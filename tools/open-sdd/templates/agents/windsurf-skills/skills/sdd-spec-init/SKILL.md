...
## Execution Steps
1. **Check for Brief**: If `{{SDD_DIR}}/specs/{feature-name}/brief.md` exists (created by `@sdd-discovery`), read it. The brief contains problem, approach, scope, and constraints from the discovery session. Use this to pre-fill the project description and skip clarification questions that the brief already answers.
...
3. **Check Uniqueness / Brownfield seed**:
   - If the directory exists with `brief.md` and `spec.json` but **no** `requirements.md` (typical `/sdd-getspecs` seed): use that directory. Write `requirements.md` stub from `requirements-init.md` using the brief; **do not** recreate or overwrite `spec.json`. Stop after stub write.
   - If the directory exists with only `brief.md` (no `spec.json`, from `/sdd-discovery`): use that directory.
...
- **Template Missing**: If template files don't exist in `{{SDD_DIR}}/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
