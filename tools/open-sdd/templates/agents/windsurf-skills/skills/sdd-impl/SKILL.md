...
- **REVIEW**:
  - `required`: Apply `sdd-review` before marking the task complete. If the host supports fresh subagents in manual mode, use a fresh reviewer; otherwise perform the review in the main context using the `sdd-review` protocol. Do NOT continue until the verdict is parseably `APPROVED`.
