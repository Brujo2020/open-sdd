...
- **Karpathy Principles & Surgical Changes**:
  - *Think Before Coding*: Always diagnose and articulate intent before modifying code.
  - *Simplicity First*: Implement the minimal, cleanest change that satisfies the spec; avoid speculative abstractions or unrequested refactoring.
  - *Surgical Precision*: Keep blast radius strictly bounded to the declared task boundary.
  - *Goal-Driven Verification*: No task is complete without fresh evidence of passing tests and zero regressions.
- **Context Management & Token Efficiency**:
  - Load only task-relevant files and spec sections; avoid dumping full repository context.
</background_information>
...
- `{{SDD_DIR}}/specs/{feature}/spec.json`, `requirements.md`, `design.md`, `tasks.md`
...
- For each wave:
  - If wave has multiple tasks: concurrently dispatch implementer sub-agents for each task in the wave.
...
- If review mode is `inline`:
  - Apply `sdd-review` in the parent context using the same task evidence and the actual `git diff`
...
- If review mode is `off`:
  ...
  - Before marking the task `[x]` or making any success claim, apply `sdd-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
- Otherwise:
...
- **REVIEW**:
  - `required`: Apply `sdd-review` before marking the task complete. If the host supports fresh sub-agents in manual mode, use a fresh reviewer; otherwise perform the review in the main context using the `sdd-review` protocol. Do NOT continue until the verdict is parseably `APPROVED`.
  - `inline`: Apply `sdd-review` in the main context before marking the task complete.
  - `off`: Skip task-local review, but note that `sdd-validate-impl` becomes the primary quality gate before any feature-level completion claim.
- **MARK COMPLETE**:
  - `required|inline`: Only after review returns `APPROVED`, apply `sdd-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.
  - `off`: Apply `sdd-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.

## Step 4: Final Validation

**Autonomous mode**:
- After all tasks complete, run `$sdd-validate-impl $1` as a GO/NO-GO gate
...
- If review mode is `off`, treat `$sdd-validate-impl $1` as mandatory before any feature-level success claim
