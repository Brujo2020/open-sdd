...
1. **Check for Brief**:
   - If `{{SDD_DIR}}/specs/{feature-name}/brief.md` exists (created by `$sdd-discovery`), read it for discovery context (problem, approach, scope, constraints)
...
4. **Create Directory**:
   - Use Bash: `mkdir -p {{SDD_DIR}}/specs/{feature-name}` (skip if already exists from discovery)
...
6. **Output Progress**: "Phase 1/4 complete: Spec initialized at {{SDD_DIR}}/specs/{feature-name}/"
...
Invoke `$sdd-spec-requirements {feature-name}`.
