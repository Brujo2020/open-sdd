...
Copy from `{{SDD_DIR}}/settings/templates/specs/init.json` and replace placeholders:
...
Copy from `{{SDD_DIR}}/settings/templates/specs/requirements-init.md`.
...
Mark `[x]` only when the spec is **fully batch-complete** (requirements + design + tasks present — same meaning as `sdd-spec-batch` skip). Leave `[ ]` for seeds and for specs that only have approved requirements; otherwise batch will skip them and never finish design/tasks.
