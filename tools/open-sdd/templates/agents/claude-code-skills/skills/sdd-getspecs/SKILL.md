---
name: sdd-getspecs
description: Brownfield entry point for existing codebases. Reverse-engineers project structure into .sdd/ steering, roadmap, and spec seeds (brief.md + spec.json + requirements.md stub). Use when adopting open-sdd on a project that has code but no specs.
...
Use instead of `/sdd-discovery` when:
...
- **Specs inventory**: Glob `{{SDD_DIR}}/specs/*/spec.json`, read each for name, phase, approvals
...
4. **Next command** (choose one):
   ...
   - Steering only: `/sdd-steering` when steering needs refinement before specs
