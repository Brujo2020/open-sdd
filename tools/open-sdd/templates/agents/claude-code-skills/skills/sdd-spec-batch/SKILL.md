...
3. Execute the full spec pipeline. For each phase, read the corresponding skill's SKILL.md for complete instructions (templates, rules, review gates):
   a. Initialize: Read .claude/skills/sdd-spec-init/SKILL.md, If `spec.json` and `brief.md` already exist (from `/sdd-getspecs` or `/sdd-discovery`), run brownfield init (requirements stub only). Otherwise create `spec.json` and `requirements.md` stub
...
1. Glob `{{SDD_DIR}}/specs/*/tasks.md` to verify all specs exist
