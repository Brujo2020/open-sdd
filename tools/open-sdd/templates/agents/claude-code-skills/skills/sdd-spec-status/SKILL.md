...
- Read `{{SDD_DIR}}/specs/$ARGUMENTS/spec.json` for metadata and phase status
- Read `{{SDD_DIR}}/specs/$ARGUMENTS/brief.md` if it exists
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `{{SDD_DIR}}/specs/$ARGUMENTS/` directory for available files
- Read `{{SDD_DIR}}/steering/roadmap.md` if it exists and this spec appears in it
...
**Spec Not Found**:
- **Message**: "No spec found for `$ARGUMENTS`. Check available specs in `{{SDD_DIR}}/specs/`"
...
- Shows all specs in `{{SDD_DIR}}/specs/` with their status
