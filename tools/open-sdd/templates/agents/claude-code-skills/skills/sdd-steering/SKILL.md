---
name: sdd-steering
description: Maintain {{SDD_DIR}}/steering/ as persistent project memory (bootstrap/sync). Use when initializing or updating steering documents.
...
## Role
You are a specialized skill for maintaining `{{SDD_DIR}}/steering/` as persistent project memory.
...
- For Sync mode: Read all existing `{{SDD_DIR}}/steering/*.md` files
