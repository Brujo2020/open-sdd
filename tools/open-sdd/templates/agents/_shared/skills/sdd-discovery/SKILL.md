...
- **Specs inventory**: Scan `{{SDD_DIR}}/specs/*/spec.json` for `name`, `phase` fields and `approvals` status. Note feature names and their current status.
...
Write `{{SDD_DIR}}/specs/<feature-name>/brief.md` to disk with this structure:
...
Then write `{{SDD_DIR}}/specs/<feature>/brief.md` for **every** feature listed under `## Specs (dependency order)` using the Path C brief format. This enables parallel spec creation via `/sdd-spec-batch`.
