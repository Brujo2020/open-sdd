...
3. **Execute Gap Analysis**:
   ...
   - Brownfield Analysis (Graph before Grep): Map call graphs, module dependencies, and architectural invariants before text searching
   - Blast Radius Estimation: Quantify impact on existing components and downstream dependencies
   - Characterization Test Locking: Verify that existing behavior is captured by automated tests before planning alterations
...
5. **Write Gap Analysis to Disk**:
   ...
   **Write the gap analysis to disk so it survives session boundaries and can be referenced during design phase.**
   ...
   - Save the gap analysis to `{{SDD_DIR}}/specs/$1/research.md`
