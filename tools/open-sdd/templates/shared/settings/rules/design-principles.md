...
### 0.1 Karpathy Simplicity First & Surgical Changes
- **Minimum code and architecture that solves the problem. Nothing speculative.**
- No abstractions for single-use components or premature generalisation.
- No "flexibility" or "configurability" that wasn't explicitly requested.
- Every architectural choice must be traceable to a verified requirement.
- For Brownfield projects: preserve existing conventions and architecture unless refactoring is an approved task; touch only what is required.

### 0.2 Drift Prevention & Brownfield Characterization
- Identify existing contracts, schemas, and invariants before modifying existing services.
- Establish characterization baselines or regression tests before introducing changes.
- Architecture is executable: specifications define system truth; detect and reject drift early.
