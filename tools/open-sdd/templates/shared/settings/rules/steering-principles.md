...
- Detailed documentation of internal metadata directories (settings, automation)
...
## Living Documentation in Git (Spec-as-Code)

- **Specs are living documentation**: Specifications live in Git alongside the code, versioned together (`.sdd/specs/`).
- **Never drift**: If implementation code evolves, the corresponding spec must evolve with it via Git Pull Requests.
- **Auditable History**: Every architectural and requirement decision is documented with Git history (`git blame` / `git log`).
- **No rot**: Avoid storing architecture and requirements in dead wikis (Confluence, Notion) that diverge from reality.
...
- Light references to `.sdd/specs/`, `.sdd/steering/` are acceptable; avoid other internal metadata directories
