...
## Objective
Analyze the gap between requirements and existing codebase (especially Brownfield systems) to inform implementation strategy decisions without introducing architectural drift or unintended side-effects.

## Analysis Framework

### 1. Current State & Brownfield Investigation

- **Graph before Grep**:
  - Map the dependency graph and import tree for impacted modules before reading individual files.
  - Identify the *blast radius*: which downstream consumers depend on the modules being changed.
  - Check for existing characterization tests and contracts.
...
- Extract conventions & invariants:
  - Naming, layering, dependency direction
  - Import/export patterns and dependency hotspots
  - Testing placement and approach
  - Existing business invariants that must not be broken

- Note integration surfaces:
  - Data models/schemas, API clients, auth mechanisms, database migrations
