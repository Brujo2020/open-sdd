...
3. **Seeds, not EARS** — each seed gets `brief.md`, `spec.json`, and a `requirements.md` **stub** (project description only, same contract as `/sdd-spec-init`). EARS acceptance criteria come from `/sdd-spec-requirements`.
...
| Only README + empty src | Greenfield — redirect to `/sdd-discovery` |
| `.sdd/` partial | Merge mode — fill gaps, preserve existing |
