...
export const buildFileOperations = async (artifacts, resolved, opts = {}) => {
    ...
    for (const art of artifacts) {
                    ...
                    const loadTemplate = async () => {
                        try {
                            ...
                            if (err?.code === 'ENOENT' && fallbackRel) {
                                const fallbackAbs = resolveRelativePathInsideRoot(path.resolve(templatesRoot, '../..'), fallbackRel, `Artifact ${art.id} fallback template path`);
        ...
        if (art.source.type === 'templateDir') {
            ...
            const sharedRulesBySkill = new Map();
            for (const src of files) {
            ...
            for (const src of files) {
                ...
                if (sharedRulesBySkill.size > 0) {
                    ...
                    if (rulesIdx >= 0) {
                        ...
                        if (sharedRulesBySkill.has(skillRel))
                            continue;
                    }
                }
            ...
            for (const [skillRel, ruleNames] of sharedRulesBySkill) {
                ...
                const sharedOps = await buildSharedRuleOperations(ruleNames, skillDestDir, templatesRoot, art.id, cwd, resolved, ctx);
