...
export const ENVIRONMENT_LOCALE_KEYS = ['LC_ALL', 'LC_MESSAGES', 'LANG', 'LANGUAGE'];
/** Superficies realmente traducidas por esta capa. */
export const TRANSLATED_SURFACES = ['open-sdd tour', 'open-sdd context (render humano)'];
...
export const MISSING_KEY_MARKER = (key) => `⟦${key}⟧`;
 ...
 */
export const createTranslator = (locale = DEFAULT_LOCALE, catalog = MESSAGES) => {
    const seen = new Set();
    const fallbacks = [];
...
export const parseLangFlag = (argv) => {
...
export const resolveLocale = (input = {}) => {
    ...
    const consider = (raw, source) => {
        ...
        if (!normalized && raw.trim().length > 0)
            rejected.push({ value: raw, source });
        return normalized;
    };
    const flag = consider(parseLangFlag(input.argv ?? []), 'flag');
    if (flag)
        return { locale: flag, source: 'flag', rejected };
    const env = consider(input.env?.OPEN_SDD_LANG, 'env');
    if (env)
        return { locale: env, source: 'env', rejected };
    ...
    if (input.env !== undefined) {
        ...
        if (fromEnvironment)
            return { locale: fromEnvironment.locale, source: 'environment', rejected };
    }
...
export const rejectionsReport = (resolution) => resolution.rejected.length === 0
    ? ''
...
export const translatedLocalesList = () => SUPPORTED_LOCALES.map((locale) => `\`${locale}\``).join(', ');
/**
 ...
 */
export const localeRefusal = (value) => `unsupported locale \`${value}\`: translated console locales are ${translatedLocalesList()}; pass \`--lang ${SUPPORTED_LOCALES[0]}\` or \`--lang ${SUPPORTED_LOCALES[1]}\``;
/**
 ...
 */
export const ENVELOPE_DETAIL_LOCALE = 'en';
/**
 ...
 */
export const stableEnvelopeDetail = (input) => {
