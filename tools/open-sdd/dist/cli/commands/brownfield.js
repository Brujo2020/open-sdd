...
import { deltaCounts, deltaSpecFileName, mergeDeltaIntoBase, parseDeltaSpec, renderDeltaSpec, strangulationReport, traceDelta, validateDeltaSpec, } from '../../core/deltaSpec.js';
import { parseTasksMarkdown, resolveSddDir } from '../../core/specManager.js';
import { analyzeChangeImpact, forecastImpact } from '../../core/changeImpact.js';
...
import { analyseConvergence, appendConvergence, convergeExitCode } from '../../core/converge.js';
...
import { applyAnswers, clarifyQuestionsFile, planClarify, readClarifyAnswers, } from '../../core/clarify.js';
import { SPECIFY_QUESTIONS_FILE_KIND, applyConstitutionAnswers, existingRequirementStatements, normalizeRequirementStatement, planConstitutionInterview, requirementArea, specifyFromDescription, } from '../../core/interview.js';
...
const positionalsOf = (args, from, valueFlags) => {
 ...
 */
const normalizeInterviewAnswers = (parsed) => {
    const out = {};
    const take = (source) => {
...
export const handleDeltaCommand = async (args, io, cwd) => {
    ...
    if (sub === 'merge' || sub === 'merge-back') {
        ...
        const declaredBasePath = declaredBase
            ...
            : null;
        const basePath = declaredBasePath && (await stat(declaredBasePath).catch(() => null)) !== null
            ? declaredBasePath
        ...
        if (report.refusals.length > 0) {
            if (json) {
                io.log(JSON.stringify({ feature: report.feature, changed: false, applied: [], unchanged: report.unchanged, refusals: report.refusals, detail: report.detail, written: false }, null, 2));
                return 1;
            }
            io.log('');
            ...
            for (const change of report.changes)
                io.log(`  ${colors.red('rechazada').padEnd(18)} ${change.id.padEnd(16)} ${change.message}`);
            io.log('');
        ...
        if (!write) {
            if (json) {
                io.log(JSON.stringify({ feature: report.feature, changed: report.changed, applied: report.applied, unchanged: report.unchanged, refusals: [], detail: report.detail, written: false }, null, 2));
                return 0;
            }
            io.log('');
            io.log(heading(`Fusión de la delta — ${delta.feature}`));
        ...
        if (!report.changed) {
            if (json) {
                io.log(JSON.stringify({ feature: report.feature, changed: false, applied: [], unchanged: report.unchanged, refusals: [], detail: report.detail, written: false }, null, 2));
                return 0;
            }
            io.log('');
 ...
 */
const handleIdsAudit = async (args, io, root) => {
    ...
    if (json) {
        ...
        for (const problem of report.problems)
            io.log(`  ${colors.yellow('!')} ${problem}`);
        for (const finding of report.findings)
            io.log(`  ${colors.red('✗')} ${finding.code} ${finding.id} — ${finding.detail}`);
...
export const handleBrownfieldCommand = async (args, io, cwd) => {
    ...
    if (sub === 'bootstrap') {
        ...
        if (write) {
                ...
                if (slug.length > 0) {
                    ...
                    if ((await stat(deltaFile).catch(() => null)) === null) {
                        try {
                            ...
                            failures.push(`no se pudo escribir ${path.posix.join(sddDir, 'specs', slug, 'delta.md')} (${error.message})`);
                        }
                    }
                    else {
            ...
            if ((await stat(constitutionFile).catch(() => null)) === null) {
                try {
                    ...
                    failures.push(`no se pudo escribir ${path.posix.join(sddDir, 'steering', 'constitution.md')} (${error.message})`);
                }
            }
        }
        if (json) {
        ...
        io.log(`  stack: ${plan.project.language}${plan.project.frameworks.length > 0 ? ` · ${plan.project.frameworks.join(', ')}` : ''}${plan.project.testFramework ? ` · tests: ${plan.project.testFramework}` : ''}`);
        io.log('');
        ...
        for (const module of plan.modules) {
            ...
            io.log(`      responsabilidades: ${module.responsibilities.length > 0 ? module.responsibilities.join(' · ') : dim('(ninguna derivable de la evidencia)')}`);
            io.log(`      depende de: ${module.dependsOn.length > 0 ? module.dependsOn.join(', ') : dim('(ninguno observado)')}`);
            if (module.testDirs.length > 0)
                io.log(`      tests: ${module.testDirs.join(', ')}`);
        }
        io.log('');
        ...
        if (write) {
            io.log('');
            if (intelligence?.written) {
                io.log(`  ${colors.green('✓')} ${intelligence.path} escrito (${intelligence.complete ? 'evidencia completa' : 'evidencia parcial'})`);
            }
    ...
    if (sub === 'templates') {
        ...
        for (const template of result.templates) {
            ...
            for (const evidence of template.adaptedFrom)
                io.log(`        ${dim(`adaptado: ${evidence}`)}`);
            for (const item of template.unchanged)
                io.log(`        ${colors.yellow('sin observar:')} ${item}`);
            io.log('');
        ...
        if (write) {
            for (const written of result.written)
                io.log(`  ${colors.green('✓')} escrita ${written}`);
            const kept = result.actions.filter((item) => item.action === 'keep');
            for (const item of kept)
                io.log(dim(`  = ${item.path} existía: se conserva`));
        }
        else {
    ...
    if (sub === 'analyze') {
        ...
        for (const finding of report.findings) {
            ...
            if (finding.artifacts.length > 0)
                io.log(`      ${dim(`artefactos: ${finding.artifacts.join(', ')}`)}`);
        }
        io.log('');
    ...
    if (sub === 'survey' || sub === 'constitution') {
        ...
        if (sub === 'constitution' && args.includes('--interview')) {
            ...
            if (answersPath) {
                ...
                const blocking = validateConstitution(parseConstitution(application.text)).filter((issue) => issue.severity === 'error');
                ...
                for (const entry of application.applied)
                    io.log(`    ${colors.green('✓')} ${entry.id} → ${entry.principle}`);
                if (application.unanswered.length > 0) {
                    ...
                    for (const id of application.unanswered)
                        io.log(`    ${colors.yellow('?')} ${id}`);
                }
                io.log('');
                for (const issue of application.issues)
                    io.log(`  ${dim(issue.code)}: ${dim(issue.message)}`);
                ...
                if (write && draftPath !== null) {
                    ...
                    if (inForce !== null) {
                        io.log(`  ${colors.yellow('!')} ${path.relative(interviewTarget, paths.inForce)} existe y no se ha tocado: solo --ratify lo sustituye.`);
                    }
                    io.log('');
            ...
            for (const item of interview.decidedByEvidence)
                io.log(`    · ${item}`);
            io.log('');
            ...
            for (const question of interview.questions) {
                ...
                if (question.default !== undefined)
                    io.log(`        ${dim(`por defecto: ${question.default}`)}`);
                if (question.options)
                    io.log(`        ${dim(`opciones: ${question.options.join(' | ')}`)}`);
                if (question.fromEvidence)
                    io.log(`        ${dim(`de la evidencia: ${question.fromEvidence}`)}`);
            }
            io.log('');
        ...
        if (sub === 'constitution' && args.includes('--draft')) {
            ...
            if (inForce !== null) {
                io.log(`  ${colors.yellow('!')} ${path.relative(target, paths.inForce)} existe y no se ha tocado: el borrador va al lado y solo --ratify lo sustituye.`);
            }
            io.log('');
    ...
    if (sub === 'specify') {
        ...
        if (feature.length === 0 || description.length === 0) {
            io.error(colors.red('Faltan argumentos: open-sdd brownfield specify <feature> "<descripción en lenguaje natural>" [--area A] [--json] [--write] [--questions-file <path>]'));
            return 1;
        }
        ...
        const existingStatements = new Set(existingRequirementStatements(existing ?? '').map(normalizeRequirementStatement));
        const keep = result.requirements.filter((requirement) => existingStatements.has(normalizeRequirementStatement(requirement.statement)));
        const added = result.requirements.filter((requirement) => !existingStatements.has(normalizeRequirementStatement(requirement.statement)));
        ...
        if (questionsFileFlag) {
            ...
            const payload = {
                ...
                howToAnswer: 'Responde cada pregunta ampliando la descripción (nombra el actor, el disparador y el valor medible) y vuelve a ejecutar specify: los enunciados ya presentes se conservan y no se reescriben.',
            };
        ...
        if (result.requirements.length === 0)
            io.log(dim('    (ninguno: no se ha inventado ningún requisito)'));
        for (const requirement of result.requirements) {
            const already = keep.some((candidate) => candidate.id === requirement.id);
            io.log(`    ${already ? colors.dim('keep') : colors.green('add')} ${requirement.id} ${dim(requirement.pattern)} ${requirement.statement}`);
        }
        if (result.questions.length > 0) {
            io.log('');
            io.log(`  ${colors.bold(`Preguntas — datos que la descripción no trae (${result.questions.length})`)} ${dim('(NO son requisitos: no se ha inventado ninguno)')}:`);
        ...
        io.log('');
        if (questionsFile)
            io.log(`  ${colors.green('✓')} preguntas escritas en ${questionsFile}`);
        if (written) {
            io.log(`  ${colors.green('✓')} ${path.relative(root, requirementsPath)} escrito (${added.length} añadido(s), ${keep.length} conservado(s))`);
        }
    ...
    if (sub === 'requirements') {
        ...
        if (requirementsRaw === null && deltaRaw === null) {
            io.error(colors.red(`No hay ${relative(requirementsPath)} ni ${relative(deltaFile)} para "${feature}": no hay requisitos que analizar.`));
            return 1;
        }
        const reports = [];
        const notes = [];
        ...
        if (deltaRaw !== null) {
            ...
            if (deltaStatements.duplicates > 0) {
                notes.push(`${deltaStatements.duplicates} enunciado(s) de la delta repiten requirements.md: no se cuentan dos veces`);
            }
        }
        ...
        const asFindings = (suggestions) => suggestions.map((suggestion) => ({ id: suggestion.code, message: suggestion.problem, artifact: suggestion.target }));
        const json = args.includes('--json');
        ...
        const files = [];
        if (requirementsRaw !== null)
            files.push({ path: requirementsPath, rel: relative(requirementsPath), text: requirementsRaw });
        if (deltaRaw !== null)
            files.push({ path: deltaFile, rel: relative(deltaFile), text: deltaRaw });
        ...
        const selectSuggestions = () => {
            ...
            for (const suggestion of selected) {
                if (!isEarsProposalApplicable(suggestion)) {
                    refusals.push(`${suggestion.code} en "${suggestion.target}" es una PREGUNTA, no una frase: falta un dato que no se inventa. Respóndela y reescribe el requisito a mano.`);
                }
            }
        ...
        if (selection !== null || write) {
            if (selection === null || selection.length === 0) {
                ...
                io.error(colors.red('--write exige una selección explícita: open-sdd brownfield requirements <feature> --apply <índice|código> --write. Nada se ha escrito.'));
                return 1;
            }
            ...
            if (refusals.length > 0) {
                ...
                for (const refusal of refusals)
                    io.log(`  ${colors.red('rechazada')} ${refusal}`);
                io.log('');
            ...
            for (const suggestion of selected) {
                ...
                const file = files.find((candidate) => candidate.text.split('\n').some((line) => line.trim() === suggestion.target));
                if (!file) {
            ...
            if (missing.length > 0) {
                ...
                for (const message of missing)
                    io.log(`  ${colors.red('rechazada')} ${message}`);
                io.log('');
        ...
        io.log('');
        for (const line of renderEarsReport(report, { suggest }))
            io.log(line);
        ...
        for (const note of notes)
            io.log(dim(`  ${note}`));
        io.log('');
        io.log(dim('  Escribir exige selección: --apply <índice|código> muestra el diff; añade --write para aplicarlo. Nunca se reescribe la spec desde una heurística en bloque.'));
        io.log('');
        return errorSuggestions.length > 0 ? 1 : 0;
    }
    ...
    if (sub === 'forecast') {
        ...
        if (forecast.modules.length > 0) {
            ...
            for (const module of forecast.modules) {
                ...
                for (const evidence of module.matched.slice(0, 5))
                    io.log(`      · ${evidence}`);
            }
        }
        else {
        ...
        if (forecast.tests.length > 0) {
            ...
            for (const test of forecast.tests.slice(0, 10))
                io.log(`    ${test.file}  ${dim(test.reasons.join('; '))}`);
        }
        if (forecast.unknown.length > 0) {
            io.log('');
            io.log(`  ${colors.bold('Desconocido (no se informa como cero)')}:`);
            for (const item of forecast.unknown)
                io.log(`    · ${item}`);
        }
        io.log('');
    ...
    if (sub === 'repair') {
        ...
        io.log(`  hash antes: ${report.beforeHash ?? dim('(ausente)')} · registrado: ${report.recordedHash ?? dim('(ninguno)')} · después: ${report.afterHash ?? dim('(sin ejecutar)')}`);
        if (report.drift)
            io.log(`  ${colors.yellow('!')} deriva: el artefacto no coincidía con el hash que la spec registra`);
        for (const refusal of report.refusals)
            io.log(`  ${colors.red('✗')} ${refusal}`);
        io.log('');
    ...
    if (sub === 'clarify') {
        ...
        const renderQuestions = () => {
            ...
            if (session.questions.length === 0) {
                ...
                session.questions.forEach((question, index) => {
                    ...
                    if (question.template)
                        io.log(dim(`      plantilla: ${question.template}`));
                    if (question.options)
                        for (const option of question.options)
                            io.log(dim(`      opción: ${option}`));
                });
            }
            ...
            io.log(dim('  No se ha escrito nada. Ciclo no interactivo: --questions-file <ruta> → rellena answers → --answers <ruta> --write.'));
        };
        ...
        const applyWith = async (answers, doWrite) => {
        ...
        const renderApply = (result, doWrite) => {
            io.log('');
            ...
            for (const { file, report } of result.reports) {
                ...
                for (const refusal of report.refused)
                    io.log(`  ${colors.red('rechazada')} ${refusal.id}: ${refusal.why}`);
                const verification = report.verification;
                io.log(`  ${file}: códigos antes ${verification.codeBefore} · después ${verification.codeAfter} · ` +
            ...
            for (const file of result.unreadable)
                io.log(`  ${colors.red('✗')} no se pudo leer ${file}`);
            io.log('');
            io.log(`  ${result.written} fichero(s) reescrito(s). ${doWrite ? 'Todo lo demás se ha conservado byte a byte.' : 'Sin --write la especificación NO se ha tocado.'}`);
            io.log(`  ${session.detail}`);
        };
        let interactiveAnswers = null;
            ...
            try {
                ...
                for (const question of session.questions) {
                    ...
                    if (answer.trim().length > 0)
                        interactiveAnswers[question.id] = answer.trim();
                }
            }
            finally {
                rl.close();
        ...
        if (answers === null) {
            if (write) {
                io.error(colors.red('--write exige respuestas: usa --answers <ruta> (o ejecuta en un TTY para responder una a una). Nada se ha escrito.'));
                return 1;
            }
            if (json) {
                io.log(JSON.stringify(jsonEnvelope({
        ...
        const answered = result.reports.reduce((count, { report }) => count + report.applied.length + report.refused.length, 0);
        if (answered === 0) {
            io.error(colors.red(`Ninguna respuesta del fichero coincide con las ${session.questions.length} pregunta(s) de esta sesión: nada que aplicar.`));
            return 1;
        }
        const refusals = result.reports.flatMap((entry) => entry.report.refused.map((refusal) => ({ id: refusal.id, message: refusal.why, artifact: entry.file })));
    ...
    if (sub === 'converge') {
        ...
        const asFindings = (findings) => findings.map((finding) => ({ id: finding.id, message: finding.remainingWork, artifact: finding.source }));
        ...
        const renderConvergence = (written) => {
            io.log('');
            io.log(heading(`Convergencia brownfield — ${feature}`));
            io.log('');
            if (report.findings.length === 0) {
                io.log(`  ${report.converged
                ...
                for (const finding of report.findings) {
                    const mark = finding.severity === 'high'
                    ...
                    for (const item of finding.evidence)
                        io.log(dim(`        evidencia: ${item}`));
                }
            }
            const metrics = report.metrics;
            io.log('');
            io.log(`  métricas: requisitos ${metrics.requirementsChecked} · tareas ${metrics.tasksChecked} · principios ${metrics.principlesChecked} · contratos ${metrics.contractsChecked}`);
            io.log(`  por tipo: ${Object.entries(metrics.byGapType).map(([gap, count]) => `${gap} ${count}`).join(' · ')}`);
            io.log(`  por severidad: alto ${metrics.bySeverity.high} · medio ${metrics.bySeverity.medium} · bajo ${metrics.bySeverity.low}`);
            const tasksAfter = written ? written.tasksAfter : report.tasksAfter;
            io.log(`  tareas: ${report.tasksBefore} → ${tasksAfter}${written ? '' : ' (proyección: sin --write no se escribe)'}`);
            io.log(`  score que debería moverse: ${report.scoreImpact.length === 0
                ? dim('ningún componente (no hay huecos nuevos)')
                : report.scoreImpact.map((entry) => `${entry.component} ↑`).join(', ')}`);
            io.log('');
            io.log(`  ${colors.bold('Comprobado')} (${report.checked.length}):`);
            for (const item of report.checked)
                io.log(`    ${colors.green('✓')} ${item}`);
            if (report.notChecked.length > 0) {
                io.log(`  ${colors.bold('NO comprobado')} (${report.notChecked.length}) — no cuenta como aprobado:`);
                for (const item of report.notChecked)
                    io.log(`    ${colors.yellow('!')} ${item}`);
