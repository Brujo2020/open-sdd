...
import { DEFAULT_PROGRESS_LIMIT, readProgress, recordProgress, renderProgress, } from '../../core/progress.js';
...
const parsePhase = (raw) => raw === '1' ? 1 : raw === '2' ? 2 : raw === '3' ? 3 : null;
const read = async (args, io, cwd) => {
...
const record = async (args, io, cwd) => {
    const json = args.includes('--json');
    ...
    if (kind === undefined || kind.trim().length === 0)
        problems.push('falta --kind <tipo> (spec, implement, verify, gate, backup…)');
    if (summary === undefined || summary.trim().length === 0)
        problems.push('falta --summary "<una línea que nombre lo conseguido>"');
    if (!Number.isFinite(score))
        problems.push('falta --score <0..100>');
    if (phase === null)
        problems.push('falta --phase <1|2|3>');
    if (problems.length > 0) {
        const usage = 'Usage: open-sdd progress record --kind <tipo> --summary "<una línea>" --score <0..100> --phase <1|2|3> [--evidence <p>]… [--json]';
        if (json) {
...
export const handleProgressCommand = async (args, io, cwd = process.cwd()) => {
    const sub = args[0];
    if (sub === 'record')
        return record(args.slice(1), io, cwd);
