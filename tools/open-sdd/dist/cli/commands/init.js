...
import { HOST_INTEGRATIONS, detectIntegration, integrationById, mcpRegistration, } from '../../core/integrations.js';
import { installStopHook, stopHookFor } from '../../core/stopHook.js';
import { COMMAND_TEMPLATE_IDS, commandHostById, hostForAgent, installCommandTemplates, planCommandTemplates, summarizeCommandTemplates, } from '../../core/commandTemplates.js';
import { IMPORT_SOURCES, applyImport, planImport, } from '../../core/importers.js';
// ---------------------------------------------------------------------------------------------
// utilidades
// ---------------------------------------------------------------------------------------------
...
const flagFor = (agent) => getAgentDefinition(agent).aliasFlags[0] ?? `--${agent}`;
const detectAgent = async (target) => {
    const matches = [];
    ...
    const ordered = [...matches].sort((a, b) => b.score - a.score || b.path.length - a.path.length || agentList.indexOf(a.agent) - agentList.indexOf(b.agent));
...
// ---------------------------------------------------------------------------------------------
const SPANISH_MARKERS = /\b(el|la|los|las|un|una|unos|unas|de|del|que|para|con|por|como|se|su|sus|es|son|no|más|también|desde|hasta|entre|sobre|cada|cuando|donde|puede|debe|este|esta|estos|estas|sin|al|lo)\b/gi;
const ENGLISH_MARKERS = /\b(the|and|of|to|in|is|are|for|with|that|this|from|as|on|by|be|or|not|it|its|will|can|should|must|when|where|which|each|all|any|use|run)\b/gi;
const countMatches = (text, pattern) => {
    const global = new RegExp(pattern.source, pattern.flags.includes('g') ? pattern.flags : `${pattern.flags}g`);
    return [...text.matchAll(global)].length;
};
const detectLanguage = async (target, sddDir) => {
    const candidates = [];
    ...
    for (const entry of steeringEntries) {
        if (entry.isFile() && entry.name.endsWith('.md'))
            candidates.push(posixJoin(sddDir, 'steering', entry.name));
    }
    ...
    const lang = english > spanish * 1.15 ? 'en' : 'es';
    return {
        lang,
        source: 'docs',
...
const rationaleFor = (level) => `Nivel declarado por \`open-sdd init\` (${level}): exige requisitos en forma comprobable (EARS) y una constitución válida, y no exige todavía delta, ligado de evidencia ni detección de drift. Se sube editando este archivo, nunca bajando lo que el suelo ya comprueba.`;
/**
...
const supportsBrownfieldDraft = (cwd, cliPath) => {
    if (cliPath === null)
        return false;
...
export const planInit = async (input) => {
    const { cwd } = input;
    ...
    if (input.agent !== undefined) {
        if (!agentList.includes(input.agent)) {
            throw new Error(`Agente desconocido: "${input.agent}". Admitidos: ${agentList.join(', ')} (o sus alias ${agentList
    ...
    if (input.level !== undefined && !isRigorLevel(input.level)) {
        throw new Error(`Nivel de rigor inválido: "${input.level}". Admitidos: ${RIGOR_LEVELS.map((level) => level.level).join(', ')}.`);
    }
    const level = input.level !== undefined ? input.level : DEFAULT_RIGOR_LEVEL;
    ...
    const hasCode = project.language !== 'unknown' ||
    ...
    if (commandHostId && commandHost) {
        try {
            ...
            artifacts.push({
                ...
                reason: `${summary.reason} Fuente: plantillas de comando propias (${COMMAND_TEMPLATE_IDS.length}), instaladas sin MCP ni red. ` +
                    `Cada workflow queda como \`${commandHost.invocation('constitution')}\` en el chat del anfitrión.`,
            });
        }
        catch (error) {
            commandSummary.reason = `no se pudieron planificar las plantillas de comando (${error.message}); no se escribe nada.`;
    ...
    let mcpSummary = { requested: input.mcp === true, path: null, action: null, verified: false, reason: null };
    if (input.mcp) {
    ...
    if (!hasCode) {
        steps.splice(1, 0, '· No se observó código (ni manifiestos, ni directorios de origen ni de tests): no hay nada que reverse-ingeniar, así que la constitución se escribe a mano antes de especificar.');
    }
    if (input.skills && cliPath === null) {
        steps.push('ATENCIÓN: se pidió --skills pero no se encontró el instalador (CLI): la instalación del agente NO se puede ejecutar.');
    }
...
// Escritura
// ---------------------------------------------------------------------------------------------
const writeRigorSettings = async (plan) => {
    ...
    try {
        ...
        return { action: 'keep', failure: `no se pudo escribir ${artifact?.path ?? resolved} (${error.message})` };
    }
};
const writeConstitution = async (plan) => {
    ...
    try {
        ...
        return { action: 'keep', failure: `no se pudo escribir ${artifact.path} (${error.message})` };
    }
};
/**
 ...
 */
export const installAgentSkillSet = async (root, agent, lang) => {
    ...
    const result = spawnSync(process.execPath, [cliPath, `--${alias}`, '--lang', lang, '--overwrite=prompt'], { cwd: root, encoding: 'utf8', timeout: 300_000 });
    if (result.status !== 0) {
        return {
            action: 'keep',
...
const installAgentSkills = async (plan) => {
    if (!plan.skills)
        return { action: 'keep' };
...
const applyInit = async (plan) => {
    const outcome = { written: [], kept: [], failures: [], details: [] };
    const rigorPath = plan.artifacts.find((item) => item.kind === 'rigor').path;
    const rigor = await writeRigorSettings(plan);
    if (rigor.failure)
        outcome.failures.push(rigor.failure);
    else if (rigor.action === 'create')
        outcome.written.push(rigorPath);
    else
        outcome.kept.push(rigorPath);
    ...
    if (constitutionArtifact) {
        ...
        if (constitution.failure)
            outcome.failures.push(constitution.failure);
        else if (constitution.action === 'create')
            outcome.written.push(constitutionArtifact.path);
        else
            outcome.kept.push(constitutionArtifact.path);
    }
    ...
    if (hookArtifact && (hookArtifact.action === 'create' || hookArtifact.action === 'update')) {
        ...
        if (cliPath === null) {
            outcome.failures.push(`no se pudo instalar el hook de commit (${hookArtifact.path}): el instalador (CLI) no está alcanzable. Ejecútalo tú: open-sdd floor install ${plan.root}`);
        }
        else {
            ...
            if (result.status === 0)
                outcome.written.push(hookArtifact.path);
            else
                outcome.failures.push(`el instalador del hook falló (código ${result.status ?? 'desconocido'}): ${(result.stderr ?? '').trim().split('\n').slice(-1)[0] ?? 'sin detalle'}`);
        }
    }
    ...
    if (plan.skills) {
        ...
        if (skills.failure)
            outcome.failures.push(skills.failure);
        else if (skills.action === 'create')
            outcome.written.push(skillsArtifactPath);
        else
            outcome.kept.push(skillsArtifactPath);
        if (skills.detail)
            outcome.details.push(skills.detail);
    }
    ...
    if (templatesArtifact && templatesHost && plan.commandTemplates.verified) {
        ...
        if (!plan.commandTemplates.verified)
            outcome.details.push(templatesArtifact.reason);
    }
        ...
        if (mcpArtifact && host) {
            ...
            if (merge.resolvedPath === null || merge.action === 'keep') {
                ...
                try {
                    ...
                    outcome.failures.push(`no se pudo escribir ${merge.resolvedPath} (${error.message})`);
                }
            }
        }
        else if (mcpArtifact) {
 ...
 */
export const isProjectInitInvocation = (args, cwd) => {
    // Contrato heredado: `--title`/`--git` son banderas del init de spec y no existen en el de proyecto.
    if (args.some((arg) => arg === '--title' || arg.startsWith('--title=')))
        return false;
    if (args.some((arg) => arg === '--git' || arg.startsWith('--git=')))
        return false;
    // Cualquier bandera propia del inicializador de proyecto decide sin ambigüedad.
    if (args.some((arg) => PROJECT_FLAGS.has(arg) || arg.startsWith('--sdd-dir=')))
        return true;
...
const handleProjectInit = async (args, io, cwd) => {
    ...
    try {
        plan = await planInit({
            cwd,
            target: positional ?? '.',
            ...(flag('agent') !== undefined ? { agent: flag('agent') } : {}),
            ...(flag('level') !== undefined ? { level: flag('level') } : {}),
            ...(flag('lang') !== undefined ? { lang: flag('lang') } : {}),
            ...(flag('sdd-dir') !== undefined ? { sddDir: flag('sdd-dir') } : {}),
            skills,
            write,
            mcp,
        });
        ...
        io.error(colors.red(`Error: ${error.message}`));
        return 1;
    }
    let outcome = { written: [], kept: [], failures: [], details: [] };
    if (write)
        outcome = await applyInit(plan);
    if (json) {
    ...
    io.log(`  agente: ${colors.bold(`${plan.agent.id} (${plan.agent.label})`)} ${colors.dim(`[${plan.agent.source}]`)}`);
    for (const evidence of plan.agent.evidence)
        io.log(colors.dim(`      por qué: ${evidence}`));
    ...
    if (plan.agent.alternatives.length > 0) {
        io.log(`      alternativas detectadas: ${plan.agent.alternatives
    ...
    io.log('');
    io.log(`  rigor: ${colors.bold(`${plan.level.name} (${plan.level.level})`)} ${colors.dim(`[${plan.level.source}]`)} · gates activos: ${plan.level.gates.join(', ')}`);
    for (const rung of plan.level.ladder) {
        const mark = rung.level === plan.level.level ? colors.green('→') : colors.dim(' ');
        io.log(`    ${mark} ${rung.level.padEnd(15)} gates ${rung.gates.join(',').padEnd(16)} constitución ${rung.constitutionRequired ? colors.bold('OBLIGATORIA') : 'recomendada'}`);
    }
    io.log(colors.dim('      La constitución es el SUELO de la escalera, obligatoria en los tres niveles: sin autoridad citable ningún veredicto bloqueante puede justificarse.'));
    // Idioma.
    io.log('');
    io.log(`  idioma: ${colors.bold(plan.language.lang)} ${colors.dim(`[${plan.language.source}]`)}`);
    for (const evidence of plan.language.evidence)
        io.log(colors.dim(`      por qué: ${evidence}`));
    // Plan de artefactos.
    io.log('');
    io.log(`  ${colors.bold('Artefactos')}`);
    for (const artifact of plan.artifacts) {
        const mark = artifact.action === 'create'
    ...
    if (write) {
        ...
        for (const item of outcome.written)
            io.log(`    ${colors.green('✓')} escrito: ${item}`);
        for (const item of outcome.kept)
            io.log(colors.dim(`    = conservado: ${item}`));
        for (const detail of outcome.details)
            io.log(colors.dim(`    · ${detail}`));
        for (const failure of outcome.failures)
            io.log(`    ${colors.red('✗')} ${failure}`);
    }
    else {
        io.log('');
    ...
    if (!plan.artifacts.some((artifact) => artifact.kind === 'constitution')) {
        io.log('');
        io.log(colors.yellow('  ! No se observó código: no hay constitución que derivar del repositorio. Escríbela a mano en .sdd/steering/constitution.md antes de especificar (el nivel declarado la exige).'));
    }
...
// ---------------------------------------------------------------------------------------------
export const handleSpecInitCommand = async (argv, io, cwd = process.cwd()) => {
 ...
 */
export const handleInitCommand = async (args, io, cwd = process.cwd()) => {
    if (isProjectInitInvocation(args, cwd))
        return handleProjectInit(args, io, cwd);
...
const expandConfigPath = (declared, cwd) => {
    if (declared.startsWith('~/') || declared === '~') {
        return path.join(homedir(), declared.slice(2));
    }
    const expandEnv = (prefix, base) => base === undefined ? null : path.join(base, declared.slice(prefix.length).replace(/^[\\/]+/, ''));
...
const mergeTopLevel = (current, incoming) => {
    const merged = { ...current };
    for (const [key, value] of Object.entries(incoming)) {
        const prior = merged[key];
        const bothPlainObjects = prior !== null &&
            typeof prior === 'object' &&
            !Array.isArray(prior) &&
            value !== null &&
            typeof value === 'object' &&
 ...
 */
export const mergeMcpConfig = async (host, cliPath, cwd) => {
    ...
    if (registration.path === null) {
        return {
            ...
            reason: 'el anfitrión no declara una ruta de configuración MCP conocida: open-sdd no inventa un archivo. Configúralo desde la interfaz del anfitrión; el snippet se imprime como NO VERIFICADO.',
        };
    }
    if (!registration.verified) {
        return {
            ...
            resolvedPath: null,
            content: registration.content,
            reason: 'el snippet de este anfitrión está NO VERIFICADO: open-sdd no escribe a ciegas una configuración que no puede garantizar. Cópialo y compruébalo contra la documentación del anfitrión.',
        };
    }
    ...
    if (registration.format === 'json') {
        try {
            ...
            const incoming = JSON.parse(registration.content);
            ...
            return {
                ...
                resolvedPath,
                content: serialized,
                reason: 'se añade o actualiza SOLO la entrada del servidor open-sdd dentro de su objeto: el resto de claves del archivo se conserva.',
            };
        }
        catch (error) {
            return {
                ...
                resolvedPath,
                content: registration.content,
                reason: `el archivo existe pero no parsea como JSON (${error.message}): NO se toca, porque reescribirlo borraría lo que no entendemos. Pega el snippet a mano.`,
            };
        }
    }
    if (registration.format === 'yaml') {
        ...
        return {
            ...
            resolvedPath,
            content: registration.content,
            reason: 'el archivo existe con contenido distinto al nuestro: se CONSERVA y no se reescribe (puede ser un archivo de una persona con el mismo nombre). Compara el bloque impreso y mézclalo a mano.',
        };
    }
...
export const planIntegrate = async (input) => {
    const { cwd } = input;
    let host;
    let source;
    let evidence;
    ...
    if (input.host !== undefined && input.host.trim().length > 0) {
        ...
        if (!declared) {
            throw new Error(`Anfitrión desconocido: "${input.host}". Admitidos: ${HOST_INTEGRATIONS.map((item) => item.id).join(', ')}. Consulta la matriz con \`open-sdd integrate --list\`.`);
        }
        host = declared;
        source = 'declarado';
    ...
    const cliPath = resolvedCli ?? (process.argv[1]?.endsWith('cli.js') === true ? process.argv[1] : '<ruta-al-cli-open-sdd>');
...
export const applyIntegrate = async (plan, cwd) => {
    ...
    if (agent && skillsArtifact) {
        ...
        if (skills.failure)
            ...
            outcome.written.push(skillsArtifact.path);
        else
...
const printIntegrationMatrix = (io) => {
    io.log('');
    io.log(formatHeading(colors.cyan('Matriz de integración open-sdd — un anfitrión por fila')));
    io.log('');
    io.log(`  ${'host'.padEnd(14)} ${'skills layout'.padEnd(34)} ${'invocación'.padEnd(30)} ${'MCP config'.padEnd(46)} verificado`);
    for (const host of HOST_INTEGRATIONS) {
        ...
        io.log(`  ${host.id.padEnd(14)} ${host.skills.layout.padEnd(34)} ${host.invocation.padEnd(30)} ${configPath.padEnd(46)} ${host.mcp.verified ? 'verificado' : 'NO VERIFICADA'}`);
    }
    io.log('');
    io.log(colors.dim('  «NO VERIFICADA» significa que este proyecto no conoce con certeza la forma o la ruta del archivo del anfitrión: se imprime para que la pegues y la compruebes, y `--write` no la escribe. Las rutas son las de la matriz; el archivo que `--write` toca puede ser el de proyecto o el de usuario según el anfitrión.'));
    io.log('');
    return 0;
};
export const handleIntegrateCommand = async (args, io, cwd = process.cwd()) => {
    if (args.includes('--list'))
        return printIntegrationMatrix(io);
    const json = args.includes('--json');
    ...
    let outcome = { written: [], kept: [], failures: [], details: [] };
    if (write)
        outcome = await applyIntegrate(plan, cwd);
    if (json) {
        io.log(JSON.stringify({ ...plan, outcome }, null, 2));
        return outcome.failures.length > 0 ? 1 : 0;
    }
    ...
    for (const item of plan.host.evidence)
        io.log(colors.dim(`      por qué: ${item}`));
    ...
    io.log(`  invocación en el chat de ${plan.host.label}: ${colors.bold(colors.cyan(plan.invocation))}`);
    io.log('');
    ...
    if (plan.mcp.resolvedPath)
        io.log(`    ruta a escribir: ${plan.mcp.resolvedPath}`);
    ...
    if (!plan.mcp.verified) {
        io.log('');
        io.log(colors.yellow('  ! El snippet MCP de este anfitrión está NO VERIFICADO: no se escribe automáticamente. Cópialo, compruébalo contra la documentación del anfitrión y pégalo tú.'));
    }
    io.log('');
    io.log(colors.dim(`  ${plan.detail}`));
    io.log('');
...
// ---------------------------------------------------------------------------------------------
export const handleImportCommand = async (args, io, cwd = process.cwd()) => {
    const json = args.includes('--json');
    const write = args.includes('--write') && !args.includes('--dry-run');
    ...
    if (sourceArg !== undefined && !IMPORT_SOURCES.includes(sourceArg)) {
        io.error(colors.red(`Error: fuente desconocida "${sourceArg}". Admitidas: ${IMPORT_SOURCES.join(', ')}.`));
        return 1;
    }
    let plans;
    try {
        plans = await planImport(cwd, sourceArg);
    }
    catch (error) {
        io.error(colors.red(`Error: ${error.message}`));
        return 1;
    ...
    io.log('');
    if (plans.length === 0) {
        io.log(colors.dim('  No se observó ningún incumbente (Kiro, spec-kit, cc-sdd): ni `.kiro/`, ni `.specify/`, ni `specs/` con `spec.md`, ni un marcador de cc-sdd.'));
        io.log('');
        return 0;
    }
    plans.forEach((plan, index) => {
        ...
        for (const warning of plan.warnings)
            io.log(`      ${colors.yellow('!')} ${warning}`);
        if (outcome) {
            for (const item of outcome.written)
                io.log(`      ${colors.green('✓')} escrito: ${item}`);
            for (const item of outcome.skipped)
                io.log(colors.dim(`      = omitido: ${item}`));
