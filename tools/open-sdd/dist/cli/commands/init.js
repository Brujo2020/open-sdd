...
const installAgentSkills = async (plan) => {
    ...
    if (cliPath === null) {
        return {
            action: 'keep',
            failure: 'se pidió --skills pero no se encontró el instalador (CLI) alcanzable: la instalación del agente NO se ha ejecutado. Instálalo con `npx open-sdd@latest --version` o compílalo con `npm --prefix tools/open-sdd run build`.',
        };
    }
    ...
    const result = spawnSync(process.execPath, [cliPath, `--${alias}`, '--lang', plan.language.lang, '--overwrite=prompt'], { cwd: plan.root, encoding: 'utf8', timeout: 300_000 });
    if (result.status !== 0) {
        return {
            action: 'keep',
