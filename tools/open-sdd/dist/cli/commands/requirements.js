...
import { applyCoachFix, reviewRequirements, summarizeReview, } from '../../core/requirementsCoach.js';
import { parseChecklist, renderChecklistReport, verifyChecklist, } from '../../core/checklist.js';
import { renderSpecReview, reviewSpec } from '../../core/specReview.js';
...
const heading = (title) => formatHeading(colors.bold(title));
...
const renderReview = (report, io, locale) => {
    ...
    for (const finding of report.findings)
        io.log(findingLine(finding));
    for (const skipped of report.skipped)
        io.log(colors.dim(`  skipped ${skipped}`));
    io.log('');
    io.log(colors.dim(`  ${messages.useFix} open-sdd requirements fix ${report.feature} --apply <id>`));
    io.log('');
};
const toEnvelopeFindings = (report) => {
    const errors = [];
    const warnings = [];
    for (const finding of report.findings) {
        const entry = {
            id: finding.id,
            message: `${finding.standardId} ${finding.file}:${finding.line}:${finding.column} — ${finding.message}${finding.remedies[0] ? ` → ${finding.remedies[0].text}` : finding.question ? ` ? ${finding.question}` : ''}`,
            artifact: finding.file,
        };
        if (finding.mayBlock && finding.severity === 'error')
            errors.push(entry);
        else
            warnings.push(entry);
    }
...
const loadArtifacts = async (cwd, sddDir, feature, deps) => {
    if (deps.readArtifacts)
        return deps.readArtifacts(feature);
...
const loadBaseArtifacts = async (cwd, sddDir, feature, base, deps) => {
    if (deps.readBaseArtifacts)
        return deps.readBaseArtifacts(feature, base);
    const artifacts = [];
    for (const name of ARTIFACT_NAMES) {
 ...
 */
const loadStandards = async (cwd, deps) => {
    if (deps.runner)
        return { entries: deps.entries ?? [], runner: deps.runner, engine: true };
    try {
        const specifier = ['..', 'core', 'standards.js'].join('/');
        const mod = (await import(/* @vite-ignore */ specifier));
        if (typeof mod.runStandard !== 'function')
...
const withApplicability = (bundle) => {
    if (!bundle.appliesTo)
...
const readApprovalState = async (cwd, sddDir, feature, deps) => {
    if (deps.approvals)
        return deps.approvals;
    const meta = await readSpecMetadata(cwd, feature, sddDir);
    const approvals = meta?.approvals;
    const accepted = (value) => typeof value === 'boolean' ? value : value && typeof value === 'object' && 'approved' in value ? Boolean(value.approved) : false;
    return {
        approved: Boolean(approvals && accepted(approvals.requirements)),
        ...(meta?.updated_at ? { approvedAt: meta.updated_at } : {}),
    };
};
const resolveLocaleOrRefuse = (argv, io) => {
    const requested = parseLangFlag([...argv]) ?? process.env.OPEN_SDD_LANG;
    if (requested && !normalizeLocale(requested)) {
        io.error(`unsupported locale "${requested}" · open-sdd requirements is translated for: ${SUPPORTED_LOCALES.join(', ')}`);
        return null;
    }
    return (normalizeLocale(requested) ?? DEFAULT_LOCALE);
};
const runReview = async (feature, flags, io, cwd, deps, locale) => {
...
const runChecklist = async (feature, flags, io, cwd, deps, locale) => {
    const sddDir = await resolveSddDir(cwd);
    ...
    if (!verify) {
        ...
        for (const problem of parsed.problems)
            io.log(colors.red(`  problem ${problem.itemId} line ${problem.line}: ${problem.reason}`));
        io.log('');
        return parsed.problems.length > 0 ? 1 : 0;
    }
    const artifacts = await loadArtifacts(cwd, sddDir, feature, deps);
    ...
    const writer = deps.writeFile ?? ((target, content) => writeFile(path.join(cwd, target), content, 'utf8'));
    try {
...
const runFix = async (feature, flags, io, cwd, deps, locale) => {
    const applyId = flagValue(flags, '--apply');
    if (!applyId)
        return usage(io, 'requirements fix needs --apply <finding-id>');
    const sddDir = await resolveSddDir(cwd);
    const artifacts = await loadArtifacts(cwd, sddDir, feature, deps);
    const bundle = await loadStandards(cwd, deps);
    ...
    if (!machineApplicable && !force) {
        ...
        for (const remedy of finding.remedies)
            io.log(`  ${remedy.grade}: ${remedy.text}`);
        if (finding.question)
            io.log(`  question: ${finding.question}`);
        return 1;
    }
...
const runChangeReview = async (feature, flags, io, cwd, deps, locale) => {
    const base = flagValue(flags, '--base');
    if (!base)
        return usage(io, 'review needs --base <ref>');
    const sddDir = await resolveSddDir(cwd);
    const artifacts = await loadArtifacts(cwd, sddDir, feature, deps);
 ...
 */
export const handleRequirementsCommand = async (argv, io, cwd = process.cwd(), deps = {}) => {
    ...
    if (head === 'review') {
        ...
        if (group === 'review' || group === 'checklist' || group === 'fix') {
            ...
            if (!feature || feature.startsWith('-'))
                return usage(io, 'requirements needs a <feature>');
            if (group === 'review')
        ...
        if (!feature || feature.startsWith('-'))
            return usage(io, 'review needs a <feature>');
    ...
    if (head === 'requirements') {
        ...
        if (group === 'review') {
            if (!feature || feature.startsWith('-'))
                return usage(io, 'requirements review needs a <feature>');
            return runReview(feature, flags, io, cwd, deps, locale);
        }
        if (group === 'checklist') {
            if (!feature || feature.startsWith('-'))
                return usage(io, 'requirements checklist needs a <feature>');
            return runChecklist(feature, flags, io, cwd, deps, locale);
        }
        if (group === 'fix') {
            if (!feature || feature.startsWith('-'))
                return usage(io, 'requirements fix needs a <feature>');
            return runFix(feature, flags, io, cwd, deps, locale);
        }
