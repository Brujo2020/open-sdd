...
import { buildContextPack, contextAbsences } from '../../core/contextPack.js';
import { handleBrownfieldCommand } from './brownfield.js';
import { handleGovernCommand } from './paper.js';
import { handleStatusCommand } from './status.js';
import { openI18n, rejectionsReport } from '../i18n.js';
const dim = (value) => colors.dim(value);
...
export const planTour = async (input) => {
    ...
    // ── 2. Borrador de constitución (la decisión humana) ────────────────────────────────────────
    {
        ...
        if (inForce > 0) {
            ...
            const run = await runCommand((io) => handleBrownfieldCommand(['constitution', absTarget, '--draft', ...(readOnly ? [] : ['--write'])], io, root));
    ...
    // ── 4. Panel de estado ──────────────────────────────────────────────────────────────────────
    {
        ...
        steps.push(haltedAt !== null
            ...
            : await (async () => {
                const run = await runCommand((io) => handleStatusCommand(feature ? [feature] : [], io, root));
                return {
                    id: 'status',
                    title: t('tour.status.title'),
                    commands,
...
export const renderTour = (report, io, translator) => {
    ...
    report.steps.forEach((step, index) => {
        ...
        if (step.executed) {
            ...
            if (step.output.length === 0)
                io.log(`      ${dim(t('tour.noOutput'))}`);
            else
                for (const line of indent(step.output))
                    io.log(`  ${line}`);
        }
        ...
        if (step.humanDecision) {
            ...
            if (step.handoverCommand)
                io.log(`    ${t('tour.handover', { command: step.handoverCommand })}`);
        }
    });
    io.log('');
    ...
    if (report.fallbackReport)
        io.log(`  ${colors.yellow(report.fallbackReport)}`);
    io.log('');
};
export const handleTourCommand = async (argv, io, cwd = process.cwd()) => {
 ...
 */
export const renderContextPack = (pack, io, translator) => {
    const t = translator.t;
    const mark = (label, present) => `${present ? colors.green('✓') : colors.yellow('✕')} ${colors.bold(label)}: ${present ? t('context.present') : t('context.absentLabel')}`;
    io.log('');
    io.log(formatHeading(t('context.heading', { feature: pack.feature ?? t('context.featureNone') })));
    io.log(`  ${pack.complete
    ...
    if (pack.constitution.present) {
        io.log(`      ${dim(`${t('context.principles', { n: pack.constitution.principlesInForce.length })}${pack.constitution.issues.length > 0 ? `; ${t('context.issues', { n: pack.constitution.issues.length })}` : ''}`)}`);
    }
    else {
    ...
    if (pack.spec.present) {
        for (const key of ['requirements', 'plan', 'tasks', 'delta']) {
            const piece = pack.spec[key];
            if (!piece)
                continue;
            io.log(`      ${piece.present ? colors.green('✓') : colors.yellow('✕')} ${key}${piece.present ? '' : ` — ${dim(t('context.reason', { reason: piece.reason ?? '—' }))}`}`);
        }
    }
    else {
    ...
    if (pack.rigor.present) {
        io.log(`      ${dim(`${pack.rigor.level ?? '—'}; ${t('context.gates', { gates: pack.rigor.activeGates.join(', ') || '(ninguno)' })}`)}`);
    }
    else {
    ...
    if (absences.length === 0)
        io.log(`  ${colors.green(t('context.absentNone'))}`);
    else
        for (const absence of absences)
            io.log(`  ${colors.yellow('·')} ${absence.key}: ${t('context.reason', { reason: absence.reason })}`);
    if (translator.hasFallbacks())
        io.log(`  ${colors.yellow(translator.report())}`);
    io.log('');
};
export const handleContextCommand = async (argv, io, cwd = process.cwd()) => {
    const json = argv.includes('--json');
    ...
    if (rejections)
        io.log(colors.yellow(`  ${rejections}`));
    return 0;
};
