...
const USAGE = 'Usage: open-sdd backup create [--out <dir>] [--force] [--json] | verify <archive> [--json] | restore <archive> [--write] [--only <ruta>]… [--json]';
...
const create = async (args, io, cwd) => {
...
const verify = async (args, io, cwd) => {
    const json = args.includes('--json');
...
const restore = async (args, io, cwd) => {
    const json = args.includes('--json');
    ...
    const rejections = result.skipped.filter((entry) => entry.reason.startsWith('escapa') || entry.reason.startsWith('fuera de la raíz'));
    ...
    for (const entry of warnings)
        io.log(colors.yellow(`  ! ${entry.path}: ${entry.reason}`));
    for (const entry of rejections)
        io.error(colors.red(`  ✗ ${entry.path}: ${entry.reason}`));
    if (!write)
        io.log(colors.dim('  Simulación: no se escribió nada. Añade --write para aplicar.'));
    return errors.length > 0 ? 1 : 0;
};
export const handleBackupCommand = async (args, io, cwd = process.cwd()) => {
    const sub = args[0];
    const rest = args.slice(1);
    if (sub === 'create')
        return create(rest, io, cwd);
    if (sub === 'verify')
        return verify(rest, io, cwd);
    if (sub === 'restore')
        return restore(rest, io, cwd);
