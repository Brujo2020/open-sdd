...
import { RECEIPT_FILE, RECEIPT_SCHEMA, applyUninstall, planFromReceipt, planUninstall, } from '../../core/receipt.js';
...
const renderPlan = (plan, command) => {
    ...
    for (const problem of plan.problems)
        lines.push(`  ${colors.yellow('!')} ${problem}`);
    if (!plan.hasReceipt) {
    ...
    if (remove.length === 0)
        lines.push(`    ${colors.dim('ninguno')}`);
    for (const entry of remove)
        lines.push(`    ${colors.green('-')} ${entry.path}  ${colors.dim(`(${entry.reason})`)}`);
    if (refused.length > 0) {
        lines.push(`  Se conservan por seguridad (${refused.length}):`);
        for (const entry of refused)
            lines.push(`    ${colors.yellow('!')} ${entry.path}  ${colors.dim(`(${entry.reason})`)}`);
    }
    if (missing.length > 0) {
        lines.push(`  Ya no existen (${missing.length}):`);
        for (const entry of missing)
            lines.push(`    ${colors.dim(`- ${entry.path}`)}`);
    }
    lines.push(`  ${plan.sddDir.willRemove ? colors.yellow('.sdd/') : '.sdd/'}  ${colors.dim(`(${plan.sddDir.reason})`)}`);
    return lines;
};
const readAlternateReceipt = async (cwd, from) => {
...
export const handleUninstallCommand = async (args, io, cwd = process.cwd()) => {
    ...
    const remove = plan.entries.filter((entry) => entry.state === 'remove');
    if (!write) {
        for (const line of renderPlan(plan, command))
            io.log(line);
    ...
    for (const rel of result.removed)
        io.log(`  ${colors.green('-')} ${rel}`);
    for (const rel of result.pruned)
        io.log(`  ${colors.green('-')} ${rel}/  ${colors.dim('(quedó vacío)')}`);
    for (const failure of result.failed)
        io.error(`  ${colors.red('!')} ${failure.path}: ${failure.reason}`);
    io.log(`${command}: ${result.removed.length} eliminado(s)` +
