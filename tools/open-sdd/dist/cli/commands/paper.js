...
import { analyseEars, deltaStatementText, isEarsProposalApplicable, mergeEarsReports, renderEarsReport, } from '../../core/earsAssistant.js';
...
export const handleGovernCommand = async (args, io, cwd) => {
    ...
    if (args.includes('--advise-ears')) {
        ...
        const allFeatures = (await readdir(specsDir).catch(() => [])).filter((entry) => !entry.startsWith('.'));
        ...
        const detail = `${features.length} spec(s), ${report.total} requisito(s), ${report.conforming} conforme(s): ` +
        ...
        for (const entry of perSpec) {
            io.log(`  ${entry.feature.padEnd(22)} ${String(entry.report.total).padStart(4)} req · ${String(entry.report.conforming).padStart(4)} conforme(s) · ${entry.report.suggestions.length} sugerencia(s)`);
        }
        if (report.suggestions.length > 0) {
            ...
            io.log('');
            io.log(dim(`  Detalle con propuestas y ejemplos: open-sdd brownfield requirements <feature> --suggest (${report.suggestions.length} sugerencia(s)).`));
        }
        ...
        for (const note of notAnalysed)
            io.log(`  ${colors.yellow('!')} ${note}`);
        io.log('');
        return errorSuggestions.length > 0 ? 1 : 0;
    }
    if (sub === 'invariants') {
