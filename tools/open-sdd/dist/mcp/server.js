...
import { RPC_ERROR_CODES, RpcFault, JsonRpcPeer, asRecord, buildInitializeResult, } from './protocol.js';
import { callTool, listResources, listToolDescriptors, readResource } from './tools.js';
...
const buildHandlers = (context, control) => ({
    initialize: (request) => {
...
const buildNotifications = (control) => ({
 ...
 */
export const runMcpServer = async (options) => {
    ...
    const context = { cwd: root, sddDir: await resolveSddDir(root) };
    return await new Promise((resolve) => {
        ...
        const onData = (chunk) => {
            ...
            queue = queue
                ...
                .catch((error) => log(`error procesando una línea: ${error instanceof Error ? error.message : String(error)}`));
        };
        const onEnd = () => {
