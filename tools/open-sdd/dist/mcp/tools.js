...
const requireString = (args, key, tool) => {
...
const optionalString = (args, key) => {
    ...
    if (typeof value !== 'string')
        throw new Error(`El argumento "${key}" debe ser una cadena.`);
...
const optionalStringArray = (args, key) => {
    ...
    if (!Array.isArray(value))
        throw new Error(`El argumento "${key}" debe ser una lista de cadenas.`);
    return value
        .filter((entry) => typeof entry === 'string')
        .map((entry) => entry.trim())
        .filter((entry) => entry.length > 0);
};
const optionalNumber = (args, key) => {
    const value = args[key];
    if (value === undefined || value === null)
        return undefined;
...
const optionalBoolean = (args, key) => {
    ...
    if (typeof value !== 'boolean')
        throw new Error(`El argumento "${key}" debe ser booleano.`);
    return value;
};
const optionalEnum = (args, key, allowed) => {
    const value = args[key];
    if (value === undefined || value === null)
        return undefined;
...
const relativeSpecDir = (context, feature) => path.join(context.sddDir, 'specs', feature);
const readDeltaIfPresent = async (context, feature) => {
...
const TOOL_DEFINITIONS = [
    {
        ...
        description: 'Estado completo del proyecto en un solo objeto: constitución, specs (tríada, trazabilidad, evidencia), delta, contratos, pivote constitucional y rigor, con la siguiente acción concreta. Es la primera llamada que un host debería hacer para saber dónde está.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Valida el contrato del cambio (delta.md): identificadores REQ-<ÁREA>-<NNN> propios de la delta, enunciados en forma EARS, objetivos, previous/contracts en modificaciones y eliminaciones, recuento por tipo, progreso del estrangulamiento y trazabilidad delta→tareas.',
        inputSchema: {
            type: 'object',
            properties: {
        ...
        handler: async (args, context) => {
            ...
            const raw = inlineDelta ?? (await readTextIfPresent(path.join(context.cwd, relativeSpecDir(context, feature), 'delta.md')));
            if (raw === null) {
                return {
            ...
            const tasksRaw = optionalString(args, 'tasks') ??
    ...
    },
    {
        ...
        description: 'Extrae el conjunto de contratos de ejecución del cambio: qué pruebas descubiertas protegen cada fichero cambiado, qué contratos declara la delta y qué cambios quedan SIN cobertura. Un run verde con un contrato declarado ausente no es un aprobado.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Analiza el radio de impacto de un cambio: ficheros/módulos dependientes, cambios incompatibles (símbolos exportados retirados), puntos de integración, migraciones y su rollback y superficie de API pública, con el tamaño del blast radius.',
        inputSchema: {
            type: 'object',
            properties: {
                changedFiles: {
    ...
    },
    {
        ...
        description: 'Busca símbolos ya existentes que podrían reutilizarse antes de crear código nuevo. Es la llamada que un agente debe hacer ANTES de escribir un fichero: si hay candidato, crear sería duplicar. Devuelve similitud, fichero y línea.',
        inputSchema: {
            type: 'object',
            properties: {
                symbols: {
    ...
    },
    {
        ...
        description: 'Mapa de módulos observados del repositorio: ruta, nombre, ficheros que posee, responsabilidades derivadas de evidencia, dependencias y directorios de test.',
        inputSchema: {
            type: 'object',
            properties: {
                sddDir: { type: 'string', description: 'Directorio SDD (por defecto .sdd).' },
    ...
    },
    {
        ...
        description: 'Plan de arranque para un repositorio existente: módulos, artefactos que se crearían/regenerarían/conservarían (constitución, inteligencia del código, semilla de delta) y los pasos ejecutables en orden. No escribe nada.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Nivel de rigor declarado en .sdd/settings/rigor.json, gates que activa y evaluación del repositorio contra ese nivel (constitución, tríada, delta, trazabilidad, evidencia, drift, contratos, regeneración). Incluye la escalera completa de niveles.',
        inputSchema: {
            type: 'object',
            properties: {
        ...
        handler: async (args, context) => {
            ...
            const level = optionalEnum(args, 'level', ['spec-first', 'spec-anchored', 'spec-as-source']) ?? settings.level;
    ...
    },
    {
        ...
        description: 'Ejecuta la cadena Zero-Trust sobre la feature indicada (por defecto los gates que activa el rigor declarado) y devuelve el veredicto por gate con su autoridad. `passed` es falso si algún gate falla.',
        inputSchema: {
            type: 'object',
            properties: {
                gates: {
        ...
        handler: async (args, context) => {
            ...
            const regime = optionalEnum(args, 'regime', ['flexible', 'strict']) ?? 'flexible';
            const feature = optionalString(args, 'feature') ?? (await listSpecs(context.cwd, context.sddDir))[0] ?? 'sin-feature';
            const changedFiles = optionalStringArray(args, 'changedFiles') ?? getModifiedFiles(context.cwd);
    ...
    },
    {
        ...
        description: 'Devuelve en UN objeto todo lo que un host debe inyectar antes de editar: la constitución (texto + principios en vigor), la especificación aplicable (requirements/plan/tasks/delta), el mapa de módulos y el rigor declarado con sus gates activos. Cada pieza declara explícitamente `present: false` y `reason` cuando falta, en lugar de omitir la clave.',
        inputSchema: {
            type: 'object',
            properties: {
 ...
 */
const assessRigorSafely = async (context, input) => assessRigor(context.cwd, {
    level: input.level,
...
const TOOL_INDEX = new Map(TOOL_DEFINITIONS.map((tool) => [tool.name, tool]));
...
export const listToolDescriptors = () => TOOL_DEFINITIONS.map((tool) => ({
    name: tool.name,
    title: tool.title,
 ...
 */
export const callTool = async (name, args, context) => {
    const tool = TOOL_INDEX.get(name);
    if (!tool) {
        throw new RpcFault(RPC_ERROR_CODES.invalidParams, `Herramienta desconocida: "${name}". Herramientas disponibles: ${TOOL_NAMES.join(', ')}.`);
    }
    try {
...
const SPEC_RESOURCE_FILES = ['requirements.md', 'plan.md', 'tasks.md', 'delta.md'];
const SPEC_RESOURCE_FILES_WITH_ALIAS = [...SPEC_RESOURCE_FILES, 'design.md'];
const specUri = (feature, file) => `sdd://specs/${encodeURIComponent(feature)}/${file}`;
/** Recursos que EXISTEN ahora mismo. Un recurso ausente no se lista; pedirlo da un error claro. */
export const listResources = async (context) => {
    const resources = [
        {
            uri: STATUS_URI,
            name: 'sdd-status',
    ...
    for (const feature of await listSpecs(context.cwd, context.sddDir)) {
        const specPath = relativeSpecDir(context, feature);
        const names = await readdir(path.join(context.cwd, specPath)).catch(() => []);
        const files = SPEC_RESOURCE_FILES.filter((file) => names.includes(file));
        if (!names.includes('plan.md') && names.includes('design.md'))
...
const resourceNotFound = (uri, detail) => new RpcFault(RPC_ERROR_CODES.resourceNotFound, `Recurso no presente: ${uri} — ${detail}`);
/** Contenido de una pieza: si existe pero está vacía, se dice; nunca se confunde con ausente. */
const textContent = (uri, mimeType, text) => text.trim().length > 0
    ? { uri, mimeType, text }
 ...
 */
export const readResource = async (uri, context) => {
    if (uri === STATUS_URI) {
    ...
    if (uri === CONSTITUTION_URI) {
        ...
        throw resourceNotFound(uri, `no existe ninguna de las rutas ${constitutionCandidates(context.sddDir).join(' ni ')} (la ausencia no se devuelve como cadena vacía)`);
    }
    ...
    if (specMatch) {
        ...
        const text = await readTextIfPresent(path.join(context.cwd, rel));
        if (text === null)
            throw resourceNotFound(uri, `no existe ${rel}`);
        return textContent(uri, 'text/markdown', text);
    }
...
export const findTool = (name) => TOOL_INDEX.get(name);
