 ...
 */
export const extractExportNames = (content) => {
    const names = new Set();
...
const walkSources = async (cwd, dirs) => {
    const files = [];
    const unreadable = [];
    let truncated = false;
    const walk = async (rel, depth) => {
        ...
        try {
            ...
            unreadable.push({ path: rel, reason: err.message });
            return;
        }
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    unreadable.push({ path: norm(child), reason: err.message });
                }
            }
        }
    };
    for (const dir of dirs)
        await walk(norm(dir), 0);
 ...
 */
const resolveSpecifier = (importer, specifier, canonFiles) => {
...
const buildDependencyGraph = (files) => {
    ...
    for (const file of files) {
        ...
        for (const spec of extractImportSpecifiers(file.content)) {
            for (const target of resolveSpecifier(file.path, spec, canonFiles))
                resolved.add(target);
        }
 ...
 */
const walkImporters = (graph, seeds, maxDepth) => {
    ...
    for (let depth = 0; depth < maxDepth && frontier.length > 0; depth += 1) {
        ...
        for (const target of frontier) {
            for (const importer of graph.importers.get(target) ?? []) {
                if (seedSet.has(importer))
                    continue;
                const via = dependents.get(importer) ?? new Set();
                via.add(target);
                dependents.set(importer, via);
                if (!next.includes(importer))
                    next.push(importer);
            }
        }
        frontier = next;
...
// Migraciones
// ---------------------------------------------------------------------------------------------
const normalizeMigrationStem = (p) => path.posix
    .basename(stem(p))
    .toLowerCase()
...
// ---------------------------------------------------------------------------------------------
const isIdentifierTarget = (target) => /^[A-Za-z_$][\w$]*$/.test(target);
/** ¿Un objetivo declarado en la delta corresponde a este fichero cambiado? */
const targetMatchesFile = (file, target, exportsOfFile) => {
    const t = norm(target).trim();
    if (!t)
        return false;
    const f = norm(file);
    if (f === t || f.endsWith(`/${t}`) || canon(f) === canon(t))
        return true;
    if (f.split('/').includes(t))
        return true;
    if (stem(path.posix.basename(f)) === t)
        return true;
    if (isIdentifierTarget(t) && exportsOfFile.includes(t))
        return true;
    return false;
};
/**
...
// El análisis
// ---------------------------------------------------------------------------------------------
export const analyzeChangeImpact = async (input) => {
    const cwd = input.cwd;
    const maxDepth = input.maxDepth ?? 3;
    const findings = [];
    let complete = true;
    const incomplete = (message, artifacts) => {
        complete = false;
    ...
    try {
        ...
        incomplete(`No se pudo reconocer el proyecto en ${cwd} (${err.message}): el impacto se informa como no evaluado, no como pequeño.`, [cwd]);
    }
    // ── Grafo de dependencias ───────────────────────────────────────────────────────────────────
    const walk = sourceDirs.length > 0
        ? await walkSources(cwd, sourceDirs)
        : { files: [], unreadable: [], truncated: false };
    if (sourceDirs.length === 0) {
        incomplete('No se detectó ningún directorio de código fuente: no se puede recorrer el grafo de dependencias, así que el radio de impacto no es fiable.', [cwd]);
    }
    for (const bad of walk.unreadable) {
        incomplete(`No se pudo leer ${bad.path} (${bad.reason}): parte del grafo de dependencias queda sin inspeccionar.`, [bad.path]);
    }
    if (walk.truncated) {
        incomplete(`Se superó el límite de ${MAX_WALK_FILES} ficheros fuente: el recorrido no fue exhaustivo y el radio de impacto es un mínimo, no una medida.`, sourceDirs);
    }
    ...
    const display = (key) => graph.paths.get(key) ?? seedDisplay.get(key) ?? key;
    ...
    const contentByCanon = new Map();
    for (const file of walk.files)
        contentByCanon.set(canon(file.path), file.content);
    const readChanged = async (file) => {
        const cached = contentByCanon.get(canon(file));
        if (cached !== undefined)
            return { exists: true, content: cached };
        try {
    ...
    if (input.previousExports) {
        for (const previous of input.previousExports) {
            ...
            if (read.content === null && read.exists) {
                incomplete(`No se pudo leer ${file} para comparar sus exportaciones anteriores (${read.error ?? 'error de lectura'}): el veredicto de compatibilidad no se emite.`, [file]);
                continue;
            }
            ...
            for (const entry of input.delta.entries) {
                ...
                for (const target of entry.targets) {
                    for (const file of changedFiles) {
                        ...
                        if (targetMatchesFile(file, target, exportsForFile))
                            declaredApiChange.add(canon(file));
                    }
                }
            }
            for (const file of changedFiles) {
        ...
        for (const file of changedFiles) {
            ...
            if (!hasRollback) {
                ...
                if (read.content !== null)
                    hasRollback = INLINE_ROLLBACK.some((pattern) => pattern.test(read.content));
            }
    ...
    if (!complete) {
        lines.push('Análisis incompleto: hay directorios o ficheros que no se pudieron inspeccionar, así que el radio de impacto es un mínimo y no una medida. No se declara un aprobado sobre lo que no se inspeccionó.');
    }
    return {
...
const forecastKeywords = (description) => Array.from(new Set(description
    .toLowerCase()
    .split(/[^a-z0-9_]+/)
    .filter((token) => token.length >= 3 && !FORECAST_STOPWORDS.has(token))));
const mentions = (haystack, keyword) => haystack.toLowerCase().includes(keyword);
export const forecastImpact = async (input) => {
    const cwd = input.cwd;
    const maxDepth = input.maxDepth ?? 3;
    ...
    let sourceDirs = [];
    let testDirs = [];
    let sourceWalk = { files: [], unreadable: [], truncated: false };
    let testWalk = { files: [], unreadable: [], truncated: false };
    try {
        const project = await scanProject(cwd);
        sourceDirs = project.sourceDirs;
        ...
        if (!map.complete) {
            unknown.push(`El mapa de módulos quedó incompleto (${map.detail}): hay módulos cuyas dependencias no se inspeccionaron, así que la coincidencia y el radio son un mínimo, no una medida.`);
        }
        ...
        if (sourceWalk.truncated || testWalk.truncated) {
            unknown.push(`Se superó el límite de ficheros recorridos: el pronóstico no fue exhaustivo y el radio de impacto es un mínimo.`);
        }
        const ownedFiles = (module) => sourceWalk.files.filter((file) => module.owns.some((dir) => isInside(file.path, dir))).map((file) => file.path);
        ...
        unknown.push(`No se pudo reconocer el proyecto en ${cwd} (${error.message}): el pronóstico se informa como no evaluado, no como pequeño.`);
    }
    if (keywords.length === 0) {
        ...
        unknown.push('Ningún módulo coincide con la descripción: no se puede identificar qué parte del sistema cambia, así que el radio esperado es desconocido (no cero).');
    }
    const candidateFiles = Array.from(new Set(moduleEntries.flatMap((module) => module.matched.filter((entry) => SOURCE_EXT.test(entry))))).sort();
    if (moduleEntries.length > 0 && candidateFiles.length === 0) {
        unknown.push('Los módulos coinciden por su nombre o responsabilidades, pero ningún fichero concreto coincide: el radio esperado no se puede calcular sin inventar objetivos.');
    }
    // ── Radio esperado: grafo de dependencias + ficheros candidatos ─────────────────────────────
    let expectedBlastRadius = null;
    ...
    for (const file of testWalk.files) {
        ...
        if (parentModule)
            reasons.push(`vive en los tests del módulo "${parentModule.path}"`);
        const hitKeyword = keywords.find((keyword) => mentions(path.posix.basename(file.path), keyword));
        if (hitKeyword)
            reasons.push(`el nombre menciona "${hitKeyword}"`);
        ...
        if (reasons.length === 0) {
            ...
            if (keywordInContent)
                reasons.push(`su contenido menciona "${keywordInContent}"`);
        }
        if (reasons.length > 0)
            tests.push({ file: file.path, reasons });
    }
    tests.sort((a, b) => a.file.localeCompare(b.file));
    if (tests.length === 0) {
        unknown.push(testDirs.length === 0
    ...
    const reuseViolations = [];
    const symbols = (input.symbols ?? []).map((symbol) => symbol.trim()).filter(Boolean);
    if (symbols.length === 0) {
        unknown.push('No se declararon símbolos propuestos: la comprobación de reutilización primero NO se ejecutó, así que no se sabe si el cambio duplicaría algo existente.');
    }
    else {
        try {
            ...
            unknown.push(`No se pudo ejecutar la búsqueda de reutilización (${error.message}): no se sabe si los símbolos propuestos ya existen.`);
        }
    }
    const complete = unknown.length === 0;
    const radiusText = expectedBlastRadius === null
    ...
    const detail = `Pronóstico de impacto (nada escrito todavía): ${keywords.length} palabra(s) clave, ${moduleEntries.length}/${modulesConsidered} módulo(s) coincidente(s), ${candidateFiles.length} fichero(s) candidato(s), ${tests.length} test(s) que podrían protegerlo, ${reuseCandidates.length} candidato(s) de reutilización para ${symbols.length} símbolo(s) propuesto(s); ${radiusText}.${complete ? '' : ` ${unknown.length} incógnita(s) declarada(s).`}`;
    return {
        description,
        keywords,
