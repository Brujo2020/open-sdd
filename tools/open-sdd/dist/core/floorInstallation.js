...
const fileExists = async (p) => (await stat(p).catch(() => null)) !== null;
 ...
 * claim beyond that.
 */
const resolveHookPath = async (cwd) => {
...
export const detectInstalledFloor = async (cwd, options = {}) => {
    ...
    for (const entry of entries) {
        ...
        if (/pull_request/.test(content))
            ciGateMatrix = true;
    }
    const floorInstalled = commitHookShipped && commitHookInstalled && ciGateMatrix;
    const missing = [];
    if (!commitHookShipped)
        missing.push('hook no incluido en el repositorio');
    ...
    if (!ciGateMatrix)
        missing.push('ningún workflow corre la cadena en pull_request');
