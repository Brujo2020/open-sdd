    ...
    try {
        ...
        if (gates.length === 0) {
            ...
            if (gateReport.findings.length > 0 && selfAuthorized.length === gateReport.findings.length) {
                outcomes.push(unmeasured('gates', `todos los gates se auto-autorizaron (sensores no disponibles): ${gates.join(', ')}`));
            }
            else {
                gatesMeasured = true;
                gatesPassed = gateReport.passed && gateReport.unavailable.length === 0;
                const credit = gateReport.findings.filter((finding) => finding.outcome === 'pass' || finding.outcome === 'advisory').length;
                const failed = gateReport.findings.filter((finding) => finding.outcome === 'fail').length;
                outcomes.push(measured('gates', credit / gateReport.findings.length, `${credit}/${gateReport.findings.length} gate(s) acreditan el control · ${failed} fallo(s)` +
...
export const renderScoreFooter = (report) => {
    ...
    for (const component of report.components) {
        if (report.notMeasured.includes(component.label))
            continue;
