...
import { HOST_COMMAND_TEMPLATES, } from './commandTemplates.js';
import { HOST_INTEGRATIONS, MCP_SNIPPETS, } from './integrations.js';
import { registerLocalAgent } from '../agents/registry.js';
...
export const defaultLocalHostsPath = (env = process.env) => {
...
const argumentSyntaxOf = (value) => str(value);
const toPosix = (value) => value.split(path.sep).join('/');
/** A repo-relative directory, never escaping it: the installer already checks, this checks earlier. */
const safeDir = (value) => value.startsWith('.') && !value.includes('..') && !path.isAbsolute(value);
/**
 ...
 */
export const validateLocalHost = (row, taken) => {
    const id = str(row.id);
    if (!id)
        return { ok: false, id: '(sin id)', reason: 'la fila no declara `id`' };
    if (!/^[a-z0-9-]+$/.test(id)) {
    ...
    if (verified && !sourceArtifact) {
        return {
            ok: false,
            id,
            reason: 'declara `verified: true` sin `sourceArtifact`: el artefacto que aporta el equipo dueño es la unica evidencia que un anfitrion privado puede tener, y nombrarlo es obligatorio',
        };
    }
    const host = {};
    ...
    if (dir !== null) {
        ...
        host.command = {
            ...
            label,
            dir: toPosix(dir),
            fileName: (templateId) => fileName.split('<id>').join(templateId),
            format: 'markdown',
            invocation: (templateId) => invocation.split('<id>').join(templateId),
            ...
            ...(str(row.forkOf) ? { forkOf: str(row.forkOf) } : {}),
            verified,
    ...
    if (mcp) {
        ...
        const notes = Array.isArray(mcp.notes) ? mcp.notes.filter((n) => typeof n === 'string') : [];
        ...
        host.integration = {
            ...
            skills: {
                ...
                mode: str(row.skills?.mode) ?? 'prompt-file',
            },
            invocation: str(row.invocation) ?? '/sdd-<id>',
            mcp: {
                configPaths,
                snippetFormat: str(mcp.snippetFormat) ?? 'json',
                snippet,
                verified: mcpVerified,
                ...(str(mcp.docUrl) ? { docUrl: str(mcp.docUrl) } : {}),
            },
            detect: Array.isArray(row.detect) ? row.detect.filter((d) => typeof d === 'string') : [],
    ...
    if (agent) {
        ...
        const flags = Array.isArray(agent.aliasFlags)
            ? agent.aliasFlags.filter((f) => typeof f === 'string')
            : [`--${id}`];
        host.agent = {
            label,
...
export const parseLocalHosts = (raw) => {
    const issues = [];
    let parsed;
    try {
        ...
        return { rows: [], issues: [`el fichero no parsea como JSON (${error.message}): se ignora entero`] };
    }
    ...
    return { rows: parsed.hosts.filter(isRecord).map((entry) => entry), issues };
};
/** Append the validated rows to the shipped matrices and report what happened. */
export const installLocalHosts = (rows) => {
    const report = { path: null, loaded: [], skipped: [], issues: [] };
    const taken = new Set([
    ...
    for (const row of rows) {
        ...
        if (result.host.command)
            HOST_COMMAND_TEMPLATES.push(result.host.command);
        if (result.host.integration)
            HOST_INTEGRATIONS.push(result.host.integration);
 ...
 */
export const loadLocalHosts = (options = {}) => {
...
export const renderLocalHostsReport = (report) => {
    const lines = [];
