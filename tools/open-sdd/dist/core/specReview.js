...
import { classifyArtifact, countByCheck, parseRequirements, reviewRequirements, } from './requirementsCoach.js';
const MAX_DIFF_TOKENS = 900;
/** Word-level diff over a statement. Whitespace is a token, so the result renders as readable prose. */
export const wordDiff = (before, after) => {
...
export const renderWordDiff = (diff) => diff
    .map((segment) => {
    ...
    .replace(/\s+/g, ' ')
    .trim();
const requirementMap = (artifacts) => {
    const map = new Map();
    for (const artifact of artifacts) {
        if (classifyArtifact(artifact.file) !== 'requirements')
            continue;
...
const coveringTasks = (artifacts, requirementId) => {
    const out = [];
    for (const artifact of artifacts) {
        if (classifyArtifact(artifact.file) !== 'tasks')
            continue;
        for (const task of parseTasksMarkdown(artifact.text)) {
            if (!task.raw.includes(requirementId))
                continue;
...
const failingTestsFor = (contracts, requirementId) => {
    const out = [];
    for (const test of contracts.tests) {
        const covers = test.covers ?? (test.requirementId ? [test.requirementId] : []);
        if (!covers.includes(requirementId))
            continue;
        if (test.wouldFail === false)
            continue;
        out.push(`${test.id}${test.file ? ` (${test.file})` : ''}`);
    }
    return out;
};
const findingsInRange = (findings, file, line, endLine) => findings.filter((f) => f.file === file && f.line >= line && f.line <= endLine);
...
export const reviewSpec = (input) => {
    ...
    let unchanged = 0;
    let failingTests = 0;
    const isApproved = (id) => approvals.approved === true && (!approvals.approvedIds || approvals.approvedIds.includes(id));
    const buildRow = (id, change, statement, previous) => {
...
export const renderSpecReview = (result) => {
    const lines = [];
