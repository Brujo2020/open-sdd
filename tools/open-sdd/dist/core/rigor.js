...
export const RIGOR_REQUIREMENTS = {
...
export const rigorRequirements = (level) => {
...
export const RIGOR_LEVELS = [
    ...
    },
    {
        ...
        name: 'Spec-as-Source',
        definition: 'La spec ES la fuente: el código se deriva de ella y toda reparación se hace primero en la spec, con contratos declarados y regeneración como mecanismo.',
        requires: requiresOf('spec-as-source'),
        gatesActive: ['C1', 'C2', 'C3', 'C4', 'C5', 'C6'],
        evaluatorCheck: '¿Puede regenerarse el código desde la spec y se reparó el cambio en la spec antes que en el código? ¿Los contratos declarados son los que gobiernan el cambio?',
        missingArtifactPolicy: 'blocking',
    },
];
 ...
 */
export const DEFAULT_RIGOR_LEVEL = 'spec-first';
/**
 ...
 */
export const selectRigorLevel = (input) => {
    if (input.scopeKnown === false) {
        return {
            level: 'spec-first',
            reason: 'Alcance poco claro: exploration-first. No se puede especificar lo que no se conoce, así que la spec más ligera es la única honesta y probablemente se reescriba.',
        };
    }
    if (input.highConsequence === true) {
        return {
            level: 'spec-as-source',
            reason: 'Consecuencia alta o trazabilidad por regulador (autenticación, autorización, pagos, contratos entre equipos, modernización de legacy): la spec es la fuente y la regeneración es la reparación.',
        };
    }
    if (input.misreadingIsCheap === true) {
        return {
            level: 'spec-first',
            reason: 'Leer mal un requisito es barato: el coste de mantener la spec supera el coste del error que previene. Spec mínima y desechable.',
        };
    }
    ...
    if (input.brownfield === true) {
        return {
            level: 'spec-anchored',
            reason: 'Proyecto brownfield: el SDD no exige reescribir el legado, exige gobernar su evolución. Spec viva y trazabilidad de la delta (§9.4–§9.6).',
        };
    }
    return {
        level: 'spec-as-source',
        reason: 'Reversibilidad desconocida: sin señal de que el daño sea reversible se aplica el nivel conservador. Declara reversible=true para bajar a spec-anchored.',
    };
};
// ---------------------------------------------------------------------------------------------
 ...
 */
export const constitutionRequired = (level, brownfield) => {
...
const originRationale = (level, origin) => {
    const spec = RIGOR_BY_NAME.get(level);
 ...
 */
export const resolveRigorSettings = (parsed, fallbackLevel = DEFAULT_RIGOR_LEVEL) => {
    const declared = parsed ?? {};
    if (!isRigorLevel(fallbackLevel)) {
        throw new Error(`Nivel de rigor de reserva inválido: "${String(fallbackLevel)}". Admitidos: ${RIGOR_LEVELS.map((l) => l.level).join(', ')}.`);
    }
    if (declared.level !== undefined && !isRigorLevel(declared.level)) {
        throw new Error(`Nivel de rigor inválido: "${String(declared.level)}". Admitidos: ${RIGOR_LEVELS.map((l) => l.level).join(', ')}.`);
    }
    if (declared.rationale !== undefined && declared.rationale.trim().length === 0) {
        throw new Error('El nivel de rigor exige un rationale no vacío: una elección de rigor sin motivo declarado no es auditable.');
    }
    const level = declared.level ?? fallbackLevel;
    const origin = declared.level !== undefined ? 'declarado' : fallbackLevel !== DEFAULT_RIGOR_LEVEL ? 'fallback' : 'defecto';
    return {
        level,
 ...
 */
export const toSddRigorLevel = (mode) => {
    switch (mode) {
        case 'spec-as-source':
...
export const rigorRequires = (level, brownfield) => {
    if (!isRigorLevel(level))
        throw new Error(`Nivel de rigor desconocido: ${String(level)}`);
    ...
    if (!brownfield && requires.delta === 'required')
        requires.delta = 'not-required';
    return requires;
};
/** Read a file and distinguish «no existe» from «existe pero no se pudo leer». */
const readIfPresent = async (absPath) => {
    try {
 ...
 */
export const assessRigor = async (cwd, options) => {
    const spec = RIGOR_BY_NAME.get(options.level);
    if (!spec)
        throw new Error(`Nivel de rigor desconocido: ${String(options.level)}`);
    ...
    if (constitutionPath === null) {
        ...
        const severity = constitutionDemand === 'required' ? 'error' : 'warning';
        if (errors.length > 0) {
            findings.push({
                aspect: 'constitution',
                severity,
        ...
        if (inForce.length === 0) {
            findings.push({
                ...
                severity,
                artifact: constitutionPath,
                message: 'La constitución no tiene principios en vigor: sin principios citables ningún veredicto bloqueante puede citar autoridad.',
            });
        }
        else {
    ...
    if (specDirRel) {
        try {
            ...
            findings.push({
                ...
                artifact: specDirRel,
                message: `No se pudo leer ${specDirRel} (${err.message}): la tríada no se evalúa y no se da por buena.`,
            });
        }
    }
        ...
        if (delta) {
            ...
            if (planPath === null) {
                ...
                findings.push({
                    ...
                    severity: 'error',
                    artifact: planPath,
                    message: 'El nivel spec-as-source exige contratos declarados y el plan no declara ninguno (sección "Contracts" o campo "Contracts:"): sin ellos no hay oráculo de regresión.',
                });
            }
            else {
                findings.push({
    ...
    if (specDirRel && tasksRel && requires.traceability !== 'not-required') {
        if (brownfield && delta) {
            ...
            if (!tasksRead.exists) {
                ...
                const deltaTasks = parseTasksMarkdown(tasksRead.content).map((t) => ({
                    id: t.id,
                    raw: t.raw,
            ...
            try {
                ...
                findings.push({
                    ...
                    severity: 'info',
                    artifact: specDirRel,
                    message: `Trazabilidad no evaluable (${err.message}); no se da por buena.`,
                });
            }
        }
    }
    ...
    if (specDirRel && tasksRel && requires.evidence !== 'not-required') {
        const tasksRead = tasksContentCache !== null
        ...
        if (!isGitRepo(cwd)) {
            ...
            try {
                ...
                findings.push({
                    ...
                    severity: 'info',
                    artifact: specDirRel,
                    message: `Detección de drift no evaluable (${err.message}); no se da por buena.`,
                });
            }
        }
    }
    ...
    if (requires.regeneration === 'required') {
        ...
        findings.push({
            ...
            message: 'La regeneración como mecanismo de reparación no es verificable desde los artefactos de este repositorio: se declara como brecha declarada, no como aprobado.',
        });
    }
