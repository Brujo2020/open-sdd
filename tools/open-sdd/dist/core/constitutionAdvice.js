...
import { parseConstitution, parseEvidenceArtifact, principlesInForce, } from './constitution.js';
...
const readText = async (file) => readFile(file, 'utf8').catch(() => null);
const ageInDays = (iso, now) => {
...
const mentions = (text, token) => new RegExp(`(^|[^a-z0-9])${escapeRegExp(token)}([^a-z0-9]|$)`, 'i').test(text);
const normalizePath = (value) => value.trim().replace(/\\/g, '/').replace(/^\.\//, '');
/**
 ...
 */
const amendmentExample = (input) => {
    const lines = [`### ${input.id} — ${input.title}`, `- Level: ${input.level}`];
    if (input.cweReference)
        lines.push(`- CWE: ${input.cweReference}`);
    else
        lines.push(`- Threat: ${input.threat ?? 'envejecimiento de la constitución'}`);
...
const listFiles = async (root, dirs, limit = 800) => {
    const out = [];
...
const checkEvidenceExpired = (ctx, inForce) => {
    let pathReferences = 0;
    let unverifiable = 0;
    const unverifiablePrinciples = new Set();
    ...
    for (const principle of inForce) {
        ...
        for (const entry of principle.evidence ?? []) {
            ...
            pathReferences += 1;
            if (!artifact.resolvable)
                expired.push(trimmed);
        }
        if (expired.length === 0)
            continue;
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                restriction: 'Toda evidencia de un principio descriptivo debe resolver a un artefacto presente en el repositorio.',
                pattern: 'Sustituir la referencia caducada por la ruta vigente; si no existe sustituto, degradar el principio a borrador hasta re-anclarlo.',
                justification: 'Una prueba que ya no existe convierte un hecho observado en una afirmación no verificable y erosiona la autoridad del documento completo.',
            }),
            apply: 'open-sdd brownfield constitution . --write',
        });
    }
    if (withEvidence.length > 0) {
        ctx.checked.push(`evidencia de principios en vigor: ${pathReferences} referencia(s) con forma de ruta comprobada(s) contra el disco en ${withEvidence.length} principio(s)`);
    }
    else {
    ...
    if (unverifiable > 0) {
        ...
        ctx.notChecked.push(`evidencia no verificable (no tiene forma de ruta, no se cuenta como caducada): ${unverifiable} referencia(s) en ${Array.from(unverifiablePrinciples).join(', ')}`);
    }
};
// ---------------------------------------------------------------------------------------------
 ...
 */
const checkPracticeUnstated = async (ctx, inForce) => {
    ...
    if (project.sourceDirs.length > 0 && project.testDirs.length > 0 && project.modules.length > 0) {
            ...
            if (covered.length === project.modules.length && !TEST_TOPIC.test(principlesText)) {
                ctx.advice.push({
                    ...
                    example: amendmentExample({
                        ...
                        pattern: 'Añadir el test del módulo en el directorio de test y nombrarlo con el módulo, como ya hace todo el código observado.',
                        justification: 'Una práctica que el repositorio ya cumple al cien por cien es una decisión de ingeniería sin registrar; sin principio, el siguiente cambio puede abandonarla sin que nadie lo note.',
                    }),
                    apply: 'open-sdd govern constitution --draft --write',
                });
            }
            else if (covered.length < project.modules.length) {
                ctx.checked.push(`práctica "test por módulo": ${covered.length}/${project.modules.length} módulos nombrados en test (no es una práctica del 100%, no se aconseja)`);
            }
            else {
        ...
        ctx.notChecked.push('práctica "test por módulo": no se observaron a la vez directorios de código con submódulos y directorios de test');
    }
    ...
    if (manifestPaths.length >= 2) {
            ...
            for (const dependency of reported) {
                ctx.advice.push({
                    ...
                    example: amendmentExample({
                        ...
                        justification: 'Una dependencia presente en todos los manifiestos es una decisión transversal del proyecto; declararla permite auditar su retirada sin depender de la memoria del equipo.',
                    }),
                    apply: 'open-sdd govern constitution --draft --write',
                });
            }
            ctx.checked.push(`práctica "dependencia en el 100% de manifiestos": ${readable.length} manifiesto(s) comparados, ${universal.length} dependencia(s) universal(es) no enunciadas, ${reported.length} reportada(s)`);
        }
        else {
        ...
        ctx.notChecked.push('práctica "dependencia en el 100% de manifiestos": se necesitan al menos dos manifiestos package.json legibles');
    }
};
// ---------------------------------------------------------------------------------------------
 ...
 */
const checkAmendmentAged = (ctx, constitution) => {
    ...
    const dated = [];
    const undated = [];
    const consider = (id, title, iso, principleId) => {
        if (iso && !Number.isNaN(Date.parse(iso)))
            dated.push({ id, title, iso, ...(principleId ? { principleId } : {}) });
        else
            undated.push(principleId ?? id);
    };
    ...
    for (const principle of drafts) {
        ...
        if (record && undecided.some((a) => a.id === record.id))
    ...
    for (const item of dated) {
        ...
        if (ageDays === undefined || ageDays <= ctx.thresholdDays)
            continue;
        ...
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                pattern: 'Ratificarla con --promote y su plan de migración, o cerrarla como rechazada con motivo escrito; la ambigüedad no es un tercer estado.',
                justification: 'Una propuesta vieja sin decisión deja a los agentes sin saber si la regla gobierna, y el silencio se interpreta como permiso para ignorarla.',
            }),
            ...(canPromote ? { apply: `open-sdd govern constitution --promote ${item.id} --plan "<plan de migración>"` } : {}),
        });
    }
    ctx.checked.push(`enmiendas sin decisión: ${undecided.length} registro(s) y ${drafts.length} principio(s) en borrador; con fecha comprobable: ${dated.length}`);
    if (undated.length > 0) {
        ctx.notChecked.push(`antigüedad de enmiendas/borradores sin fecha registrada: ${Array.from(new Set(undated)).join(', ')} (el modelo no guarda una fecha de propuesta; sin dato no se inventa una edad)`);
    }
};
const collectViolationHits = (payload, source) => {
 ...
 */
const checkRecurringViolation = async (ctx, inForce) => {
    const existing = [];
    const hits = [];
    ...
    if (existing.length === 0) {
        ctx.notChecked.push(`violaciones recurrentes: no existe ningún histórico de recibos/claims/auditoría/adhesión (${VIOLATION_HISTORY_CANDIDATES.join(', ')})`);
        return;
    }
    ...
    for (const hit of relevant) {
        if (hit.at) {
            ...
            if (!Number.isNaN(at) && at < windowStart)
                continue; // outside the 90-day recency window
        }
    ...
    for (const [principleId, occurrences] of recurring) {
        ...
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                pattern: 'Exigir la comprobación del principio en la puerta de cambio y registrar cada relajación con actor, motivo y resultado posterior.',
                justification: 'Cuando el mismo principio se incumple dos o más veces, el fallo está en el control y no en la persona: el principio necesita una comprobación ejecutable o debe reescribirse.',
            }),
        });
    }
    ctx.checked.push(`violaciones recurrentes: ${existing.length} histórico(s) leído(s) (${existing.join(', ')}), ${relevant.length} referencia(s) a principios en vigor, ${recurring.size} principio(s) con al menos un episodio reciente`);
    if (ignored.length > 0) {
        ctx.notChecked.push(`violaciones recurrentes: ${ignored.length} registro(s) citan identificadores que no son principios en vigor (${Array.from(new Set(ignored.map((i) => i.principleId))).join(', ')})`);
    }
};
const TECH_FAMILIES = [
...
const tokensOf = (text) => new Set(text
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((token) => token.length >= 4 && !STOPWORDS.has(token)));
const principleText = (principle) => [principle.id, principle.title, principle.restriction, principle.pattern, principle.justification, (principle.evidence ?? []).join(' ')].join(' ');
/** Technology members a principle LOCKS: names a member and either is a MUST or says "only/never". */
const lockedMembers = (principle) => {
    const text = principleText(principle);
    const locks = [];
    for (const family of TECH_FAMILIES) {
        for (const member of family.members) {
            if (!mentions(text, member))
                continue;
...
const checkContradictoryPrinciples = (ctx, inForce) => {
                    ...
                    for (const b of rightMembers) {
                        ...
                        ctx.advice.push({
                            ...
                            example: amendmentExample({
                                ...
                                pattern: 'Retirar o degradar uno de los dos principios, o acotar su ámbito hasta que ambas restricciones puedan cumplirse en el mismo cambio.',
                                justification: 'Un agente que lee ambas reglas no puede obedecerlas: la contradicción garantiza que al menos un veredicto bloqueará trabajo correcto.',
                            }),
                        });
                    }
                }
    ...
    for (let i = 0; i < inForce.length; i += 1) {
        for (let j = i + 1; j < inForce.length; j += 1) {
            ...
            const sameTarget = (left.cweReference !== undefined && left.cweReference === right.cweReference) ||
            ...
            if (!pairLevels.includes('MUST'))
                continue;
            if (!pairLevels.every((level) => level === 'MUST' || level === 'SHOULD'))
                continue;
            ...
            ctx.advice.push({
                ...
                example: amendmentExample({
                    ...
                    pattern: 'Elegir una polaridad, degradar la otra a SHOULD con su excepción declarada, o reescribir el objetivo para que cada principio cubra un caso distinto.',
                    justification: 'Una regla que se exige y se prohíbe sobre el mismo objetivo convierte el veredicto en una moneda al aire y la constitución en un documento que no puede obedecerse.',
                }),
            });
        }
    }
    ctx.checked.push(`contradicciones: ${inForce.length} principio(s) en vigor comparados por familia tecnológica (${TECH_FAMILIES.length} familia(s)) y por objetivo compartido`);
};
// ---------------------------------------------------------------------------------------------
 ...
 */
const isBoundaryToken = (value) => /^[a-z][a-z0-9_-]{1,}$/i.test(value.trim()) && !value.includes('/') && !value.includes('.') && !/\s/.test(value);
const checkBoundaryAged = async (ctx, inForce) => {
    const project = await scanProject(ctx.root);
    const discovered = Array.from(new Set((project.modules.length > 0 ? project.modules : project.sourceDirs).map(normalizePath))).filter(Boolean);
    const boundaryPrinciples = inForce.filter((p) => /\b(frontera|fronteras|boundary|l[ií]mite|l[ií]mites|m[oó]dulo|m[oó]dulos|module|modules|servicio|servicios)\b/i.test([p.id, p.title, p.restriction, p.pattern].join(' ')));
    const inForceText = inForce.map((p) => principleText(p)).join('\n');
    if (discovered.length === 0 && boundaryPrinciples.length === 0) {
        ctx.notChecked.push('fronteras: el mapa de módulos (scanProject) no observó módulos ni directorios de código, y ningún principio en vigor declara fronteras');
        return;
    }
    const declared = new Set();
    for (const principle of boundaryPrinciples) {
        for (const entry of principle.evidence ?? []) {
            if (isBoundaryToken(entry))
                declared.add(normalizePath(entry));
        }
    }
    const missing = discovered.filter((boundary) => !declared.has(boundary) && !mentions(inForceText, boundary));
    const vanished = Array.from(declared).filter((boundary) => !discovered.includes(boundary) && !existsSync(path.join(ctx.root, boundary)));
    if (boundaryPrinciples.length === 0 && discovered.length > 0) {
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                pattern: 'Añadir estas fronteras a la evidencia del principio de límites, como hace la constitución descriptiva cuando registra el mapa de módulos.',
                justification: 'Una frontera que existe en el código y no en la constitución convierte cada cruce en una refactorización involuntaria sin autoridad que la juzgue.',
            }),
            apply: 'open-sdd brownfield constitution . --write',
        });
    }
    for (const boundary of missing) {
        if (boundaryPrinciples.length === 0)
            break; // already covered by the aggregate finding above
        ctx.advice.push({
            kind: 'boundary-aged',
            severity: 'info',
            ...
            example: amendmentExample({
                ...
                justification: 'Una frontera que existe en el código y no en la constitución deja el límite sin autoridad citable, y el primer cruce silencioso nadie lo nota.',
            }),
            apply: 'open-sdd brownfield constitution . --write',
        });
    }
    for (const boundary of vanished) {
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                justification: 'Una frontera fantasma hace que los agentes busquen límites que no existen y que los veredictos citen una arquitectura que ya nadie mantiene.',
            }),
            apply: 'open-sdd brownfield constitution . --write',
        });
    }
    ctx.checked.push(`fronteras: ${discovered.length} observada(s) por el mapa de módulos (scanProject, la misma fuente que cita C-BOUNDARIES) frente a ${declared.size} nombrada(s) en ${boundaryPrinciples.length} principio(s) de límites`);
};
// ---------------------------------------------------------------------------------------------
// Signal 7 — stack-drifted
// ---------------------------------------------------------------------------------------------
const MANAGER_FAMILY = TECH_FAMILIES[0];
const LANGUAGE_FAMILY = { name: 'lenguaje', members: ['typescript', 'javascript', 'python', 'go', 'rust'] };
const checkStackDrifted = (ctx, inForce, project) => {
    const managerStatements = new Map(); // member -> principle ids
    const languageStatements = new Map();
    for (const principle of inForce) {
        const text = principleText(principle);
        if (principle.level !== 'MUST' && !LOCK_CUE.test(text))
            continue;
    ...
    if (managerStatements.size === 0) {
        ...
        if (MANAGER_FAMILY.members.includes(observedManager) && observedManager !== stated) {
            ...
            ctx.advice.push({
                ...
                example: amendmentExample({
                    ...
                    justification: 'Un principio que declara un gestor distinto del que el repositorio usa convierte el stack en ficción y enseña a los agentes a desconfiar de la constitución.',
                }),
                apply: 'open-sdd brownfield constitution . --write',
            });
        }
        else if (!MANAGER_FAMILY.members.includes(observedManager) && observedManager !== stated) {
            ctx.notChecked.push(`stack: el principio declara ${stated} y el gestor observado (${observedManager}) no pertenece a la misma familia comparable`);
        }
        else {
        ...
        ctx.checked.push(`stack (gestor de paquetes): ${managerStatements.size} gestores declarados (${Array.from(managerStatements.keys()).join(', ')}); el conflicto lo reporta contradictory-principles`);
    }
    else {
    ...
    if (languageStatements.size === 0) {
        ...
        if (stated !== observedLanguage.toLowerCase()) {
            ...
            ctx.advice.push({
                ...
                example: amendmentExample({
                    ...
                    justification: 'Un lenguaje declarado que ya no es el del repositorio hace que todo principio derivado de él se lea como una aspiración incumplida, no como un hecho.',
                }),
                apply: 'open-sdd brownfield constitution . --write',
            });
        }
        ...
        ctx.checked.push(`stack (lenguaje): ${languageStatements.size} lenguajes declarados (${Array.from(languageStatements.keys()).join(', ')})`);
    }
};
const readWaivers = async (ctx) => {
    ...
    if (payload === null)
        return null;
    const root = asRecord(payload);
    const list = Array.isArray(payload) ? payload : Array.isArray(root?.allow) ? root.allow : [];
    const waivers = [];
    for (const entry of list) {
        const record = asRecord(entry);
        if (!record)
        ...
        if (!expires)
            continue; // owner/expires are optional: an entry without a date is skipped
        waivers.push({
...
const checkWaiverExpiring = async (ctx) => {
    const read = await readWaivers(ctx);
    if (read === null) {
        ctx.notChecked.push(`exenciones con caducidad: no hay ${ctx.sddDir}/settings/security-allowlist.json legible`);
        return;
    }
    if (read.waivers.length === 0) {
        ctx.notChecked.push(`exenciones con caducidad: ninguna de las ${read.total} entrada(s) declara "expires" (owner/expires son opcionales; sin fecha no se puede juzgar la caducidad)`);
        return;
    }
    for (const waiver of read.waivers) {
        ...
        if (daysLeft > WAIVER_WINDOW_DAYS)
            continue;
        ...
        ctx.advice.push({
            ...
            example: amendmentExample({
                ...
                pattern: 'Revisar la entrada en .sdd/settings/security-allowlist.json, actualizar reason y expires con el responsable vigente, o eliminar la entrada y corregir el hallazgo que cubría.',
                justification: 'Una excepción que caduca sin revisión se convierte en una supresión permanente: el control sigue apagado y nadie recuerda por qué se apagó.',
            }),
        });
    }
    ctx.checked.push(`exenciones con caducidad: ${read.waivers.length} de ${read.total} entrada(s) declaran expires (ventana de ${WAIVER_WINDOW_DAYS} días a ${ctx.now.toISOString()})`);
};
const readConstitution = async (root, sddDir) => {
    const candidates = [
...
export const adviseConstitution = async (cwd, options = {}) => {
    ...
    ctx.checked.push(`constitución: ${constitution.principles.length} principio(s) (${inForce.length} en vigor, ${constitution.principles.length - inForce.length} fuera de vigor), ${constitution.amendments.length} enmienda(s), leída de ${normalizePath(path.relative(root, loaded.path))}`);
    if (inForce.length === 0) {
        ctx.notChecked.push('principios en vigor: la constitución no tiene ningún principio en vigor; solo un borrador viejo sin ratificar puede reportarse como amendment-aged');
    }
    ...
    return {
        ...
        detail: `Consejo de constitución: ${ctx.advice.length} hallazgo(s) (${errors} error, ${warnings} aviso, ${infos} info) ` +
...
export const renderAdvice = (report) => {
    const lines = [];
    lines.push(`Consejo de constitución — ${report.project}`);
    lines.push(`${report.advice.length} consejo(s) · ${report.complete ? 'análisis completo' : 'INCOMPLETO (ver «no comprobado»)'} · umbral de enmienda y ventana de exención aplicados`);
    lines.push('');
    ...
    for (const advice of report.advice) {
        ...
        if (advice.evidence.length > 0)
            lines.push(`      evidencia: ${advice.evidence[0]}`);
        if (advice.apply)
            lines.push(`      aplicar: ${advice.apply}`);
    }
    lines.push('');
