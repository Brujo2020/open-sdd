...
import { analyseEars, deltaStatementText } from './earsAssistant.js';
...
export const CONSTITUTION_RATIFY_COMMAND = 'open-sdd govern constitution --ratify --by "<nombre>" --rationale "<texto>" --write';
/**
...
export const resetAssistLedger = () => {
 ...
 * deduplica.
 */
const keyOf = (suggestion) => `${suggestion.trigger}|${suggestion.headline}|${suggestion.proposal}|${suggestion.action}`;
// ---------------------------------------------------------------------------------------------
...
const isEarsFinding = (code) => {
    const upper = code.toUpperCase();
    if (EARS_GENERIC_CODES.has(upper))
        return true;
    if (EARS_NONTESTABLE_CODES.has(upper))
        return true;
    return (upper === 'COMPOUND_REQUIREMENT' ||
        upper === 'VAGUE_TERM' ||
...
const checkConstitution = async (root, sddDir) => {
...
const evidenceLine = (project, detected) => {
    const stack = [
 ...
 */
const constitutionCollaboration = async (root, state) => {
    ...
    if (deferred.length > 0) {
        questions.push(`2) ¿Se adopta "${deferred[0]}"? Propuesta por defecto: NO todavía — el código NO muestra la práctica, ` +
    ...
    const headline = state.kind === 'missing'
    ...
    return {
        ...
        headline,
        proposal,
        example: 'Flujo ya implementado en este repositorio: tools/open-sdd/src/core/constitutionDraft.ts (borrador con evidencia y puerta humana `ratifyDraft`). ' +
...
// ---------------------------------------------------------------------------------------------
const earsTriggerOf = (code) => EARS_NONTESTABLE_CODES.has(code.toUpperCase()) ? 'ears-nontestable' : 'ears-ambiguity';
const toEarsSuggestion = (suggestion, source, action) => ({
...
const placeholderSuggestion = (cited, feature, taskId) => {
    ...
    if (!feature) {
        ...
        if (wantsEars) {
            if (requirementsRaw === null && deltaRaw === null) {
                notChecked.push(`EARS: no existe ${requirementsRel} ni ${deltaRel} para "${feature}": no hay requisitos que analizar`);
            }
            else {
                const action = `open-sdd brownfield requirements ${feature} --suggest`;
                const requirementsReport = requirementsRaw !== null ? analyseEars(requirementsRaw, { source: requirementsRel }) : null;
                const deltaReport = deltaRaw !== null
                    ? (() => {
                        const deltaText = deltaStatementText(deltaRaw, requirementsReport?.statements ?? []);
                        return deltaText.statements > 0
                            ? { report: analyseEars(deltaText.text, { source: deltaRel }), origin: 'delta' }
                            : null;
                    })()
                    : null;
                const tagged = [
                ...
                for (const entry of tagged) {
                    ...
                    const include = findings === undefined ||
                        findingCodes.has(code) ||
                        (entry.origin === 'delta' && [...findingCodes].some((finding) => EARS_GENERIC_CODES.has(finding)));
                    if (include)
                        collected.push(toEarsSuggestion(entry.suggestion, entry.source, action));
                }
            }
        }
        else {
            notChecked.push(`EARS: los hallazgos de esta ejecución no incluyen ningún código EARS (${findingCodes.size} código(s) ajenos): no se re-analizan las specs`);
        }
        ...
        if (wantsPlaceholders) {
            ...
            if (findings !== undefined) {
                for (const finding of findings) {
                    if (finding.code.toUpperCase() !== 'UNFILLED_REQUIREMENT_PLACEHOLDER')
                        continue;
                    const token = finding.artifact?.match(TEMPLATE_PLACEHOLDER)?.[0];
                    if (token)
                        cited.push({ cited: token });
                }
            }
...
export const renderAssist = (suggestions, opts = {}) => {
