...
import { getSpecStatus, listSpecs, parseRequirementsMarkdown, parseTasksMarkdown, readSpecMetadata, resolveSddDir, } from './specManager.js';
...
const normalizedStartsWith = (file, prefix) => {
 ...
 */
const taskDeclaresRequirement = (task, req) => {
    ...
    if (declared) {
        ...
        if (ids.some((id) => id === req.id.toLowerCase() || id === numeric))
            return true;
    }
    return (task.raw.toLowerCase().includes(req.id.toLowerCase()) || task.raw.includes(req.title));
};
export const auditFeature = async (cwd, feature, options = {}) => {
...
export const auditAll = async (cwd, options = {}) => {
    const sddDir = options.sddDir ?? (await resolveSddDir(cwd));
    ...
    const results = await Promise.all(specs.map((spec) => auditFeature(cwd, spec, { ...options, sddDir })));
