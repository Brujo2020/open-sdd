...
export const ADAPTED_TEMPLATE_FILE = {
...
const readManifests = async (cwd, modules) => {
    const scripts = [];
    const dependencies = [];
    const read = async (rel, scope) => {
        ...
        for (const field of ['dependencies', 'devDependencies', 'peerDependencies']) {
            ...
            for (const name of Object.keys(declared))
                dependencies.push({ manifest: rel, field, name });
        }
    };
...
const scriptRank = (script) => {
    if (/watch/i.test(script.name))
        return null;
    if (/--watch\b|\s-w\b/.test(script.command))
        return null;
    if (script.name === 'test')
        return 0;
    if (script.name === 'tests')
        return 1;
    if (/^test:/.test(script.name))
        return 2;
    if (/(^|:)tests?(:|$)/.test(script.name))
        return 3;
    return null;
};
const deriveInvocation = (packageManager, scope, name) => {
...
const resolveRunner = async (project, scripts, dependencies) => {
    const discarded = [];
    const candidates = [];
    ...
    const labelFromCommand = (command) => RUNNER_LABELS.find(([pattern]) => pattern.test(command))?.[1];
    ...
    if (framework) {
        const dependency = RUNNER_DEPENDENCIES[framework]?.find((name) => dependencies.some((hit) => hit.name === name));
...
const ev = (label, evidence) => `${label} ← ${evidence}`;
/** A boundary that covers nothing the code shows is not a boundary; it is a guess. */
const observedBoundaryHint = (facts) => facts.boundaries.boundaries.map((boundary) => `\`${boundary}\``).join(', ');
const boundaryForModule = (module) => {
    const sourceOwns = module.owns.filter((own) => !module.testDirs.includes(own) && !/\.(json|xml|toml|mod|py|sln|slnx)$/i.test(own));
    const list = sourceOwns.length > 0 ? sourceOwns : [module.path];
    return list.map((own) => `\`${own}\``).join(', ');
};
const testDirForModule = (facts, module) => module.testDirs[0] ?? (module.path === '.' ? facts.tests[0] : undefined);
/**
 ...
 */
const sourceDirForModule = (facts, module) => {
    const owned = module.owns.find((own) => !module.testDirs.includes(own) && facts.sources.includes(own));
    return owned ?? '{{DIRECTORIO_DE_CÓDIGO}}';
};
const commonUnobserved = (facts) => {
    const items = [
    ...
    if (!facts.runner.hasCycle) {
        items.push('el ciclo rojo-verde y el comando de test: no se emiten porque ningún manifiesto declara un runner ejecutable (ver la sección «Disciplina rojo-verde»).');
    }
...
// requirements
// ---------------------------------------------------------------------------------------------
const buildRequirementsTemplate = (facts) => {
    ...
    lines.push(`- directorios de código: ${facts.sources.length > 0 ? facts.sources.map((dir) => `\`${dir}\``).join(', ') : 'ninguno observado'}`);
    ...
    lines.push(`- directorios de test: ${facts.tests.length > 0 ? facts.tests.map((dir) => `\`${dir}\``).join(', ') : 'ninguno observado'}`);
    if (facts.tests.length > 0) {
    ...
    lines.push(`- Targets: \`${facts.sources[0] ?? '{{DIRECTORIO_DE_CÓDIGO}}'}/{{FICHERO}}\``);
    if (runner.hasCycle) {
        lines.push(`- Contracts: \`${facts.tests[0] ?? '{{DIRECTORIO_DE_TEST}}'}/{{FICHERO}}.test.{{EXT}}\` — la prueba que debe existir para este enunciado`);
    }
    ...
    for (const module of facts.modules) {
        lines.push(`- \`${module.path}\` — ${module.name}: posee ${module.owns.length > 0 ? module.owns.join(', ') : 'nada observado'}; tests: ${module.testDirs.length > 0 ? module.testDirs.join(', ') : 'no observados'}`);
    }
    lines.push('');
    ...
    return {
        ...
        unchanged,
        rationale: 'Los requisitos nombran el lenguaje, el runner y los directorios de código y de test REALES, y cada requisito declara sus objetivos dentro de las fronteras observadas: sin eso el análisis de impacto y la trazabilidad se hacen sobre rutas inventadas.',
    };
};
// ---------------------------------------------------------------------------------------------
// plan
// ---------------------------------------------------------------------------------------------
const buildPlanTemplate = (facts) => {
    const { project, runner } = facts;
    const evidence = [];
    const unchanged = [
    ...
    lines.push(`| Tests | ${runner.label ? `${runner.label}${runner.command ? ` — \`${runner.command}\`` : ' (sin comando declarado)'}` : 'no observado'} | manifiestos |`);
    ...
    evidence.push(ev('«módulos, responsabilidades y dependsOn»', `buildModuleMap(cwd) → ${facts.modules.map((module) => module.path).join(', ')}`));
    ...
    lines.push(`- Fuera de frontera: ${facts.modules.length > 1 ? facts.modules.map((module) => `\`${module.path}\``).join(', ') : 'la raíz del repositorio'}; cruzar una frontera exige declararlo en la delta.`);
    lines.push(`- Fronteras observadas: ${observedBoundaryHint(facts)} (vocabulario único \`resolveBoundaries\`).`);
    lines.push('');
    lines.push('## Estructura de ficheros observada');
    lines.push('');
    if (facts.sources.length > 0) {
        for (const dir of facts.sources)
            lines.push(`- \`${dir}/\` — directorio de código observado`);
    }
    else {
        lines.push('- No se observó ningún directorio de código convencional.');
    }
    if (facts.tests.length > 0) {
        for (const dir of facts.tests)
            lines.push(`- \`${dir}/\` — directorio de test observado: aquí vive la prueba que se escribe primero`);
    }
    lines.push('');
    ...
    return {
        ...
        adaptedFrom: evidence,
        unchanged,
        rationale: 'El plan describe los módulos, responsabilidades, dependencias y directorios que el repositorio ya tiene (buildModuleMap + resolveBoundaries), en lugar del árbol de ejemplo genérico, y solo nombra un comando de verificación que un manifiesto declara.',
    };
};
// ---------------------------------------------------------------------------------------------
// tasks
// ---------------------------------------------------------------------------------------------
const buildTasksTemplate = (facts) => {
    const { project, runner } = facts;
    const evidence = [];
    const unchanged = commonUnobserved(facts);
    const lines = [];
    ...
    if (runner.hasCycle) {
        ...
        lines.push(`- directorios de test observados: ${facts.tests.length > 0 ? facts.tests.map((dir) => `\`${dir}\``).join(', ') : 'ninguno'}`);
        ...
        evidence.push(ev(`«runner: ${runner.label}»`, runner.labelEvidence ?? 'scanProject().testFramework'));
        if (runner.derivedCandidate) {
            evidence.push(ev(`«candidato derivado (NO verificado): ${runner.derivedCandidate}»`, 'derivación del runner observado; ningún script del repositorio lo declara, por eso no es el comando del ciclo'));
        }
    }
    else {
    ...
    for (const note of facts.discarded)
        lines.push(`- ${note}`);
    lines.push('');
    ...
    facts.modules.forEach((module, index) => {
        ...
        lines.push(`- [ ] ${number} {{DESCRIPCIÓN_DEL_CAMBIO}} — _Requirements: ${requirement}_ — _Boundary:_ ${boundary}${tdd}`);
        if (runner.hasCycle) {
            if (testDir) {
                lines.push(`  - test primero: \`${testDir}/{{FICHERO}}.test.{{EXT}}\` debe expresar el criterio de aceptación de ${requirement} y FALLAR por la razón correcta (no por un error de sintaxis).`);
            }
            else {
                lines.push(`  - test primero: \`{{RUTA_DE_TEST}}/{{FICHERO}}.test.{{EXT}}\` — este módulo no tiene directorio de test observado: decide y declara dónde vive el test antes de escribir código.`);
            }
            lines.push(`  - rojo: \`${runner.command}\` debe fallar.`);
            lines.push(`  - implementación: \`${sourceDirForModule(facts, module)}/{{FICHERO}}.{{EXT}}\` en el módulo \`${module.path}\`.`);
            ...
            lines.push(`  - implementación: \`${sourceDirForModule(facts, module)}/{{FICHERO}}.{{EXT}}\` en el módulo \`${module.path}\`.`);
            lines.push('  - cierre: {{CÓMO_SE_VERIFICA}} — no hay comando de test declarado, así que la verificación se declara ANTES de implementar.');
            if (module.dependsOn.length > 0) {
                lines.push(`  - _Depends:_ ${module.dependsOn.map((dep) => `\`${dep}\``).join(', ')}`);
            }
        }
    ...
    if (facts.modules.length > 1) {
        ...
        lines.push(`- [ ] ${facts.modules.length + 1}.1 {{DESCRIPCIÓN_DEL_CAMBIO_QUE_CRUZA}} — _Requirements: ${requirement}_ — _Boundary:_ ${facts.boundaries.boundaries.map((boundary) => `\`${boundary}\``).join(', ')}${crossTdd}`);
        if (runner.hasCycle) {
            lines.push(`  - test primero: \`${facts.tests[0] ?? '{{DIRECTORIO_DE_TEST}}'}/{{FICHERO}}.test.{{EXT}}\` en el módulo que recibe la dependencia.`);
            lines.push(`  - rojo: \`${runner.command}\` debe fallar.`);
...
export const adaptTemplates = async (input) => {
    ...
    try {
        ...
        return unreadableResult(`No se pudo reconocer el proyecto en ${cwd}: ${error.message}`);
    }
    ...
    if (!modules) {
        try {
            ...
            return unreadableResult(`No se pudo construir el mapa de módulos: ${error.message}`);
        }
    }
    ...
    for (const template of templates) {
        ...
        try {
            ...
            problems.push(`no se pudo escribir ${template.path}: ${error.message}`);
            actions.push({ path: template.path, action: 'create', reason: `falló la escritura: ${error.message}` });
        }
    }
