...
import { ALL_CRITICAL_INVARIANTS, } from './types.js';
/**
 * Governance profiles.
 *
...
const isValidInvariant = (v) => typeof v === 'string' && ALL_CRITICAL_INVARIANTS.includes(v);
/**
 ...
 */
export const resolveGovernanceSettings = (parsed = {}) => {
    const profile = parsed.profile && parsed.profile in governanceProfiles ? parsed.profile : 'solo';
    ...
    return {
        ...
        critical_gates_only: typeof parsed.critical_gates_only === 'boolean' ? parsed.critical_gates_only : base.critical_gates_only,
        non_blocking_warnings: typeof parsed.non_blocking_warnings === 'boolean'
 ...
 */
export const evaluateGates = (settings, facts) => {
    const raw = [
        {
    ...
    return raw.map(({ id, ok, detail }) => {
        ...
        const outcome = ok ? 'pass' : enforced ? 'fail' : 'advisory';
...
export const gatesPass = (gates) => !gates.some((g) => g.outcome === 'fail');
