...
export const ENFORCEMENT_LEVELS = [
    {
        ...
        universal: false,
        borrowed: true,
        description: 'El arnés bloquea la acción del agente antes de que toque el disco o el shell. Requiere que la herramienta exponga hooks bloqueantes; es el nivel más fuerte y el menos disponible.',
    },
    {
        id: 'B',
        name: 'commit',
        ...
        universal: true,
        borrowed: false,
        description: 'Hooks de pre-commit/pre-push ejecutan los gates críticos (secretos, comandos destructivos, evidencia, tríada SDD). Funciona en el 100% de las herramientas, porque git no pertenece a ninguna.',
    },
    {
        id: 'C',
        name: 'merge',
        ...
        universal: true,
        borrowed: false,
        description: 'La matriz completa de gates corre en CI sobre cada pull request. También universal, también barato.',
    },
    {
        id: 'D',
        name: 'MCP',
        ...
        universal: false,
        borrowed: false,
        description: 'El proxy media las llamadas a herramientas para cualquier agente que hable MCP, con independencia del IDE. En parte propio: se configura desde la organización, pero media un protocolo de terceros.',
    },
];
/**
...
export const TOOL_ENFORCEMENT = [
    ...
    },
    {
        ...
        caveat: 'Las ediciones de fichero no tienen evento pre-escritura y solo admiten auditoría posterior.',
        commit: true,
        merge: true,
        mcpProxy: true,
    },
    {
        ...
        caveat: 'Solo si el fork ejecuta el runtime de hooks y el usuario lo activa.',
        commit: true,
        merge: true,
        mcpProxy: true,
    },
...
export const getToolEnforcement = (tool) => TOOL_ENFORCEMENT.find((t) => t.tool === tool);
export const DEFAULT_SENTINEL = {
    ...
    notes: 'El centinela debe relanzarse periódicamente: la tabla describe el techo, no el suelo. Una sola verificación en la puesta en marcha no establece la garantía.',
};
/**
 ...
 * failure.
 */
export const interpretSentinel = (exitCode, spec = DEFAULT_SENTINEL) => {
...
const writeCeilingReachesLevelA = (ceiling) => ceiling === 'total' || ceiling === 'partial' || ceiling === 'conditional';
/**
 ...
 */
export const resolveFloor = (tool, input = {}) => {
    ...
    const ceiling = ['B', 'C'];
    if (writeCeilingReachesLevelA(def.writeCeiling))
        ceiling.unshift('A');
    if (def.mcpProxy)
        ceiling.push('D');
    const guaranteesA = writeCeilingReachesLevelA(def.writeCeiling) && input.levelAVerified === true;
    const floorIds = ['B', 'C'];
    if (def.mcpProxy && input.mcpProxyConfigured)
        floorIds.push('D');
    if (writeCeilingReachesLevelA(def.writeCeiling) && !input.levelAVerified) {
        warnings.push(`${def.label} permite nivel A (${def.writeMechanism}) pero ningún centinela lo verificó: se declara techo, no garantía.`);
    }
    if (def.caveat)
        warnings.push(`${def.label}: ${def.caveat}`);
    ...
    const beyondFloor = guaranteesA
        ...
        : [];
    const guaranteed = new Set([...floorIds, ...beyondFloor.map((l) => l.id)]);
 ...
 */
export const assertFloorIsOwned = (report) => {
    const borrowed = report.floor.filter((l) => l.ownedBy === 'third-party').map((l) => l.id);
    if (borrowed.length > 0 || !report.floorIsOwned) {
        throw new Error(`I3 violado: el suelo declarado incluye nivel(es) ${borrowed.join(', ') || 'marcados como no propios'} prestados por un tercero. ` +
 ...
 */
export const applyDefaultFail = (observation, regime = 'flexible') => {
    ...
    if (observation.inspects === false) {
        return {
            ...
            authority,
            detail: 'Declarado pero vacío: el gate no inspecciona nada, así que no puede acreditar un control (activación ≠ medición).',
    ...
    if (!observation.sensorAvailable) {
        ...
        return {
            ...
            authority,
            detail: 'Sensor no disponible: se auto-autoriza bajo responsabilidad del operador y deja registro (bloquear ante un instrumento roto enseña a desactivar instrumentos).',
    ...
    if (observation.fired) {
        return {
            ...
            outcome: 'fail',
            authority,
            detail: 'El escáner corrió y encontró algo: no se auto-autoriza nunca, porque eso sería control de acceso por exclusión con pasos adicionales.',
            requiresReceipt: false,
        };
    }
    return {
...
export const posturePasses = (verdicts) => !verdicts.some((v) => v.outcome === 'fail');
export const unresolvedReceipts = (verdicts) => verdicts.filter((v) => v.requiresReceipt);
