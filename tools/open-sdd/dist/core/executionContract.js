...
export const testCommandFor = (testFramework) => {
    const framework = (testFramework ?? '').trim();
    if (/vitest/i.test(framework))
        return { command: 'npx vitest run', derived: true };
    if (/jest/i.test(framework))
        return { command: 'npx jest', derived: true };
    if (/mocha/i.test(framework))
        return { command: 'npx mocha', derived: true };
