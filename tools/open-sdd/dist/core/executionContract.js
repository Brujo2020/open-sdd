 ...
 */
export const testCommandFor = (testFramework, declaredScript) => {
    const declared = (declaredScript ?? '').trim();
    if (declared)
        return { command: declared, derived: false };
    const framework = (testFramework ?? '').trim();
    for (const [pattern, command] of ECOSYSTEM_TEST_COMMANDS) {
        if (pattern.test(framework))
            return { command, derived: true };
    }
 ...
 */
const declaredMatchesTest = (declared, extractedTest) => fileMatchesTarget(extractedTest, declared);
const walkTestDirs = async (cwd, testDirs) => {
    const files = [];
    const unreadable = [];
    let truncated = false;
    const walk = async (rel, depth) => {
        ...
        try {
            ...
            unreadable.push({ dir: rel, reason: err.message });
            return;
        }
 ...
 */
export const extractContracts = async (input) => {
    const cwd = input.cwd;
    const notes = [];
    let complete = true;
    ...
    if (derived.command === 'npm test') {
        notes.push(`No se detectó framework de test con un comando asociado: se propone \`${derived.command}\` como comando por defecto. Nadie lo declaró, así que es una suposición derivada (derivado, no verificado), no una verificación.`);
    }
    else {
        notes.push(`Comando de test derivado del framework detectado (${testFramework}), no verificado ejecutándolo: \`${derived.command}\`.`);
    }
    // ── Contratos descubiertos ──────────────────────────────────────────────────────────────────
    const walk = testDirs.length > 0 ? await walkTestDirs(cwd, testDirs) : { files: [], unreadable: [], truncated: false };
    if (testDirs.length === 0) {
        notes.push('No se detectó ningún directorio de test: no hay contratos descubiertos que extraer. Esto no es un aprobado, es la ausencia de oráculo.');
    }
    for (const bad of walk.unreadable) {
        complete = false;
        notes.push(`No se pudo leer el directorio de test ${bad.dir} (${bad.reason}): el oráculo queda sin inspeccionar en esa parte y no se declara completo.`);
    }
    if (walk.truncated) {
        complete = false;
    ...
    const discoveredByTest = new Map();
    for (const test of walk.files) {
        ...
        for (const source of changedSources) {
            ...
            if (!matches)
                continue;
            const set = discoveredByTest.get(test) ?? new Set();
            set.add(source);
    ...
    if (input.delta) {
        for (const entry of input.delta.entries) {
            ...
            if (entry.kind === 'MODIFIED' && declared.length === 0) {
                notes.push(`Aviso: la entrada MODIFIED ${entry.id} no declara contratos; el oráculo está incompleto para esa entrada y su comportamiento modificado no queda protegido por nada declarado.`);
            }
            for (const test of declared) {
                const key = norm(test);
                const existing = declaredByTest.get(key) ?? { protects: new Set(), entries: [] };
                for (const target of entry.targets)
                    existing.protects.add(target);
    ...
    if (missingDeclared.length > 0) {
        complete = false;
        notes.push(`ERROR: ${missingDeclared.length} contrato(s) declarado(s) por entradas REMOVED no existen en el conjunto extraído: ${Array.from(new Set(missingDeclared)).join(', ')}. La delta promete una protección que el repositorio no tiene.`);
    }
    ...
    if (uncoveredChanges.length > 0) {
        notes.push(`${uncoveredChanges.length} fichero(s) cambiado(s) sin ningún contrato que los cubra: ${uncoveredChanges.join(', ')}. Es el hueco del oráculo y se informa tal cual.`);
    }
 ...
 */
export const verifyContracts = (set, run) => {
    ...
    const missing = Array.from(new Set(declaredTests.filter((test) => !discovered.some((d) => declaredMatchesTest(test, d.test)))));
    const namedFailures = run.failedTests ?? [];
    const failed = Array.from(new Set(declaredTests.filter((test) => namedFailures.some((f) => declaredMatchesTest(test, f)))));
    const unexplainedFailures = namedFailures.filter((f) => !declared.some((c) => declaredMatchesTest(c.test, f)) && !discovered.some((d) => declaredMatchesTest(d.test, f)));
    ...
    if (missing.length > 0) {
        lines.push(`Contratos declarados que no existen en el conjunto extraído: ${missing.join(', ')}. Un código de salida 0 con contratos declarados ausentes no es un aprobado.`);
    }
    if (failed.length > 0) {
    ...
    if (declared.length === 0) {
        lines.push('El conjunto no declara contratos: el código de salida es la única señal disponible y no demuestra que lo que debía protegerse siga intacto.');
    }
