 ...
 */
export const resolveBoundaries = (project) => {
    const declared = project.declaredModules ?? [];
    const moduleRoots = Array.from(new Set([...(project.workspaceRoots ?? []), ...declared.map((module) => module.path)]))
    ...
    const source = moduleRoots.length > 0 ? 'declared-modules' : 'root-only';
    const labels = Array.from(new Set(moduleRoots.map((root) => ecosystems[root]))).filter((ecosystem) => ecosystem !== 'unknown');
    const detail = source === 'declared-modules'
...
const existsUnder = async (cwd, rel) => (await stat(path.join(cwd, rel)).catch(() => null)) !== null;
const readTextUnder = async (cwd, rel) => readFile(path.join(cwd, rel), 'utf8').catch(() => null);
...
const subdirsContaining = async (cwd, parent, manifestName) => {
    ...
    for (const entry of entries) {
        ...
        if (await existsUnder(cwd, `${rel}/${manifestName}`))
            found.push(rel);
    }
    return found;
};
/** Bounded walk for manifest files, so a monorepo is detected without scanning an entire tree. */
const walkFiles = async (cwd, matches, maxDepth) => {
    const found = [];
    let visited = 0;
    const walk = async (relDir, depth) => {
        if (depth > maxDepth || visited > MAX_WALK_ENTRIES)
            return;
...
const expandMember = async (cwd, member, manifestName) => {
    const normalized = member.replace(/\\/g, '/').replace(/\/$/, '');
    if (!normalized.includes('*'))
        return normalized ? [normalizeRel(normalized)] : [];
 ...
 */
export const tomlSections = (text) => {
...
export const tomlArrayTables = (text, name) => {
    const bodies = [];
    let current = null;
    for (const line of text.split(/\r?\n/)) {
...
const tomlRawArray = (sectionBody, key) => {
    ...
    for (let i = start + 1; i < sectionBody.length; i += 1) {
        ...
        if (char === ']')
            return sectionBody.slice(start + 1, i);
    }
    return null;
};
/** Quoted strings inside a TOML array (`members = ["a", "b"]`). */
const tomlStringArray = (sectionBody, key) => {
 ...
 */
const poetryPackageMembers = (sectionBody, key) => {
    const raw = tomlRawArray(sectionBody, key);
    if (raw === null)
        return [];
    const members = [];
...
const emptyDetection = () => ({ modules: [], unreadable: [] });
/** Maven: `<modules><module>x</module></modules>`, followed one nesting level at a time. */
const detectMavenModules = async (cwd, rootPom = 'pom.xml') => {
    const modules = [];
    const unreadable = [];
    const seen = new Set();
    ...
    while (queue.length > 0) {
        ...
        if (seen.has(dir) || depth > 4)
            continue;
        seen.add(dir);
        const pomRel = dir === '.' ? rootPom : `${dir}/${rootPom}`;
        if (!(await existsUnder(cwd, pomRel)))
            continue;
...
const detectGradleModules = async (cwd, settingsRel) => {
    const modules = [];
    if (!(await existsUnder(cwd, settingsRel)))
        return emptyDetection();
    const text = await readTextUnder(cwd, settingsRel);
    if (text === null)
        return { modules: [], unreadable: [settingsRel] };
...
const detectGoModules = async (cwd) => {
    const unreadable = [];
    const declared = new Map();
    const addUse = (raw) => {
...
const detectPythonModules = async (cwd) => {
    ...
    if (hasRoot) {
        ...
        if (text === null)
            unreadable.push('pyproject.toml');
        else {
            ...
            for (const section of ['tool.uv.workspace', 'tool.poetry.workspace']) {
                ...
                if (body)
                    memberLists.push(tomlStringArray(body, 'members'));
            }
            const poetry = sections.get('tool.poetry');
            if (poetry)
                memberLists.push(poetryPackageMembers(poetry, 'packages'));
    ...
    for (const manifest of await walkFiles(cwd, (name) => name === 'pyproject.toml' || name === 'setup.py', 4)) {
...
const detectDotnetModules = async (cwd) => {
    const modules = new Map();
    const unreadable = [];
 ...
 */
const detectDeclaredModules = async (cwd, rootPkg, subdirsWithManifest) => {
    const declared = [];
    const unreadable = [];
    // --- Node: the existing behaviour, kept intact -----------------------------------------------
    if (rootPkg)
        declared.push({ path: '.', ecosystem: 'node', manifest: 'package.json' });
    ...
    for (const glob of workspaceGlobs) {
        ...
        for (const dir of await subdirsWithManifest(parent || '.'))
            nodeRoots.add(dir);
    }
    for (const conventional of ['packages', 'apps', 'tools', 'services', 'libs', 'modules']) {
        for (const dir of await subdirsWithManifest(conventional))
            nodeRoots.add(dir);
    }
    ...
    for (const settings of ['settings.gradle.kts', 'settings.gradle']) {
        if (!(await existsUnder(cwd, settings)))
            continue;
    ...
    if ((await existsUnder(cwd, 'pyproject.toml')) ||
...
export const scanProject = async (cwd = process.cwd()) => {
    ...
    const readJson = async (rel) => {
        try {
            return JSON.parse(await readFile(path.join(cwd, rel), 'utf8'));
        }
        catch {
            return null;
        }
    };
    const subdirsWithManifest = async (parent) => {
        try {
    ...
    const { declared: declaredModules, unreadable: unreadableToolchains } = await detectDeclaredModules(cwd, rootPkg, subdirsWithManifest);
    const workspaceRoots = new Set(declaredModules.map((module) => module.path).filter((modulePath) => modulePath !== '.'));
    ...
    for (const root of roots) {
        ...
        if (!pkg)
            continue;
        sawAnyManifest = true;
        if (root === '.' && typeof pkg.name === 'string' && pkg.name)
            name = pkg.name;
        ...
        for (const [dep, label] of buildMap)
            if (!buildTool && deps[dep])
                buildTool = label;
    }
    ...
    if (sawAnyManifest && !packageManager)
        packageManager = 'npm';
...
export const bootstrapSteering = async (cwd = process.cwd(), project, sddDir) => {
    ...
    const nativeCommand = runner && /^(go test|cargo test|mvn test|gradle test|\.?\/?gradlew test|dotnet test|pytest|ctest)$/i.test(runner)
        ? runner
        : undefined;
    const testLines = [];
    if (runner) {
        ...
        if (declaredTestScript) {
            const manager = ['npm', 'pnpm', 'yarn', 'bun'].includes(proj.packageManager ?? '')
                ? proj.packageManager
                : 'npm';
            testLines.push(`- **Comando de test**: \`${manager} test\` (declarado en package.json: script "test" = "${declaredTestScript}")`);
        }
            ...
            testLines.push(`- **Comando de test**: \`${testCommandFor(runner).command}\` (derivado del runner observado, no verificado ejecutándolo)`);
        }
    }
    else {
        testLines.push('- **Testing Framework**: no se observó ningún runner de test (ni dependencia, ni configuración, ni script `test`).');
