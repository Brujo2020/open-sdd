...
export const IMPORT_SOURCES = ['kiro', 'spec-kit', 'cc-sdd'];
...
const copy = (from, to, reason) => ({
    from,
    to,
    action: 'copy',
    reason,
});
const convert = (from, to, reason) => ({
    from,
    to,
    action: 'convert',
    reason,
});
const skip = (from, reason) => ({
    from,
    to: '',
    action: 'skip',
    reason,
...
const planAliases = () => TRIAD.find((file) => file.canonical === 'plan.md')?.aliases ?? [];
const DESIGN_ALIAS_NOTE = () => {
...
const provenanceBanner = (from, extra) => [
...
const planKiroSpecs = async (cwd, specsRoot, conversions, found, warnings, options) => {
    ...
    for (const feature of features) {
        ...
        if (!names.has('requirements.md')) {
            warnings.push(`La spec "${feature.name}" no trae requirements.md: se importará sin requisitos y el gate C1 la verá incompleta.`);
        }
        if (names.has('spec.json')) {
            conversions.push(skip(posix(path.join(featureRoot, 'spec.json')), 'El spec.json de Kiro/cc-sdd declara metadatos con otra forma que el spec.json de open-sdd (fase, estado, idioma): copiarlo haría que el motor leyera un metadato ajeno como propio. Recrea la spec con `open-sdd init <feature>`.'));
        }
        if (options.reportBoundaries && names.has('tasks.md')) {
            ...
            if (boundaries + depends > 0) {
                warnings.push(`tasks.md de "${feature.name}" declara ${boundaries} \`_Boundary:_\` y ${depends} \`_Depends:_\`: open-sdd ya lee esas anotaciones (frontera de tarea y dependencia), pero conviene revisar que el vocabulario de rutas coincida con el repositorio.`);
            }
        }
    }
};
const planKiroLike = async (cwd, source, options) => {
    const found = [];
    const conversions = [];
    const warnings = [];
    ...
    if (await exists(path.join(cwd, steeringRoot))) {
        ...
        for (const entry of await listEntries(path.join(cwd, steeringRoot))) {
            ...
            conversions.push(copy(from, posix(path.join(IMPORT_TARGET_SDD_DIR, 'steering', entry.name)), 'Misma memoria de proyecto (steering), mismo rol y mismo nombre.'));
        }
    }
    ...
    if (await exists(path.join(cwd, settingsRoot))) {
        ...
        for (const entry of await listEntries(path.join(cwd, settingsRoot))) {
            ...
            if (entry.dir) {
                conversions.push(skip(from, 'Subdirectorio de settings: la forma de open-sdd no está confirmada para su contenido; revísalo a mano.'));
                continue;
            }
            ...
            try {
                ...
                conversions.push(skip(from, 'El JSON no parsea: NO se copia a ciegas, porque open-sdd lo leería como su propio settings y fallaría en silencio.'));
                continue;
            }
            conversions.push(convert(from, posix(path.join(IMPORT_TARGET_SDD_DIR, 'settings', entry.name)), 'El JSON parsea y la forma es compatible; se importa como settings.'));
        }
    }
    ...
    if (source === 'cc-sdd') {
        for (const skillsRoot of ['.kiro/skills', '.claude/skills', '.agents/skills']) {
            if (!(await exists(path.join(cwd, skillsRoot))))
                continue;
            ...
            for (const entry of foreign) {
                conversions.push(skip(posix(path.join(skillsRoot, entry.name)), 'Las skills de cc-sdd son otro conjunto (no un renombrado de las `sdd-*`): copiarlas al layout de open-sdd declararía capacidades que no son las nuestras. Elígela o adáptala a mano.'));
            }
            warnings.push(`Se encontraron ${foreign.length} skill(s) de cc-sdd en ${skillsRoot}: NO se importan. Las skills de open-sdd son ${'`sdd-*`'} y se instalan con el instalador del CLI.`);
        }
    }
    ...
    if (!(await exists(path.join(cwd, IMPORT_TARGET_SDD_DIR))) && (await exists(path.join(cwd, '.kiro')))) {
        warnings.push('El proyecto no tiene `.sdd`: open-sdd ya lee `.kiro` en su sitio (resolveSddDir). Esta importación crea además `.sdd/`; elige una raíz y elimina la otra para que no haya dos fuentes de verdad.');
    }
...
const planSpecKitSpec = async (cwd, specsRoot, feature, conversions) => {
    const featureRoot = posix(path.join(specsRoot, feature));
    const files = await listEntries(path.join(cwd, featureRoot));
    const names = new Set(files.filter((entry) => !entry.dir).map((entry) => entry.name));
    if (names.has('spec.md')) {
        conversions.push(convert(posix(path.join(featureRoot, 'spec.md')), posix(path.join(IMPORT_TARGET_SDD_DIR, 'specs', feature, 'requirements.md')), '`spec.md` de spec-kit ocupa el rol de requisitos; se importa como `requirements.md` con una banda de procedencia porque NO es EARS y nadie lo ha validado como tal.'));
    }
    for (const name of SPECKIT_AUX) {
        if (!names.has(name))
            continue;
        conversions.push(copy(posix(path.join(featureRoot, name)), posix(path.join(IMPORT_TARGET_SDD_DIR, 'specs', feature, name)), 'Documento auxiliar del mismo rol; se copia tal cual.'));
    }
    if (files.some((entry) => entry.dir && entry.name === 'contracts')) {
        ...
        for (const entry of await listEntries(path.join(cwd, contractsRoot))) {
            conversions.push(copy(posix(path.join(contractsRoot, entry.name)), posix(path.join(IMPORT_TARGET_SDD_DIR, 'specs', feature, 'contracts', entry.name)), 'Contrato de spec-kit: mismo rol (interfaz ejecutable de la spec).'));
        }
    }
    for (const entry of files) {
        if (entry.dir) {
            if (entry.name !== 'contracts') {
                conversions.push(skip(posix(path.join(featureRoot, entry.name)), 'Directorio no reconocido por open-sdd: no se importa a ciegas.'));
            }
            continue;
        }
        if (entry.name === 'spec.md' || SPECKIT_AUX.includes(entry.name))
            continue;
        conversions.push(skip(posix(path.join(featureRoot, entry.name)), 'Documento de spec-kit sin equivalente declarado en la tríada ni en los auxiliares conocidos.'));
    }
};
const planSpecKit = async (cwd) => {
    const found = [];
    const conversions = [];
    const warnings = [];
    ...
    if (await exists(path.join(cwd, constitutionFrom))) {
        ...
        conversions.push(convert(constitutionFrom, posix(path.join(IMPORT_TARGET_SDD_DIR, 'steering', 'constitution.md')), 'Misma autoridad (constitución del proyecto). Se copia VERBATIM: open-sdd no «arregla» una constitución ajena, solo reporta lo que su validador encuentra.'));
        if (parsed.principles.length === 0) {
            warnings.push('La constitución de spec-kit no expone ningún principio con la anatomía de open-sdd (id citable, nivel, restricción, patrón, justificación): se importa verbatim y debe reescribirse a mano antes de que un veredicto bloqueante pueda citarla.');
        }
        for (const issue of issues) {
            warnings.push(`validateConstitution [${issue.severity}${issue.code ? ` ${issue.code}` : ''}] ${issue.id}: ${issue.message}`);
        }
        if (issues.length === 0 && parsed.principles.length > 0) {
            warnings.push(`validateConstitution no encontró problemas en ${parsed.principles.length} principio(s), pero la importación sigue siendo un MAPEO: el contenido no se ha revisado por una persona.`);
        }
    }
    ...
    for (const specsRoot of specsRoots) {
        ...
        for (const feature of features) {
            if (seenFeatures.has(feature.name)) {
                warnings.push(`La feature "${feature.name}" aparece en más de una raíz de specs: solo se planifica una vez; revisa cuál es la vigente.`);
                continue;
            }
    ...
    for (const dir of ['.specify/templates', '.specify/scripts']) {
        ...
        conversions.push(skip(dir, dir.includes('templates')
    ...
    if (await exists(path.join(cwd, '.specify/memory'))) {
        for (const entry of await listEntries(path.join(cwd, '.specify/memory'))) {
            if (entry.name === 'constitution.md' || entry.dir)
                continue;
            conversions.push(skip(posix(path.join('.specify/memory', entry.name)), 'Documento de memoria de spec-kit sin rol declarado en el steering de open-sdd.'));
        }
    }
    const skipped = conversions.filter((item) => item.action === 'skip');
    const complete = found.length > 0 && skipped.length === 0 && warnings.length === 0;
...
// ---------------------------------------------------------------------------------------------
const hasKiroEvidence = async (cwd) => (await exists(path.join(cwd, '.kiro/steering'))) || (await exists(path.join(cwd, '.kiro/specs')));
const hasSpecKitEvidence = async (cwd) => {
    if (await exists(path.join(cwd, '.specify')))
        return true;
    const rootSpecs = path.join(cwd, 'specs');
    if (!(await exists(rootSpecs)))
        return false;
    const features = (await listEntries(rootSpecs)).filter((entry) => entry.dir);
    for (const feature of features) {
        if (await exists(path.join(rootSpecs, feature.name, 'spec.md')))
            return true;
    }
    return false;
};
/** cc-sdd is a `.kiro`-compatible generator: the specs are Kiro's, the marker is its own. */
const hasCcSddEvidence = async (cwd) => {
    const pkg = await readIfExists(path.join(cwd, 'package.json'));
    if (pkg !== null) {
        try {
            const parsed = JSON.parse(pkg);
            const declared = { ...(parsed.dependencies ?? {}), ...(parsed.devDependencies ?? {}) };
            if (Object.keys(declared).some((name) => name === 'cc-sdd' || name.startsWith('cc-sdd')))
                return true;
        }
        catch {
    ...
    if (await exists(path.join(cwd, '.kiro/skills')))
        return true;
    for (const skillsRoot of ['.claude/skills', '.agents/skills']) {
        const entries = await listEntries(path.join(cwd, skillsRoot));
        if (entries.some((entry) => entry.dir && entry.name.startsWith('kiro-')))
            return true;
    }
    return false;
};
const buildPlan = async (cwd, source) => {
    if (source === 'spec-kit')
        return planSpecKit(cwd);
 ...
 */
export const planImport = async (cwd, source) => {
    if (source !== undefined)
        return [await buildPlan(cwd, source)];
    const plans = [];
    if (await hasKiroEvidence(cwd))
        plans.push(await buildPlan(cwd, 'kiro'));
    if (await hasSpecKitEvidence(cwd))
        plans.push(await buildPlan(cwd, 'spec-kit'));
    if (await hasCcSddEvidence(cwd))
        plans.push(await buildPlan(cwd, 'cc-sdd'));
    return plans;
};
/**
 ...
 */
export const applyImport = async (cwd, plan, opts) => {
    if (opts.write !== true) {
        return {
            written: [],
            skipped: [],
    ...
    for (const conversion of plan.conversions) {
        ...
        const payload = conversion.from.endsWith('spec.md') && conversion.to.endsWith('requirements.md')
            ? provenanceBanner(conversion.from, 'spec-kit escribe escenarios de usuario y requisitos funcionales, no la sintaxis EARS.') + content
            : content;
        try {
            ...
            skipped.push(`${conversion.to} — no se pudo escribir (${error.message}).`);
        }
    }
