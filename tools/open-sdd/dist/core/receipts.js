...
export const makeReceipt = (input) => {
    if (!input.reason || input.reason.trim().length === 0) {
        throw new Error('I6: una relajación sin motivo no es un evento registrado; el motivo es obligatorio.');
    }
 ...
 * system's own.
 */
export const recordOverride = (appeal, journal) => {
 ...
 */
export const recalibrate = (counts, threshold = OVERRIDE_REVIEW_THRESHOLD) => counts.map(({ gate, overrides, blocks }) => {
...
export const assessI6 = (relaxationsObserved, journal) => {
 ...
 */
export const acceptedRiskLedger = (journal) => ({
