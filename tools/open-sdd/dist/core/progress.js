...
export const progressStatePath = (sddDir) => path.join(sddDir, PROGRESS_STATE_DIR, PROGRESS_STATE_FILE);
const isFiniteNumber = (value) => typeof value === 'number' && Number.isFinite(value);
const isPhase = (value) => value === 1 || value === 2 || value === 3;
const isPhaseDelta = (value) => value === 0 || value === 1 || value === -1;
const isEntry = (value) => {
 ...
 * historia falsa.
 */
export const readProgress = async (cwd, opts = {}) => {
    ...
    try {
        ...
        return {
            entries: [],
            corrupt: true,
            detail: `El historial de progreso no se pudo leer (${statePath}: ${err.message}).`,
        };
    }
    let parsed;
    try {
        ...
        return {
            entries: [],
            corrupt: true,
            detail: `El historial de progreso no se pudo interpretar (${statePath}: JSON inválido — ${err.message}).`,
        };
    }
    ...
    return { entries: candidate.entries, corrupt: false, detail: `Historial de progreso con ${candidate.entries.length} entrada(s) en ${statePath}.` };
};
const persist = async (statePath, ledger) => {
...
const normalizeSummary = (summary) => summary
    .normalize('NFD')
...
export const recordProgress = async (input) => {
    ...
    if (!validated.ok)
        return { written: false, entry: null, detail: validated.detail };
    ...
    const delta = previous
        ? {
            ...
            phase: Math.sign(current.score.phase - previous.score.phase),
        }
        : { score: 0, phase: 0 };
    const entry = { at: new Date().toISOString(), ...current, delta };
    ...
    try {
        ...
        return {
            written: false,
            entry: null,
            detail: `El hito no se pudo escribir en ${statePath} (${err.message}): el libro queda como estaba.`,
        };
    }
    const movement = delta.score === 0 && delta.phase === 0
...
const arrow = (phase) => (phase > 0 ? ' ↑' : phase < 0 ? ' ↓' : '');
/**
 ...
 */
export const renderProgress = (input) => {
