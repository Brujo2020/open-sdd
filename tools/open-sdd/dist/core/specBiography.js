...
import { computeSddScore } from './sddScore.js';
...
export const ACTIVITY_IS_NOT_QUALITY = 'Esto mide RITMO, no CALIDAD: una especificación que nunca cambia puede ser correcta y una que cambia todos los días puede ser un desastre.';
// ---------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------
const collectRatifications = (constitution) => {
    if (!constitution)
        return [];
    return constitution.amendments
        .filter((amendment) => amendment.status === 'in-force' ||
...
const decideBreathing = (measured) => {
    ...
    if (born === null) {
        return {
            breathing: 'orphan',
            reason: 'git nunca ha registrado esta especificación: no hay ni un commit que la contenga, así que no tiene historia que medir.',
        };
    }
    if (codeCommits === 0) {
        return {
...
export const specBiography = async (input) => {
    ...
    const amendments = deltaRead.content
    ...
    if (constitutionRead.exists && constitutionRead.constitution === null) {
        notes.push(`constitución presente pero ilegible (${constitutionRead.path ?? sddDir}): no se pudo leer el registro de ratificaciones.`);
    }
    ...
    if (!isGitRepo(cwd)) {
        return unknown('no es un repositorio git (o git no está disponible): sin historia no se puede medir el ritmo y no se adivina.');
    }
    ...
    const fromCwd = (target) => toPosix(path.relative(cwd, path.resolve(cwd, target)));
    /** La misma ruta relativa a la raíz: lo que imprime `--numstat` y lo que espera `<rev>:<path>`. */
    const fromRoot = (target) => path.posix.normalize(path.posix.join(prefix, fromCwd(target)));
    /**
     ...
     */
    const atTop = (rootRel) => `:(top,literal)${rootRel}`;
    ...
    const lastLine = gitOut(cwd, ['log', '-n', '1', '--format=%H|%h|%aI', '--', specPathspec])
        ?.split('\n')
        .map((line) => line.trim())
    ...
    if (bornSha) {
        ...
        if (lastSha)
            codeSinceLastChange = countCommits(cwd, [`${lastSha}..HEAD`]);
    }
    // ── Eventos: un evento por fichero cambiado y commit, más reciente primero ────────────────────
    const classify = (file) => {
        const rel = normalizeNumstatPath(toPosix(file));
        if (constitutionRels.includes(rel))
            return 'constitution';
        if (rel !== specRel && !rel.startsWith(`${specRel}/`))
            return null;
        const base = rel.slice(rel.lastIndexOf('/') + 1);
        if (base === 'requirements.md')
            return 'requirements';
        if (base === 'plan.md' || base === 'design.md')
            return 'plan';
        if (base === 'tasks.md')
            return 'tasks';
        if (base === deltaSpecFileName())
            return 'delta';
        if (base === 'spec.json')
            return 'spec.json';
        return 'other';
    ...
    for (const event of events) {
        if (event.artifact !== 'spec.json' || seenCommits.has(event.commit))
            continue;
        ...
        if (phase)
            phaseHistory.push({ date: event.date, commit: event.commit, phase });
    }
    ...
    if (status.exists) {
        try {
            ...
            notes.push(`el estado actual no se pudo medir (${err.message}).`);
            score = null;
        }
    }
...
// ---------------------------------------------------------------------------------------------
const renderAmendment = (amendment) => {
...
export const renderBiography = (bio) => {
    const lines = [];
    lines.push(`Biografía de la especificación «${bio.feature}»`);
    lines.push(`Nacimiento: ${bio.born ?? '(git nunca registró esta especificación)'} · Último cambio: ${bio.lastChange ?? '(sin cambios registrados)'}`);
    lines.push(`Actividad: ${bio.activity.specCommits} commit(s) de la spec · ${bio.activity.codeCommits} commit(s) de código · ${bio.activity.sinceDays} día(s) desde el nacimiento`);
    ...
    if (bio.amendments.length === 0) {
        ...
        for (const amendment of bio.amendments)
            lines.push(renderAmendment(amendment));
    }
    lines.push('');
    ...
    if (bio.phaseHistory.length === 0) {
        ...
        for (const phase of bio.phaseHistory)
            lines.push(`  - ${phase.date} · ${phase.commit} · ${phase.phase}`);
    }
    ...
    if (bio.events.length === 0) {
        ...
        for (const event of bio.events) {
            lines.push(`  ${event.date} · ${event.commit} · ${event.author} · ${event.artifact} +${event.added}/-${event.removed} · ${event.subject}`);
        }
    }
    if (bio.notes.length > 0) {
        lines.push('');
        lines.push('Notas de lectura (lo que no se pudo leer, dicho en vez de omitido)');
        for (const note of bio.notes)
            lines.push(`  - ${note}`);
    }
    return lines;
};
