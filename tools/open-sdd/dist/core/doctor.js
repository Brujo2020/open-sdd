...
const inClause = (version, operator, base, partial) => {
    switch (operator) {
        case '>':
        ...
        default:
            if (partial) {
                ...
                const upper = base[2] === 0 && base[1] === 0 ? [base[0] + 1, 0, 0] : [base[0], base[1] + 1, 0];
                return compare(version, base) >= 0 && compare(version, upper) < 0;
            }
            return compare(version, base) === 0;
    }
    ...
    for (const alternative of alternatives) {
        ...
        for (const part of parts) {
            ...
            if (raw.some((piece) => /^[xX*]$/.test(piece))) {
                ...
                if (/^[xX*]$/.test(raw[0]))
                    continue;
                const base = [Number(raw[0]), raw[1] && /^[xX*]$/.test(raw[1]) ? 0 : Number(raw[1] ?? 0), 0];
                if (operator === '<' || operator === '<=')
                    continue;
                if (!inClause(parsed, '>=', base, false))
                    matches = false;
                continue;
            }
            const base = [Number(raw[0]), Number(raw[1] ?? 0), Number(raw[2] ?? 0)];
            if (!inClause(parsed, operator, base, raw.length < 3))
                matches = false;
        }
        if (!clauseUnderstood)
            continue;
...
const CLI_CANDIDATES = (cwd) => Array.from(new Set([
...
const HOOK_TEMPLATE_CANDIDATES = (cwd) => {
    const bases = [
    ...
    for (const base of bases) {
        for (const file of HOOK_TEMPLATE_FILES)
            out.push(path.join(base, file));
    }
    return Array.from(new Set(out));
};
/** Plantillas embarcadas legibles, la preferida (portátil) primero. */
const shippedTemplates = async (cwd) => {
    const templates = [];
    for (const candidate of HOOK_TEMPLATE_CANDIDATES(cwd)) {
        ...
        if (content !== null)
            templates.push({ path: candidate, content });
    }
    return templates;
};
// ---------------------------------------------------------------------------------------------
// Comprobaciones
// ---------------------------------------------------------------------------------------------
const checkNode = async (cwd, manifestPaths) => {
 ...
 */
export const resolveCliExecutable = async (cwd) => {
...
const probeCliVersion = (candidate, cwd) => {
    if (cliVersionCache.has(candidate))
        return cliVersionCache.get(candidate) ?? null;
...
export const inspectCommitHook = async (cwd, platform = process.platform) => {
...
const checkHookPortability = async (cwd, platform, hook) => {
...
const checkSettings = async (cwd, sddDir) => {
    ...
    try {
        ...
        return {
            check: {
                ...
                label: 'Rigor declarado',
                status: 'fail',
                detail: `${display} no es JSON válido (${error.message}). No se degrada en silencio: un archivo roto no puede convertirse en una bajada invisible de exigencia.`,
    ...
    try {
        const settings = resolveRigorSettings(parsed);
        ...
        return {
            check: {
                ...
                label: 'Rigor declarado',
                status: 'fail',
                detail: `${display} es JSON válido pero no declara un rigor utilizable: ${error.message}`,
...
const constitutionCandidates = (sddDir) => [
    path.join(sddDir, 'steering', 'constitution.md'),
    path.join(sddDir, 'constitution.md'),
];
const checkConstitution = async (cwd, sddDir) => {
...
const checkSpecs = async (cwd, sddDir) => {
    ...
    try {
        ...
        return {
            ...
            label: 'Especificaciones',
            status: 'warn',
            detail: `La tríada no se pudo comprobar en ninguna especificación: ${path.posix.join(sddDir, 'specs')} no se pudo leer (${error.message}).`,
    ...
    for (const feature of features) {
        ...
        if (!verdict.complete)
            incomplete.push({ feature: feature.name, missing: verdict.missing });
    }
...
const checkModelOffline = () => ({
    id: 'model-offline',
    label: 'Sin modelo y sin red',
    status: 'ok',
    detail: 'No se requiere backend de modelo ni acceso a red: el CLI es determinista y funciona offline. La alineación de intención (C5) se reporta como `mode=degraded` cuando no hay modelo, y NO se cuenta como evidencia.',
});
const checkEnvironment = (cwd, sddDir, repoRoot) => {
...
export const runDoctor = async (cwd, options) => (await computeDoctor(cwd, options)).report;
/** Una línea por comprobación, sin ANSI: el JSON queda limpio y el render es testeable. */
export const renderDoctor = (report) => {
...
const DEFAULT_RATIONALE = (level) => `Nivel ${level} elegido por \`open-sdd doctor --fix\` como valor por defecto del inicializador: exige requisitos en forma comprobable (EARS) y una constitución válida, y no exige todavía delta, ligado de evidencia ni detección de drift. Se sube editando este archivo, nunca bajando lo que el suelo ya comprueba.`;
/**
 ...
 */
const applyFixes = async (cwd, facts) => {
    const done = [];
    ...
    if (!facts.settingsExists) {
        try {
            ...
            await writeFile(facts.settingsPath, `${JSON.stringify({
            ...
            done.push(`no se pudo crear ${display} (${error.message})`);
        }
    }
    else {
        done.push(`= ${display} ya existe: se conserva sin tocar`);
    }
    const hookFixable = facts.hookPath !== null &&
    ...
    if (hookFixable && facts.hookTemplateContent !== null && facts.hookTemplatePath !== null) {
        try {
            await mkdir(path.dirname(facts.hookPath), { recursive: true });
            await writeFile(facts.hookPath, facts.hookTemplateContent, 'utf8');
            await chmod(facts.hookPath, 0o755);
            done.push(`reinstalado el hook de commit en ${path.relative(cwd, facts.hookPath)} desde la plantilla vigente (${path.basename(facts.hookTemplatePath)})`);
        }
        catch (error) {
            done.push(`no se pudo reinstalar el hook (${error.message})`);
        }
    }
        ...
        try {
            ...
            done.push(`no se pudo marcar el hook como ejecutable (${error.message})`);
        }
    }
    else if (facts.hookState === 'foreign') {
        done.push(`el hook instalado es ajeno: no se toca (una reparación automática no debe reemplazar decisiones humanas). Ejecuta: open-sdd floor install ${quote(cwd)} --force`);
    }
...
export const handleDoctorCommand = async (args, io, cwd = process.cwd()) => {
    ...
    for (const check of computed.report.checks) {
        const mark = check.status === 'ok' ? colors.green('✓') : check.status === 'warn' ? colors.yellow('!') : colors.red('✗');
    ...
    if (repaired.length > 0) {
        ...
        for (const item of repaired)
            io.log(`  ${item.startsWith('=') ? colors.dim(item) : colors.green(`✓ ${item}`)}`);
        io.log('');
    }
    io.log(formatVerdict(computed.report.ok ? 'pass' : 'fail', computed.report.ok ? colors.green(computed.report.detail) : colors.red(computed.report.detail)));
    io.log('');
    return computed.report.ok ? 0 : 1;
};
