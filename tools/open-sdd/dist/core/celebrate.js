...
const isPhase = (value) => value === 1 || value === 2 || value === 3;
const missingEars = (ears) => {
    if (!ears || typeof ears !== 'object')
        return 'no hay requisitos medidos (EARS no corrió)';
    if (ears.evaluated === false)
        return 'los requisitos están sin comprobar (EARS notChecked)';
    ...
    if (ears.total <= 0)
        return 'no hay ningún requisito que comprobar (0 requisitos)';
    return null;
};
const missingPivot = (pivot) => {
    if (!pivot || typeof pivot !== 'object')
        return 'el pivote constitucional no se midió';
    if (pivot.evaluated !== true)
        return 'el pivote constitucional no se evaluó (evaluated ≠ true)';
...
const missingGates = (gates) => {
    if (!gates || typeof gates !== 'object')
        return 'no hay resultado de gates';
    if (gates.evaluated === false)
        return 'los gates están sin comprobar (notChecked)';
    ...
    if (gates.total <= 0)
        return 'no se ejecutó ningún gate (0 gates)';
    return null;
};
const percent = (alignment) => `${Math.round(alignment * 100)}%`;
const gateList = (failed) => failed.length === 1 ? `el gate ${failed[0]} no pasa` : `los gates ${failed.join(', ')} no pasan`;
/**
 ...
 */
const judgeVerdict = (parts, prefix) => {
    const earsGap = missingEars(parts.ears);
    if (earsGap)
        return { verdict: 'refused', reason: `${prefix}no se puede celebrar: ${earsGap}.`, detail: null };
    const pivotGap = missingPivot(parts.pivot);
    if (pivotGap)
        return { verdict: 'refused', reason: `${prefix}no se puede celebrar: ${pivotGap}.`, detail: null };
    const gatesGap = missingGates(parts.gates);
    if (gatesGap)
        return { verdict: 'refused', reason: `${prefix}no se puede celebrar: ${gatesGap}.`, detail: null };
...
export const renderStreakPhrase = (rises) => {
    const n = isFiniteNumber(rises) && rises > 0 ? Math.floor(rises) : 0;
    if (n === 0)
        return 'racha: 0 cambios seguidos sin bajar la adhesión (aún sin racha)';
    return n === 1
...
export const renderStreak = (input) => {
    const parts = [renderStreakPhrase(input.rises)];
    if (isFiniteNumber(input.score))
        parts.push(`score ${Math.round(input.score)}/100`);
    if (isPhase(input.phase))
        parts.push(`fase ${input.phase}/3`);
    return parts.join(' · ');
};
const progressLine = (score, streak) => {
    ...
    if (isFiniteNumber(streak))
        return renderStreakPhrase(streak);
    return null;
};
const summaryLine = (input) => {
    ...
    if (isFiniteNumber(input.streak))
        parts.push(`racha ${input.streak}`);
    return parts.join(' · ');
};
const buildStaticLines = (input) => {
...
export const renderCelebrationFrame = (index, summary, opts = {}) => {
...
export const renderCelebrationFrames = (summary, opts = {}) => Array.from({ length: CELEBRATION_FRAME_COUNT }, (_unused, index) => renderCelebrationFrame(index, summary, opts));
// ── Opciones → modo de salida ───────────────────────────────────────────────────────────────────
const truthyEnv = (value) => value !== undefined && value !== '' && value !== '0' && value !== 'false' && value !== 'no';
const resolveMode = (opts) => {
    ...
    const animate = (opts.anim ?? true) && isTty === true && !quiet && !json && !noColorEnv && !noAnimEnv && !ciEnv;
    const color = opts.noColor !== true && !json && !noColorEnv && isTty === true;
    return { animate, color, minimal: quiet || json };
};
const paint = (verdict, line, color) => {
    if (!color)
        return line;
...
const assemble = (verdict, reason, block, opts) => {
...
export const judgeCelebration = (snapshot, opts = {}) => {
    const parts = {
        ears: snapshot?.ears,
...
export const judgeFeatureCelebration = (input, opts = {}) => {
 ...
 */
export const playCelebration = async (report, opts = {}) => {
...
export const levelFor = (score, _phase) => {
...
const dateOnly = (iso) => (iso.length >= 10 ? iso.slice(0, 10) : iso);
/** Logros derivados del libro + hechos explícitos. Nada se concede sin su evento. */
export const deriveAchievements = (entries, context = {}) => {
    const out = [];
    ...
    const best = entries.reduce((top, entry) => (top === null || entry.streak > top.streak ? entry : top), null);
...
export const CELEBRATIONS_STATE_VERSION = 1;
export const celebrationsStatePath = (sddDir) => path.join(sddDir, CELEBRATIONS_STATE_DIR, CELEBRATIONS_STATE_FILE);
const isCelebrationEntry = (value) => {
    if (!value || typeof value !== 'object')
        return false;
    const entry = value;
    if (typeof entry.feature !== 'string' || entry.feature.length === 0)
        return false;
    if (typeof entry.date !== 'string' || entry.date.length === 0)
        return false;
    if (!isFiniteNumber(entry.score) || !isPhase(entry.phase))
        return false;
    if (!isFiniteNumber(entry.streak) || entry.streak < 0)
        return false;
    const ears = entry.ears;
    const pivot = entry.pivot;
    const gates = entry.gates;
    if (!ears || !isFiniteNumber(ears.conforming) || !isFiniteNumber(ears.total))
        return false;
    if (!pivot || !isFiniteNumber(pivot.alignment))
        return false;
    if (!gates || !isFiniteNumber(gates.passed) || !isFiniteNumber(gates.total))
        return false;
    return true;
};
export const readCelebrations = async (sddDir) => {
    ...
    try {
        ...
        if (code === 'ENOENT' || code === 'ENOTDIR')
            return { ledger: null, corrupt: false, path: statePath };
        return { ledger: null, corrupt: true, path: statePath, error: `${statePath}: ${err.message}` };
    }
    let candidate;
    try {
        ...
        return {
            ...
            corrupt: true,
            path: statePath,
            error: `${statePath}: JSON inválido (${err.message})`,
        };
    }
    ...
    return { ledger: { version: CELEBRATIONS_STATE_VERSION, entries: ledger.entries }, corrupt: false, path: statePath };
};
/** Escritura atómica: tmp + rename, el mismo patrón que `ratchet.ts`. */
const persistLedger = async (statePath, ledger) => {
    await mkdir(path.dirname(statePath), { recursive: true });
    const tmp = `${statePath}.tmp`;
    await writeFile(tmp, `${JSON.stringify(ledger, null, 2)}\n`, 'utf8');
    await rename(tmp, statePath);
};
const celebrationKey = (entry) => `${entry.feature}|${entry.date}|${entry.score}`;
/**
 ...
 */
export const appendCelebration = async (sddDir, input) => {
    const statePath = celebrationsStatePath(sddDir);
    const reject = (reason, corrupt = false, stateError) => ({
        appended: false,
        reason,
        path: statePath,
        corrupt,
    ...
    const ledger = { version: CELEBRATIONS_STATE_VERSION, entries: [...entries, entry] };
    try {
        await persistLedger(statePath, ledger);
    }
    catch (err) {
        return reject(`no se pudo escribir el libro en ${statePath} (${err.message}): esta validación no quedó registrada`, read.corrupt, read.error);
    }
    return {
        appended: true,
        reason: read.corrupt
 ...
 */
export const celebrateFeature = async (sddDir, input, opts = {}) => {
    const report = judgeFeatureCelebration(input, opts);
    if (report.verdict !== 'celebrate' || opts.record === false)
        return { report, ledger: null };
...
export const celebrationsToJson = (read, context = {}) => {
    const entries = read.ledger?.entries ?? [];
    return {
 ...
 */
export const renderCelebrations = (read, opts = {}) => {
    const color = opts.noColor !== true;
    const paintOk = (line) => (color ? `${GREEN}${line}${RESET}` : line);
    const paintWarn = (line) => (color ? `${YELLOW}${line}${RESET}` : line);
    const lines = [];
    if (read.corrupt) {
    ...
    for (const entry of entries) {
        lines.push(paintOk(`  ✔ ${entry.feature}  ${dateOnly(entry.date)}  score ${entry.score}/100 · fase ${entry.phase} · ` +
