 ...
 */
export const parseClaimsRegistry = (raw) => {
    const claims = [];
    const rejected = [];
    let current = null;
    let currentId = '';
    ...
    for (const line of raw.split(/\r?\n/)) {
        ...
        else if (key === 'statement_es')
            current.statementEs = value;
        else if (key === 'statement_en')
            current.statementEn = value;
        else if (key === 'verifier')
            current.verifier = value;
...
const runVerifier = (verifier, cwd, timeoutMs) => {
...
export const runClaimsRegistry = async (raw, options) => {
...
export const readClaimsRegistry = async (cwd, relPath = 'docs/claims/paper-claims.yaml') => readFile(path.join(cwd, relPath), 'utf8');
/** Human-readable one-line verdict per claim, for CLI output. */
export const renderClaimLines = (result, statements) => result.report.results.map(({ claim, status }) => {
