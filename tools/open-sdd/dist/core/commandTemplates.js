...
export const HOST_COMMAND_TEMPLATES = [
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: '$ARGUMENTS',
        argumentEvidence: 'VERIFIED: `https://docs.factory.ai/harness/custom-slash-commands.md` (fetched) documents project commands in `<repo>/.factory/commands/`, invoked `/command-name args`, with the `$ARGUMENTS` placeholder (and notes that `$1`/`$2` are NOT supported in the Markdown form).',
        verified: true,
        evidence: 'Factory Droid documents custom slash commands as Markdown files in `.factory/commands/` (user scope `~/.factory/commands/`), with optional YAML frontmatter (`description`, `argument-hint`), invoked as `/command-name`.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'NOT DOCUMENTED as an engine substitution: `https://roocodeinc.github.io/Roo-Code/features/slash-commands` (fetched) documents project commands in `.roo/commands/` (global `~/.roo/commands/`) as Markdown files with frontmatter, and `argument-hint` is display-only — there is no placeholder the engine substitutes, so the input arrives as a plain instruction.',
        verified: true,
        evidence: 'Roo Code documents project slash commands as Markdown files in `.roo/commands/`, invoked `/name`.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'NOT DOCUMENTED as an engine substitution: `https://kilocode.ai/docs/llms.txt` → `## Source: /customize/workflows` (fetched) documents project commands in `.kilo/commands/` (global `~/.config/kilo/commands/`) as Markdown files with frontmatter, and no argument placeholder; the page states there is no character limit on rule files.',
        verified: true,
        evidence: 'Kilo Code documents project workflow files as Markdown in `.kilo/commands/`, invoked `/name`.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: '$ARGUMENTS',
        argumentEvidence: 'VERIFIED: `https://mimo.xiaomi.com/mimocode/commands` (fetched) documents project commands in `.mimocode/commands/` (global `~/.config/mimocode/commands/`), "Place them in: Global … Per-project `.mimocode/commands/`", invoked `/test`, with `$ARGUMENTS`, `$1`..`$N`, `` !`cmd` `` and `@file`.',
        verified: true,
        evidence: 'MiMoCode documents custom commands as Markdown files in `.mimocode/commands/` with YAML frontmatter (`description`, `agent`, `model`), invoked `/name`.',
    ...
    },
    {
        ...
        verified: true,
        evidence: 'iFlow CLI documents project commands in `.iflow/commands/`, invoked `/command-name`; `--write` uses the Markdown form, which the same page documents.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: '$ARGUMENTS',
        argumentEvidence: 'SOURCE-VERIFIED, not documented: `https://zcode.z.ai/en/docs/commands` (fetched) states only that "workspace-level commands live in the project directory"; the exact path comes from the vendor\'s own resolver at `https://raw.githubusercontent.com/zai-org/ZCode/main/apps/zcode-cli/packages/adapters/src/commands/roots.ts`, which resolves `<base>/.zcode/commands` and `<base>/.agents/commands` per directory up to the worktree root, and `$ARGUMENTS` from `packages/contracts/src/commands/index.ts`. The evidence names the source file so a reader can re-check it.',
        verified: true,
        evidence: 'ZCode documents commands (user scope `~/.zcode/commands` verified) and its own resolver computes the project path `.zcode/commands`; the docs do not state the project path, so this row is SOURCE-VERIFIED and says so.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: '$ARGUMENTS',
        argumentEvidence: 'VERIFIED: `https://docs.augmentcode.com/using-augment/custom-commands.md` (fetched, and note that this Mintlify host serves real Markdown bodies on 404 — always check the status) documents project commands in `.augment/commands/` (user `~/.augment/commands/`), invoked `/name` with `dir:name` namespacing, with `$ARGUMENTS`.',
        verified: true,
        evidence: 'Augment Code documents custom commands as Markdown files in `.augment/commands/` with frontmatter (`description`, `argument-hint`, `model`).',
    ...
    },
    {
        ...
        verified: true,
        evidence: 'Qoder documents project commands in `.qoder/commands/` with `name`+`description` frontmatter, invoked `/name`; skills live in `.qoder/skills/` and rules in `.qoder/rules/` (with a documented 100,000-character budget across ALL active rule files).',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: '$ARGUMENTS',
        argumentEvidence: 'VERIFIED: `https://www.codebuddy.ai/docs/cli/slash-commands` (fetched; VitePress) documents project commands in `.codebuddy/commands/` as `<name>.md` with `argument-hint` frontmatter, nested commands becoming `/frontend:build`, and the `$ARGUMENTS` placeholder alongside `$1`..`$N`.',
        verified: true,
        evidence: 'CodeBuddy documents project slash commands as Markdown files in `.codebuddy/commands/`, invoked `/name`; native instructions live in CODEBUDDY.md with AGENTS.md as fallback only when CODEBUDDY.md is absent.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://moonshotai.github.io/kimi-code/llms.txt` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED y con la prueba del silencio: el indice completo de su propia documentacion (`https://moonshotai.github.io/kimi-code/llms.txt`, fetched) NO tiene pagina de comandos, y la pagina de barra dice que son "built-in control commands": un `/x` desconocido se envia como mensaje normal. NO existe directorio de comandos de proyecto, y por eso su integracion es skills + MCP, no plantillas.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://raw.githubusercontent.com/charmbracelet/crush/main/README.md` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED: su README (`https://raw.githubusercontent.com/charmbracelet/crush/main/README.md`, fetched) documenta las Agent Skills y el contexto de proyecto, y NO documenta comandos personalizados ni un directorio donde definirlos. Su superficie en este proyecto es MCP + skills.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://ampcode.com/llms.txt` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED por decision del fabricante: la propia documentacion dice "You do not need a slash command" (`https://ampcode.com/llms.txt`, fetched, indice de sus 54 paginas). No hay comandos de barra ni directorio de comandos: su superficie son las Agent Skills y AGENTS.md.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://docs.warp.dev/_llms-txt/agents.txt` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED: la copia para agentes de su documentacion (`https://docs.warp.dev/_llms-txt/agents.txt`, fetched) documenta `.warp/.mcp.json`, las diez rutas de skills y las reglas, y NO un directorio de comandos de proyecto (sus prompts viven en Warp Drive, en la nube). Su superficie aqui es MCP + skills.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://goose-docs.ai/docs/guides/context-engineering/using-skills.md` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED: su superficie documentada son las RECIPES (`.goose/recipes/`, ficheros YAML/JSON) y las skills, no comandos (`https://goose-docs.ai/docs/guides/context-engineering/using-skills.md`, fetched). Una recipe se invoca desde el mapa de `recipe_path` de la configuracion GLOBAL, que no es una convencion de proyecto: no se escribe ninguna.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://docs.openhands.dev/` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED: los comandos existen dentro de PAQUETES de plugin (`commands/<n>.md` con frontmatter `name: /x`), no como convencion de proyecto; su documentacion (`https://docs.openhands.dev/`, fetched por la investigacion) documenta AGENTS.md, las skills y `~/.openhands/mcp.json`, y ningun directorio de comandos del repositorio. Su superficie aqui son las skills.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://raw.githubusercontent.com/deepseek-ai/deepseek-harness/master/docs/subsystems/commands.md` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED y con la cita del fabricante: `https://raw.githubusercontent.com/deepseek-ai/deepseek-harness/master/docs/subsystems/commands.md` (fetched) describe los comandos como "Plugin-owned command registration", es decir, se registran por plugin y no hay directorio de ficheros de prompt en ninguna parte de su documentacion. Su superficie aqui son las skills y AGENTS.md.',
    ...
    },
    {
        ...
        invocation: (id) => `/sdd-${id}`,
        argumentSyntax: null,
        argumentEvidence: 'Sin marcador que declarar mientras no haya directorio de comandos: la convencion no existe y un placeholder seria una invencion; la documentacion revisada esta en `https://aider.chat/docs/usage/conventions.html` (fetched).',
        verified: false,
        evidence: 'NOT VERIFIED tras revisar sus 84 paginas de documentacion: no hay comandos personalizados (solo los `/` incorporados), ni skills, ni MCP. Su unica superficie de integracion es el fichero de convenciones (`.aider.conf.yml` -> `read: CONVENTIONS.md`), documentado en `https://aider.chat/docs/usage/conventions.html` (fetched), y AGENTS.md aparece CERO veces en toda su documentacion. Es un rechazo, no un hueco pendiente.',
 ...
 */
export const mapAgentToHost = (agent, host) => {
 ...
 */
const adaptFrontmatter = (raw, host, format) => {
    if (format !== 'markdown' || !host.frontmatterKeys)
        return raw;
