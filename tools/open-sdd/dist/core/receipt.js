...
export const receiptPath = (cwd) => path.join(cwd, RECEIPT_FILE);
const emptyReceipt = () => ({ schema: RECEIPT_SCHEMA, entries: [] });
/**
 ...
 */
export const normalizeRel = (cwd, target) => {
...
export const readReceipt = async (cwd) => {
    const file = receiptPath(cwd);
    if (!existsSync(file))
        return { receipt: emptyReceipt(), problem: null };
    try {
 ...
 */
export const recordReceipt = async (cwd, writes, now = new Date().toISOString()) => {
 ...
 */
export const planFromReceipt = async (cwd, receipt, options = {}) => {
    const entries = [];
...
const pruneEmptyParents = async (cwd, removed, keepSdd) => {
    const pruned = [];
    ...
    for (const dir of ordered) {
        ...
        if (keepSdd && (rel === '.sdd' || rel.startsWith('.sdd/')))
            continue;
...
export const applyUninstall = async (cwd, plan) => {
    const removed = [];
    const failed = [];
