...
const isTemplatePlaceholder = (token) => /^\{\{.*\}\}$/.test(token.trim());
/**
...
export const traceDelta = (delta, tasks) => {
     ...
     */
    const declaredRequirementTokens = (task) => {
        const placeholders = Array.from(new Set(Array.from(task.raw.matchAll(TEMPLATE_PLACEHOLDER_RE)).map((m) => m[0])));
    ...
    for (const entry of delta.entries) {
        const owners = tasks.filter((t) => declaredRequirementTokens(t).ids.some((cited) => cited.toUpperCase() === entry.id));
        if (owners.length === 0) {
    ...
    for (const task of tasks) {
        ...
        for (const cited of ids) {
            if (!requirementIds.has(cited.toUpperCase()))
    ...
    return {
        ...
        detail: delta.entries.length === 0
            ...
            : `${mapped.length}/${delta.entries.length} requisito(s) de la delta con tarea (${Math.round(coverage * 100)}%)${unmapped.length > 0 ? `; sin tarea: ${unmapped.join(', ')}` : ''}${phantomTasks.length > 0 ? `; tareas citando ids inexistentes: ${phantomTasks.map((p) => `${p.taskId}→${p.cited}`).join(', ')}` : ''}${unfilledPlaceholders.length > 0
    ...
    for (const raw of markdown.split('\n')) {
        ...
        if (field && current) {
            ...
            if (key === 'Statement')
                ...
                current.merged = /^true$/i.test(value.trim());
        }
    }
    push();
    return delta;
...
const sameText = (a, b) => collapse(a) === collapse(b);
const parseBaseRequirementBlocks = (lines) => {
    const blocks = [];
    let headingIndex = -1;
    let id = '';
    let title = '';
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (headingIndex >= 0 && /^#{2,3}\s/.test(lines[i]))
            flush(i);
    }
    flush(lines.length);
    return blocks;
};
const blockStatements = (block, lines) => lines
 ...
 */
const resolveBaseTarget = (blocks, entry) => {
...
const requirementsSectionRange = (lines) => {
...
const addedInsertionIndex = (lines, blocks, section) => {
    const inside = blocks.filter((b) => b.headingIndex > section.start && b.headingIndex < section.end);
    if (inside.length > 0)
        return inside[inside.length - 1].endIndex;
...
const renderRequirementBlock = (id, title, statement) => [
    `### ${id} — ${title}`,
    `- Statement: ${statement.trim()}`,
];
export const mergeDeltaIntoBase = (delta, base, options = {}) => {
    ...
    for (const entry of delta.entries) {
        ...
        if (entry.kind === 'ADDED') {
            ...
            if (existing) {
                ...
                if (statements.length > 0 && statements.every((s) => sameText(s, statement))) {
                    ...
                    refused(entry, `La base ya define ${existing.id} con otro contenido: una entrada ADDED no puede añadir lo que ya existe; el cambio de lo existente es MODIFIED.`, existing.id);
                }
                continue;
            }
        ...
        if (entry.kind === 'MODIFIED') {
            ...
            if (!target) {
                if (alreadyApplied)
                    unchanged(entry, `El objetivo de ${id} ya no está en la base y la delta consta como fusionada: se interpreta como ya aplicado.`);
                else
                    refused(entry, `La entrada MODIFIED no encuentra en la base ni ${id} ni "${entry.previous}": no hay comportamiento que sustituir.`);
                continue;
            }
        ...
        if (entry.kind === 'REMOVED') {
            ...
            if (!target) {
                if (alreadyApplied)
                    unchanged(entry, `El objetivo de ${id} ya no está en la base y la delta consta como fusionada: la eliminación ya se aplicó.`);
                else
                    refused(entry, `La entrada REMOVED no encuentra en la base ni ${id} ni "${entry.previous ?? ''}": no se puede eliminar lo que no está.`);
                continue;
            }
        ...
        if (newBlock && oldBlock && newBlock !== oldBlock) {
            refused(entry, `Ya existen ${oldBlock.id} y ${id}: renombrar colisionaría con un requisito distinto.`, oldBlock.id);
            continue;
        }
        if (!oldBlock) {
            if (alreadyApplied)
                unchanged(entry, `El objetivo de ${id} ya no está en la base y la delta consta como fusionada: el renombrado ya se aplicó.`);
            else
                refused(entry, `La entrada RENAMED no encuentra "${entry.previous}" en la base: no hay requisito que renombrar.`);
            continue;
        }
        ops.push({
    ...
    if (refusals.length > 0) {
        ...
        const withheld = changes.map((change) => change.outcome === 'refused' || change.outcome === 'unchanged'
            ? change
    ...
    for (const op of ops)
        lines.splice(op.start, op.end - op.start, ...op.replacement);
    ...
    if (text === base) {
        ...
        const normalized = changes.map((change) => change.outcome === 'unchanged' || change.outcome === 'refused'
            ? change
