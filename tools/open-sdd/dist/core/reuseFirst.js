...
export const REUSE_FIRST_RULE = 'Antes de generar cualquier clase o método nuevo, busca si ya existe uno reutilizable. Crear código nuevo solo está permitido cuando no existe nada reutilizable: la duplicación es un riesgo enorme en brownfield.';
...
export const scanDeclarations = (content) => {
    const found = [];
    const seen = new Set();
    content.split(/\r?\n/).forEach((line, index) => {
...
const buildDeclarationIndex = async (cwd, sourceDirs) => {
    const symbols = [];
    const unreadable = [];
    let truncated = false;
    let filesScanned = 0;
...
const tokensOf = (name) => Array.from(new Set(name
 ...
 */
export const similarityOf = (requested, declared) => {
    const a = normalizeName(requested);
    const b = normalizeName(declared);
    if (!a || !b)
        return null;
    if (a === b)
        return { similarity: 1, kind: 'same-name' };
    ...
    if (pluralVariant || prefixOrSuffix)
        return { similarity: 0.8, kind: 'name-variant' };
    if (shorter.length >= 4 && levenshtein(a, b) <= 2)
        return { similarity: 0.6, kind: 'same-module' };
    if (tokenOverlap(requested, declared) >= 0.6)
        return { similarity: 0.6, kind: 'same-module' };
    return null;
};
// ---------------------------------------------------------------------------------------------
// La búsqueda
// ---------------------------------------------------------------------------------------------
export const findReuseCandidates = async (input) => {
    const cwd = input.cwd;
    ...
    const index = sourceDirs.length > 0
        ? await buildDeclarationIndex(cwd, sourceDirs)
        : { symbols: [], unreadable: [], truncated: false };
    if (sourceDirs.length === 0) {
        complete = false;
        notes.push('No se detectó ningún directorio de código fuente legible: la búsqueda de reutilización NO se ejecutó, así que «no hay candidatos» no está demostrado y no se informa como si lo estuviera.');
    }
    ...
    for (const request of input.requests) {
        ...
        for (const declared of index.symbols) {
            ...
            if (!verdict || verdict.similarity < threshold)
                continue;
            matches.push({
        ...
        if (matches.length > 0) {
            ...
            if (loose)
                belowThreshold.push(request.symbol);
            else
                unmatched.push(request.symbol);
        }
        matches
    ...
    const finalCandidates = Array.from(deduped.values()).sort((a, b) => b.similarity - a.similarity || a.symbol.localeCompare(b.symbol) || a.file.localeCompare(b.file));
    if (violations.length > 0) {
        notes.push(`Violaciones de reutilización primero (${violations.length}): ${violations.join(', ')}. Estos símbolos ya tienen candidato reutilizable: crear una versión nueva duplica lógica existente.`);
    }
    if (belowThreshold.length > 0) {
        notes.push(`Sin candidato por encima del umbral ${threshold} (revisar antes de crear): ${belowThreshold.join(', ')}.`);
    }
    if (unmatched.length > 0) {
        notes.push(`El símbolo no existe en el proyecto, así que crearlo es legítimo: ${unmatched.join(', ')}. Esto no es una violación, es la excepción que la política permite.`);
    }
