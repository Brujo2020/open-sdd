...
export const CHECKLIST_PREDICATE_KINDS = ['cmd', 'artifact', 'property', 'trace'];
 ...
 */
export const parseChecklist = (markdown) => {
...
const defaultRunCommand = (cmd, cwd) => {
    try {
...
export const resolveTrace = (value, artifacts) => {
...
const runPredicates = (item, input) => {
...
const renderEvidence = (result, recordedAt) => {
 ...
 */
export const verifyChecklist = (input) => {
    ...
    while (i < lines.length) {
        ...
        if (result && result.digest !== '')
            kept.push(renderEvidence(result, input.recordedAt));
        out.push(...kept);
    }
...
export const renderChecklistReport = (result) => {
    const lines = [];
    lines.push(`${result.items.length} item(s) · ${result.checkedItems} with a predicate · ${result.passed} verified · ${result.failures.length} fail · ${result.stale.length} stale`);
    for (const failure of result.failures)
        lines.push(`fail  ${failure.itemId} line ${failure.line}: ${failure.reason}`);
    for (const item of result.stale)
        lines.push(`stale ${item.itemId} line ${item.line}: ${item.reason}`);
    return lines.join('\n');
};
