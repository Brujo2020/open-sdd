 ...
 */
import { EARS_LIMIT, EARS_TEMPLATES, containsVagueTerm } from './ears.js';
export const EARS_PATTERNS = ['ubiquitous', 'event', 'state', 'option', 'unwanted'];
...
export const isEarsProposalApplicable = (suggestion) => !suggestion.proposal.trim().startsWith(EARS_NEEDS_INFORMATION);
// ---------------------------------------------------------------------------------------------
// Plantillas y ejemplos reales de este repositorio
// ---------------------------------------------------------------------------------------------
const VALIDATOR_TO_ASSISTANT = {
 ...
 */
export const EARS_REPO_EXAMPLES = [
    {
        pattern: 'ubiquitous',
...
export const EARS_ASSISTANT_TEMPLATES = EARS_TEMPLATES.map((template) => {
 ...
 */
export const EARS_SUGGESTION_CATALOGUE = [
    {
 ...
 */
const isDomainFlexible = (text) => /\bflexible\s+(regime|régimen|governance|gobierno)\b/i.test(text);
const VAGUE_HINTS = [
    {
...
const hintFor = (term) => VAGUE_HINTS.find((entry) => entry.re.test(term))?.hint ?? 'el valor o criterio observable que la sustituya';
...
const keywordToPattern = (keyword) => {
    ...
    if (lower === 'if' || lower === 'si')
        return 'unwanted';
    if (lower === 'while' || lower === 'mientras')
        return 'state';
    if (lower === 'where')
        return 'option';
    return 'event';
...
const PERSON_ACTOR = /\b(?:the|a|an|el|la|los|las|un|una)?\s*\[?(?:user|users|usuario|usuarios|developer|developers|desarrollador|desarrolladores|client|clients|cliente|clientes|customer|customers|person|persona|human|humano|operator|operador|engineer|ingeniero|auditor|auditors)\b/i;
const capitalizeFirst = (text) => text.length === 0 ? text : text[0].toUpperCase() + text.slice(1);
const lowerFirstWord = (text) => /^\[/.test(text) ? text : text.replace(/^([A-ZÀ-Þ])/, (m) => m.toLowerCase());
const SENTENCE_END = /[.!?]\s*$/;
const withPeriod = (text) => {
    const trimmed = text.trim().replace(/[,;:]+$/, '').trim();
    return SENTENCE_END.test(trimmed) ? trimmed : `${trimmed}.`;
};
const NON_STATEMENT_FIELD = /^(?:previous|anterior|targets|objetivos|contracts|contratos|rationale|motivo|justificaci[oó]n|strangler|merged|base|status|estado|depends|depende|acceptance|aceptaci[oó]n|objective|objetivo|scope|alcance|risks|riesgos|notes|notas)\b/i;
const looksLikeRequirementStart = (body) => /^(?:the|el|la|los|las|un|una|when|whenever|while|where|if|once|after|upon|shall|cuando|si|mientras|all|every|cada|todo|toda)\b/i.test(body) || /^\[[^\]]+\]/.test(body);
/**
...
const extractRequirements = (text) => {
    ...
    for (const raw of text.split('\n')) {
        ...
        if (trimmed.length === 0 || /^#{1,6}\s/.test(trimmed) || trimmed.startsWith('|'))
            continue;
        ...
        if (bullet) {
            ...
            if (statementField) {
                ...
                if (statement.length === 0)
                    ...
                    requirements.push({ line: trimmed, statement });
                continue;
            }
        ...
        if (/\bshall\b/i.test(trimmed))
            requirements.push({ line: trimmed, statement: trimmed });
    }
 ...
 */
const dominantActor = (candidates) => {
    ...
    if (count < 2 || count / candidates.length < 0.6)
        return null;
...
const baseVerb = (word) => {
    ...
    if (IRREGULAR_BASE[lower])
        return IRREGULAR_BASE[lower];
    if (/(at|it|ut|ov|ur|iz|is|os|us|as|or|u|o)ed$/.test(lower))
        return lower.slice(0, -1);
    if (/ed$/.test(lower)) {
        let stem = lower.slice(0, -2);
        if (/([bdgklmnprt])\1$/.test(stem))
            stem = stem.slice(0, -1);
...
const DETERMINER_OR_PREPOSITION = /^(?:the|a|an|its|their|his|her|that|this|those|these|to|for|with|in|on|at|from|into|by|each|every)$/;
/**
...
const negateClause = (clause) => {
    ...
    for (let i = 1; i < words.length; i += 1) {
        ...
        if (bare.length < 3 || !DETERMINER_OR_PREPOSITION.test(next))
            continue;
...
const NON_PASSIVE_PARTICIPLES = /^(?:able|unable|available|responsible|consistent|idempotent|present|absent|visible|empty|valid|true|false|required|allowed|expected|possible|necessary|ready|sure)$/i;
 ...
 */
const splitCompound = (statement) => {
    const marks = [...statement.matchAll(/\bshall\b/gi)].map((match) => match.index ?? -1);
    if (marks.length < 2 || marks.some((index) => index < 0))
        return null;
    ...
    for (const segment of segments) {
        ...
        if (subject.length === 0 || response.replace(/\bshall\b/i, '').trim().length === 0)
            return null;
...
const classifyStatement = (statement) => {
    ...
    for (const entry of KEYWORD_PATTERNS) {
        if (entry.re.test(trimmed))
            return { pattern: entry.pattern, keyword: entry.keyword };
    }
    return { pattern: 'ubiquitous', keyword: null };
};
const isGoal = (statement) => /\b(quiero|necesito|deseo|objetivo|meta|i want|we want|be able to|ser capaz de|as an? [a-zà-ÿ]+)\b/i.test(statement) ||
...
const deriveShall = (statement) => {
    const match = statement.match(/^((?:The|El|La|Los|Las|the|el|la|los|las)\s+(?:\[[^\]]+\]|[A-Za-zÀ-ÿ][\w.-]*))\s+([A-Za-zÀ-ÿ]+)\b(.*)$/);
    if (!match)
        return null;
...
const analyseCandidate = (candidate, context) => {
    ...
    if (shallMarks.length > 1) {
        ...
        if (classified.pattern === 'ubiquitous' && shallIndex >= 0) {
            ...
            const trailing = response.match(/,\s*(when|whenever|if|while|once|after|upon|si|cuando|mientras)\s+(.+)$/i);
    ...
    for (const term of VAGUE_TERMS) {
        if (!containsVagueTerm(statement, term))
            continue;
        if (term.toLowerCase() === 'flexible' && isDomainFlexible(statement))
            continue;
 ...
 */
export const analyseEars = (text, options = {}) => {
    const source = options.source?.trim() || '(texto sin fuente)';
    const extraction = extractRequirements(text ?? '');
    const context = { dominant: dominantActor(extraction.requirements) };
    ...
    if (analysed.length === 0) {
        ...
        detail.push(`${analysed.length} requisito(s) analizado(s) en ${source}: ${conforming} conforme(s), ${suggestions.length} sugerencia(s) (${errors} error, ${warnings} aviso).`);
        ...
        if (conforming > 0 && patterns.ubiquitous === conforming) {
            detail.push('Todos los requisitos conformes son ubicuos: el documento no dice CUÁNDO ocurre nada, así que no hay disparadores que probar.');
        }
        else if (patterns.unwanted === 0) {
            detail.push('Ningún requisito conforme usa IF…THEN: es el patrón que los equipos omiten y el único hogar de primera clase para requisitos negativos.');
        }
        if (open > 0) {
    ...
    if (extraction.goals.length > 0) {
        detail.push(`${extraction.goals.length} objetivo(s) declarados como "Objective" no se han contado como requisitos: son metas, no comportamientos.`);
    }
...
export const mergeEarsReports = (reports) => {
    ...
    for (const report of usable) {
        ...
        for (const pattern of EARS_PATTERNS)
            patterns[pattern] += report.patterns[pattern] ?? 0;
    ...
    return {
        ...
        suggestions,
        patterns,
        detail: `Análisis agregado de ${usable.length} fuente(s): ${total} requisito(s), ${conforming} conforme(s), ` +
 ...
 */
export const deltaStatementText = (deltaMarkdown, alreadyAnalysed = []) => {
...
// ---------------------------------------------------------------------------------------------
const INTENT_PREFIX = /^\s*(?:yo\s+)?(?:quiero|necesito|deseo|me\s+gustar[íi]a|pretendo|i\s+want|we\s+want|i\s+need|we\s+need|the\s+system\s+should|el\s+sistema\s+deber[íi]a)\s+(?:que\s+)?/i;
...
const normalizeResponse = (text) => text
    .trim()
    .replace(/^(?:que\s+)?/i, '')
    .replace(MODAL_PREFIX, '')
    .replace(/[.;,]+$/, '')
    .trim();
const looksSpanish = (text) => /[áéíóúñ¿¡]|\b(?:el|la|los|las|que|cuando|sistema|usuario)\b/i.test(text);
...
const reorderPassiveFragment = (response) => {
    const match = response.match(PASSIVE_FRAGMENT);
    if (!match)
        return { response, reordered: false };
 ...
 */
export const describeFromPlainLanguage = (description) => {
    ...
    const suggestion = {
        ...
        proposal,
        why: `Patrón ${pattern}${keyword ? ` (${keyword})` : ''}: ` +
            (pattern === 'ubiquitous'
 ...
 */
export const earsEvidencePack = (report, options = {}) => {
    ...
    if (!report.complete) {
        undetermined.push(report.total === 0
    ...
    if (!report.complete)
        incompleteBecause.push(`La cobertura de ${report.source} está incompleta (ver undetermined).`);
    if (open.length > 0) {
        incompleteBecause.push(`${open.length} hallazgo(s) no traen una frase lista: falta un dato que solo puede aportar quien escribe el requisito. No los rellenes inventando: pregúntalo.`);
    }
    if (report.total === 0)
        incompleteBecause.push('No había requisitos que analizar: no declares que el documento es conforme.');
    ...
    return {
        ...
        incompleteBecause,
        limit: EARS_LIMIT,
        noModelShipped: 'Esta herramienta no embarca ningún modelo: el texto final lo escribe tu modelo anfitrión a partir de este paquete. Toda propuesta marcada como pregunta exige un dato que solo conoce quien escribe el requisito: no la rellenes inventando.',
    };
};
// ---------------------------------------------------------------------------------------------
// Salida humana
 ...
 */
export const renderEarsReport = (report, options = {}) => {
    ...
    lines.push(`  ${report.total} requisito(s) · ${report.conforming} conforme(s) · ${report.suggestions.length} sugerencia(s) (${errors} error, ${warnings} aviso) · ${report.complete ? 'análisis completo' : 'INCOMPLETO'}`);
    lines.push(`  patrones (solo conformes): ${EARS_PATTERNS.map((pattern) => `${pattern} ${report.patterns[pattern] ?? 0}`).join(' · ')}`);
    lines.push('');
    if (report.suggestions.length === 0) {
        lines.push(report.total === 0
    ...
    report.suggestions.forEach((suggestion, index) => {
        ...
        if (suggest) {
            ...
            if (suggestion.example)
                lines.push(`      ejemplo: ${suggestion.example}`);
        }
    });
    lines.push('');
