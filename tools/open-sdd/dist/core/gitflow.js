...
import { DEFAULT_RIGOR_LEVEL, effectiveGates, isRigorLevel } from './rigor.js';
...
const DEVELOP_NAMES = ['develop', 'development'];
/**
...
export const CORE_CHAIN_IDS = EXECUTABLE_CHAIN.filter((g) => g.posture !== 'opt-in').map((g) => g.id);
...
const readRole = (branch) => {
    ...
    if (DEVELOP_NAMES.includes(branch)) {
        return {
            role: 'develop',
            feature: null,
            version: null,
    ...
    for (const { prefix, role } of ROLE_PREFIXES) {
        ...
        return {
            ...
            feature,
            version,
            reason: `el prefijo "${prefix}" nombra el rol ${role}${feature ? ` y la feature "${feature}"` : ''}${version ? ` y la versión "${version}"` : ''}.`,
        };
    }
    ...
    switch (role) {
        ...
        case 'other': {
            if (defaultBranch !== null && defaultBranch !== branch)
                return defaultBranch;
            if (developBranch !== null && developBranch !== branch)
 ...
 */
export const detectGitFlow = async (cwd) => {
    if (!isGitRepo(cwd)) {
        return {
            model: 'none',
            branch: null,
    ...
    const evidence = [
        ...
        `rama actual: ${branch ?? (rawBranch === 'HEAD' ? 'HEAD desprendido' : 'sin rama (repositorio sin commits)')}`,
    ];
    let model;
    let modelReason;
    if (branches.length === 0) {
        ...
        model = 'gitflow';
        modelReason = `gitflow: "${defaultBranch}" y "${developBranch}" existen a la vez${roleBranches.length > 0
        ...
        model = 'none';
        modelReason = `existe "${defaultBranch}" pero no develop, y hay más de una rama (${branches.join(', ')}): no es gitflow ni un trunk de una sola rama, así que no se afirma ningún modelo.`;
    }
    else {
        model = 'none';
        modelReason = `las ramas observadas (${branches.join(', ')}) no corresponden ni a gitflow (develop + main) ni a un trunk de una sola rama, así que no se afirma ningún modelo.`;
    }
    ...
    if (base !== null) {
        const fallback = reading.role === 'feature' && developBranch === null && defaultBranch !== null
        ...
        evidence.push(reading.role === 'main'
    ...
    const detail = `${modelReason} Rol ${reading.role}${reading.feature ? ` de "${reading.feature}"` : ''}${reading.version ? ` (versión "${reading.version}")` : ''}${base !== null ? `; base de comparación "${base}"` : '; sin base de comparación'}.`;
    return {
        model,
        branch,
        role: reading.role,
...
// Política por rol
// ---------------------------------------------------------------------------------------------
const deltaPath = (feature) => feature ? `.sdd/specs/${feature}/delta.md` : '.sdd/specs/<feature>/delta.md';
/**
 ...
 */
const changeContract = (brownfield) => [
    brownfield
...
const levelOf = (opts) => isRigorLevel(opts?.level) ? opts.level : DEFAULT_RIGOR_LEVEL;
const union = (base, extra) => [
    ...base,
 ...
 */
export const branchPolicy = (state, opts) => {
    ...
    switch (state.role) {
        case 'feature':
            return {
                ...
                detail: `Rol feature: el contrato del cambio es la delta de ${feature ? `"${feature}" (${deltaPath(feature)})` : 'la feature que nombra la rama'}; los gates se ejecutan contra "${base}". Lo exigido que no figura en "bloquea hoy" es consejo, no candado.`,
            };
        case 'release':
            return {
                role: 'release',
                ...
                detail: `Rol release${state.version ? ` (versión "${state.version}")` : ''}: exige todo lo de una rama de cambio MÁS changelog/versión, bundle de auditoría y CI verde; se fusiona en ${state.base ?? 'main'} y vuelve a develop. Solo la CI del PR bloquea hoy.`,
            };
        case 'hotfix':
            return {
                role: 'hotfix',
                required: [
                    ...
                    `gates contra ${base} y fusión a main y a develop con tag de la versión${state.version ? ` "${state.version}"` : ''}`,
                ],
                ...
                nextStep: 'open-sdd gates run',
                detail: `Rol hotfix${state.version ? ` (versión "${state.version}")` : ''}: la constitución y la evidencia siguen siendo exigibles, y la delta posterior a la fusión es obligatoria para que el arreglo no desaparezca de la especificación. Nada de eso bloquea hoy por sí solo.`,
            };
        case 'main':
            return {
                role: 'main',
                ...
                nextStep: 'open-sdd gates run',
                detail: 'Rol main: pivote de fusiones, no de trabajo. El pivote debe pasar la cadena completa, pero hoy nada impide un commit directo en main.',
            };
        case 'develop':
            return {
                role: 'develop',
                ...
                detail: 'Rol develop: la rama de integración. Las features se fusionan aquí y el pivote debe pasar antes de cortar una release; no hay candado propio más allá del suelo.',
            };
        case 'other':
        default:
            return {
                ...
                required: feature
                    ...
                    : [
                        `rol no reconocido: la rama ${state.branch ? `"${state.branch}"` : '(sin rama)'} no declara rol; nómbrala feature/<x>, feat/<x>, release/<v>, hotfix/<v> o bugfix/<x>`,
...
const clip = (text, width = LINE_WIDTH) => text.length > width ? `${text.slice(0, width - 1)}…` : text;
export const renderGitFlow = (state, policy) => {
    const lines = [];
    lines.push(`GitFlow — modelo ${state.model} · rama ${state.branch ?? '(sin rama)'} · rol ${policy.role}`);
    lines.push(`  base: ${state.base ?? '—'} · feature: ${state.feature ?? '—'} · versión: ${state.version ?? '—'}`);
    lines.push(`  ${clip(state.detail)}`);
    lines.push('  evidencia:');
    for (const item of state.evidence.slice(0, 5))
        lines.push(`    - ${clip(item)}`);
    lines.push('');
    lines.push(`Requerido por el rol ${policy.role}:`);
    for (const item of policy.required)
        lines.push(`  · ${clip(item)}`);
    lines.push(`  gates: ${policy.gates.join(', ') || '—'}`);
    lines.push('');
    lines.push('Bloquea hoy (solo el suelo instalado):');
    for (const item of policy.blocking)
        lines.push(`  ! ${clip(item)}`);
