 ...
 */
export const SUPPORTED_PROTOCOL_VERSIONS: readonly string[] = ['2024-11-05', '2024-10-07'];
...
export const toErrorObject = (error: unknown, method?: string): JsonRpcErrorObject => {
...
export const errorResponse = (id: JsonRpcId, error: JsonRpcErrorObject): JsonRpcErrorResponse => ({
  jsonrpc: JSONRPC_VERSION,
  id,
  error,
});
...
export const buildInitializeResult = (params: unknown, info: McpServerInfo): McpInitializeResult => {
...
// ---------------------------------------------------------------------------------------------
// Parseo y validación de un mensaje
// ---------------------------------------------------------------------------------------------

export type ParsedMessage =
  | { kind: 'request'; request: JsonRpcRequest }
  | { kind: 'notification'; notification: JsonRpcNotification }
  | { kind: 'invalid'; id: JsonRpcId; error: JsonRpcErrorObject };

const invalidRequest = (id: JsonRpcId, message: string): ParsedMessage => ({
  kind: 'invalid',
  id,
...
const readId = (value: unknown): { valid: boolean; id: JsonRpcId } => {
  if (value === undefined || value === null) return { valid: true, id: null };
  if (typeof value === 'string' || typeof value === 'number') return { valid: true, id: value };
 ...
 */
export const parseJsonRpcMessage = (raw: string): ParsedMessage => {
  let decoded: unknown;
  try {
  ...
  if (!valid) return invalidRequest(null, 'El campo "id" debe ser una cadena, un número o null.');
...
// ---------------------------------------------------------------------------------------------
// Peer: despacho con la garantía de «nunca romper el stream»
// ---------------------------------------------------------------------------------------------
...
export interface RpcRequestContext {
  ...
  method: string;
  params: unknown;
  /** true cuando el mensaje era una notificación (no se responde). */
  notification: boolean;
}

export type RpcHandler = (context: RpcRequestContext) => Promise<unknown> | unknown;
export type RpcNotificationHandler = (context: RpcRequestContext) => Promise<void> | void;

export interface JsonRpcPeerOptions {
  /** Escribe UNA línea de protocolo (sin salto de línea; lo añade el peer). */
  write: (line: string) => void;
  handlers: Record<string, RpcHandler>;
  notifications?: Record<string, RpcNotificationHandler>;
  /** Diagnóstico humano; NUNCA va a stdout. */
...
export class JsonRpcPeer {
  ...
  async handleChunk(chunk: string): Promise<void> {
    for (const line of this.framer.push(chunk)) await this.handleLine(line);
  }

  /** Procesar lo que quedó sin salto final al cerrarse la entrada. */
  async flush(): Promise<void> {
    for (const line of this.framer.flush()) await this.handleLine(line);
  }
...
/** Tipo estructural mínimo de los flujos que consume el servidor; facilita inyectar dobles. */
