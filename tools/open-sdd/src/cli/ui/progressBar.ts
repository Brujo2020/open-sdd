...
export const renderProgressBar = (state: ExecutionState): string[] => {
  const lines: string[] = [];
  ...
  lines.push(
    `  ${bar} ${statusColor(
      `${progress}/${state.totalTasks}`
    )} (${percent}%) | Wave ${state.currentWave}/${state.totalWaves}`
  );
  lines.push('');
...
export const renderTaskUpdate = (task: TaskProgress, action: 'start' | 'complete' | 'fail'): string => {
  const icons = {
    start: colors.cyan('⟳'),
...
export const renderWaveStart = (waveNum: number, totalWaves: number, parallelCount: number): string => {
  return colors.cyan(`\n▶︎ Wave ${waveNum}/${totalWaves} (${parallelCount} parallel subagents running...)\n`);
};

export const renderExecutionSummary = (state: ExecutionState): string[] => {
  const lines: string[] = [];
  lines.push('');
