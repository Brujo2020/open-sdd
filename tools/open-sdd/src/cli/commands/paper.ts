...
import { INVARIANTS, CONFORMITY_LEVELS, assessConformity, type InvariantEvidence } from '../../core/invariants.js';
import { HITL_DEFAULTS, evaluateEscalations, selectRigorMode, calibrateEscalationThreshold } from '../../core/hitl.js';
import { recalibrate, assessI6, acceptedRiskLedger, type RelaxationReceipt } from '../../core/receipts.js';
...
const row = (label: string, value: string, width = 22): string =>
  `  ${label.padEnd(width)} ${value}`;

const outcomeColor = (outcome: string): string => {
  if (outcome === 'pass') return colors.green(outcome);
...
export const detectRepoSignals = async (cwd: string): Promise<{ signals: RepoSignals; evidence: [string, string][] }> => {
  const root = await findRepoRoot(cwd);
  const evidence: Record<string, string | undefined> = {};

  const exists = async (rel: string): Promise<boolean> =>
    (await stat(path.join(root, rel)).catch(() => null)) !== null;
  ...
  for (const rel of ['.mcp.json', '.cursor/mcp.json', '.vscode/mcp.json']) {
    ...
    if (file && /"mcpServers"\s*:\s*\{[^}]*\}/s.test(file)) {
      ...
      if (names.length > 0) evidence.declaresThirdPartyMcpServers = `${names.join(', ')} en ${rel}`;
    }
  }

  // Architecture decisions registered.
  for (const rel of ['docs/adr', 'docs/decisions']) {
    const entries = await readdir(path.join(root, rel)).catch(() => [] as string[]);
    if (entries.length > 0) evidence.hasAdrRecords = `${entries.length} registro(s) en ${rel}/`;
  }
  ...
  for (const rel of ['.sdd/memory', '.sdd/.memory-inbox']) {
    const entries = await readdir(path.join(root, rel)).catch(() => [] as string[]);
    if (entries.length > 0) evidence.hasLivingMemory = `${entries.length} ítem(s) en ${rel}/`;
  }
  ...
  if (files > 400) evidence.exceedsContextWindow = `${files} ficheros de código`;

  const { signals, evidence: rows } = detectSignals(evidence);
  return { signals, evidence: rows.map((r) => [r.signal, r.detail] as [string, string]) };
};

const countSourceFiles = async (dir: string, depth = 0): Promise<number> => {
  if (depth > 4) return 0;
  const entries = await readdir(dir, { withFileTypes: true }).catch(() => []);
  let count = 0;
  for (const e of entries) {
    if (['node_modules', '.git', 'dist', 'build', 'coverage'].includes(e.name)) continue;
    if (e.isDirectory()) count += await countSourceFiles(path.join(dir, e.name), depth + 1);
    else if (/\.(ts|tsx|js|mjs|cjs|py|go|java|rb|rs)$/.test(e.name)) count += 1;
  }
  return count;
};
 ...
 */
const readFlagValue = (args: string[], long: string, short?: string): string | undefined => {
  for (let i = 0; i < args.length; i += 1) {
    const token = args[i];
    const eq = token.indexOf('=');
    const name = eq > -1 ? token.slice(0, eq) : token;
    if (name !== long && (short === undefined || name !== short)) continue;
...
const parseProfile = (args: string[]): ChainProfile => {
 ...
 */
const parseGatesRunArgs = (args: string[]): { ok: true; value: GatesRunArgs } | { ok: false; error: string } => {
  const gateIds: string[] = [];
  let base: string | undefined;
  let staged = false;
  let strict = false;
  let profile: ChainProfile = 'solo';
...
export const handleGatesCommand = async (args: string[], io: CliIO, cwd: string): Promise<number> => {
  ...
  if (sub === 'chain') {
    ...
    io.log(
      `  ${colors.bold('Activables por perfil')}: ${
        chain.profileMandated.map((c) => c.id).join(' ') || '(ninguno)'
      }`,
    );
    io.log(
      `  ${colors.bold('Activables por señal')}: ${
        chain.signalActivated.map((c) => `${c.id} (${c.reason})`).join('; ') || '(ninguna)'
      }`,
    );
    io.log('');
    ...
    io.log('');
    io.log(
      `  ${colors.bold('Declarados')}: ${chain.declared.length}   ${colors.bold('Ejecutables')}: ${chain.executed.length}   ${colors.bold('No implementados')}: ${chain.notImplemented.length}   ${colors.bold('Vacíos')}: ${chain.vacuous.length}`,
    );
    if (chain.notImplemented.length === 0) {
      ...
      io.log(
        `  ${dim('«No implementados: 0» se deriva de este catálogo: ningún control declarado aquí carece de implementación. No significa que la brecha declarado-vs-ejecutado del prototipo de referencia (§9.5) se haya cerrado.')}`,
      );
    }
    if (chain.vacuous.length > 0) {
      for (const v of chain.vacuous) io.log(`  ${colors.yellow('!')} ${v.id}: ${v.reason}`);
    }
    io.log('');
  ...
  if (sub === 'crosswalk') {
    ...
    io.log('');
    io.log(
      `  ${summary.logical} controles lógicos · ${summary.executable} ejecutables · ${summary.vacuous} vacíos · ${summary.covered} cubiertos · ${summary.residue} en el residuo`,
    );
    io.log('');
    return 0;
  }
  ...
  if (sub === 'enforcement') {
    ...
    for (const t of TOOL_ENFORCEMENT) {
      ...
      io.log(
        `  ${t.label.padEnd(16)} ${t.writeCeiling.padEnd(14)} ✓  ✓  ${t.mcpProxy ? '✓' : '—'}   ${floor.floor
          .map((f) => f.id)
          .join('')}           ${floor.beyondFloor.map((f) => f.id).join('') || '—'}`,
      );
      if (t.caveat) io.log(`      ${dim(t.caveat)}`);
    }
    io.log('');
  ...
  if (sub === 'run') {
    ...
    if (unknownIds.length > 0) {
      io.error(
        colors.red(
          `Control(es) no reconocido(s): ${unknownIds.join(', ')}. Ids válidos: ${EXECUTABLE_CHAIN.map((g) => g.id).join(', ')}.`,
        ),
      );
      return 1;
    }
    ...
    if (base !== undefined && runGit(root, ['rev-parse', '--verify', '--quiet', `${base}^{commit}`]).status !== 0) {
      io.error(
        colors.red(
          `La referencia base "${base}" no existe en este repositorio: la cadena no puede juzgar un cambio que no puede resolver.`,
        ),
      );
      return 1;
    }
    ...
    io.log(
      heading(
        `Ejecutando la cadena resuelta (${gateIds.length} control(es), perfil ${runProfile}, régimen ${strict ? 'estricto' : 'flexible'})`,
      ),
    );
    io.log(
      `  ${dim(
    ...
    if (report.unavailable.length > 0) {
      io.log(
        `  ${colors.yellow('Auto-autorizados por sensor no disponible')}: ${report.unavailable.join(', ')} ${dim('(requieren recibo I6)')}`,
      );
    }
    io.log('');
...
const firstSpec = async (cwd: string): Promise<string | null> => {
  const root = await findRepoRoot(cwd);
...
export const handleGovernCommand = async (args: string[], io: CliIO, cwd: string): Promise<number> => {
  ...
  if (sub === 'conformance') {
    ...
    for (const tier of CONFORMITY_LEVELS) {
      ...
      io.log(
        `  ${hit ? colors.green('▶') : ' '} ${tier.level} ${tier.name.padEnd(14)} requiere: ${
          tier.requires.join(', ') || '—'
        }`,
      );
    }
  ...
  if (sub === 'constitution') {
    ...
    if (args.includes('--advise')) {
      ...
      if (args.includes('--json')) {
        ...
        io.log('');
        for (const line of renderAdvice(report)) io.log(line);
        io.log('');
      }
  ...
  if (sub === 'appeal') {
    ...
    for (const e of ledger.entries) io.log(`    ${e.gate.padEnd(5)} ${e.actor.padEnd(16)} ${dim(e.reason.slice(0, 80))}`);
    io.log('');
  ...
  if (sub === 'meta-eval') {
    ...
    io.log(
      `  Piso de acuerdo sustancial (Landis–Koch): ${SUBSTANTIAL_KAPPA} · reinicio del evaluador por debajo de 0,6 · n preregistrado: ${PREREGISTERED_N}`,
    );
    io.log('');
  ...
  if (sub === 'budget') {
    ...
    const verdict = evaluateGovernanceBudget(
      cycle,
      Number(process.env.SDD_META_TOKENS ?? '8000'),
      Number(process.env.SDD_AUDIT_TOKENS ?? '6000'),
      Number(process.env.SDD_DISTILL_TOKENS ?? '4000'),
    );
    io.log('');
    ...
    for (const m of METRIC_DEFINITIONS) io.log(`  ${m.metric.padEnd(18)} ${dim(m.definition)}`);
    io.log('');
  ...
  if (sub === 'discipline') {
    ...
    const findings = checkDecidableDiscipline(
      ...
      f.property === 'scope-containment' && declaredScope.length === 0
        ? {
            ...f,
            decidable: false,
            violated: false,
...
const loadReceipts = async (root: string): Promise<RelaxationReceipt[]> => {
  const raw = await readFile(path.join(root, '.sdd', 'receipts.json'), 'utf8').catch(() => null);
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as RelaxationReceipt[]) : [];
  } catch {
    return [];
  }
};
...
export const handleAssureCommand = async (args: string[], io: CliIO, cwd: string): Promise<number> => {
  ...
  if (sub === 'threats') {
    ...
    for (const n of ZERO_TRUST_BOUNDARY.notClaimed) io.log(`  ${colors.yellow('No reclamado')}: ${n}`);
    io.log('');
    return 0;
  }
  ...
  if (sub === 'claims') {
    ...
    if (args.includes('--verify')) {
      ...
      io.log('');
      io.log(
        `  ${result.report.total} afirmaciones | ${result.report.verified} verificadas | ${result.report.notImplemented} declaradas | ${result.report.notMeasured} no medidas | ${result.report.broken} rotas | ${result.report.outdatedText} desactualizadas`,
      );
  ...
  if (sub === 'skills') {
    ...
    for (const m of SKILL_METRICS) io.log(`  ${m.metric.padEnd(34)} ${dim(m.note)}`);
    io.log('');
    return 0;
  }
  ...
  if (sub === 'memory') {
    ...
    io.log('');
    for (const b of MEMORY_MESH) io.log(`  ${b.backend.padEnd(18)} ${b.contents} ${dim(`(${b.lifetime})`)}`);
    io.log('');
...
export const handleWavesCommand = async (args: string[], io: CliIO, cwd: string): Promise<number> => {
  const root = await findRepoRoot(cwd);
  ...
  for (const inv of WAVE_INVARIANTS) io.log(`    · ${dim(inv)}`);
  io.log('');
  ...
  for (const wave of waves) {
    ...
    for (const t of wave.tasks) io.log(`      ${t.id.padEnd(8)} ${t.title.slice(0, 60)}`);
