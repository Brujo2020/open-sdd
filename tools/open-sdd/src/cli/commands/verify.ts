...
export const handleVerifyCommand = async (
  ...
  if (isJson) {
    io.log(
      JSON.stringify(
        { feature: featureArg, passed, allTasksDone, isApproved, profile: gov.profile, mode: gov.mode, gates, status },
        null,
        2,
      ),
    );
  ...
  if (advisory.length) {
    ...
    for (const g of advisory) io.log(colors.dim(`    - ${g.label}: ${g.detail}`));
  }
