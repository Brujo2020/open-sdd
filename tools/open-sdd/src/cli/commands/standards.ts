...
import type { StandardEntry, StandardFinding } from '../../core/standardsTypes.js';

const USAGE =
  'Usage: open-sdd standards <list|show <id>|explain <id>|check|fix <id>> [--json] [--apply]';
 ...
 */
const MACHINE_APPLICABLE_FIXES: Record<
  string,
  (text: string, findings: StandardFinding[]) => { text: string; applied: number }
> = {
  'REQ-EARS-010': (text, findings) => {
    ...
    for (const finding of findings) {
      ...
      if (!/^(?:when|while|where|if)$/i.test(span) || span === span.toUpperCase()) continue;
...
const posture = (entry: StandardEntry): string =>
  isBlocking(entry) ? 'blocking' : 'advisory until a measured corpus exists';

const renderEntry = (entry: StandardEntry, explain: boolean): string[] => {
  const lines: string[] = [];
  ...
  lines.push(
    `  calibration: ${
      typeof corpus === 'string' && corpus.length > 0
  ...
  if (explain) {
    lines.push(
      `  why: blocking is only honoured when \`calibrated.corpus\` is a measured string (REQ-STD-005); this entry is ${posture(entry)}`,
    );
...
const renderFinding = (finding: StandardFinding): string[] => {
  const lines: string[] = [];
  lines.push(
    `${colors.yellow(finding.severity)} ${finding.standardId} ${finding.file}:${finding.line}:${finding.column}${
      finding.span === undefined ? '' : ` (${JSON.stringify(finding.span)})`
    }`,
  );
  lines.push(`  ${finding.message}`);
  if (finding.question !== undefined) lines.push(`  question: ${finding.question}`);
  for (const remedy of finding.remedies) lines.push(`  fix(${remedy.grade}): ${remedy.text}`);
  return lines;
};
...
interface LoadedContext {
  ...
  byId: Map<string, StandardEntry>;
}

const loadContext = async (cwd: string): Promise<LoadedContext> => {
  const { entries, rejected } = await loadStandards(cwd);
  return { entries, rejected, byId: new Map(entries.map((entry) => [entry.id, entry])) };
};

const catalogueSummary = (entry: StandardEntry): Record<string, unknown> => ({
  id: entry.id,
  title: entry.title,
...
const standardFindingRecord = (finding: StandardFinding): Record<string, unknown> => ({
...
export const handleStandardsCommand = async (
  args: string[],
  io: CliIO,
  ...
  if (sub === 'show' || sub === 'explain') {
      ...
      const data =
        sub === 'show'
          ...
          : {
              ...
              blocking: isBlocking(entry),
              rationale:
                'blocking is only honoured when calibrated.corpus is a measured string (REQ-STD-005)',
              offline: true,
            };
      io.log(
        JSON.stringify(
    ...
    for (const line of renderEntry(entry, sub === 'explain')) io.log(line);
    return 0;
  }
  ...
  if (sub === 'check') {
    ...
    const findings: { entry: StandardEntry; finding: StandardFinding }[] = [];
    ...
    for (const entry of entries) {
      ...
      for (const artifact of applicable) {
        for (const finding of runStandard(entry, artifact)) findings.push({ entry, finding });
      }
    }
    ...
    for (const { finding } of findings) {
      for (const line of renderFinding(finding)) io.log(line);
    }
    for (const s of skipped) io.log(colors.dim(`skipped ${s.id}: ${s.reason}`));
    for (const r of rejected) io.error(colors.red(`rejected ${r.file}: ${r.reason}`));
    for (const d of drift) io.log(colors.dim(`drift ${d.file} (${d.reason}): ${d.entries.join(', ')}`));
  ...
  if (sub === 'fix') {
          ...
          JSON.stringify(
            jsonEnvelope({
              ...
              data: {
                ...
                findings: findingsByFile.flatMap((item) =>
                  item.findings.map((finding) => standardFindingRecord(finding)),
                ),
                rejected,
              },
              ok: false,
    ...
    if (!apply) {
      ...
      for (const item of findingsByFile) {
        for (const finding of item.findings) for (const line of renderFinding(finding)) io.log(line);
      }
      io.log(colors.dim('preview: añade `--apply` para escribir los remedios `machine-applicable`'));
      return 0;
    }

    const applied: { file: string; applied: number }[] = [];
    for (const item of findingsByFile) {
      const result = transform(item.artifact.text, item.findings);
      if (result.applied === 0 || result.text === item.artifact.text) continue;
    ...
    for (const item of applied) io.log(colors.green(`applied ${item.applied} cambio(s) en ${item.file}`));
    if (applied.length === 0) io.log(`\`${entry.id}\` no cambió nada`);
    return 0;
  }
