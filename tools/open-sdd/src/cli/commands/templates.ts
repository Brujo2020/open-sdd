...
import {
  ...
  type CommandTemplateCatalogueEntry,
...
const clip = (value: string, width: number): string =>
  value.length <= width ? value : `${value.slice(0, width - 1).trimEnd()}…`;

export interface InstalledTemplates {
  host: string;
  label: string;
  dir: string;
  /** How many of the 22 artifacts are already in place (identical or edited by hand). */
  installed: number;
  total: number;
  /** Artifacts whose generated body a human changed: present, and deliberately not overwritten. */
  handEdited: number;
  stale: number;
}

export interface TemplatesReport {
  templates: CommandTemplateCatalogueEntry[];
  hosts: {
    id: string;
    label: string;
    dir: string | null;
...
const scanInstalled = async (
  ...
  total: number,
): Promise<InstalledTemplates | null> => {
  if (!host.verified || host.dir === null) return null;
  const dir = path.join(cwd, host.dir);
  if (!(await fileExists(dir))) return null;

  const artifacts: CommandTemplateArtifact[] = await planCommandTemplates({ cwd, hosts: [host.id] });
  const files = artifacts.filter((artifact) => artifact.command !== '*');
  const handEdited = files.filter(
    (artifact) => artifact.action === 'keep' && /editad|no es una plantilla generada/.test(artifact.reason),
  ).length;
...
export const buildTemplatesReport = async (cwd: string): Promise<TemplatesReport> => {
...
export const handleTemplatesCommand = async (
  args: string[],
  io: CliIO,
  ...
  io.log(
    formatHeading(
      `Plantillas de comando — ${report.templates.length} workflow(s) que se instalan como comandos del anfitrión, SIN MCP y sin red`,
    ),
  );
  io.log('');
  ...
  for (const [index, entry] of report.templates.entries()) {
    ...
    io.log(
      `  ${pad(String(index + 1), 3)} ${pad(entry.id, idWidth)}  ${pad(clip(writes, 34), 34)}  ${clip(entry.description, 62)}`,
    );
  }
  io.log('');
  ...
  const dirLabel = (host: TemplatesReport['hosts'][number]): string => {
    if (host.dir === null) return '(sin directorio documentado)';
...
const describeInstalled = async (report: TemplatesReport): Promise<string> => {
  ...
  return report.installed
    .map((row) => {
      const notes: string[] = [];
      if (row.handEdited > 0) notes.push(`${row.handEdited} editada(s) a mano y conservadas`);
      if (row.stale > 0) notes.push(`${row.stale} desactualizada(s) y refrescables`);
