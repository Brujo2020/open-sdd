...
export const EXIT_CODES: ExitCodeDoc[] = [
...
export const suggestCommands = (
  ...
  for (const candidate of candidates) {
    if (candidate === input || candidate.toLowerCase() === needle) continue;
    const distance = levenshtein(needle, candidate.toLowerCase());
    if (distance <= maxDistance) scored.set(candidate, distance);
  }
...
// ---------------------------------------------------------------------------------------------
// Una tabla, tres proyecciones
// ---------------------------------------------------------------------------------------------

export type HelpGroup = 'templates' | 'skills' | 'console' | 'brownfield' | 'experience' | 'adoption' | 'daily' | 'reversibility' | 'score';
...
export interface CommandDoc {
  ...
  summary: string;
  usage: string;
  /** Líneas tal y como aparecen en el índice de `--help` (con su sangría de dos espacios). */
  indexLines: string[];
  /** Subcomandos válidos; `strictSubcommands` convierte un valor desconocido en error de uso. */
...
const SUB = (name: string, summary?: string) => (summary ? { name, summary } : { name });
...
export const COMMANDS: CommandDoc[] = [
  ...
  },
  {
    ...
    usage: 'open-sdd brownfield <survey|bootstrap|constitution|templates|specify|requirements|clarify|converge|analyze|impact|contracts|reuse|forecast|repair>',
    indexLines: [
...
export const COMMAND_INDEX: Record<string, CommandDoc> = (() => {
  const index: Record<string, CommandDoc> = {};
  for (const doc of COMMANDS) {
    index[doc.name] = doc;
    for (const alias of doc.aliases ?? []) index[alias] = doc;
  }
  return index;
})();

/** Todo comando enrutable por su nombre canónico o alias. */
export const KNOWN_COMMANDS: Set<string> = new Set(Object.keys(COMMAND_INDEX));

/** Los comandos canónicos, en el orden de la tabla (uno por ayuda). */
export const ROUTED_COMMANDS: string[] = COMMANDS.map((doc) => doc.name);

/** Subcomandos por comando, para validar el despacho y sugerir el más cercano. */
export const SUBCOMMAND_INDEX: Record<string, string[]> = (() => {
  const index: Record<string, string[]> = {};
  for (const doc of COMMANDS) {
    if (!doc.subcommands || doc.subcommands.length === 0) continue;
    index[doc.name] = doc.subcommands.map((sub) => sub.name);
    for (const alias of doc.aliases ?? []) index[alias] = index[doc.name]!;
  }
  return index;
})();

/** Solo los comandos cuyo primer posicional ES un subcomando: un valor desconocido es error de uso. */
export const STRICT_SUBCOMMANDS: Record<string, string[]> = (() => {
  const strict: Record<string, string[]> = {};
  for (const doc of COMMANDS) {
    if (!doc.strictSubcommands || !doc.subcommands) continue;
    strict[doc.name] = doc.subcommands.map((sub) => sub.name);
    for (const alias of doc.aliases ?? []) strict[alias] = strict[doc.name]!;
  }
  return strict;
})();
...
const HELP_SECTIONS: { group: HelpGroup; heading: string; staticLines?: string[]; after?: string[] }[] = [
...
export const renderFullHelp = (): string => {
  const lines: string[] = ['Usage: open-sdd [options] (alias: open-sdd)', '', 'Options:', ...globalOptions()];
  lines.push('', 'Quick Examples:', ...quickExamples());
  for (const section of HELP_SECTIONS) {
    lines.push('', section.heading);
    if (section.staticLines) lines.push(...section.staticLines);
    for (const doc of COMMANDS.filter((candidate) => candidate.group === section.group)) {
      lines.push(...doc.indexLines);
    }
    if (section.after) lines.push(...section.after);
  }
...
export const renderCommandHelp = (name: string): string | undefined => {
  const doc = COMMAND_INDEX[name.toLowerCase()];
  if (!doc) return undefined;
  const lines: string[] = ['', formatHeading(`Command: open-sdd ${doc.name}`), `  ${doc.summary}`, ''];
  if (doc.aliases && doc.aliases.length > 0) lines.push(`Aliases: ${doc.aliases.map((alias) => `open-sdd ${alias}`).join(', ')}`, '');
  ...
  if (doc.details && doc.details.length > 0) {
    lines.push('Details:');
    for (const detail of doc.details) lines.push(`  ${detail}`);
    lines.push('');
  }
...
const RIGOR_USAGE = (): string => {
  const lines: string[] = ['', formatHeading('RIGOR — the cumulative ladder'), ''];
...
const FORMATTING_USAGE = (): string => {
  const lines: string[] = ['', formatHeading('FORMATTING — color, diagnostics and the machine surface'), ''];
...
export const HELP_TOPICS: Record<string, { title: string; render: () => string }> = {
...
export const renderHelpTopic = (topic: string): string | undefined => HELP_TOPICS[topic]?.render();
...
export const renderExplain = (code: string): ExplainResult => {
...
const usageErrorLines = (message: string, suggestions: string[], helpCommand: string): string[] => {
...
export const handleHelpCommand = async (argv: string[], io: CliIO): Promise<number> => {
  ...
  for (const line of usageErrorLines(
    `unknown help topic \`${topic}\``,
    suggestCommands(topic, [...KNOWN_COMMANDS]),
    'open-sdd help',
  )) {
    io.error(line);
  }
