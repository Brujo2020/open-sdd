...
import { runChain, type GateRunReport } from '../../core/gateRunner.js';
import { resolveGateChain, type ChainProfile } from '../../core/gateCatalog.js';
...
import { alignSpecWithConstitution, type SpecAlignment } from '../../core/specConstitution.js';
import { evaluateTriad, type TriadVerdict } from '../../core/triad.js';
import { parseDeltaSpec } from '../../core/deltaSpec.js';
import {
  assessRigor,
  effectiveGates,
  loadRigorSettings,
  type RigorAssessment,
  type RigorSettings,
} from '../../core/rigor.js';
import { readClaimsRegistry, runClaimsRegistry, type RegistryRunResult } from '../../core/claimsRegistry.js';
 ...
 */
export const AUDIT_EXIT_CODES = { pass: 0, blocking: 1, couldNotRun: 2 } as const;
...
export type BundleArtifactRole =
  | 'constitution'
  | 'compliance-matrix'
  | 'spec'
  | 'gates'
...
export interface BundleArtifact {
  ...
  /** Bundle-relative POSIX path; for external artifacts, repository-relative. Null when absent. */
  path: string | null;
  present: boolean;
  /** sha256 of the exact bytes on disk, or null when the source artifact was absent. */
  sha256: string | null;
  bytes: number | null;
  /** Repository-relative path of the source this artifact was derived from. */
  source?: string;
  /** True when the artifact lives outside the bundle directory (e.g. a CI SARIF path). */
  external?: boolean;
}

export type BundleFindingKind = 'gate' | 'alignment' | 'claim';

export interface BundleFinding {
  kind: BundleFindingKind;
  ruleId: string;
  level: 'error' | 'warning' | 'note';
  message: string;
  /** Repository-relative URI, always present so a SARIF result is addressable. */
  artifact: string;
  startLine?: number;
  /** Named authority a blocking verdict cites (I1). */
  authority?: string;
}
...
export interface AuditBundleManifest {
  ...
  verdict: {
    passed: boolean;
    exitCode: number;
    /** Findings that made the bundle fail: gate FAILs and broken claims. */
    blockedBy: string[];
  };
  scope: {
    root: string;
    ...
    /** Changed files the chain judged; empty means the run judged the declared controls only. */
    changedFiles: string[];
  };
  rigor: RigorSettings & { gates: string[]; assessment: RigorAssessment };
  gates: {
    declared: string[];
    executed: string[];
    unavailable: string[];
    vacuous: string[];
    regime: 'flexible' | 'strict';
    report: GateRunReport;
  };
  constitution: {
    present: boolean;
    source: string | null;
    principlesInForce: ConstitutionPrinciple[];
    complianceMatrix: ComplianceMatrix;
  };
  specs: BundleSpecEntry[];
  claims: {
    registry: string;
    present: boolean;
    report: RegistryRunResult['report'] | null;
    runs: RegistryRunResult['runs'];
    rejected: RegistryRunResult['rejected'];
    timedOut: string[];
    detail: string;
  };
  ...
  summary: {
    gates: { declared: number; failed: number; selfAuthorized: number };
    claims: { total: number; broken: number; verified: number; executed: number } | null;
    alignment: { error: number; warning: number; info: number };
    findings: number;
  };
}

/** One file the bundle wants persisted, before the output directory exists. */
...
export interface AssembleOptions {
  cwd: string;
  feature?: string;
  /** Exact output directory. Default: `<sddDir>/audit/<timestamp>`. */
  outDir?: string;
  /** Chain profile. Default: derived from `.sdd/settings/governance.json`. */
  profile?: ChainProfile;
  sddDir?: string;
  claimsPath?: string;
  claimsTimeoutMs?: number;
  /** SARIF output path (relative paths resolve against the repository root). */
  sarifPath?: string;
...
export const bundleTimestamp = (date: Date): string => date.toISOString().replace(/[:.]/g, '-');

const profileFromGovernance = (governance: string | undefined): ChainProfile => {
  if (governance === 'team') return 'team';
  if (governance === 'enterprise') return 'regulated';
  return 'solo';
};

const SPEC_FILES: { name: string; role: string; required: boolean }[] = [
...
const specFileUri = (sddDir: string, feature: string, name: string): string =>
  toPosix(path.join(sddDir, 'specs', feature, name));
...
export const gateLocation = (
  ...
  sddDir: string,
  feature: string,
): { artifact: string; startLine?: number } => {
  if (gateId === 'C1') return { artifact: specFileUri(sddDir, feature, 'requirements.md') };
  if (gateId === 'C3') return { artifact: specFileUri(sddDir, feature, 'tasks.md') };
  if (gateId === 'C4') return { artifact: specFileUri(sddDir, feature, 'plan.md') };
  if (gateId === 'C6') {
    const missing = evidence?.[0];
    if (missing && /\.(md|json|ya?ml)$/i.test(missing)) return { artifact: toPosix(missing) };
    return { artifact: 'README.md' };
  }
  if (gateId === 'C2') {
    // Evidence is rendered as `kind:id path:line` by the C2 control.
    const match = evidence?.[0]?.match(/(\S+):(\d+)$/);
    if (match) return { artifact: toPosix(match[1]), startLine: Number(match[2]) };
  }
  return { artifact: specFileUri(sddDir, feature, 'requirements.md') };
};

const alignmentUri = (sddDir: string, feature: string, artifactId: string | undefined): string => {
  if (artifactId && /^TASK/i.test(artifactId)) return specFileUri(sddDir, feature, 'tasks.md');
  return specFileUri(sddDir, feature, 'requirements.md');
};

const claimLine = (registryText: string, claimId: string): number | undefined => {
...
export const collectEvidence = async (options: AssembleOptions): Promise<CollectedEvidence> => {
  ...
  let rigor: RigorSettings;
  try {
    rigor = await loadRigorSettings(root, sddDir);
  } catch (error) {
    throw new AuditBundleError(`No se pudo leer el nivel de rigor declarado: ${(error as Error).message}`);
  }
  ...
  const regime: 'flexible' | 'strict' = profile === 'regulated' ? 'strict' : 'flexible';
  ...
  const gateReport = await runChain(
    gateIds,
    {
      cwd: root,
      sddDir,
  ...
  const specs: BundleSpecEntry[] = [];
  for (const feature of features) {
    const specDirAbs = path.join(root, sddDir, 'specs', feature);
    const entries = await readdir(specDirAbs).catch(() => [] as string[]);
    const triad = evaluateTriad(entries);
    const contents = new Map<string, string>();
    const files: BundleSpecEntry['files'] = [];
    for (const candidate of SPEC_FILES) {
      const text = entries.includes(candidate.name)
        ...
        : null;
      if (text !== null) contents.set(candidate.name, text);
  ...
  let claimsRun: RegistryRunResult | null = null;
  let claimsText: string | null = null;
  let claimsDetail: string;
  try {
    ...
    claimsDetail = `Registro de afirmaciones no ejecutable en ${claimsRelPath}: ${(error as Error).message}`;
  }
  ...
  for (const finding of gateReport.findings) {
    if (finding.outcome !== 'fail') continue;
  ...
  const addAbsent = (id: string, role: BundleArtifactRole, source: string): void => {
  ...
  addText(
    ...
    'constitution.json',
    JSON.stringify(
      { present: constitutionText !== null, source: constitutionSource, principlesInForce: inForce, complianceMatrix },
      null,
      2,
    ) + '\n',
  ...
  for (const spec of specs) {
    for (const file of spec.files) {
      if (file.present && file.bundlePath !== null && file.source !== null) {
        ...
        addAbsent(
          `spec:${spec.feature}:${file.name}`,
          'spec',
          toPosix(path.join(sddDir, 'specs', spec.feature, file.name)),
        );
      }
    }
    if (!spec.present) addAbsent(`spec:${spec.feature}`, 'spec', toPosix(path.join(sddDir, 'specs', spec.feature)));
  }
  ...
  addText(
    ...
    'alignment.json',
    JSON.stringify(
      { specs: specs.map((spec) => ({ ...spec.alignment, feature: spec.feature, present: spec.present })) },
      null,
      2,
    ) + '\n',
  );
  addText(
    ...
    'rigor',
    'rigor.json',
    JSON.stringify({ ...rigor, gates: effectiveGates(rigor.level, rigor.gates) }, null, 2) + '\n',
  );

  const alignmentCounts = { error: 0, warning: 0, info: 0 };
  for (const spec of specs) {
    for (const finding of spec.alignment.findings) alignmentCounts[finding.severity] += 1;
  }
...
export const writeBundle = async (
  ...
  try {
    ...
    throw new AuditBundleError(`No se pudo crear el directorio del bundle ${dir}: ${(error as Error).message}`);
  }
...
export const assembleAuditBundle = async (options: AssembleOptions): Promise<AssembleResult> => {
...
export interface SarifLog {
  ...
  runs: {
    ...
    results: SarifResult[];
    invocations: { executionSuccessful: boolean; exitCode: number; endTimeUtc: string }[];
  }[];
}

const ruleText = (ruleId: string): string => {
  if (/^[CO]\d$/.test(ruleId)) return `Control de la cadena Zero-Trust (${ruleId}).`;
  if (/^(CLAIM|PAPER)-/.test(ruleId)) return `Afirmación registrada ${ruleId} del registro ejecutable (§9.6).`;
  return `Hallazgo de alineación constitucional (${ruleId}).`;
};

/** Build the minimal-but-valid SARIF 2.1.0 log for a manifest's findings. */
export const buildSarifLog = (manifest: AuditBundleManifest): SarifLog => {
 ...
 */
export const validateSarifShape = (log: unknown): string[] => {
  const problems: string[] = [];
  if (typeof log !== 'object' || log === null) return ['el SARIF no es un objeto'];
  const candidate = log as Partial<SarifLog>;
  if (candidate.version !== SARIF_VERSION) problems.push(`version debe ser "${SARIF_VERSION}"`);
  if (candidate.$schema !== SARIF_SCHEMA) problems.push('$schema debe ser el esquema OASIS de SARIF 2.1.0');
...
export const parseBundleArgs = (argv: string[]): ParsedBundleArgs => {
  const sub: 'bundle' | 'sarif' = argv[0] === 'sarif' ? 'sarif' : 'bundle';
  const rest = argv[0] === 'sarif' || argv[0] === 'bundle' ? argv.slice(1) : argv;
  const parsed: ParsedBundleArgs = { sub, json: false };
  ...
  for (let i = 0; i < rest.length; i += 1) {
    ...
    if (token.startsWith('--') && token.includes('=')) {
      ...
      if (flag === '--out') parsed.outDir = value;
      else if (flag === '--sarif') parsed.sarifPath = value;
      else if (flag === '--profile') parsed.profile = value === 'team' || value === 'regulated' ? value : 'solo';
      else if (flag === '--sdd-dir') parsed.sddDir = value;
      else if (flag === '--claims') parsed.claimsPath = value;
      else if (flag === '--claims-timeout') parsed.claimsTimeoutMs = Number(value);
      continue;
    }
    if (VALUE_FLAGS.has(token)) {
      const value = rest[i + 1];
      if (value !== undefined) {
        if (token === '--out') parsed.outDir = value;
        else if (token === '--sarif') parsed.sarifPath = value;
        else if (token === '--profile') parsed.profile = value === 'team' || value === 'regulated' ? value : 'solo';
        else if (token === '--sdd-dir') parsed.sddDir = value;
        else if (token === '--claims') parsed.claimsPath = value;
        else if (token === '--claims-timeout') parsed.claimsTimeoutMs = Number(value);
        i += 1;
      }
      continue;
    }
  ...
  if (positionals.length > 0) parsed.feature = positionals[0];
...
const bundleOptions = (cwd: string, parsed: ParsedBundleArgs): AssembleOptions => ({
  cwd,
...
export const renderBundleSummary = (result: AssembleResult): string[] => {
  ...
  if (result.sarifPath) lines.push(`  SARIF:            ${result.sarifPath}`);
  lines.push('');
  return lines;
};

export const handleAuditBundleCommand = async (
  argv: string[],
  io: CliIO,
  ...
  try {
    ...
    for (const line of renderBundleSummary(result)) io.log(line);
