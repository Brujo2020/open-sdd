...
const paint = (text: string, line: StatusLine): string =>
  line.value ? TONE_PAINT[line.tone](text) : colors.bold(colors.cyan(text));

interface PivotFinding {
...
interface PivotRun {
  ...
  errorCount: number;
  warningCount: number;
  /** Veredicto de hoy: solo los errores fallan. */
  exitCode: number;
  /** Veredicto de `--strict`: los avisos también fallan. */
 ...
 */
const legacySpecList = async (cwd: string, sddDir: string | undefined): Promise<unknown[]> => {
 ...
 */
const handleCelebrations = async (args: string[], io: CliIO, cwd: string): Promise<number> => {
  ...
  for (const line of renderCelebrations(read, { noColor: args.includes('--no-color') })) io.log(line);
  return 0;
};
 ...
 */
const celebrateValidatedFeature = async (
  io: CliIO,
  root: string,
  sddDir: string,
  ...
  if (ears.total === 0 || earsErrors > 0 || ears.conforming !== ears.total) return;

  let gates: { passed: number; total: number; failed: string[]; evaluated: boolean };
  let gateRun: GateRunReport;
  try {
    ...
    if (gateRun.findings.length === 0) return;
    gates = {
  ...
  for (let i = series.length - 1; i > 0; i -= 1) {
    if (series[i].score >= series[i - 1].score) streak += 1;
    else break;
  }

  const celebration = await celebrateFeature(
    path.resolve(root, sddDir),
    {
      feature,
  ...
  if (celebration.report.verdict !== 'celebrate') return;

  await playCelebration(celebration.report, {
    out: {
      write: (chunk: string) => {
        for (const line of chunk.split('\n')) if (line.length > 0) io.log(line);
        return true;
      },
    },
  });
...
export const handleStatusCommand = async (
  ...
  if (args.includes('--celebrations')) return handleCelebrations(args, io, cwd);
  ...
  const flagValue = (flag: string): string | undefined => {
    ...
    if (at >= 0 && args[at + 1] !== undefined && !args[at + 1].startsWith('-')) return args[at + 1];
    return undefined;
  };
      ...
      io.log(
        JSON.stringify(
          pivot.alignment
            ...
            : { ...report, alignmentError: pivot.error ?? 'el pivote no se pudo ejecutar', ratchet: pivot.ratchet, checkExitCode: pivot.exitCode },
          null,
          2,
        ),
      );
  ...
  if (isCheck) {
    ...
    if (!pivot.alignment) {
      ...
      for (const finding of pivot.findings) {
        const mark =
          finding.severity === 'error' ? colors.red('error') : finding.severity === 'warning' ? colors.yellow('aviso') : colors.dim('info');
      ...
      if (isStrict) {
        io.log(
          `  ${colors.yellow('Modo estricto')}: los avisos también fallan. ${pivot.warningCount > 0 ? `Añade ${pivot.warningCount} aviso(s) al veredicto (${warningCodes.join(', ')}).` : 'No hay avisos que escalar.'}`,
        );
      } else {
        io.log(
          `  ${colors.dim(`Modo estricto (--strict) añadiría ${pivot.warningCount} aviso(s) al veredicto${warningCodes.length > 0 ? ` (${warningCodes.join(', ')})` : ''}.`)}`,
        );
      }
    }
