...
import type { StandardFinding } from './standardsTypes.js';

export interface GateRunContext {
  cwd: string;
  sddDir: string;
  feature: string;
  /** Files changed by the change under evaluation (for scope and symbol checks). */
  changedFiles: string[];
  /** Declared scope of the task, for the scope gate. */
  declaredScope: string[];
  /** Symbols referenced by the change, as `module.symbol`. */
  referencedSymbols?: string[];
  /** MCP servers declared by host configuration. */
  mcpServers?: string[];
  /** Allow-list for MCP proxy validation. */
  mcpAllowlist?: string[];
  /** Living-memory state, when configured. */
  memory?: { undigestedItems: number; hoursSinceLastDistillation: number; maxHoursBetweenDistillations: number };
  /** Structural index file, when a graph has been built. */
  graphIndexPath?: string;
  /** Files the loaded context references. */
...
export interface GateFinding {
  ...
  detail: string;
  /** Named authority so a blocking verdict can cite what rejected it (I1). */
  authority: string;
  evidence?: string[];
}
...
export interface GateRunReport {
  feature: string;
  regime: PostureRegime;
  findings: GateFinding[];
  passed: boolean;
  /** Declared controls that could not be executed, reported instead of hidden. */
  unavailable: string[];
}
...
const DESTRUCTIVE_PATTERNS: { id: string; re: RegExp }[] = [
...
export type SecurityFinding = { id: string; kind: 'secret' | 'destructive' | 'injection'; file: string; line: number };
...
export const scanSecurity = (
  ...
  for (const file of files) {
    ...
    lines.forEach((line, i) => {
      for (const p of SECRET_PATTERNS) if (p.re.test(line)) findings.push({ id: p.id, kind: 'secret', file: file.path, line: i + 1 });
      for (const p of DESTRUCTIVE_PATTERNS) if (p.re.test(line)) findings.push({ id: p.id, kind: 'destructive', file: file.path, line: i + 1 });
      for (const p of INJECTION_PATTERNS) if (p.re.test(line)) findings.push({ id: p.id, kind: 'injection', file: file.path, line: i + 1 });
    });
  }
  return findings;
};

// --- symbol membership (C4 / G13) -------------------------------------------------------------

/**
 * Declared domain: only a symbol whose prefix is a module of THIS repository is judgeable,
 * because only then does the absence of a definition mean something. Outside that domain the
 * verdict is undecidable, and saying so is legitimate information — a control that answers
 * outside its domain is not measuring, it is guessing, and its hits and misses are
 * indistinguishable to the reader.
 */
export interface SymbolIndex {
  /** module -> declared symbol names */
  modules: Map<string, Set<string>>;
}

export const buildSymbolIndex = (sources: { path: string; content: string }[]): SymbolIndex => {
  const modules = new Map<string, Set<string>>();
  for (const src of sources) {
    const mod = path.basename(src.path).replace(/\.(ts|tsx|js|jsx|mjs|cjs|py|go|rb|java|kt|rs)$/, '');
    const set = modules.get(mod) ?? new Set<string>();
    const re =
      /\b(?:export\s+)?(?:async\s+)?(?:function|class|const|let|var|interface|type|enum|def|struct|func)\s+([A-Za-z_$][\w$]*)/g;
    let m: RegExpExecArray | null;
    while ((m = re.exec(src.content)) !== null) set.add(m[1]);
    modules.set(mod, set);
  }
  return { modules };
};
...
export interface SymbolVerdict {
  symbol: string;
  classification: 'present' | 'absent' | 'external' | 'undecidable';
  detail: string;
}
 ...
 */
export const checkSymbols = (index: SymbolIndex, referenced: string[]): SymbolVerdict[] =>
  referenced.map((ref) => {
...
const HARD_CONTROL_BY_GATE: Partial<Record<string, HardControl>> = {
  C2: 'secretos',
...
export const runGate = async (
  ...
  const posture: 'blocking' | 'advisory' = gate?.posture === 'blocking' ? 'blocking' : 'advisory';
  ...
  switch (gateId) {
    case 'C1': {
      ...
      let standardFindings: StandardFinding[] = [];
      try {
      ...
      return finalize(
        true,
        fired,
        `${triad.detail}${bad.length > 0 ? ` ${bad.length} requisito(s) no conformes a EARS.` : ''}${
    ...
    case 'C2': {
      ...
      const expiredNote =
        expiredWaivers.length > 0
          ? ` ${expiredWaivers.length} hallazgo(s) se mantienen porque su excepción CADUCÓ: ${expiredWaivers
              .map(
                (waiver) =>
                  `«excepción caducada el ${waiver.expires ?? '(sin fecha)'}; responsable: ${
                    waiver.owner ?? '(sin dueño declarado)'
                  }» (${waiver.id} en ${waiver.file}:${waiver.line})`,
              )
              .join('; ')}.`
          : '';
      const weakNote =
        weakWaivers.length > 0
      ...
      const evidence = [
        ...findings.map((f) =>
          f.waiverExpired
            ? `${f.kind}:${f.id} ${f.file}:${f.line} — excepción caducada el ${
                f.expires ?? '(sin fecha)'
              }; responsable: ${f.owner ?? '(sin dueño declarado)'}`
    ...
    case 'C4': {
      ...
      const walk = async (dir: string): Promise<void> => {
        ...
        for (const e of entries) {
          ...
          if (e.isDirectory()) await walk(full);
            ...
            if (content) sources.push({ path: full, content });
          }
        }
      };
      ...
      return finalize(
        true,
        absent.length > 0,
        `${verdicts.length - absent.length}/${verdicts.length} referencia(s) resueltas o declaradas indecidibles fuera del dominio.`,
        absent.map((a) => a.symbol),
      );
    }
    ...
    case 'C5': {
      ...
      return finalize(
        false,
        false,
        'mode=degraded: sin backend de modelo alcanzable se declara heurística de palabras clave, no juicio, y NO cuenta como evidencia de alineamiento de intención.',
      );
    }
    ...
    case 'C6': {
      ...
      for (const rel of candidates) {
        if ((await readIfExists(path.join(ctx.cwd, rel))) === null) missing.push(rel);
      }
      return finalize(
        true,
        missing.length > 0,
        missing.length > 0
          ...
          : 'Afirmaciones documentales con soporte presente en el repositorio.',
        missing,
      );
    }
    ...
    case 'C8': {
      ...
      let loaded: { entries: Awaited<ReturnType<typeof loadStandards>>['entries']; rejected: Awaited<ReturnType<typeof loadStandards>>['rejected'] };
      try {
    ...
    case 'O2': {
      ...
      if (declared.length === 0) return finalize(true, false, 'No hay servidores MCP de terceros declarados.');
    ...
    case 'O3': {
      ...
      if (!exists) return finalize(false, false, `El índice declarado ${ctx.graphIndexPath} no existe.`);
    ...
    case 'O4': {
      ...
      if (plan === null) return finalize(true, false, 'Sin plan.md/design.md: no hay referencias a ADRs que verificar.');
      const refs = Array.from(plan.matchAll(/ADR[-\s]?(\d+)/gi)).map((m) => m[1]);
      if (refs.length === 0) return finalize(true, false, 'El plan no cita ADRs.');
      const adrDir = path.join(ctx.cwd, 'docs', 'adr');
      const present = await readdir(adrDir).catch(() => [] as string[]);
    ...
    case 'O6': {
      ...
      if (refs.length === 0) return finalize(true, false, 'El contexto cargado no referencia ficheros.');
      const missing: string[] = [];
      for (const rel of refs) {
        if ((await stat(path.join(ctx.cwd, rel)).catch(() => null)) === null) missing.push(rel);
      }
    ...
    case 'O7': {
      ...
      for (const c of claims) {
        if (now - new Date(c.grantedAt).getTime() >= c.ttlMs) continue;
...
export const runChain = async (
  ...
  for (const id of gateIds) findings.push(await runGate(id, ctx, regime));
