...
export const ADHESION_STATE_VERSION = 1 as const;

export interface AdhesionBaseline {
  /** Ratio 0..1 devuelta por el pivote en el momento de registrar la base. */
  alignment: number;
  /** Ids de los principios EN VIGOR en la constitución en ese momento. */
  principles: string[];
  /** sha256 del contenido de la constitución: distingue «mismo veredicto, otra autoridad». */
  constitutionHash: string;
  /** ISO-8601 del momento en que se registró. */
  recordedAt: string;
}

/** Un descenso aceptado explícitamente: quién, qué y cuándo. */
export interface AdhesionAcceptance {
  feature: string;
  reason: string;
  actor: string;
  at: string;
  /** Ratio antes y después del descenso aceptado. */
  from: number;
  to: number;
...
export interface AdhesionState {
  version: typeof ADHESION_STATE_VERSION;
  features: Record<string, AdhesionBaseline>;
  acceptances: Record<string, AdhesionAcceptance[]>;
}
...
export type RatchetVerdict =
  ...
  | 'drop'
  | 'accepted-drop'
  | 'corrupt-reestablished';

export interface RatchetFinding {
  severity: 'error' | 'warning' | 'info';
  /** Código estable para máquinas. */
  code: string;
  /** Principio citado, cuando el hallazgo trata de uno. */
  principleId?: string;
  message: string;
}
...
export interface RatchetRun {
  ...
  /** Línea base vigente DESPUÉS de la ejecución. */
  baseline: AdhesionBaseline;
  /** Línea base previa, si existía. */
  previous: AdhesionBaseline | null;
  /** Principios que estaban en vigor en la base y ya no lo están. */
  droppedPrinciples: string[];
  /** Cierto cuando el fichero existía y no se pudo leer/interpretar. */
  stateCorrupt: boolean;
  statePath: string;
  /** Motivo del fallo de lectura o de escritura, cuando lo hubo. */
  stateError?: string;
}

/** `.sdd/state/adhesion.json` a partir del directorio SDD (absoluto o relativo). */
export const adhesionStatePath = (sddDir: string): string =>
  path.join(sddDir, ADHESION_STATE_DIR, ADHESION_STATE_FILE);

/** Hash de contenido de la constitución: sha256 hex. */
export const hashConstitution = (text: string): string =>
...
const isBaseline = (value: unknown): value is AdhesionBaseline => {
  if (!value || typeof value !== 'object') return false;
  const candidate = value as Record<string, unknown>;
  return (
    isFiniteNumber(candidate.alignment) &&
...
export interface AdhesionStateRead {
  state: AdhesionState | null;
  /** El fichero existe pero no se pudo leer o no tiene la forma esperada. */
  corrupt: boolean;
  /** Motivo concreto, para poder decirlo en voz alta. */
  error?: string;
}
 ...
 */
export const readAdhesionState = async (sddDir: string): Promise<AdhesionStateRead> => {
  ...
  try {
    ...
    if (code === 'ENOENT' || code === 'ENOTDIR') return { state: null, corrupt: false };
    return { state: null, corrupt: true, error: `${statePath}: ${(err as Error).message}` };
  }
  ...
  try {
    ...
    return { state: null, corrupt: true, error: `${statePath}: JSON inválido (${(err as Error).message})` };
  }

  const candidate = parsed as Partial<AdhesionState> | null;
  ...
  const acceptances: Record<string, AdhesionAcceptance[]> = {};
  if (candidate.acceptances && typeof candidate.acceptances === 'object') {
    for (const [feature, list] of Object.entries(candidate.acceptances as Record<string, unknown>)) {
      if (Array.isArray(list)) acceptances[feature] = list as AdhesionAcceptance[];
    }
  }

  return {
    state: { version: ADHESION_STATE_VERSION, features: features as Record<string, AdhesionBaseline>, acceptances },
    corrupt: false,
  };
};

const persistState = async (statePath: string, state: AdhesionState): Promise<void> => {
...
export interface RatchetInput {
  /** Directorio SDD que contiene `state/adhesion.json`. */
  sddDir: string;
  feature: string;
  /** Ratio 0..1 del pivote en esta ejecución. */
  alignment: number;
  /** Ids de los principios en vigor en esta ejecución. */
  principles: string[];
  /** Hash del contenido de la constitución en esta ejecución. */
  constitutionHash: string;
  /** Razón con la que se acepta un descenso; su presencia autoriza el descenso de ESTA ejecución. */
  acceptDrop?: string;
  /** Quién acepta. Sin actor, la aceptación se registra igual pero se nombra como desconocido. */
  actor?: string;
  now?: Date;
  /** `false` para evaluar sin escribir (dry-run): el veredicto se calcula, el fichero no se toca. */
  persist?: boolean;
}

const dropMessage = (
  feature: string,
  previous: AdhesionBaseline,
  current: AdhesionBaseline,
  dropped: string[],
): string => {
 ...
 */
export const runAdhesionRatchet = async (input: RatchetInput): Promise<RatchetRun> => {
  const now = input.now ?? new Date();
  const statePath = adhesionStatePath(input.sddDir);
  const current: AdhesionBaseline = {
  ...
  const features: Record<string, AdhesionBaseline> = { ...(read.state?.features ?? {}) };
  const acceptances: Record<string, AdhesionAcceptance[]> = { ...(read.state?.acceptances ?? {}) };
  ...
  if (read.corrupt) {
    ...
    if (ratioDropped || dropped.length > 0) {
      if (input.acceptDrop && input.acceptDrop.trim()) {
        ...
        const acceptance: AdhesionAcceptance = {
  ...
  const state: AdhesionState = { version: ADHESION_STATE_VERSION, features, acceptances };
  ...
  if (shouldPersist) {
    try {
      ...
      findings.push({
        ...
        message: `El estado del trinquete no se pudo escribir en ${statePath} (${(err as Error).message}): esta comprobación no dejó línea base, así que la próxima volverá a empezar.`,
      });
      return {
        feature: input.feature,
        verdict,
