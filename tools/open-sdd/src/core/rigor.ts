  ...
  if (specDirRel && isGitRepo(cwd)) {
    ...
    if (modified.length === 0) {
      findings.push({
        ...
        artifact: specDirRel,
        message:
          'Binding de cobertura no evaluable: no hay ficheros modificados conocidos en el árbol de trabajo. En un checkout limpio esto NO es un aprobado, es una lista vacía.',
      });
    } else {
      ...
      if (uncovered.length > 0) {
        findings.push({
          ...
          message: `${uncovered.length} fichero(s) cambiaron sin que ningún requisito declare cubrirlos: ${uncovered
            .slice(0, 5)
            .map((item) => item.file)
            .join(', ')}. Declara la frontera en la tarea que corresponda, o acepta el cambio con un waiver en ${DRIFT_WAIVERS_FILE}.${
            driftBlocking ? '' : ' (Advisory en el nivel spec-first: informa, no bloquea.)'
          }`,
        });
      }
      if (expired.length > 0) {
