...
export interface ConsistencyFinding {
  ...
  /** Stable machine code (the codes are listed in the module tests). */
  code: string;
  /** Spanish, actionable. */
  message: string;
  /** Artifacts the finding cites: the files it is about. */
  artifacts: string[];
}
...
export interface ConsistencyReport {
  ...
  /** Checks that ran, with what they saw. */
  checked: string[];
  /** Checks that could not run, each with the reason. Absence is not a pass. */
  notChecked: string[];
  complete: boolean;
  detail: string;
}
...
export interface ConsistencyInput {
  ...
  sddDir?: string;
  /**
   * Changed files, when the caller already knows them (a CI diff, or a test). Derived from git when
   * omitted; when neither is available the delta-target check is declared uninspected.
   */
  changedFiles?: string[];
}

const SEVERITY_RANK: Record<ConsistencyFinding['severity'], number> = { error: 0, warning: 1, info: 2 };
...
const deriveChangedFiles = async (root: string, provided?: string[]): Promise<ChangedFiles> => {
...
const isPathLikeTarget = (target: string): boolean => {
...
const targetTouchedBy = (target: string, changedFiles: string[]): boolean => {
...
const contractBacked = async (root: string, declared: string, discovered: string[]): Promise<boolean> => {
  const file = norm(declared.split('::')[0] ?? declared);
  if (!file) return false;
  if (discovered.some((test) => norm(test.split('::')[0] ?? test) === file)) return true;
 ...
 */
const boundaryCoversSomething = async (root: string, boundary: string, anchors: string[]): Promise<boolean> => {
  const b = norm(boundary).replace(/\/(\*\*|\*)$/, '');
  if (!b) return false;
  if (b === '.' || b === '*' || b === '**') return anchors.length > 0;
  if (await exists(path.join(root, b))) return true;
...
const traceSourceFromRequirements = (
  markdown: string,
  feature: string,
  path: string,
): TraceSource | null => {
...
export const checkConsistency = async (input: ConsistencyInput): Promise<ConsistencyReport> => {
  ...
  const trackUnreadable = (rel: string, read: TextRead): void => {
  ...
  for (const rel of unreadable) notChecked.push(`${rel}: existe pero no se pudo leer, así que no se inspeccionó`);
  ...
  const delta: DeltaSpec | undefined = deltaRaw === null ? undefined : parseDeltaSpec(deltaRaw);
  const project: DiscoveredProject = await scanProject(root);
  ...
  if (tasks === null) {
    ...
    const traceSource: TraceSource | null = delta
    ...
    if (!traceSource) {
      ...
      const knownIds = new Set<string>(traceSource.ids);
      if (delta && requirements !== null) {
        for (const requirement of parseRequirementsMarkdown(requirements)) knownIds.add(requirement.id.toUpperCase());
      }
      checked.push(
        `trazabilidad requisito→tarea (${traceSource.label}: ${traceSource.ids.length} requisito(s) sobre ${parsedTasks.length} tarea(s))`,
      );
      ...
      for (const phantom of trace.phantomTasks) {
        if (knownIds.has(phantom.cited.toUpperCase())) continue;
        findings.push({
          severity: 'error',
  ...
  if (!delta) {
    ...
    notChecked.push(
      `objetivos de la delta contra los ficheros cambiados: no hay ficheros cambiados que analizar (origen: ${changed.source})`,
    );
  } else {
    let decided = 0;
    const untouched: { id: string; targets: string[] }[] = [];
    for (const entry of delta.entries) {
      const pathTargets = entry.targets.filter(isPathLikeTarget);
      if (pathTargets.length === 0) {
        notChecked.push(
          `objetivos de ${entry.id}: ningún objetivo es una ruta comparable con un fichero (${entry.targets.join(', ') || 'sin objetivos'})`,
        );
        continue;
      }
      decided += 1;
      if (pathTargets.some((target) => targetTouchedBy(target, changed.files))) continue;
    ...
    checked.push(
      `objetivos de la delta contra los ficheros cambiados (${decided}/${delta.entries.length} entrada(s) con objetivo de ruta, ${decided - untouched.length} con objetivo tocado por el diff, ${changed.files.length} fichero(s) cambiado(s), origen: ${changed.source})`,
    );
  }
  ...
  if (!constitutionRead.exists || constitutionRead.constitution === null) {
    ...
    checked.push(
      `pivote constitucional (${alignment.declared.length} principio(s) declarado(s), alineación ${(alignment.alignment * 100).toFixed(0)}%)`,
    );
    for (const finding of alignment.findings) {
      const artifacts = new Set<string>(specArtifacts);
      if (finding.artifactId && finding.artifactId.includes('/')) artifacts.add(norm(finding.artifactId));
      if (finding.artifactId && /^REQ-/i.test(finding.artifactId) && deltaRaw !== null) artifacts.add(deltaRel);
      findings.push({
    ...
    if (gaps) notChecked.push(`pivote constitucional — ${gaps[1]}`);
  }
  ...
  if (!delta) {
    ...
    const declaredContracts = delta.entries.flatMap((entry) =>
      (entry.contracts ?? []).map((test) => ({ entry: entry.id, test })),
    );
    if (declaredContracts.length === 0) {
      ...
      findings.push({
        ...
        message:
          'La delta no declara ningún contrato de ejecución: no hay un oráculo declarado que demuestre que lo existente sigue intacto (los contratos descubiertos solo existen si hay ficheros cambiados).',
        artifacts: [deltaRel],
      });
    } else {
      ...
      const missing: { entry: string; test: string }[] = [];
      for (const declared of declaredContracts) {
        if (await contractBacked(root, declared.test, discovered)) continue;
        missing.push(declared);
      }
      checked.push(
        `contratos declarados (${declaredContracts.length} declarado(s), ${discovered.length} descubierto(s), comando propuesto: ${set.testCommand})`,
      );
  ...
  if (tasks === null) {
    ...
    const declaredBoundaries = uniqueSorted(
    ...
    if (declaredBoundaries.length === 0) {
      findings.push({
        ...
        message:
          'Ninguna tarea declara `_Boundary:_`, así que la spec no dice qué código posee: sin frontera declarada no hay forma de detectar que el cambio se sale de su alcance.',
        artifacts: [tasksRel],
      });
      ...
      for (const boundary of declaredBoundaries) {
        if (!(await boundaryCoversSomething(root, boundary, anchors))) empty.push(boundary);
      }
      checked.push(
        `fronteras declaradas contra el código (${declaredBoundaries.length} frontera(s) declarada(s), ${anchors.length} ancla(s) observada(s))`,
      );
      if (empty.length > 0) {
        findings.push({
          ...
          message:
            empty.length === declaredBoundaries.length
