...
import { DELTA_ID_PATTERN, deltaSpecFileName, parseDeltaSpec, traceDelta, type DeltaSpec } from './deltaSpec.js';
import { parseTasksMarkdown, resolveSddDir } from './specManager.js';
import { scanProject } from './reverseEngineering.js';

export type ClarifySeverity = 'error' | 'warning' | 'info';

/** Cómo se aplica una respuesta: frase completa, valor que sustituye al término, o id del `{{…}}`. */
export type ClarifyAnswerKind = 'statement' | 'value' | 'id';

export interface ClarifyQuestion {
  /** Estable dentro de la sesión: `Q-<CÓDIGO CORTO>-<n>` (p. ej. `Q-COMPOUND-2`). */
  id: string;
  /** El código EARS/placeholder que la levanta (configuración o traza delta→tareas). */
  code: string;
  /** El fichero y la línea/requisito del que trata, legible: «<ruta> — «<línea>»». */
  artifact: string;
  /** La pregunta en lenguaje natural, en español, específica y respondible. */
  question: string;
  /** Qué cambiará la respuesta, para que la persona sepa por qué se le pregunta. */
  why: string;
  /** Las opciones cuando la respuesta es una elección; ausente cuando es texto libre. */
  options?: string[];
  /** La frase que producirá la respuesta, con el dato que falta marcado. */
  template?: string;
  severity: ClarifySeverity;
  /** Ruta relativa a la raíz del repositorio del artefacto a reescribir. */
  file: string;
  /** La línea EXACTA del artefacto (con su viñeta y su `Statement:`), para localizarla y reescribirla. */
  target: string;
  /** Qué se espera en `answers[id]` y cómo se aplica. */
  answerKind: ClarifyAnswerKind;
  /** Solo en `value`: el término exacto que la respuesta sustituye. */
  slot?: string;
}

/** Un hallazgo que se respondió sin preguntar porque el repositorio ya lo dice. */
export interface ClarifyEvidence {
  /** `E-<CÓDIGO CORTO>-<n>`: nunca colisiona con los ids de pregunta. */
  id: string;
  code: string;
  artifact: string;
  /** De dónde sale la respuesta, con el fichero y los números que la sostienen. */
  evidence: string;
  /** La frase (o el id) ya calculada, lista para escribir. */
  answer: string;
  file: string;
  target: string;
}

/** Un código que produjo CERO preguntas porque preguntar no lo puede resolver. */
export interface ClarifyUnresolvable {
  code: string;
  artifact: string;
  reason: string;
}
...
export interface ClarifySession {
  ...
  /** Códigos que no produjeron pregunta porque la ambigüedad no se resuelve preguntando. */
  unresolvable: ClarifyUnresolvable[];
  /** True cuando el lote se recortó al tope. */
  truncated: boolean;
  detail: string;
  /** Cuántas preguntas quedaron fuera del lote (≥ 0). */
  remaining: number;
  /** Hallazgos resueltos desde la evidencia del repositorio: NO se preguntan. */
  fromEvidence: ClarifyEvidence[];
  /** Artefactos analizados, rutas relativas, en el orden en que se leyeron. */
  artifacts: string[];
  /** Hechos del repositorio que se consultaron (stack, tests, módulos): lo que hace innecesaria
   *  una pregunta sobre el proyecto en vez de sobre el requisito. */
  evidenceScan: string[];
}
...
// ---------------------------------------------------------------------------------------------
// Catálogo: qué código produce qué pregunta, con qué severidad y con qué forma de respuesta
// ---------------------------------------------------------------------------------------------

interface ClarifyRule {
  code: string;
  /** Sufijo del id, legible y estable. */
  short: string;
  severity: ClarifySeverity;
  /** Orden dentro de la misma severidad: lo que desbloquea la fase siguiente va primero. */
  rank: number;
  answerKind: ClarifyAnswerKind;
}
 ...
 */
const CLARIFY_CATALOGUE: readonly ClarifyRule[] = [
...
export const CLARIFY_QUESTION_CODES: readonly string[] = CLARIFY_CATALOGUE.map((rule) => rule.code);

const ruleOf = (code: string): ClarifyRule | undefined =>
  CLARIFY_CATALOGUE.find((rule) => rule.code.toUpperCase() === code.toUpperCase());

const SEVERITY_RANK: Record<ClarifySeverity, number> = { error: 0, warning: 1, info: 2 };
...
const stripPrefix = (line: string): string => line.slice(prefixOf(line).length);

/** El término vago citado en un problema EARS: `«rápido» no es comprobable…`. */
const termOf = (problem: string): string => problem.match(/«([^»]+)»/)?.[1]?.trim() ?? '';

/** El agente nombrado en el `why` de una pasiva aplicable: `…declara ("the [engine]")`. */
const agentOf = (why: string): string => why.match(/declara \("([^"]+)"\)/)?.[1]?.trim() ?? '';
 ...
 */
const measurableHint = (suggestion: EarsSuggestion): string => {
...
const relativePath = (root: string, file: string): string => path.relative(root, file).split(path.sep).join('/');

const artifactLabel = (file: string, target: string): string => `${file} — «${target}»`;
...
interface EvidenceCandidate extends Omit<ClarifyEvidence, 'id'> {
  rank: number;
  order: number;
  short: string;
}

interface UnresolvableCandidate extends ClarifyUnresolvable {
  key: string;
  order: number;
}

const buildEvidenceScan = (project: Awaited<ReturnType<typeof scanProject>> | null): string[] => {
  if (!project) return [];
  const facts: string[] = [];
  const stack = [
  ...
  if (stack.length > 0) facts.push(`stack: ${stack.join(', ')}`);
  if (project.modules.length > 0) {
    const shown = project.modules.slice(0, 5).join(', ');
    facts.push(`módulos (${project.modules.length}): ${shown}${project.modules.length > 5 ? ', …' : ''}`);
  }
  if (project.testDirs.length > 0) facts.push(`directorios de test: ${project.testDirs.slice(0, 5).join(', ')}`);
  return facts;
};
 ...
 */
const deriveFromSuggestion = (
  suggestion: EarsSuggestion,
  context: { file: string; rel: string; text: string; order: () => number },
  out: { candidates: Candidate[]; evidence: EvidenceCandidate[]; unresolvable: UnresolvableCandidate[] },
): void => {
  ...
  const unresolvable = (code: string, reason: string): void => {
    const key = `${context.rel}|${code}|${target}`;
    if (out.unresolvable.some((entry) => entry.key === key)) return;
  ...
  if (occurrences > 1) {
    unresolvable(
      suggestion.code,
      `la línea objetivo aparece ${occurrences} veces idénticas en ${context.rel}: la respuesta no se podría escribir en un lugar único, así que no se pregunta.`,
    );
    return;
  }
  ...
  if (!rule) {
    unresolvable(
      suggestion.code,
      `no hay pregunta de clarificación definida para el código ${suggestion.code}: inventarla sería adivinar qué dato falta (fail-closed).`,
    );
    return;
  }
  ...
  const evidence = (detail: string, answer: string): void => {
    out.evidence.push({
      short: rule.short,
      order: context.order(),
      rank: rule.rank,
  ...
  const ask = (question: string, why: string, extra: { options?: string[]; template?: string; slot?: string } = {}): void => {
  ...
  switch (rule.code) {
    case 'COMPOUND_REQUIREMENT': {
      ...
      ask(
        `Esta frase encadena ${shallCount} respuestas "shall" y cada una es un comportamiento. ¿Cómo se escribe cada comportamiento como su propia frase EARS, con su actor y su disparador?`,
        `El requisito se partirá en ${shallCount} frases EARS verificables por separado: hoy no se puede probar un comportamiento sin probar el otro. La partición conserva cada respuesta palabra por palabra; lo que falta es la decisión de si son de verdad comportamientos distintos.`,
        {
          options: [
    ...
    case 'VAGUE_TERM': {
      ...
      ask(
        `¿Cuál es ${hint}? Sustituirá a «${term}» en la frase y la hará comprobable.`,
        suggestion.why,
        { ...(template ? { template } : {}), ...(term.length > 0 ? { slot: term } : {}) },
      );
      return;
    }
    case 'MISSING_TRIGGER': {
      if (applicable) {
        ...
        evidence(
          `la condición necesaria de ${context.rel} ya está escrita en la propia frase («${target}»); moverla al disparador no añade ningún dato y la reescritura conserva el sentido.`,
          suggestion.proposal,
        );
        return;
      }
      ask(
        'La condición necesaria ("only …") está dentro de la respuesta y no se puede negar sin cambiar el sentido. ¿Es una condición necesaria (solo se cumple si X) o un disparador (cuando X, entonces …)?',
        suggestion.why,
        {
          options: [
    ...
    case 'PASSIVE_VOICE': {
      if (applicable) {
        ...
        evidence(
          `en ${context.rel} la frase nombra al agente${agent ? ` («${agent}»)` : ''}: pasarla a voz activa usa ese mismo agente y no añade ningún dato.`,
          suggestion.proposal,
        );
        return;
      }
      ask(
        'La respuesta está en pasiva y no dice quién ejecuta la acción. ¿Qué componente del sistema responde, y con qué acción?',
        suggestion.why,
        { template: 'WHEN <disparador>, the <agente> shall <verbo> <objeto>.' },
      );
      return;
    }
    case 'NO_ACTOR': {
      if (applicable) {
        evidence(
          `el actor dominante de ${context.rel} ya resuelve la frase: ${suggestion.why}`,
          suggestion.proposal,
        );
        return;
      }
      ask(
        'No hay sujeto antes de "shall" y el documento no repite ningún actor del que tomarlo. ¿Qué componente del sistema debe actuar?',
        suggestion.why,
        { template: 'WHEN <disparador>, the <sistema> shall <respuesta>.' },
      );
      return;
    }
    case 'ACTOR_NOT_SYSTEM': {
      ...
      ask(
        `El sujeto de la frase es una persona («${subject}»), no el sistema. ¿Qué respuesta observable del sistema produce esa acción, y cuándo?`,
        suggestion.why,
        { template: 'WHEN <disparador>, the <sistema> shall <respuesta observable>.' },
      );
      return;
    }
    case 'NOT_TESTABLE': {
      ask(
        'Esta frase es un objetivo, no un comportamiento. ¿Qué respuesta observable del sistema demuestra que se cumple, y cuándo?',
        suggestion.why,
        { template: 'WHEN <disparador>, the <sistema> shall <respuesta observable>.' },
      );
      return;
    ...
    case 'NO_SHALL': {
      ...
      ask(
        `La frase no lleva el operador "shall"${should ? ' (lleva "should", que es una recomendación)' : ''}. ¿Cuál es la acción observable y obligatoria del sistema?`,
        suggestion.why,
        { template: 'WHEN <disparador>, the <sistema> shall <acción observable>.' },
      );
      return;
    }
    default: {
      unresolvable(
        suggestion.code,
        `el código ${suggestion.code} no tiene una derivación de pregunta implementada en este motor.`,
      );
    }
  }
};
...
export const planClarify = async (input: {
  ...
  const max = Number.isFinite(requestedMax) && (requestedMax as number) >= 0
    ? Math.floor(requestedMax as number)
    : DEFAULT_CLARIFY_MAX;
  ...
  if (requirementsRaw !== null) artifacts.push(relativePath(root, requirementsPath));
  if (deltaRaw !== null) artifacts.push(relativePath(root, deltaPath));
  if (tasksRaw !== null) artifacts.push(relativePath(root, tasksPath));

  const candidates: Candidate[] = [];
  const evidenceCandidates: EvidenceCandidate[] = [];
  const unresolvableCandidates: UnresolvableCandidate[] = [];
  ...
  if (tasksRaw !== null) {
    const delta: DeltaSpec = deltaRaw !== null ? parseDeltaSpec(deltaRaw) : parseDeltaSpec('');
  ...
  const questions: ClarifyQuestion[] = kept.map((candidate) => {
  ...
  const fromEvidence: ClarifyEvidence[] = evidenceCandidates.map((entry) => {
  ...
  const unresolvable: ClarifyUnresolvable[] = unresolvableCandidates.map(({ code, artifact, reason }) => ({
    code,
    artifact,
    reason,
  }));
  ...
  if (artifacts.length === 0) {
    detailParts.push(
      `No hay requirements.md, delta.md ni tasks.md para "${feature}": no hay nada que clarificar y NO se declara limpia (no se analizó nada).`,
    );
  } else if (questions.length === 0 && fromEvidence.length === 0 && unresolvable.length === 0) {
    detailParts.push(
      `La especificación de "${feature}" está limpia: 0 preguntas de clarificación sobre ${artifacts.join(', ')} (${analysedRequirements} requisito(s) analizado(s), ninguno ambiguo). Nada que preguntar.`,
    );
  } else {
    detailParts.push(
      `${questions.length} pregunta(s) de clarificación sobre ${artifacts.join(', ')} (${analysedRequirements} requisito(s) analizado(s)).`,
    );
  }
  if (fromEvidence.length > 0) {
    detailParts.push(
      `${fromEvidence.length} hallazgo(s) NO se preguntan: el repositorio ya los responde (ver fromEvidence, ids ${fromEvidence.map((entry) => entry.id).join(', ')}).`,
    );
  }
  ...
  if (remaining > 0) {
    detailParts.push(
      `Lote RECORTADO al tope de ${max}: quedan ${remaining} pregunta(s) sin mostrar (sube --max para verlas).`,
    );
  }
...
// ---------------------------------------------------------------------------------------------
// applyAnswers: escribir SOLO la línea afectada y verificar contra el análisis
// ---------------------------------------------------------------------------------------------

export interface ClarifyApplyReport {
  text: string;
...
const looksLikePlaceholderAnswer = (answer: string): boolean =>
  PLACEHOLDER_TOKEN.test(answer) ||
  /^<[^<>\n]+>$/.test(answer) ||
...
export const applyAnswers = (input: {
  ...
  for (const question of input.session.questions) {
    ...
    if (question.answerKind === 'id') {
      ...
      replacement = [before.replace(slotRe, (_match, left: string, right: string) => `${left}${answer}${right}`)];
    } else {
  ...
  for (const entry of attempted) {
    ...
    const beforeHit = isPlaceholder
      ? PLACEHOLDER_TOKEN.test(entry.before)
        ? 1
        : 0
      : (beforeReport?.suggestions ?? []).some(
          (suggestion) => suggestion.code === entry.question.code && suggestion.target === entry.before.trim(),
        )
        ? 1
        : 0;
    const afterHit = isPlaceholder
      ? entry.after.some((line) => PLACEHOLDER_TOKEN.test(line))
        ? 1
        : 0
      : (afterReport?.suggestions ?? []).some(
          (suggestion) => suggestion.code === entry.question.code && afterTargets.includes(suggestion.target),
        )
        ? 1
        : 0;
    ...
    codeAfter += afterHit;
    if (afterHit > 0) stillOpen.push(entry.question.id);
    else resolved.push(entry.question.id);
  }
...
export const clarifyQuestionsFile = (session: ClarifySession): Record<string, unknown> => ({
  ...
  howToAnswer:
    'Rellena `answers` con el id de cada pregunta. `answerKind: statement` espera la frase EARS completa; ' +
 ...
 */
export const readClarifyAnswers = (raw: string): { answers: Record<string, string>; error?: string } => {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    return { answers: {}, error: `JSON ilegible: ${(error as Error).message}` };
  }
  ...
  for (const [key, value] of Object.entries(source)) {
    if (!/^[QE]-[A-Z0-9]+-\d+$/.test(key)) continue;
  ...
  if (Object.keys(answers).length === 0) {
    return {
      answers: {},
      error:
        'no se encontró ninguna respuesta: se esperaba `{ "answers": { "Q-…": "…" } }` o un mapa de ids de pregunta a texto.',
    };
  }
  return { answers };
};
