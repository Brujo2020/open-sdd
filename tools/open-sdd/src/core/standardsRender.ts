...
import type { StandardEntry } from './standardsTypes.js';
...
export interface RuleDrift {
  file: string;
  /** `missing` = el render no existe en disco; `differs` = existe pero no coincide con el catálogo. */
  reason: 'missing' | 'differs';
  /** Ids de las entradas cuya prosa renderizada no está en el fichero de disco. */
  entries: string[];
}

const normalize = (value: string): string => value.replace(/^\.\//, '').split('\\').join('/');
 ...
 */
export const ruleFileOf = (source: string): string | null => {
...
const fileTitle = (file: string): string => {
  const base = path.posix.basename(file, '.md').replace(/[-_]+/g, ' ').trim();
  if (base.length === 0) return 'Standards';
  return `${base.charAt(0).toUpperCase()}${base.slice(1)}`;
};

const beginMarker = (id: string): string => `<!-- standards:${id}:begin -->`;
const endMarker = (id: string): string => `<!-- standards:${id}:end -->`;
 ...
 */
export const renderEntryBlock = (entry: StandardEntry): string => {
  const lines: string[] = [];
  ...
  lines.push(
    bullet(
      'Severity',
      `\`${entry.severity}\` (${isBlocking(entry) ? 'blocking, corpus measured' : 'advisory until a measured corpus exists'})`,
    ),
  );
...
const entriesByFile = (entries: readonly StandardEntry[]): Map<string, StandardEntry[]> => {
  ...
  for (const list of grouped.values()) list.sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
  return grouped;
};
 ...
 * texto, byte a byte.
 */
export const renderRules = (entries: readonly StandardEntry[]): RenderedRule[] => {
...
const readIfPresent = async (abs: string): Promise<string | null> => {
  try {
 ...
 */
export const writeRules = async (cwd: string, entries: readonly StandardEntry[]): Promise<string[]> => {
