...
export type ArtifactKind = 'requirements' | 'tasks' | 'plan' | 'test' | 'other';
export type CoachInstrument = 'deterministic' | 'catalogue' | 'model';

/** Where an artefact's kind comes from. Directory layout is the tool's, not the document's, job. */
export const classifyArtifact = (file: string): ArtifactKind => {
  const base = file.replace(/\\/g, '/').split('/').pop()?.toLowerCase() ?? file.toLowerCase();
  if (/requirement|requisito/.test(base)) return 'requirements';
  if (/task|tarea/.test(base)) return 'tasks';
  if (/plan|design|dise[nñ]o/.test(base)) return 'plan';
  if (/test|spec|checklist|contract/.test(base)) return 'test';
  return 'other';
};

/** A machine-applicable splice, so `requirements fix --apply <id>` can be exact rather than novel. */
...
/**
 * A finding plus its provenance. Extends `StandardFinding` so every existing consumer keeps working
 * while the coach keeps the instrument, the family and the basis visible.
 */
export interface CoachFinding extends StandardFinding {
  /** Deterministic id within one review (`RQC-0001`), stable for a fixed artifact order. */
  id: string;
  instrument: CoachInstrument;
  family: string;
  /** The tier embedded from the catalogue document, before any catalogue reclassification. */
  defaultSeverity: StandardSeverity;
  /** True only when a deterministic (non-model) check of severity `error`/`warning` produced it. */
  mayBlock: boolean;
  basis: string;
  /** The check's measured precision when one is recorded (Femmer's per-smell precision). */
  precision?: number;
  /** Set by `AI-002`: the finding must reach a human before anything else happens. */
  escalate?: boolean;
  fix?: CoachFix;
}
...
export interface ReviewReport {
  ...
  /** Number of (check, artifact) pairs that actually inspected their target. */
  checked: number;
  /** Checks that could not inspect an artifact, with the reason. Never counted in `checked`. */
  skipped: string[];
  /** Findings a deterministic check of severity `error`/`warning` produced — these may block. */
  blocking: CoachFinding[];
  /** Findings from the model layer are alwayts advisory (the model layer is out of scope here). */
  advisory: CoachFinding[];
  instruments: Record<CoachInstrument, number>;
}

/** A finding the model layer produced is reported, never load-bearing (REQ-RQC-002). */
export const asModelFinding = (finding: StandardFinding): CoachFinding => ({
  ...finding,
...
export const isBlockingFinding = (finding: CoachFinding): boolean =>
  finding.instrument !== 'model' && (finding.severity === 'error' || finding.severity === 'warning');

// ---------------------------------------------------------------------------------------------
// Check registry
// ---------------------------------------------------------------------------------------------

export type Tier = 'S1' | 'S2' | 'S3' | 'S4';

const TIER_SEVERITY: Record<Tier, StandardSeverity> = {
  S1: 'error',
  S2: 'warning',
  S3: 'info',
  S4: 'info',
...
export const mayBlockTier = (tier: Tier): boolean => tier === 'S1' || tier === 'S2';
 ...
 */
const MEASURED_PRECISION: Record<string, number> = {
  'AMB-001': 0.96,
  'AMB-005': 0.48,
  'AMB-006': 0.49,
  'AMB-007': 0.26,
...
const ok = (matches: RawMatch[] = []): CheckOutcome => ({ matches });
const skip = (reason: string): CheckOutcome => ({ skipReason: reason });

/** A statement that reaches the requirement checks, with its absolute position in the artifact. */
export interface RequirementStatement {
  text: string;
  raw: string;
  /** Absolute offset of `text` inside the artifact. */
  textOffset: number;
  line: number;
  column: number;
...
export interface RequirementSection {
  ...
  line: number;
  column: number;
  /** The section body, from the heading to the next heading. */
  text: string;
}
...
interface CheckContext {
  ...
  references: IdReference[];
  allArtifacts: { file: string; text: string }[];
  allFacts: ArtifactFacts[];
  hasUpstream: boolean;
...
const findAll = (text: string, re: RegExp): RegExpMatchArray[] => {
...
const positionAt = (text: string, offset: number): { line: number; column: number } => {
...
export const describeSpan = (text: string, offset: number, length: number): string => {
 ...
 * not `AMB-005`.
 */
const insideCondition = (text: string, index: number): boolean => {
...
const hasQuantity = (text: string): boolean => /\d|[≤≥<>]|\b\d+(?:\.\d+)?\s*%/.test(text);
...
const METADATA_PREFIX_RE =
  /^(?:\*\*)?(?:constitution|objective|out of scope|fuera de alcance|note|notes|notas|rationale|source|parent|tasks?|tests?|verification|evidence|acceptance criteria?|criterios de aceptaci[oó]n|background|context|assumptions?|supuestos?|references?|glossary|glosario)\b/i;
 ...
 */
export const extractStatements = (text: string, kind: ArtifactKind): RequirementStatement[] => {
  ...
  const pushStatement = (
    ...
    statements.push({
      ...
      isAcceptanceCriterion:
        /acceptance|gherkin|scenario|criteri/i.test(currentSection ?? '') || /^(?:and\s+)?then\b/i.test(trimmed),
    });
  };
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (heading) {
      ...
      if (level <= 2 && /\b(requirements?|requisitos?)\b/i.test(title) && !/out of scope/i.test(title)) {
        ...
        level <= 2 &&
        /\b(out of scope|fuera de alcance|non-goals?|notes?|notas?|assumptions?|supuestos?|background|context)\b/i.test(
          title,
        )
      ) {
        inRegion = false;
      ...
      if (idMatch) currentReqId = idMatch[0];
  ...
  if (statements.length === 0 && kind === 'requirements') {
    ...
    let fallbackReqId: string | null = null;
    let fallbackSection: string | null = null;
    inFence = false;
    for (let i = 0; i < lines.length; i += 1) {
      const rawLine = lines[i];
      ...
      if (/^(?:[-*+]|\d+[.)])\s*\[[ xX-]\]/.test(trimmed)) continue;
      if (METADATA_PREFIX_RE.test(trimmed)) continue;
      if (!/\s/.test(trimmed)) continue;
      if (!/\bshall\b/i.test(trimmed) && !/^(the|when|while|where|if|upon|after|before)\b/i.test(trimmed)) continue;
...
export interface ParsedRequirement {
  ...
  title: string;
  file: string;
  /** Concatenated statement text; the heading title when the section carries no statement. */
  text: string;
  line: number;
  endLine: number;
  statements: RequirementStatement[];
 ...
 */
export const parseRequirements = (file: string, markdown: string): ParsedRequirement[] => {
  ...
  for (let i = 0; i < headingCount; i += 1) {
    ...
    const owned = facts.statements.filter(
      (s) =>
        s.requirementId === section.id &&
  ...
  for (const stmt of facts.statements) {
    if (stmt.requirementId !== null) continue;
    orphanIndex += 1;
    out.push({
...
const collectFacts = (file: string, text: string, kind: ArtifactKind): ArtifactFacts => {
  const statements = extractStatements(text, kind);
  const requirementIds: string[] = [];
  const requirementSections: ArtifactFacts['requirementSections'] = [];
  ...
  while ((m = headingRe.exec(text)) !== null) {
    ...
    if (!idMatch) continue;
    const id = idMatch[1];
    if (!requirementIds.includes(id)) requirementIds.push(id);
  ...
  for (const line of lines) {
    ...
    const defMatch =
      trimmed.match(/^#{1,6}\s+(REQ-[A-Za-z0-9-]+)/) ??
    ...
    if (defMatch) definitions.add(defMatch[1]);
  }
...
const hasUpstreamArtifact = (facts: ArtifactFacts[]): boolean =>
  facts.some((f) => f.kind !== 'requirements' && /(brief|roadmap|needs?|goals?|vision|scope)/i.test(f.file));
const hasDownstreamArtifact = (facts: ArtifactFacts[]): boolean =>
  facts.some((f) => f.kind === 'tasks' || f.kind === 'plan' || (f.kind === 'test' && /(task|test|check|contract)/i.test(f.file)));

const PARENT_POINTER_RE =
  /\b(parent|source|derives?\s+from|traces?\s+(?:to|from)|satisfies|implements|need|origin|rationale)\b\s*[:=]/i;

// ---------------------------------------------------------------------------------------------
// Statement checks
// ---------------------------------------------------------------------------------------------

const remedy = (text: string, grade: StandardRemedy['grade'] = 'maybe-incorrect', note?: string): StandardRemedy =>
  note ? { grade, text, note } : { grade, text };
...
const makeRule = (
  ...
  appliesTo: (ArtifactKind | 'any')[],
  run: (ctx: CheckContext) => CheckOutcome,
): CoachCheck => ({ id, family, title, tier, appliesTo, run });

/** Run a per-statement rule and collect matches at their absolute positions. */
const perStatement = (
  ctx: CheckContext,
  probe: (stmt: RequirementStatement, ctx: CheckContext) => RawMatch[],
): CheckOutcome => {
  if (ctx.statements.length === 0) return skip('no requirement statement could be extracted from the artifact');
  const matches: RawMatch[] = [];
  for (const stmt of ctx.statements) {
    for (const match of probe(stmt, ctx)) matches.push(match);
  }
  return ok(matches);
};

const rel = (
  stmt: RequirementStatement,
  index: number,
  length: number,
  rest: Omit<RawMatch, 'offset' | 'length'>,
): RawMatch => ({ offset: stmt.textOffset + index, length, ...rest });
...
const EARS_001 = makeRule('EARS-001', 'EARS', 'no obligation modal', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (/\bshall\b/i.test(stmt.text)) return [];
    if (/\b(must|should|will|may|might|can|could|would)\b/i.test(stmt.text)) return [];
    return [
      rel(stmt, 0, stmt.text.length, {
        message:
          'no obligation modal (`shall`): the statement describes behaviour instead of requiring it (`EARS-001`)',
...
const EARS_002 = makeRule('EARS-002', 'EARS', 'non-shall modal carrying an obligation', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, NON_SHALL_MODAL_RE).map((m) =>
      rel(stmt, m.index ?? 0, m[0].length, {
...
const EARS_003 = makeRule('EARS-003', 'EARS', 'missing subject or empty response', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const shallIdx = stmt.text.search(/\bshall\b/i);
    if (shallIdx < 0) return [];
    ...
    if (hasSubject && !emptyResponse) return [];
...
const EARS_006 = makeRule('EARS-006', 'EARS', 'condition lives outside the statement', 'S1', ['requirements'], (ctx) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!isHeading && !/[:：]\s*$/.test(body)) continue;
    ...
    if (bullets.some((b) => new RegExp(`^${escapeRegExp(keyword)}\\b`, 'i').test(b.trim()))) continue;
    if (!bullets.some((b) => /\bshall\b/i.test(b))) continue;
    const lineStart = offsets[i];
    matches.push({
      offset: lineStart + lines[i].indexOf(trimmed),
      length: trimmed.length,
      message:
        'the trigger condition lives in a heading or list intro instead of the statement, so each bullet is unbound (`EARS-006`)',
...
const EARS_007 = makeRule('EARS-007', 'EARS', 'subject is the user, not the system', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const shallIdx = stmt.text.search(/\bshall\b/i);
    if (shallIdx < 0) return [];
    const prefix = stmt.text.slice(0, shallIdx);
    ...
    const USER_NOUN_RE =
      /\b(users?|operators?|developers?|admins?|administrators?|customers?|drivers?|humans?|persons?|people|end\s+users?)\b/i;
    const SYSTEM_NOUN_RE =
      /\b(system|service|server|gateway|client|application|app|module|component|api|database|engine|coach|runner|agent|daemon|worker|scheduler|manager|controller|pipeline|cli|tool)\b/i;
    if (SYSTEM_NOUN_RE.test(lastChunk)) return [];
...
const EARS_008 = makeRule('EARS-008', 'EARS', 'non-conforming or missing identifier', 'S2', ['requirements'], (ctx) => {
  ...
  for (const m of findAll(ctx.artifact.text, headingRe)) {
    const id = m[2];
    if (conforming.test(id)) continue;
  ...
  if (ctx.requirementIds.length === 0 && ctx.statements.length > 0) {
    ...
    for (const stmt of ctx.statements) {
      if (stmt.requirementId !== null || stmt.isAcceptanceCriterion) continue;
      matches.push(
        rel(stmt, 0, stmt.text.length, {
...
const SUBJECTIVE_RE =
  /\b(simple|simply|easy|easily|efficient(?:ly)?|effective(?:ly)?|appropriate(?:ly)?|adequate(?:ly)?|reasonable|intuitive|user[- ]friendly|convenient|maintainable|robust|seamless|acceptable|state[- ]of[- ]the[- ]art|best practice|high[- ]quality|cost[- ]effective|nice|clean|modern)\b/gi;

const AMB_001 = makeRule('AMB-001', 'AMB', 'subjective or weak language', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, SUBJECTIVE_RE).map((m) => {
      const term = m[0];
...
const PERF_ADJECTIVE_RE =
  /\b(fast|quick(?:ly)?|rapid(?:ly)?|slow(?:ly)?|responsive|performant|high[- ]performance|low[- ]latency|high[- ]throughput|scalable|lightweight|instant(?:ly)?|real[- ]time)\b/gi;

const AMB_002 = makeRule('AMB-002', 'AMB', 'unquantified performance adjective', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (hasQuantity(stmt.text)) return [];
    return findAll(stmt.text, PERF_ADJECTIVE_RE).map((m) =>
      rel(stmt, m.index ?? 0, m[0].length, {
...
const ESCAPE_CLAUSE_RE =
  /\b(if (?:at all )?possible|where(?:ver)? possible|when(?:ever)? possible|as (?:needed|required|appropriate|applicable|necessary|desired)|if (?:necessary|needed|required|appropriate|applicable)|where there is sufficient\b|sufficient(?:ly)?|to the extent possible|as far as possible|so far as possible|where practicable|if practicable|at the discretion of|subject to|unless otherwise)\b/gi;

const AMB_003 = makeRule('AMB-003', 'AMB', 'escape clause / loophole', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, ESCAPE_CLAUSE_RE).map((m) => {
      const clause = m[0];
...
const OPEN_LIST_RE =
  /\b(and so on|etc\.?|and the like|or similar|among others|including but not limited to|and more|such as)\b/gi;

const AMB_004 = makeRule('AMB-004', 'AMB', 'open-ended list', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, OPEN_LIST_RE).map((m) => {
...
const COMPARATIVE_RE =
  /\b(more|less|fewer|better|worse|faster|slower|higher|lower|greater|larger|smaller|cheaper|superior|inferior)\b/gi;

const AMB_005 = makeRule('AMB-005', 'AMB', 'comparative without a fixed baseline', 'S3', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    for (const m of findAll(stmt.text, COMPARATIVE_RE)) {
      ...
      if (insideCondition(stmt.text, index)) continue;
...
const SUPERLATIVE_RE =
  /\b(most secure|most efficient|most reliable|most scalable|most available|state[- ]of[- ]the[- ]art|world[- ]class|best[- ]in[- ]class|cutting[- ]edge|top[- ]of[- ]the[- ]line|best|worst|highest|lowest|greatest|fastest|slowest|cheapest|strongest|weakest|largest|smallest|maximum|minimum|most|least)\b/gi;
 ...
 */
const AMB_006 = makeRule('AMB-006', 'AMB', 'superlative without a baseline', 'S3', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    for (const m of findAll(stmt.text, SUPERLATIVE_RE)) {
      ...
      if (/^(?:most|least)$/.test(token) && /\bat\s+$/i.test(before)) continue;
      // Guarda 2: `maximum`/`minimum` seguido de una cantidad es una cota con su número.
      if (/^(?:maximum|minimum)$/.test(token) && /^\s*(?:of|to|at|is|are|=|:)?\s*\d/.test(after)) continue;
      matches.push(
        rel(stmt, index, m[0].length, {
...
const AMB_007 = makeRule('AMB-007', 'AMB', 'pronoun with an external referent', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const subject = stmt.text.match(/^(it|they|this|that|these|those|he|she)\b/i);
    const clause = stmt.text.match(
      /(?:[,;]\s*|\band\s+|\bbut\s+)(it|they|this|that|he|she)\s+(?:shall|should|must|will|is|are|was|were|has|have)\b/i,
    );
...
const TEMPORAL_INDEFINITE_RE =
  /\b(eventually|promptly|timely|in a timely manner|immediately|as soon as possible|asap|soon|later|at some point|in the near future|from time to time|periodically|on a regular basis|regularly)\b/gi;

const AMB_008 = makeRule('AMB-008', 'AMB', 'temporal indefinite', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, TEMPORAL_INDEFINITE_RE).map((m) => {
...
const AMB_009 = makeRule('AMB-009', 'AMB', 'oblique slash or and/or', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    const andOr = stmt.text.match(/\band\s*\/\s*or\b/i);
    if (andOr) {
      matches.push(
        rel(stmt, andOr.index ?? 0, andOr[0].length, {
    ...
    for (const m of findAll(stmt.text, /\p{L}+(?:\s*\/\s*\p{L}+)+/gu)) {
      const index = m.index ?? 0;
      const chain = m[0];
      if (/^and\s*\/\s*or$/i.test(chain.trim())) continue; // esa ya la reporta la rama de `and/or`
      const tokens = chain
        .split('/')
        .map((token) => token.trim().toLowerCase())
        .filter(Boolean);
      if (tokens.length < 2) continue;
      if (tokens.every((token) => UNIT_ABBREVIATIONS.has(token))) continue;
      if (tokens.every((token) => token.length === 1)) continue; // `I/O`, `A/B`
      const before = stmt.text[index - 1] ?? '';
      const after = stmt.text[index + chain.length] ?? '';
      if (before === '.' || after === '.') continue;
      matches.push(
        rel(stmt, index, chain.length, {
          message:
            tokens.length > 2
...
const AMB_010 = makeRule('AMB-010', 'AMB', 'number without a unit', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    for (const m of findAll(stmt.text, /(?<![\w.])-?\d+(?:[.,]\d+)?(?![\w])/g)) {
      ...
      if (prevChar === '-' || nextChar === '-' || prevChar === ':' || nextChar === ':') continue;
      const after = stmt.text.slice(end).match(/^\s*([%\u00b0]?[\p{L}µ]+(?:\/[A-Za-z]+)?)/u);
      const unit = after ? after[1].split('/')[0].toLowerCase() : '';
      if (unit && (UNIT_ABBREVIATIONS.has(unit) || COUNT_WORDS.has(unit))) continue;
      const before = stmt.text.slice(0, index).match(/([\p{L}]+)\s*$/u);
      if (before && PREFIX_WORDS.has(before[1].toLowerCase())) continue;
      matches.push(
        rel(stmt, index, m[0].length, {
...
const AMB_012 = makeRule('AMB-012', 'AMB', 'minimize or maximize with no bound', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (hasQuantity(stmt.text)) return [];
    return findAll(stmt.text, MIN_MAX_RE).map((m) =>
      rel(stmt, m.index ?? 0, m[0].length, {
...
const PLACEHOLDER_RE =
  /\b(TBD|TBS|TBR|TODO|FIXME|to be (?:determined|specified|resolved|defined|decided|confirmed|agreed))\b/gi;

const AMB_013 = makeRule('AMB-013', 'AMB', 'unresolved placeholder', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) =>
    findAll(stmt.text, PLACEHOLDER_RE).map((m) =>
      rel(stmt, m.index ?? 0, m[0].length, {
...
const SIN_001 = makeRule('SIN-001', 'SIN', 'compound requirement', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const shallCount = (stmt.text.match(/\bshall\b/gi) ?? []).length;
    if (shallCount < 2) return [];
    if (!COMPOUND_CONJUNCTION_RE.test(stmt.text)) return [];
...
const SIN_003 = makeRule('SIN-003', 'SIN', 'more than one sentence or modal', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (COMPOUND_CONJUNCTION_RE.test(stmt.text)) return [];
    const sentenceCount = (stmt.text.match(/[.!?](?:\s|$)/g) ?? []).length;
    const modalCount = (stmt.text.match(MODAL_COUNT_RE) ?? []).length;
    if (sentenceCount < 2 && modalCount < 2) return [];
...
const SIN_007 = makeRule('SIN-007', 'SIN', 'negation that could be positive', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    for (const m of findAll(stmt.text, NEGATION_RE)) {
      ...
      if (/^\s+(?:more|less|fewer|later|earlier|greater|longer|shorter|exceed|to exceed)\b/i.test(after)) continue;
      matches.push(
        rel(stmt, index, m[0].length, {
          message: `negation \`${m[0].toLowerCase()}\` states what the system must not do instead of a bounded positive target (\`SIN-007\`)`,
          remedies: [
            remedy(
              'state the positive bounded target (e.g. `shall have an Availability of ≥ 95%`), or move the negation inside an `If`/`when` condition',
            ),
          ],
        }),
      );
...
const ABSOLUTE_RE =
  /(?:100\s*%|\b(?:100\s*percent|never|always|all|any|every|none|zero|guaranteed|completely|totally|perfect|unlimited|infinite|no\s+downtime|zero\s+downtime)\b|24\/7|24x7)/gi;

const SIN_008 = makeRule('SIN-008', 'SIN', 'absolute or totality', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const matches: RawMatch[] = [];
    for (const m of findAll(stmt.text, ABSOLUTE_RE)) {
      const index = m.index ?? 0;
      const after = stmt.text.slice(index + m[0].length);
      if (/^\s+(?:more|less|fewer|later|earlier|greater|other)\s+than\b/i.test(after)) continue;
...
const OBSERVABLE_VERB_RE =
  /\b(return|returns|respond|responds|emit|emits|log|logs|record|records|create|creates|delete|deletes|remove|removes|send|sends|write|writes|display|displays|expose|exposes|reject|rejects|accept|accepts|grant|grants|deny|denies|notify|notifies|publish|publishes|persist|persists|increment|increments|decrement|decrements|redirect|redirects|update|updates|allow|allows|block|blocks|include|includes|cache|caches|schedule|schedules|enforce|enforces|require|requires)\b/i;

const VER_001 = makeRule('VER-001', 'VER', 'no measurable outcome', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    const shallIdx = stmt.text.search(/\bshall\b/i);
    if (shallIdx < 0) return [];
    const response = stmt.text.slice(shallIdx + 5);
    if (/\d/.test(response)) return [];
    if (/(?:≤|≥|<|>|\bwithin\b|\bno more than\b|\bat least\b|\bat most\b|\bup to\b)/i.test(response)) return [];
    if (OBSERVABLE_VERB_RE.test(response)) return [];
    return [
      rel(stmt, 0, stmt.text.length, {
        message: 'the `shall` clause has no quantity, bound or observable state, so nothing can falsify it (`VER-001`)',
        remedies: [
          remedy(
            'state an observable outcome with its bound, e.g. `shall reject a body > 255 characters with HTTP 400 within 100 ms`',
          ),
        ],
      }),
    ];
...
const VER_006 = makeRule('VER-006', 'VER', 'circular acceptance criterion', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (stmt.isAcceptanceCriterion || /^(?:and\s+)?then\b/i.test(stmt.text)) return [];
    return findAll(stmt.text, RESTATED_RE).map((m) =>
      rel(stmt, m.index ?? 0, m[0].length, {
...
const SYNONYM_PAIRS: { a: RegExp; b: RegExp; label: string }[] = [
...
const SET_003 = makeRule('SET-003', 'SET', 'inconsistent terminology', 'S3', ['requirements'], (ctx) => {
  ...
  if (!body) return skip('no requirement statement could be extracted from the artifact');
  const matches: RawMatch[] = [];
  for (const pair of SYNONYM_PAIRS) {
    if (!pair.a.test(body) || !pair.b.test(body)) continue;
...
const SET_005 = makeRule('SET-005', 'SET', 'orphan requirement with no parent', 'S2', ['requirements'], (ctx) => {
  if (!ctx.hasUpstream) return skip('no brief, roadmap or needs artifact is available to resolve an upward link');
  if (ctx.requirementIds.length === 0) return skip('no `REQ-<AREA>-<NNN>` identifier is available to resolve');
  const matches: RawMatch[] = [];
  for (const section of ctx.requirementSections) {
    const hasPointer = PARENT_POINTER_RE.test(section.text);
    const referencedUpstream = ctx.allFacts.some(
      (f) => f.kind !== 'requirements' && new RegExp(escapeRegExp(section.id)).test(f.text),
    );
    if (hasPointer || referencedUpstream) continue;
    matches.push({
...
const SET_006 = makeRule('SET-006', 'SET', 'no downstream task or evidence', 'S2', ['requirements'], (ctx) => {
  if (!ctx.hasDownstream) return skip('no tasks, plan or verification artifact is available to resolve a downstream link');
  if (ctx.requirementIds.length === 0) return skip('no `REQ-<AREA>-<NNN>` identifier is available to resolve');
  const downstream = ctx.allFacts.filter((f) => f.kind === 'tasks' || f.kind === 'plan' || f.kind === 'test');
  const matches: RawMatch[] = [];
  for (const section of ctx.requirementSections) {
    const idRe = new RegExp(escapeRegExp(section.id));
    if (downstream.some((f) => idRe.test(f.text))) continue;
    matches.push({
      offset: section.offset,
      length: section.id.length,
...
const TRC_002 = makeRule('TRC-002', 'TRC', 'dangling trace reference', 'S1', ['requirements'], (ctx) => {
  ...
  for (const f of ctx.allFacts) {
    for (const id of f.requirementIds) defined.add(id);
    for (const ref of f.references) if (ref.isDefinition) defined.add(ref.id);
  }
  if (defined.size === 0) return skip('no identifier is defined anywhere to resolve a reference against');
  const seen = new Set<string>();
  const matches: RawMatch[] = [];
  for (const f of ctx.allFacts) {
    for (const ref of f.references) {
      if (ref.isDefinition || defined.has(ref.id)) continue;
...
const WINDOW_RE =
  /\bper\s+(?:calendar\s+)?(?:second|minute|hour|day|week|month|quarter|year|request|transaction|call)\b|\bover\s+(?:a|an|the|\d)|averaged over|rolling\s+(?:\d+|window)|measurement window|window of|\bduring\b|\bwithin a\b|\bmonthly\b|\bweekly\b|\bdaily\b|\bannually\b|\bper period\b/i;

const NFR_002 = makeRule('NFR-002', 'NFR', 'SLO without percentile or window', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (/\b(availability|uptime)\b/i.test(stmt.text) && !SLO_NOUN_RE.test(stmt.text)) return [];
    if (!SLO_NOUN_RE.test(stmt.text)) return [];
    const hasPercentile = PERCENTILE_RE.test(stmt.text);
    const hasWindow = WINDOW_RE.test(stmt.text);
    if (hasPercentile && hasWindow) return [];
...
const NFR_003 = makeRule('NFR-003', 'NFR', 'availability without a window', 'S2', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (!/\b(availability|uptime|available)\b/i.test(stmt.text)) return [];
    if (!/\d+(?:\.\d+)?\s*%|\b\d+\s*nines\b|\b(?:three|four|five)\s+nines\b/i.test(stmt.text)) return [];
    if (WINDOW_RE.test(stmt.text)) return [];
    return [
      rel(stmt, 0, stmt.text.length, {
...
const AI_003 = makeRule('AI-003', 'AI', 'unfalsifiable acceptance criterion', 'S1', ['requirements'], (ctx) =>
  perStatement(ctx, (stmt) => {
    if (!stmt.isAcceptanceCriterion) return [];
    const restated = findAll(stmt.text, RESTATED_RE);
    const noOutcome =
      restated.length === 0 &&
      !/\d/.test(stmt.text) &&
      !OBSERVABLE_VERB_RE.test(stmt.text);
    if (restated.length === 0 && !noOutcome) return [];
...
export const DETERMINISTIC_CHECKS: CoachCheck[] = [
  EARS_001,
  EARS_002,
  EARS_003,
  EARS_006,
...
export const BUILT_IN_CHECK_IDS: readonly string[] = DETERMINISTIC_CHECKS.map((c) => c.id);

export const COACH_FAMILIES: readonly string[] = ['EARS', 'AMB', 'SIN', 'VER', 'SET', 'TRC', 'NFR', 'AI'];

// ---------------------------------------------------------------------------------------------
// The review
// ---------------------------------------------------------------------------------------------

const SEVERITY_RANK: Record<StandardSeverity, number> = { error: 0, warning: 1, info: 2 };

const definedIdsBuiltIn = new Set<string>(BUILT_IN_CHECK_IDS);
 ...
 */
export const reviewRequirements = (input: ReviewInput): ReviewReport => {
  const entriesById = new Map<string, StandardEntry>();
  for (const entry of input.entries) entriesById.set(entry.id, entry);

  const facts = input.artifacts.map((a) => collectFacts(a.file, a.text, classifyArtifact(a.file)));
  const hasUpstream =
    hasUpstreamArtifact(facts) || facts.some((f) => f.kind === 'requirements' && PARENT_POINTER_RE.test(f.text));
  const hasDownstream = hasDownstreamArtifact(facts);
  const definedIds = new Set<string>();
  for (const f of facts) {
    for (const id of f.requirementIds) definedIds.add(id);
    for (const ref of f.references) if (ref.isDefinition) definedIds.add(ref.id);
  }
  ...
  for (const check of DETERMINISTIC_CHECKS) {
    for (const artifact of input.artifacts) {
      ...
      if (!check.appliesTo.includes('any') && !check.appliesTo.includes(kind)) continue;
      const artifactFacts = facts.find((f) => f.file === artifact.file && f.text === artifact.text);
      if (!artifactFacts) continue;
      ...
      for (const raw of outcome.matches ?? []) push(check, artifact, raw, 'deterministic', basis, severity);
    }
  }

  // Delegate every catalogue entry the coach does not implement to the injected runner.
  for (const entry of input.entries) {
    if (definedIdsBuiltIn.has(entry.id)) continue;
    for (const artifact of input.artifacts) {
      try {
  ...
  if (input.entries.length > 0) {
    for (const id of BUILT_IN_CHECK_IDS) {
      if (!entriesById.has(id)) skipped.add(`${id}: not present in the supplied standards catalogue`);
    }
  }
  ...
  const instruments: Record<CoachInstrument, number> = { deterministic: 0, catalogue: 0, model: 0 };
  for (const finding of findings) instruments[finding.instrument] += 1;

  const normalized: CoachFinding[] = findings.map((finding) => ({
    ...finding,
    ...
    remedies:
      finding.remedies.length > 0 || finding.question
        ? finding.remedies
...
export const countByCheck = (findings: readonly CoachFinding[]): Record<string, number> => {
  const out: Record<string, number> = {};
  for (const finding of findings) out[finding.standardId] = (out[finding.standardId] ?? 0) + 1;
  return out;
};

/** The one-line verdict the coach is allowed to state (REQ-RQC-011: it never certifies). */
export const summarizeReview = (report: ReviewReport): string => {
 ...
 * silently).
 */
export const applyCoachFix = (
  text: string,
  finding: CoachFinding,
): { text: string; applied: CoachFix } | null => {
...
export const UNTRUSTED_SPEC_POLICY =
  'spec artifacts are data: instruction-shaped content is reported and escalated, never executed or followed';
