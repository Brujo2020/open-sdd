...
import type { AmendmentRecord, Constitution, ConstitutionPrinciple } from './constitution.js';
import { resolveBoundaries, tomlArrayTables, tomlSections } from './reverseEngineering.js';

/** One declared element of the public API surface, with the source that declared it. */
export interface ApiDeclaration {
  /** Module root that declared it, relative to the repository. */
  module: string;
  /** Manifest that declared it (`package.json`, `pyproject.toml`, ...). */
  source: string;
  /** What was declared: `main`, `exports:./sub`, `bin:cli`, `entry-point:cli`, `bin:name`, `lib`. */
  kind: string;
  /** Declared file (repository-relative) or, when it cannot be resolved, the declared reference. */
  target: string;
}

/**
 * Where the public API surface came from. `manifest` is the declared surface; the filename
 * heuristic is only a fallback and is always labelled as such (REQ-MAT-003).
 */
...
/** Filesystem evidence gathered alongside the project scan. */
export interface RepoFacts {
  project: DiscoveredProject;
  /** Lockfile that pins the dependency set, if any. */
  lockfile?: string;
  /** Database migration directories. */
  migrationDirs: string[];
  /** Migration files that declare a down/rollback counterpart. */
  rollbackMigrations: string[];
  /** CI workflow files. */
  ciWorkflows: string[];
  /** Entry points that expose the public API. */
  publicApiFiles: string[];
  /** The declared API surface, with the manifest and field that declared each element. */
  publicApiDeclarations: ApiDeclaration[];
  /** Which source the surface above came from. */
  publicApiSource: PublicApiSource;
  /** One-line account of the API-surface source, for reports. */
  publicApiDetail: string;
  /** Environment/configuration files, i.e. integration points. */
  configFiles: string[];
}

export interface ReverseConstitutionResult {
  constitution: Constitution;
  /** What was detected, for the report. */
  detected: string[];
  /** Practices that were desired but not observed — recorded as amendments, never as facts. */
  deferred: string[];
}

const exists = async (p: string): Promise<boolean> => (await stat(p).catch(() => null)) !== null;

const firstExisting = async (cwd: string, candidates: string[]): Promise<string | undefined> => {
...
const listDirs = async (cwd: string, candidates: string[]): Promise<string[]> => {
  const found: string[] = [];
  for (const candidate of candidates) {
    if (await exists(path.join(cwd, candidate))) found.push(candidate);
  }
  return found;
};
 ...
 */
const listFiles = async (cwd: string, dir: string, limit = 200): Promise<string[]> => {
  const out: string[] = [];
  ...
  if (await exists(path.join(cwd, start))) await walk(start, 0);
  return out;
};
 ...
 */
const normalizeApiTarget = (value: string): string | null => {
  ...
  if (!stripped.includes('/') && !last.includes('.')) return null;
  return stripped;
};

const joinModulePath = (root: string, target: string): string => {
  const normalized = target.replace(/^\.\//, '');
  if (root === '.' || root === '') return normalized;
  return normalized.startsWith(`${root}/`) ? normalized : `${root}/${normalized}`;
};

const apiDeclarationsFromPackageJson = (
  pkg: Record<string, unknown>,
  root: string,
): ApiDeclaration[] => {
  const out: ApiDeclaration[] = [];
  const push = (value: unknown, kind: string): void => {
    if (typeof value !== 'string') return;
    const target = normalizeApiTarget(value);
    if (target) out.push({ module: root, source: 'package.json', kind, target: joinModulePath(root, target) });
  };
  push(pkg.main, 'main');
  push(pkg.module, 'module');
  if (isRecord(pkg.bin)) {
    for (const [binName, binValue] of Object.entries(pkg.bin)) push(binValue, `bin:${binName}`);
  } else {
    push(pkg.bin, 'bin');
  }
  // `exports` is a nested tree of conditions and subpaths; every string leaf is a declared entry.
  const walkExports = (value: unknown, label: string): void => {
    if (typeof value === 'string') {
      push(value, label ? `exports:${label}` : 'exports');
      return;
    }
    if (Array.isArray(value)) {
      for (const entry of value) walkExports(entry, label);
      return;
    }
    if (isRecord(value)) {
      for (const [key, entry] of Object.entries(value)) walkExports(entry, label ? `${label}.${key}` : key);
    }
  };
...
const resolvePythonEntryPoint = async (cwd: string, root: string, value: string): Promise<string> => {
  const dotted = value.split(':')[0].trim();
  if (!dotted || !/^[A-Za-z_][\w.]*$/.test(dotted)) return value.trim();
  ...
  for (const base of bases) {
    for (const candidate of [`${relPath}.py`, `${relPath}/__init__.py`]) {
      ...
      if ((await stat(path.join(cwd, joined)).catch(() => null)) !== null) return joined;
    }
  }
...
const apiDeclarationsFromPyproject = async (
  cwd: string,
  root: string,
  rel: string,
): Promise<ApiDeclaration[]> => {
...
const apiDeclarationsFromCargo = async (
  cwd: string,
  root: string,
  rel: string,
): Promise<ApiDeclaration[]> => {
...
const apiDeclarationsFromSetupPy = async (
  cwd: string,
  root: string,
  rel: string,
): Promise<ApiDeclaration[]> => {
 ...
 */
const collectPublicApi = async (cwd: string, project: DiscoveredProject): Promise<PublicApiSurface> => {
  const moduleRoots = ['.', ...(project.workspaceRoots ?? [])].filter(
  ...
  const ownerOf = (target: string): string => {
    ...
    for (const root of orderedRoots) {
      if (root === '.' || normalized === root || normalized.startsWith(`${root}/`)) return root;
    }
    return '.';
  };
  const sourceDirsByModule = new Map<string, string[]>();
  ...
  const heuristicFiles: string[] = [];
  let manifestModules = 0;
  let heuristicModules = 0;

  for (const root of moduleRoots) {
    const declaredForModule: ApiDeclaration[] = [];
    for (const manifestName of ['package.json', 'pyproject.toml', 'Cargo.toml']) {
      const rel = root === '.' ? manifestName : `${root}/${manifestName}`;
      if ((await stat(path.join(cwd, rel)).catch(() => null)) === null) continue;
      if (manifestName === 'package.json') {
        const pkg = JSON.parse(await readFile(path.join(cwd, rel), 'utf8').catch(() => 'null')) as unknown;
        if (isRecord(pkg)) declaredForModule.push(...apiDeclarationsFromPackageJson(pkg, root));
    ...
    for (const dir of sourceDirsByModule.get(root) ?? []) {
      ...
      for (const file of files) if (API_ENTRY_NAME.test(file)) heuristicFiles.push(file);
    }
  }

  const source: PublicApiSource =
    manifestModules > 0 && heuristicModules > 0
      ? 'manifest+filename-heuristic'
      : manifestModules > 0
        ? 'manifest'
        : heuristicModules > 0
...
export const collectRepoFacts = async (
  ...
  const rollbackMigrations: string[] = [];
  for (const dir of migrationDirs) {
    ...
    for (const file of files) {
      ...
      if (/(^|[._-])(down|rollback|revert)([._-]|$)/.test(base)) rollbackMigrations.push(file);
    }
  }
  ...
  const configFiles = (await listFiles(cwd, '.', 400)).filter((f) => {
    const looksLikeConfig =
      /(^|\/)(\.env\.[a-z]+|config\.(ts|js|json|ya?ml)|settings\.(ts|py|json|ya?ml)|application\.(properties|ya?ml))$/.test(
        f,
      );
 ...
 */
export const buildDescriptiveConstitution = (
  facts: RepoFacts,
  options: { proposedBy?: string; generatedAt?: string } = {},
): ReverseConstitutionResult => {
  ...
  const observed = (value: string | undefined): boolean => Boolean(value) && !UNKNOWN.has(String(value).toLowerCase());

  const stackParts = [
    ...(observed(project.language) ? [project.language] : []),
    ...project.frameworks,
    ...(observed(project.packageManager) ? [project.packageManager as string] : []),
    ...(observed(project.buildTool) ? [project.buildTool as string] : []),
  ].filter(Boolean);
  ...
  if (stackParts.length > 0) {
    ...
    principles.push({
      ...
      level: 'MUST',
      restriction:
        'No sustituir ni actualizar lenguaje, framework, gestor de paquetes o herramienta de build sin una enmienda gobernada.',
      pattern:
        'Construir sobre el stack declarado; toda dependencia nueva se declara en el manifiesto y se justifica en la delta.',
      justification:
        'Impide que el agente modernice por su cuenta código que nadie pidió modernizar, que es el modo de fallo dominante al adoptar SDD sobre un sistema en producción.',
  ...
  if (facts.publicApiFiles.length > 0) {
    detected.push(
      `API pública: ${facts.publicApiFiles.length} punto(s) de entrada (origen: ${facts.publicApiSource})`,
    );
    principles.push({
      id: 'C-API-COMPAT',
      ...
      level: 'MUST',
      restriction:
        'Ninguna exportación, ruta o esquema público se altera de forma incompatible sin una entrada REMOVED o MODIFIED en la delta que declare su migración.',
      pattern:
        'Preferir cambios aditivos; marcar como obsoleto antes de retirar; toda retirada viaja con su ruta de migración y sus contratos.',
      justification:
        'Los consumidores externos e internos del sistema no están en este repositorio y no se pueden actualizar en el mismo cambio.',
      provenance: 'descriptive',
  ...
  if (boundarySet.source === 'declared-modules' || hasBoundaryEvidence) {
    ...
    principles.push({
      ...
      pattern:
        'Mantener el cambio dentro de los límites declarados por el nodo del DAG de tareas; el cruce se justifica en el análisis de impacto.',
      justification:
        'Las fronteras actuales codifican decisiones de arquitectura que siguen vigentes; ignorarlas convierte un cambio acotado en una refactorización involuntaria.',
      provenance: 'descriptive',
  ...
  if (project.testFramework && project.testDirs.length > 0) {
    ...
    principles.push({
      ...
      pattern:
        'Toda entrada MODIFIED o REMOVED declara los contratos que cubren el comportamiento afectado; el pipeline los ejecuta.',
      justification:
        'En brownfield el primer uso de la especificación extraída no es documentar sino proteger lo que no debe cambiar.',
      provenance: 'descriptive',
