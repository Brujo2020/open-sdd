import type { SchedulePlan, TaskItem, TaskWave } from './types.js';
import { parseTasksMarkdown } from './specManager.js';

export const buildTaskDependencyWaves = (
  tasks: TaskItem[],
  ...
  while (remaining.length > 0) {
    ...
    const waveTasks: TaskItem[] = [];
    const waveBoundaries = new Set<string>();

    for (const task of readyTasks) {
      if (waveTasks.length >= maxParallel) break;
...
export const createSchedulePlan = (
  feature: string,
