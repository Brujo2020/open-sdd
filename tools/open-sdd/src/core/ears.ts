...
export interface EarsTemplate {
  pattern: EarsPattern;
  /** Canonical template with the trigger keyword kept in English, as the paper fixes it. */
  template: string;
  example: string;
  /** Trigger keyword, absent for the ubiquitous pattern. */
  keyword: string | null;
}
...
const KEYWORD_PATTERNS: { pattern: EarsPattern; re: RegExp }[] = [
 ...
 */
export const containsVagueTerm = (text: string, term: string): boolean =>
 ...
 */
export const validateEarsRequirement = (text: string): EarsVerdict => {
  const issues: EarsIssue[] = [];
  const trimmed = text.trim();

  const matched = KEYWORD_PATTERNS.filter((k) => k.re.test(trimmed));
  const pattern: EarsPattern | null = matched.length === 0 ? 'ubiquitous' : matched[0].pattern;
  ...
  if (triggerCount > 1) {
    issues.push({
      ...
      message:
        'Más de un patrón EARS en la misma condición de disparo: exactamente una plantilla por requisito funcional.',
    });
  }
  ...
  if (matched.length === 0 && /\bshall\b/i.test(trimmed)) {
    ...
    if (looksAmbiguous) {
      issues.push({
        ...
        message:
          'No encaja en ninguna plantilla y contiene términos ambiguos: casi siempre es un requisito ambiguo, o dos requisitos vestidos de uno.',
      });
    }
  }
...
export interface EarsReport {
  total: number;
  conforming: number;
  byPattern: Record<EarsPattern, number>;
  uncoveredPatterns: EarsPattern[];
  verdicts: EarsVerdict[];
}

/** Report over a whole requirements document, including which patterns nobody used. */
export const validateRequirements = (texts: string[]): EarsReport => {
  const verdicts = texts.map(validateEarsRequirement);
  const byPattern: Record<EarsPattern, number> = {
    ubiquitous: 0,
    'event-driven': 0,
    'state-driven': 0,
    optional: 0,
  ...
  for (const v of verdicts) if (v.pattern) byPattern[v.pattern] += 1;

  return {
    total: verdicts.length,
    conforming: verdicts.filter((v) => v.conforms).length,
    byPattern,
    uncoveredPatterns: (Object.keys(byPattern) as EarsPattern[]).filter((p) => byPattern[p] === 0),
    verdicts,
  };
};
 ...
 */
export const negativeRequirementGap = (report: EarsReport): string | null =>
  report.byPattern.unwanted === 0
...
export const EARS_LIMIT =
  'EARS disciplina el enunciado de un requisito, no su corrección: un requisito impecable puede especificar el comportamiento equivocado.';
