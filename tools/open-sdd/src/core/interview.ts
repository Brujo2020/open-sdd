...
const truncate = (values: string[], max: number, label: string): string[] =>
  values.length <= max ? [...values] : [...values.slice(0, max), `… (+${values.length - max} ${label})`];
...
export interface InterviewQuestion {
  id: string;
  question: string;
  /** Por qué esto es una elección humana y no un dato del repositorio. */
  why: string;
  default?: string;
  options?: string[];
  /** La evidencia del repositorio de la que sale el valor por defecto (cuando existe). */
  fromEvidence?: string;
}

export interface ConstitutionInterview {
  project: string;
  /** Líneas de reconocimiento que sostienen la entrevista (hechos + prácticas con evidencia). */
  evidence: string[];
  questions: InterviewQuestion[];
  /** Lo que la evidencia YA decidió y por tanto NO se pregunta, con su evidencia citada. */
  decidedByEvidence: string[];
  /** El bloque serializable que se entrega a un modelo/host para que rellene las respuestas. */
  deferForHost?: Record<string, unknown>;
  detail: string;
}

/** Contexto derivado del reconocimiento que decide qué preguntas normativas aplican. */
interface InterviewContext {
  publicApiFiles: string[];
  publicApiDetail: string;
...
interface NormativeQuestionSpec {
  id: string;
  needs: 'public-api' | 'test-oracle' | 'boundaries' | 'config';
  question: string;
  why: string;
  /** `options[0]` es la opción ESTRICTA (afirmativa); `options[1]`, la permisiva. */
  options: [string, string];
  /** Principio observado que la respuesta refina; se registra cuando no añade un principio nuevo. */
  confirms: string;
  evidence: (ctx: InterviewContext) => string;
  /** El principio normativo que la opción estricta añade. Sin él, la respuesta solo confirma. */
  principle?: {
    id: string;
    title: string;
    level: 'MUST' | 'SHOULD';
 ...
 */
const NORMATIVE_QUESTIONS: readonly NormativeQuestionSpec[] = [
  {
    id: 'Q-NORM-API-DEPRECATION',
    needs: 'public-api',
    question: '¿Toda retirada de la API pública exige deprecación previa antes de retirarla?',
    why:
      'La API pública ya existe y la evidencia la declara, pero cuánto tiempo debe sobrevivir un consumidor que no está en este repositorio es una decisión del equipo, no un hecho del código.',
    ...
    principle: {
      ...
      justification:
        'Los consumidores de una API pública no están en este repositorio y no se pueden actualizar en el mismo cambio: una retirada sin aviso los rompe sin que el pipeline de este proyecto lo detecte.',
    },
  },
  {
    ...
    needs: 'test-oracle',
    question:
      '¿Un cambio MODIFIED exige añadir un test que cubra el nuevo comportamiento, o basta con que los tests actuales sigan verdes?',
    why:
      'El oráculo de regresión existe y la evidencia lo demuestra, pero exigir cobertura NUEVA para cada modificación es una elección de rigor, no un hecho del código.',
    ...
    principle: {
      ...
      justification:
        'Un cambio de comportamiento que solo reutiliza la cobertura anterior deja el caso nuevo sin oráculo: el pipeline seguirá verde aunque el comportamiento añadido sea incorrecto.',
    },
  },
  {
    ...
    why:
      'Las fronteras actuales están observadas, pero si cruzarlas es una decisión defendible en la delta o una violación que solo una enmienda puede autorizar es una elección del equipo.',
  ...
  },
  {
    ...
    why:
      'La presencia de configuración e integraciones está observada, pero QUÉ datos viajan por ellas y con qué obligación es una decisión de negocio que ningún manifiesto declara.',
    ...
    principle: {
      ...
      justification:
        'Un dato sensible sin principio explícito acaba copiado en un log o en una respuesta de error, que es donde una fuga se descubre tarde y sin rastro de quién la autorizó.',
    },
  },
];

const normativeApplies = (needs: NormativeQuestionSpec['needs'], ctx: InterviewContext): boolean => {
  if (needs === 'public-api') return ctx.publicApiFiles.length > 0;
  if (needs === 'test-oracle') return Boolean(ctx.testFramework) && ctx.testDirs.length > 0;
  if (needs === 'boundaries') return ctx.boundaries.length > 0;
  return ctx.configFiles.length > 0;
};

const UNKNOWN = new Set(['unknown', '', 'n/a', 'undefined']);

const observed = (value: string | undefined): value is string =>
  Boolean(value) && !UNKNOWN.has(String(value).toLowerCase());
 ...
 */
export const planConstitutionInterview = async (
  cwd: string,
  opts: { max?: number; sddDir?: string } = {},
): Promise<ConstitutionInterview> => {
  ...
  for (const candidate of [
    ...
    if (await exists(path.join(root, candidate))) manifestEvidence.push(candidate);
  }
  if (facts.lockfile) manifestEvidence.push(facts.lockfile);
  ...
  const decidedByEvidence: string[] = [];
  if (stackParts.length > 0) {
    decidedByEvidence.push(
      `stack (hecho establecido, no se pregunta): ${stackParts.join(', ')} — evidencia: ` +
  ...
  for (const principle of evidencedPrinciples) {
    decidedByEvidence.push(
      `práctica observada ${principle.id} "${principle.title}" (${principle.level}) — evidencia: ${(principle.evidence ?? []).join('; ')}`,
    );
  }
  if (ctx.boundaries.length > 0 && (project.sourceDirs.length > 0 || project.modules.length > 0)) {
    decidedByEvidence.push(
      `fronteras de módulo (${ctx.boundaries.length}): ${truncate(ctx.boundaries, 6, 'frontera(s)').join(', ')} — ${ctx.boundaryDetail}`,
    );
  }
  if (ctx.publicApiFiles.length > 0) {
    decidedByEvidence.push(
      `API pública: ${ctx.publicApiDetail} — evidencia: ${truncate(ctx.publicApiFiles, 5, 'punto(s) de entrada').join(', ')}`,
    );
  }
  ...
  if (decidedByEvidence.length === 0) {
    decidedByEvidence.push(
      'el reconocimiento no pudo decidir nada por evidencia: no hay manifiesto legible ni prácticas que citar (no se inventa ninguna)',
    );
  }
  ...
  if (evidencedPrinciples.length > 0) {
    questions.push({
      ...
      why:
        'La evidencia demuestra la práctica, pero convertirla en autoridad citable es una decisión humana: el borrador no es autoridad hasta que una persona nombrada lo ratifique.',
      default: 'sí',
      options: ['sí', 'no'],
  ...
  constitution.amendments.forEach((amendment, index) => {
    if (amendment.status === 'in-force') return;
    ...
    questions.push({
      ...
      why:
        'Una práctica que el código no demuestra no puede ser un hecho: entra como principio normativo con enmienda propuesta y no gobierna hasta que exista evidencia y una persona la ratifique.',
      default: 'no todavía',
  ...
  for (const spec of NORMATIVE_QUESTIONS) {
    if (!normativeApplies(spec.needs, ctx)) continue;
    questions.push({
      id: spec.id,
  ...
  const answersTemplate: Record<string, string> = {};
  for (const question of shown) if (question.default !== undefined) answersTemplate[question.id] = question.default;

  const deferForHost: Record<string, unknown> = {
    ...
    instructions:
      'Rellena `answersTemplate` (una respuesta por id) y pásalo en un fichero JSON con --answers. ' +
  ...
  if (shown.length === 0) {
    detailParts.push(
      'La evidencia respondió todo lo que este motor puede comprobar: no hay preguntas abiertas, pero el borrador sigue sin estar en vigor hasta que una persona nombrada lo ratifique.',
    );
  }
  if (omitted.length > 0) {
    detailParts.push(
      `AVISO: ${omitted.length} pregunta(s) no se muestran por el tope --max=${max}; es un límite de presentación, no una afirmación de que no existan.`,
    );
  }
  if (stackParts.length === 0) {
    detailParts.push(
      'No se pudo determinar el stack por evidencia (sin manifiesto legible): no se declara como hecho ni se pregunta por él; queda como desconocido.',
    );
  }
  if (inForceExists) {
 ...
 */
const decideAnswer = (question: InterviewQuestion, answer: string): 'yes' | 'no' | 'unclear' => {
  const normalized = answer.trim().toLowerCase();
  if (normalized.length === 0) return 'unclear';
  if (NEGATIVE_ANSWER.test(normalized)) return 'no';
  if (AFFIRMATIVE_ANSWER.test(normalized)) return 'yes';
  ...
  if (options.length >= 2) {
    ...
    options.forEach((option, index) => {
      ...
      let score = 0;
      while (score < max && lower[score] === normalized[score]) score += 1;
      if (score > bestScore) {
        bestScore = score;
        best = index;
      }
    });
    if (best >= 0 && bestScore >= Math.min(6, options[best].length)) return best === 0 ? 'yes' : 'no';
  }

  if (answer.includes(':') || /^(?:MUST|SHOULD|MAY)\b/i.test(answer)) return 'yes';
  return 'unclear';
};

/** Texto libre de una respuesta con forma `nivel: texto` o `sí: texto`. */
const freeTextOf = (answer: string): string => {
 ...
 */
const levelOf = (answer: string): string | undefined => {
  const prefixed = answer.match(/^\s*([A-Za-z][A-Za-z-]{1,})\s*:/);
  if (prefixed) {
    const token = prefixed[1].toUpperCase();
    if (LEVELS.has(token) || /^[A-Z][A-Z-]{2,}$/.test(token)) return token;
  }
  const leading = answer.match(/^\s*(MUST|SHOULD|MAY)\b/i);
  return leading ? leading[1].toUpperCase() : undefined;
};

const amendmentRecord = (principle: ConstitutionPrinciple, title: string, rationale: string): AmendmentRecord => ({
  id: `AMD-INTERVIEW-${principle.id}`,
  title,
  proposedBy: 'constitution-interview',
  status: 'proposed',
  migrationPlan:
    'Aplicar la práctica y aportar la evidencia (ruta, símbolo o comando) que la demuestre antes de ratificarla como principio observable.',
  rationale,
});

/** Una práctica ausente que la persona adopta: principio NORMATIVO en borrador, nunca un hecho. */
const adoptedFromAmendment = (amendment: AmendmentRecord, answer: string): ConstitutionPrinciple => {
  ...
  return {
    ...
    level: level as ConstitutionPrinciple['level'],
    ...
    justification:
      'Entra por la entrevista de constitución: el reconocimiento no encontró evidencia de esta práctica, y una práctica que el código no demuestra no puede afirmarse como hecho; solo gobierna cuando exista evidencia y una persona la ratifique.',
...
const normativePrinciple = (spec: NormativeQuestionSpec, answer: string): ConstitutionPrinciple | null => {
  if (!spec.principle) return null;
  ...
  return {
    ...
    amendment: amendmentRecord(
      principle,
      `Adopción por entrevista de ${principle.id} — ${principle.title}`,
      'Decisión registrada por la entrevista de constitución; sin evidencia todavía, no puede afirmarse como hecho.',
    ),
  };
};

/** El borrador con la cabecera que dice, sin ambigüedad, qué se aplicó, qué falta y qué bloquea. */
const renderInterviewDraft = (
  document: Constitution,
  input: {
    applied: { id: string; principle: string }[];
    unanswered: string[];
    issues: { severity: 'error' | 'warning'; id: string; code?: string; message: string }[];
    adopted: string[];
  },
): string => {
  ...
  if (input.applied.length === 0) header.push('- Ninguna respuesta se aplicó: no hay decisiones registradas.');
  for (const entry of input.applied) header.push(`- ${entry.id} → ${entry.principle}`);
  header.push('');

  header.push('## Entrevista: sin responder o no reconocidas (no cuentan como decisiones)', '');
  if (input.unanswered.length === 0) header.push('- Ninguna: todas las preguntas de la entrevista quedaron resueltas.');
  for (const id of input.unanswered) header.push(`- ${id}`);
  header.push('');
  ...
  if (input.adopted.length > 0) {
    ...
    for (const id of input.adopted) header.push(`- ${id}`);
    header.push('');
  }
...
export const applyConstitutionAnswers = async (input: {
  ...
  const answerOf = (id: string): string => String(answers[id] ?? '').trim();

  const applied: { id: string; principle: string }[] = [];
  const unanswered: string[] = [];
  const additions: ConstitutionPrinciple[] = [];
  const addedAmendments: AmendmentRecord[] = [];
  ...
  for (const question of input.interview.questions) {
    ...
    if (question.id.startsWith('Q-NORM-')) {
      ...
      if (!adoptedIds.has(principle.id)) {
        adoptedIds.add(principle.id);
        additions.push(principle);
        if (principle.amendment) addedAmendments.push(principle.amendment);
      }
      applied.push({ id: question.id, principle: principle.id });
      continue;
    }
  ...
  for (const id of Object.keys(answers)) {
    if (!questionIds.has(id) && answerOf(id).length > 0) unanswered.push(id);
  }
...
export interface SpecifyRequirement {
  id: string;
  /** El patrón EARS que el analizador compartido eligió (`ubiquitous`, `event`, `state`, `option`, `unwanted`). */
  pattern: string;
  statement: string;
  /** El fragmento de la descripción del que sale el requisito (trazabilidad, no una suposición). */
  fromAnswer?: string;
}
...
export interface SpecifyQuestion {
  id: string;
  question: string;
  /** El dato que falta: `actor`, `disparador`, `valor medible`, … */
  missing: string;
}

export interface SpecifyResult {
  feature: string;
  requirements: SpecifyRequirement[];
  questions: SpecifyQuestion[];
  /** El documento completo: los requisitos existentes intactos y, debajo, los ADDED derivados. */
  text: string;
  detail: string;
}
...
const sentenceBreak =
  /(?<=[.!?])\s+(?=(?:WHEN|WHILE|WHERE|IF|Cuando|Si|Mientras|Donde|El|La|Los|Las|The|A|An|Yo|Quiero|Necesito|Una|Dado)\b)/;

/** Divide la descripción en comportamientos sin decidir nada sobre ellos. */
const splitBehaviours = (description: string): string[] => {
  const units: string[] = [];
  ...
  for (const rawLine of withBullets.split('\n')) {
    ...
    for (const piece of line.split(/[;•]/)) {
      for (const sentence of piece.split(sentenceBreak)) units.push(sentence.trim());
    }
  }
 ...
 * palabra del usuario. */
const tidyCandidate = (proposal: string): string =>
  proposal
...
const TRIGGER_HINT =
  /\b(?:cuando|when|whenever|mientras|while|si\b|if\b|donde|where|una\s+vez\s+que|once|tras\b|después\s+de|despues\s+de|upon\b|after\b|al\s+\w+(?:ar|er|ir)se\b)/i;

const patternFromCode = (code: string): string => {
...
interface UnitAnalysis {
  ...
  missing?: string;
  question?: string;
  /** El candidato se tuvo que normalizar (separador/modal) antes de aceptarlo. */
  tidied?: boolean;
  /** El candidato del analizador compartido no quedó en forma de frase y no se fabricó el requisito. */
  malformed?: boolean;
}
 ...
 */
const analyseBehaviour = (unit: string, source: string): UnitAnalysis | null => {
  ...
  const actorDefaulted =
    /(^|,\s*)the system shall\b/i.test(candidate.proposal) && !/\b(?:the\s+system|el\s+sistema)\b/i.test(unit);
  if (actorDefaulted) {
    return {
      kind: 'question',
      missing: 'actor',
      question:
        `Sobre «${unit}»: el texto no nombra el componente del sistema que responde y el analizador EARS ` +
  ...
  if (pattern === 'ubiquitous' && TRIGGER_HINT.test(unit)) {
    return {
      ...
      missing: 'disparador',
      question:
        `Sobre «${unit}»: el texto insinúa un momento o una condición, pero el disparador no se pudo extraer ` +
  ...
  if (MALFORMED_RESPONSE.some((patternRe) => patternRe.test(tidied))) {
    return {
      ...
      question:
        `Sobre «${unit}»: el candidato EARS no quedó en forma de frase (la respuesta tras "shall" no es un verbo en ` +
  ...
  const blocking = report.suggestions.find(
    (suggestion) => suggestion.severity === 'error' || REFUSAL_CODES.has(suggestion.code.toUpperCase()),
  );
  if (blocking) {
...
export const existingRequirementStatements = (text: string): string[] => {
  const out: string[] = [];
  for (const raw of (text ?? '').split('\n')) {
    const line = raw.trim();
    if (line.length === 0 || line.startsWith('#') || line.startsWith('<!--') || /^Fuente\s*:/i.test(line)) continue;
    ...
    if (/\bshall\b/i.test(line)) out.push(line.replace(/^[-*+]\s+/, '').trim());
  }
  return out;
};

/** Clave de comparación de enunciados: forma, no contenido (espacios, caso y puntuación final). */
export const normalizeRequirementStatement = (statement: string): string =>
  statement.replace(/\s+/g, ' ').trim().toLowerCase().replace(/[.;]+$/, '');
 ...
 */
const existingStatementIds = (text: string): Map<string, string> => {
  ...
  for (const raw of (text ?? '').split('\n')) {
    ...
    const field = line.match(STATEMENT_FIELD);
    if (field) {
      if (lastId) map.set(normalizeRequirementStatement(field[1].trim()), lastId);
      continue;
    }
...
export const requirementArea = (feature: string, area?: string): string => {
  const declared = (area ?? '').trim();
  if (declared.length > 0) return declared.replace(/[^A-Za-z0-9]+/g, '-').replace(/^-+|-+$/g, '').toUpperCase() || 'REQ';
...
const nextSequence = (existing: string, area: string): number => {
  const prefix = `REQ-${area}-`;
  let max = 0;
  for (const match of (existing ?? '').matchAll(REQ_ID)) {
    if (`REQ-${match[1]}-` !== prefix) continue;
...
export const specifyFromDescription = (input: {
  ...
  const existingStatements = new Set(
    existingRequirementStatements(existingText).map(normalizeRequirementStatement),
  );
  ...
  const requirements: SpecifyRequirement[] = [];
  const questions: SpecifyQuestion[] = [];
  ...
  for (const unit of units) {
    ...
    if (alreadyPresent) {
      ...
      if (mapped) {
        ...
        while (existingIds.has(id)) id = `REQ-${area}-${pad(requirementSequence++)}`;
      }
  ...
  const kept = requirements.filter((requirement) =>
  ...
  const detailParts: string[] = [];
  detailParts.push(
    `${requirements.length} requisito(s) EARS derivado(s) de ${units.length} comportamiento(s) descrito(s); ` +
  ...
  if (patternsUsed.length > 0) detailParts.push(`Patrones usados: ${patternsUsed}.`);
  detailParts.push(
    `Plantillas y análisis: ${EARS_ASSISTANT_TEMPLATES.length} plantillas EARS compartidas (no se escribe un segundo analizador).`,
  );
  if (existingText.trim().length > 0) {
    detailParts.push(
      `Se conservan ${existingStatements.size} enunciado(s) existente(s) palabra por palabra (${kept} de los derivados ya estaban): ` +
        'decidir ADDED frente a MODIFIED es competencia de la delta (open-sdd delta init|validate), no de esta función.',
    );
  }
  if (tidied > 0) {
    detailParts.push(
      `${tidied} candidato(s) se normalizaron: el analizador compartido dejó un separador vacío tras "shall" y el modal duplicado; se quitaron esas marcas de forma, sin cambiar ninguna palabra.`,
    );
  }
  if (malformed > 0) {
    detailParts.push(
      `${malformed} comportamiento(s) se rechazaron porque el candidato no quedó en forma de frase: no se fabricó el requisito.`,
    );
  }
  detailParts.push(
    'La forma verbal tras "shall" se conserva tal como la escribió la descripción (no se conjuga ni se traduce): verifícala con `open-sdd brownfield requirements <feature>`.',
  );
