...
export type StopHookHost = 'claude-code' | 'codex';

export interface StopHookConfigPaths {
  darwin: string;
  linux: string;
  win32: string;
}

export interface StopHookDefinition {
  host: StopHookHost;
  event: string;
 ...
 */
export const STOP_HOOK_GATE_ARGS: readonly string[] = [
  'gates',
  'run',
  'C1',
  'C2',
...
const argvFor = (cliPath: string): string[] => [
  'node',
  '-e',
  EXIT_ADAPTER,
  cliPath,
...
const claudeSnippet = (opts: { cliPath: string }): string => {
...
const codexSnippet = (opts: { cliPath: string }): string =>
  jsonDoc({
    hooks: {
...
export const stopHookFor = (host: string): StopHookDefinition | undefined =>
  STOP_HOOKS.find((entry) => entry.host === host);

export interface StopHookInstallation {
  /** Absolute path of the host config this call targeted ('' when refused: no path was even chosen). */
  path: string;
  action: 'create' | 'update' | 'keep' | 'refused';
  reason: string;
  /** The exact snippet for the host ('' when refused). */
  snippet: string;
}
...
export interface InstallStopHookInput {
  ...
  host: string;
  cliPath: string;
  /** Writes by default; `false` computes the action and the snippet without touching the disk. */
  write?: boolean;
}
 ...
 */
const pickConfigPath = (paths: StopHookConfigPaths): string => {
  ...
  return paths[process.platform as keyof StopHookConfigPaths] ?? entries[0];
};
...
const refused = (reason: string): StopHookInstallation => ({ path: '', action: 'refused', reason, snippet: '' });
 ...
 */
export const installStopHook = async (input: InstallStopHookInput): Promise<StopHookInstallation> => {
  const hook = stopHookFor(input.host);
  if (!hook) {
    return refused(
      `el anfitrión «${input.host}» no tiene un mecanismo Stop verificado en este repositorio (solo claude-code y codex lo tienen): no se emite un hook no verificado y no se escribe nada.`,
    );
  }
  ...
  const entry = (JSON.parse(snippet) as { hooks: { Stop: unknown[] } }).hooks.Stop[0];
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      return refused(`no se pudo leer ${target}: ${(error as Error).message}; no se escribe nada.`);
    }
  }
  ...
  try {
    ...
    return refused(
      `${relative} existe pero no es JSON válido: se niega a reescribirlo (podría destruir configuración del equipo). Nada escrito.`,
    );
  }
  ...
  const stop = (existingStop as unknown[] | undefined) ?? [];
  ...
  if (write) {
    await writeFile(
      target,
      jsonDoc({ ...config, hooks: { ...(existingHooks ?? {}), Stop: [...stop, entry] } }),
      'utf8',
    );
  }
  return {
