...
import { parseConstitution, principlesInForce, validateConstitution, type Constitution } from './constitution.js';
import {
  deltaCounts,
  deltaSpecFileName,
  parseDeltaSpec,
...
import { extractContracts, testCommandFor, type ContractSet } from './executionContract.js';
import { getModifiedFiles, isGitRepo } from './git.js';
import { scanProject } from './reverseEngineering.js';
import { effectiveGates, loadRigorSettings, rigorRequires, type RigorDemand, type SddRigorLevel } from './rigor.js';
import { getSpecStatus, listSpecs, resolveSddDir } from './specManager.js';
import { checkEvidenceLock, evaluateTriad } from './triad.js';
import { alignSpecWithConstitution, type SpecAlignment } from './specConstitution.js';

export type StatusTone = 'ok' | 'warn' | 'err' | 'dim';
...
export interface StatusReport {
  ...
  brownfield: boolean;
  lines: StatusLine[];
  /** The single next command a user should run, when it can be determined. */
  nextAction?: string;
  /** True when nothing in the report could not be inspected. */
  complete: boolean;
}

const TONE_RANK: Record<StatusTone, number> = { err: 0, warn: 1, ok: 2, dim: 3 };
 ...
 */
const STRICT_DEMANDS: Record<string, RigorDemand> = {
...
export const worstTone = (tones: StatusTone[]): StatusTone | undefined =>
  [...tones].sort((a, b) => TONE_RANK[a] - TONE_RANK[b])[0];

/** Una línea de contenido. Etiqueta + valor; `value` vacío marca una CABECERA de sección. */
const content = (label: string, value: string, tone: StatusTone, detail?: string): StatusLine => ({
  label,
  value,
  tone,
  ...(detail ? { detail } : {}),
...
const section = (label: string): StatusLine => ({ label, value: '', tone: 'dim' });
...
export const findRepoRoot = async (cwd: string): Promise<string> => {
  ...
  for (let i = 0; i < 6; i += 1) {
    for (const candidate of ['.sdd', '.kiro']) {
      try {
        if ((await stat(path.join(dir, candidate))).isDirectory()) return dir;
      } catch {
        // sigue subiendo
      }
    }
...
const readText = async (absPath: string): Promise<TextRead> => {
  try {
...
export const constitutionCandidates = (sddDir: string): string[] => [
  path.join(sddDir, 'steering', 'constitution.md'),
  path.join(sddDir, 'constitution.md'),
];

export interface ConstitutionRead {
  path: string | null;
  exists: boolean;
...
export const loadConstitution = async (root: string, sddDir: string): Promise<ConstitutionRead> => {
...
export interface FeatureInspection {
  ...
  specDir: string;
  dirExists: boolean;
  /** Nombres de fichero del directorio de la spec (entrada de `evaluateTriad`). */
  files: string[];
  requirements: string | null;
  plan: string | null;
  tasks: string | null;
  delta: DeltaSpec | null;
  /** Rutas que existen pero no se pudieron leer: no evaluadas, y por tanto no aprobadas. */
  unreadable: string[];
}

/** Leer de una vez todo lo que el panel y el pivote necesitan de una feature. */
export const inspectFeature = async (
  root: string,
  feature: string,
  sddDir: string,
): Promise<FeatureInspection> => {
  ...
  const readSpecFile = async (rel: string): Promise<string | null> => {
    const read = await readText(path.join(root, rel));
    if (read.exists && read.content === null) unreadable.push(rel);
    return read.content;
  };
...
const deltaExists = async (root: string, sddDir: string, feature: string): Promise<boolean> =>
  (await readText(path.join(root, sddDir, 'specs', feature, deltaSpecFileName()))).exists;

/** Feature sobre la que se enfocan las secciones caras: la pedida, o la primera con delta. */
export const focusFeature = async (root: string, sddDir: string): Promise<string | null> => {
  const specs = await listSpecs(root, sddDir);
  for (const feature of specs) {
    if (await deltaExists(root, sddDir, feature)) return feature;
  }
  return specs[0] ?? null;
};
...
export interface AlignmentOutcome {
  ...
  /** El pivote no pudo ejecutarse; esto dice por qué (ausencia declarada, no fallo de lectura). */
  error?: string;
  /** Motivos por los que el pivote quedó parcialmente sin inspeccionar (fallo de lectura, sin git). */
  uninspected: string[];
  changedFiles: string[];
}
...
export const alignFeature = async (
  cwd: string,
  options: { feature?: string; sddDir?: string } = {},
  ...
  if (!gitRepo) uninspected.push('no es un repositorio git: el origen del cambio queda sin determinar');
...
const sameTestFile = (a: string, b: string): boolean => {
  const norm = (value: string): string =>
    value.split('::')[0].trim().replace(/\\/g, '/').replace(/^\.\//, '');
  const left = norm(a);
  const right = norm(b);
...
const undecidedNote = (detail: string): string | undefined => {
...
export const buildStatus = async (
  ...
  let level: SddRigorLevel = 'spec-first';
  let gates: string[] = [];
  let declaredBrownfield = false;
  let rigorError: string | null = null;
  try {
    ...
    rigorError = (err as Error).message;
  ...
  const constitution = await loadConstitution(root, sddDir);
  let constitutionState: 'valid' | 'invalid' | 'missing' | 'unreadable';
  if (!constitution.exists) {
    constitutionState = 'missing';
    lines.push(
      content(
        'Constitución',
        `ausente (${constitutionCandidates(sddDir)[0]}): no hay autoridad que citar`,
        'warn',
        'todo veredicto bloqueante debe poder citar un principio en vigor',
      ),
    );
    ...
    lines.push(
      content(
        'Constitución',
        `existe pero no se pudo leer (${constitution.error ?? 'error de lectura'}): no evaluada, no válida`,
        'warn',
      ),
    );
  } else {
    ...
    const proposed = parsed.amendments.filter(
      (amendment) => amendment.status === 'proposed' || amendment.status === 'under-review',
    );
    const amendmentText =
      proposed.length > 0
    ...
    lines.push(
      content(
        'Constitución',
        `presente y ${errors.length === 0 && inForce.length > 0 ? 'válida' : 'inválida'} · ${inForce.length} principio(s) en vigor · ${amendmentText}`,
        errors.length > 0 || inForce.length === 0 ? 'err' : warnings.length > 0 ? 'warn' : 'ok',
        errors.length > 0
  ...
  const demandsFor = (bf: boolean): Record<string, RigorDemand> =>
    rigorError ? STRICT_DEMANDS : rigorRequires(level, bf);
  ...
  for (const feature of features) {
    ...
    if (!status.exists) {
      lines.push(
        content(`Specification: ${feature}`, 'no existe en .sdd/specs: no hay nada que inspeccionar', 'err', 'comprueba el nombre'),
      );
      continue;
    }
    ...
    try {
      ...
      failures.push(`la trazabilidad de "${feature}" no es evaluable (${(err as Error).message})`);
    ...
    let evidenceTone: StatusTone = 'warn';
    ...
    if (!triad?.complete) tones.push('warn');
    if (traceTone !== 'ok') tones.push(traceTone);
    if (evidenceTone !== 'ok') tones.push(evidenceTone);

    lines.push(
      content(
        `Specification: ${feature}`,
        `Phase: ${status.phase} · ${triadText} · ${traceText} · ${evidenceText}`,
        worstTone(tones) ?? 'ok',
      ),
    );
  }
  ...
  const deltas: { feature: string; delta: DeltaSpec }[] = [];
  for (const feature of deltaCandidates) {
    const inspection = await inspectFeature(root, feature, sddDir);
    if (inspection.delta) deltas.push({ feature, delta: inspection.delta });
  }
  ...
  if (deltas.length > 0 && deltas.length <= DELTA_LINES_MAX) {
    for (const { feature, delta } of deltas) {
      ...
      lines.push(
        content(
          `Delta de ${feature}`,
          `ADDED ${counts.ADDED} · MODIFIED ${counts.MODIFIED} · REMOVED ${counts.REMOVED} · RENAMED ${counts.RENAMED} (total ${delta.entries.length}) · ${strangler.detail}`,
          deltaErrors.length > 0 ? 'warn' : 'ok',
          deltaErrors.length > 0 ? `${deltaErrors.length} error(es) de validación de la delta` : undefined,
        ),
      );
    }
    ...
    for (const { delta } of deltas) {
      ...
      for (const entry of delta.entries) states[entry.strangler ?? 'legacy'] += 1;
      deltaErrors += validateDeltaSpec(delta).filter((issue) => issue.severity === 'error').length;
    }
    lines.push(
      content(
        'Delta',
        `${deltas.length} feature(s) con delta: ADDED ${totals.ADDED} · MODIFIED ${totals.MODIFIED} · REMOVED ${totals.REMOVED} · RENAMED ${totals.RENAMED} (total ${entries}) · Estrangulamiento: ${states.new}/${entries} entradas completamente en el camino nuevo; ${states.both} conviven y ${states.legacy} siguen en el legado.`,
        deltaErrors > 0 ? 'warn' : 'ok',
        deltaErrors > 0 ? `${deltaErrors} error(es) de validación de la delta` : undefined,
      ),
    );
  } else {
    ...
    if (!brownfield) {
      ...
      lines.push(
        content(
          'Delta',
          `brownfield sin delta: el nivel ${level} exige el contrato del cambio`,
          'err',
          `créala con open-sdd delta init ${focus ?? '<feature>'} "..."`,
        ),
      );
    } else {
  ...
  if (!gitRepo) {
    ...
    lines.push(
      content('Contratos', 'cambios no evaluables: no es un repositorio git', 'warn', 'sin origen del cambio no hay huecos de cobertura que medir'),
    );
  }
  const changedFiles = gitRepo ? getModifiedFiles(root) : [];

  let contractSet: ContractSet | null = null;
  let missingDeclared: string[] = [];
  if (focus && focusInspection?.delta) {
    try {
      ...
      missingDeclared = declaredContracts.filter((declared) => {
        ...
        if (file && existsSync(path.join(root, file))) return false;
      ...
      if (missingDeclared.length > 0) {
        lines.push(
          content(`Contratos de ${focus}`, `declarados por la delta que no existen: ${missingDeclared.join(', ')}`, 'warn'),
        );
      }
      ...
      failures.push(`los contratos de "${focus}" no son evaluables (${(err as Error).message})`);
      lines.push(content(`Contratos de ${focus}`, `no evaluables (${(err as Error).message})`, 'warn'));
    }
  ...
  if (!outcome.alignment) {
    lines.push(
      content(
        'Constitucional',
        `pivote no ejecutado: ${outcome.error ?? 'sin datos'}`,
        'warn',
        'no se puede afirmar que la spec respete una autoridad que no se pudo leer',
      ),
    );
  } else {
    ...
    lines.push(
      content(
        'Constitucional',
        `declarados: ${alignment.declared.join(', ') || 'ninguno'} · alineación ${(alignment.alignment * 100).toFixed(0)}% · ${alignment.findings.length} hallazgo(s)${errors.length > 0 ? ` (${errors.length} error(es))` : ''}`,
        errors.length > 0 ? 'err' : warnings.length > 0 || note ? 'warn' : 'ok',
  ...
  if (rigorError) {
    lines.push(
      content('Rigor', 'declarado no evaluable: se informa como no evaluado, no como spec-first', 'warn', rigorError),
    );
  } else {
    lines.push(
      content(
        'Rigor',
        `${level} · gates activos ${gates.join(', ') || '(ninguno)'} · ${brownfield ? 'brownfield' : 'greenfield'}`,
        'ok',
      ),
    );
  }
...
const chooseNextAction = (input: {
  constitutionState: 'valid' | 'invalid' | 'missing' | 'unreadable';
  level: SddRigorLevel;
  brownfield: boolean;
  focus: string | null;
  focusHasDelta: boolean;
  missingDeclared: string[];
  uncoveredChanges: string[];
  changeOriginKnown: boolean;
  gates: string[];
}): string | undefined => {
  ...
  if (input.constitutionState === 'unreadable') return 'open-sdd brownfield constitution .';
