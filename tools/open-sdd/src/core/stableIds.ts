...
export type IdFindingCode = 'ID-MUTATED' | 'ID-LOST';
...
export interface IdAuditReport {
  beforeCount: number;
  afterCount: number;
  findings: IdAuditFinding[];
  /** Lo que no se pudo inspeccionar, dicho en vez de omitido. */
  problems: string[];
}

/** Normaliza un enunciado para comparar SIGNIFICADO, no formato. */
export const normalizeStatement = (statement: string): string =>
  statement
    .replace(/^[\s\-*>\d.)]+/, '')
    .replace(/[`*_]/g, '')
    .replace(/\s+/g, ' ')
 ...
 */
export const parseRequirementIds = (text: string): Map<string, string> => {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (!match) continue;
    const id = match[1];
    if (!found.has(id)) found.set(id, normalizeStatement(match[2] ?? ''));
  }
  return found;
};
 ...
 */
export const auditIds = (input: { before: Map<string, string>; after: Map<string, string> }): IdAuditFinding[] => {
 ...
 */
export const allocateBlock = (area: string, existing: readonly string[], size: number = ID_BLOCK_SIZE): string[] => {
...
export const readTextAtRef = (cwd: string, ref: string, relPath: string): string | null => {
  try {
...
export const auditIdsAgainstBase = (input: {
  ...
  if (beforeText === null) {
    problems.push(
      `no se pudo leer ${input.requirementsRelPath} en \`${input.base}\`: la base no resuelve, o el fichero no existe allí. Sin base no hay auditoría.`,
    );
  }
