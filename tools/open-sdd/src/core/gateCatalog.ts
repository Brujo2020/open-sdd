...
/** Control level. The paper is explicit: tiers encode COST, not severity (§9.3). */
export type GateTier = 'Hard' | 'Structural' | 'QE' | 'Meta' | 'Transversal';

/** Whether the control runs and inspects, runs without inspecting, or was retired. */
export type ControlState = 'executable' | 'vacuous' | 'spec-only' | 'retired';

/** Admission criteria (§9.3). A control enters the blocking chain only if all three hold. */
export interface AdmissionCriteria {
  /** A1 — the failure it prevents does not self-report. */
  silence: boolean;
  /** A2 — shares no assumption, analyzer, judge or evidence source with another control. */
  independence: boolean;
  /** A3 — letting the failure through costs orders of magnitude more than stopping. */
  costAsymmetry: boolean;
}

export interface LogicalGate {
  /** G1..G21 — taxonomy id from Table 33. */
  id: string;
  /** Canonical name (Spanish, as published). */
  name: string;
  /** Stable English label for CLI output in other locales. */
  nameEn: string;
  tier: GateTier;
  /** What it checks (Table 33, "Comprobación"). */
  check: string;
  /** Failure semantics (Table 33, "Semántica de fallo"). */
  failureSemantics: string;
  admission: AdmissionCriteria;
  /**
   * State of the logical control itself. A gate can be a legitimate control in the taxonomy
   * and still have no executable counterpart today; Table 36 calls that residue "the honest
   * half of the crosswalk".
   */
  state: ControlState;
  /** Where the control went when the chain was reduced (Table 35 note). */
  retiredTo?: string;
  /** Why there is no executable counterpart (Table 36, verbatim reasoning). */
  residueReason?: string;
}

/** Declared posture of an executable check (Table 34). */
export type GatePosture = 'blocking' | 'advisory' | 'opt-in';

export interface ExecutableGate {
  /** C1..C7 (core) and O1..O7 (opt-in). */
  id: string;
  name: string;
  nameEn: string;
  /** Reference implementation command, as declared in Table 34. */
  command: string;
  posture: GatePosture;
  /** Table 35 crosswalk: which logical gates this check imposes. */
  imposes: string[];
  tier: GateTier[];
  /**
   * False when the gate runs but inspects nothing (C7 today). Default-FAIL exists precisely to
   * stop a vacuous gate from being read as a passing control.
   */
  inspects: boolean;
  /**
   * False when the control is declared and enabled by a profile but NOT implemented. Such a
   * control is counted in the chain, not executed, and reported while the gap exists — omitting
   * it understates the government actually exercised, and an empty function returning success
   * fabricates evidence. The reference architecture lived this gap for a while (declared 12,
   * executed 7), which is why the state is a first-class field instead of a footnote.
   */
  implemented?: boolean;
  /** True when the check needs a model backend and degrades honestly without one. */
  requiresModel: boolean;
  /** What the check actually does in this implementation. */
  description: string;
}
 ...
 */
export const LOGICAL_GATES: LogicalGate[] = [
  {
    id: 'G1',
  ...
  },
  {
    ...
    residueReason:
      'La cobertura de riesgos exige conocer los riesgos; una verificación automática solo comprobaría que un campo de mitigación no esté vacío, lo que mide teatro de cumplimiento y no cobertura. La configuración la retira al checklist de spec justamente por eso.',
  },
  {
    id: 'G8',
    ...
    retiredTo: 'CI',
    residueReason:
      'Retirado a CI: lint y formato ya los impone la toolchain existente del proyecto, y duplicarlos dentro del arnés agregaría latencia sin agregar control. Se lista porque la taxonomía nombra el control, no a su dueño.',
  },
  {
    id: 'G9',
  ...
  },
  {
    ...
    retiredTo: 'CI',
    residueReason:
      'Retirado a CI (conflictos, build, suite) en vez de duplicarse dentro del arnés; el gate existe en la taxonomía porque el control debe nombrarse aunque otro sistema sea su dueño.',
  },
  {
    id: 'G11',
    ...
    residueReason:
      'La seguridad ante regresiones la sostiene la suite de tests existente, cuya evidencia valida C3, y no un gate propio; el gate de rendimiento del esquema 1 que lo habría extendido nunca se implementó y no sobrevivió a la consolidación del esquema 2, así que no reclamamos el control como impuesto por derecho propio.',
  },
  {
    id: 'G12',
    name: 'Documentación',
  ...
  },
  {
    ...
    residueReason:
      'Retirado a un ajuste de modo (configs/modes.yaml): un incumplimiento de la disciplina de salida es inmediatamente visible para quien lee y no daña ningún artefacto, así que no pasa el test de asimetría que un gate debe pasar. Es una perilla de configuración, no un control.',
  },
  {
    id: 'G18',
    name: 'Memoria Viva',
  ...
  },
  {
    ...
    failureSemantics:
      'Previsto como bloqueo; no implementado: gate_C7 retorna éxito sin inspeccionar nada, y la configuración lo declara advisorio',
 ...
 */
export const EXECUTABLE_CHAIN: ExecutableGate[] = [
  {
    id: 'C1',
...
/** Chain reduction profile (§9.5). The core is constant; the rest is a function of signals. */
export type ChainProfile = 'solo' | 'team' | 'regulated';

/** Repository signals. Every activated control must cite the signal that justified it. */
export interface RepoSignals {
  /** Third-party MCP servers declared in the host config. */
  declaresThirdPartyMcpServers: boolean;
  /** Architecture decisions recorded in the repo. */
  hasAdrRecords: boolean;
  /** Repository exceeds what fits an effective context window. */
  exceedsContextWindow: boolean;
  /** Dependency manifest and lockfile present. */
  hasDependencyManifest: boolean;
  /** More than one author in the recent history. */
  multipleAuthors: boolean;
  /** A structural index/graph is available to build on. */
  requiresStructuralGraph: boolean;
  /** Living memory configured for this repository. */
  hasLivingMemory: boolean;
}

export const DEFAULT_SIGNALS: RepoSignals = {
...
const SIGNAL_LABELS: Record<keyof RepoSignals, string> = {
...
export interface ActivatedControl {
  id: string;
  reason: string;
  /** Signal key that justified activation, when there is one. */
  signal?: keyof RepoSignals;
}

export interface ResolvedChain {
  profile: ChainProfile;
  /** The constant universal core: C1-C7. */
  core: string[];
  /** Opt-in controls this profile mandates without needing a signal. */
  profileMandated: ActivatedControl[];
  /** Opt-in controls activated because a repository signal justified them. */
  signalActivated: ActivatedControl[];
  /** Everything declared for this repository, in order. */
  declared: string[];
  /** Declared controls whose implementation exists and inspects. */
  executed: string[];
  /** Declared controls counted in the chain but reported as NOT implemented. */
  notImplemented: ActivatedControl[];
  /** Declared and running, but vacuous (activation without measurement). */
  vacuous: ActivatedControl[];
  /** Advisory controls (they run and report, but never change the exit code). */
  advisory: string[];
  blocking: string[];
}
 ...
 */
const PROFILE_MANDATED: Record<ChainProfile, string[]> = {
  solo: [],
  team: ['O1', 'O5'],
  regulated: ['O1', 'O2', 'O3', 'O4', 'O5'],
};

const CONTROL_SIGNAL: Partial<Record<string, keyof RepoSignals>> = {
...
export const getExecutableGate = (id: string): ExecutableGate | undefined => gateById.get(id);
...
export const resolveGateChain = (
  ...
  if (profile !== 'solo') {
    for (const gate of EXECUTABLE_CHAIN) {
      if (gate.posture !== 'opt-in') continue;
      if (mandatedIds.has(gate.id)) continue;
      const signal = CONTROL_SIGNAL[gate.id];
      if (!signal || !signals[signal]) continue;
  ...
  const notImplemented: ActivatedControl[] = [];
  const vacuous: ActivatedControl[] = [];
  ...
  for (const id of declared) {
    ...
    if (gate.implemented === false) {
      notImplemented.push({
        id,
        reason:
          'habilitado por el perfil y NO implementado: se cuenta en la cadena, no se ejecuta y se reporta mientras exista la brecha',
      });
      continue;
    }
    executed.push(id);
    if (gate.posture === 'blocking') blocking.push(id);
    if (gate.posture === 'advisory') advisory.push(id);
  }
...
/** Detection of the repository signals used by {@link resolveGateChain}. */
...
export const detectSignals = (evidence: Partial<Record<keyof RepoSignals, string>>): {
  signals: RepoSignals;
  evidence: SignalEvidence[];
} => {
  const signals: RepoSignals = { ...DEFAULT_SIGNALS };
  const rows: SignalEvidence[] = [];

  for (const signal of Object.keys(DEFAULT_SIGNALS) as (keyof RepoSignals)[]) {
...
export const buildCrosswalk = (): {
  byLogical: { gate: string; imposedBy: string[] }[];
  byCheck: { check: string; imposes: string[] }[];
} => {
 ...
 * no instrument).
 */
export const computeResidue = (): ResidueEntry[] => {
  const covered = new Set(EXECUTABLE_CHAIN.flatMap((c) => c.imposes));
  return LOGICAL_GATES.filter(
    (g) => !covered.has(g.id) && g.state !== 'vacuous',
  ).map((g) => ({
    id: g.id,
    name: g.name,
    tier: g.tier,
