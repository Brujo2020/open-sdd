...
export interface ModuleMapEntry {
  ...
  path: string;
  name: string;
  /** Toolchain that declared this module root (`node`, `maven`, `go`, ...). */
  ecosystem: ModuleEcosystem;
  /** Files/globs that live here, as observed. */
  owns: string[];
  /** Observed responsibilities, derived from evidence, not guessed. */
  responsibilities: string[];
  /** Modules this one imports (from the manifests/source), when observable. */
  dependsOn: string[];
  /** Existing test directories for this module. */
  testDirs: string[];
}

export interface CodePlacementAnswer {
  kind: 'new-module' | 'existing-module' | 'undecidable';
  target?: string;
  evidence: string[];
  rationale: string;
  /** Existing symbols a new symbol would duplicate, from reuseFirst. */
  reuseCandidates: { symbol: string; file: string; line: number; similarity: number }[];
}
...
const readManifest = async (
  cwd: string,
  rel: string,
): Promise<{ pkg: Record<string, unknown> | null; unreadable: boolean }> => {
  if (!(await exists(path.join(cwd, rel)))) return { pkg: null, unreadable: false };
  try {
    const parsed = JSON.parse(await readFile(path.join(cwd, rel), 'utf8')) as unknown;
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return { pkg: null, unreadable: true };
    return { pkg: parsed as Record<string, unknown>, unreadable: false };
  } catch {
...
const manifestDeclaresWorkspaces = (pkg: Record<string, unknown> | null): boolean => {
  if (!pkg) return false;
...
const listModuleFiles = async (cwd: string, dirs: string[]): Promise<FileWalk> => {
  const files: string[] = [];
...
const isTypeOnlyFile = (content: string): boolean => {
...
const relativeTargets = (content: string, file: string): string[] => {
...
export const buildModuleMap = async (
  cwd: string,
): Promise<{ modules: ModuleMapEntry[]; complete: boolean; detail: string }> => {
  ...
  const moduleInfo = new Map<string, { ecosystem: ModuleEcosystem; manifest?: string }>();
  const declaredByPath = new Map<string, ModuleEcosystem[]>();
  for (const declared of project.declaredModules ?? []) {
    const ecosystems = declaredByPath.get(declared.path) ?? [];
    if (!ecosystems.includes(declared.ecosystem)) ecosystems.push(declared.ecosystem);
  ...
  for (const modulePath of modulePaths) {
    if (!moduleInfo.has(modulePath)) moduleInfo.set(modulePath, { ecosystem: 'unknown' });
  }
  for (const [modulePath, ecosystems] of declaredByPath) {
    if (ecosystems.length <= 1) continue;
    notes.push(
      `${modulePath} está declarado por más de un ecosistema (${ecosystems
  ...
  const ownerOf = (target: string): string => {
    const normalized = posix(target);
    for (const candidate of ordered) {
      if (candidate === '.' || normalized === candidate || normalized.startsWith(`${candidate}/`)) return candidate;
    }
    return '.';
  };

  const sourceByModule = new Map<string, string[]>();
  const testByModule = new Map<string, string[]>();
  for (const modulePath of modulePaths) {
    sourceByModule.set(modulePath, []);
    testByModule.set(modulePath, []);
  }
  for (const dir of project.sourceDirs) sourceByModule.get(ownerOf(dir))!.push(posix(dir));
  for (const dir of project.testDirs) testByModule.get(ownerOf(dir))!.push(posix(dir));
  ...
  const manifestRel = (modulePath: string): string =>
    moduleInfo.get(modulePath)?.manifest ??
    (modulePath === '.' ? 'package.json' : `${modulePath}/package.json`);

  const manifests = new Map<string, Record<string, unknown> | null>();
  const manifestExists = new Map<string, boolean>();
  ...
  for (const modulePath of modulePaths) {
    ...
    if (path.posix.basename(rel) === 'package.json') {
      ...
      if (unreadable) {
        complete = false;
        notes.push(
          `No se pudo leer ${rel}: las dependencias declaradas de ese módulo no se inspeccionaron y su dependsOn puede estar incompleto.`,
        );
      }
      if (pkg) readableManifests += 1;
  ...
  const nameOf = (modulePath: string): string => {
    const pkg = manifests.get(modulePath);
    const declared = pkg?.name;
    if (typeof declared === 'string' && declared) return declared;
    if (modulePath === '.') return project.name;
  ...
  for (const modulePath of modulePaths) {
    const pkg = manifests.get(modulePath);
    const declared = pkg?.name;
    if (typeof declared === 'string' && declared) nameToPath.set(declared, modulePath);
    else if (modulePath !== '.') nameToPath.set(path.posix.basename(modulePath), modulePath);
  }

  const filesByModule = new Map<string, FileWalk>();
  let declaredEdges = 0;
  let relativeEdges = 0;
  ...
  for (const modulePath of modulePaths) {
    ...
    const info = moduleInfo.get(modulePath) ?? { ecosystem: 'unknown' as ModuleEcosystem };
    ...
    if (info.ecosystem !== 'node') {
      ...
      responsibilities.push(
        `módulo ${ECOSYSTEM_LABEL[info.ecosystem]}${info.manifest ? ` declarado por ${info.manifest}` : ''}`,
      );
    }
    if (manifestDeclaresWorkspaces(pkg)) {
      ...
      responsibilities.push(
        `raíz del repositorio: agrupa ${workspaceRoots.length} módulo(s) observado(s) (${ECOSYSTEM_LABEL[info.ecosystem]})`,
      );
    }
    ...
    if (binDeclared) {
      ...
      if (cliFile) responsibilities.push(`punto de entrada CLI observado: ${cliFile}`);
    }
    ...
    if (sourceWalk.files.length > 0) {
      const contents = await Promise.all(
        sourceWalk.files.map((file) => readFile(path.join(cwd, file), 'utf8').catch(() => null)),
      );
      const readable = contents.filter((content): content is string => content !== null);
      if (readable.length === sourceWalk.files.length && readable.every((content) => isTypeOnlyFile(content))) {
        responsibilities.push(
          `solo declara tipos: ${sourceWalk.files.length} fichero(s) sin implementación en tiempo de ejecución`,
        );
      }
    }
    if (testWalk.files.length > 0 && testWalk.files.length >= sourceWalk.files.length) {
      responsibilities.push(
        `los tests dominan el módulo: ${testWalk.files.length} fichero(s) de test frente a ${sourceWalk.files.length} de código`,
      );
    }
  ...
  if (readableManifests === 0 && project.sourceDirs.length === 0 && project.testDirs.length === 0) {
    complete = false;
    notes.push(
      'No se observó ningún manifiesto legible ni ningún directorio de código o de tests: el mapa está vacío porque no había nada que inspeccionar, no porque no haya módulos.',
    );
  }
  ...
  if (project.unreadableToolchains.length > 0) {
    complete = false;
    notes.push(
      `Declaración de toolchain ilegible (${project.unreadableToolchains.join(', ')}): no se pudo leer qué módulos declara, así que el mapa puede estar incompleto y el repositorio NO se trata como un único módulo.`,
    );
  }

  const ecosystemCounts = new Map<ModuleEcosystem, number>();
  for (const modulePath of modulePaths) {
...
const moduleHaystack = (entry: ModuleMapEntry): Set<string> =>
  new Set(
    tokensOf([entry.name, entry.path, ...entry.responsibilities, ...entry.owns].join(' ')),
  );
...
export const answerCodePlacement = async (input: {
  ...
  const reuseCandidates = reuse.candidates.map((candidate: ReuseCandidate) => ({
  ...
  const ownerOfFile = (file: string): string | undefined =>
    ordered.find((entry) => entry.path === '.' || file === entry.path || file.startsWith(`${entry.path}/`))?.path;

  let match: PlacementMatch | null = null;
  ...
  const candidateOwners = new Map<string, ReuseCandidate[]>();
  ...
  if (match) {
    ...
    evidence.push(
      `señales compartidas con el módulo ${entry.path} (${entry.name}): ${match.hits.join(', ')}`,
    );
    evidence.push(
    ...
    for (const candidate of reuse.candidates.slice(0, 5)) {
      evidence.push(
        `símbolo existente ${candidate.symbol} en ${candidate.file}:${candidate.line} (similitud ${candidate.similarity})`,
      );
    }
    ...
    const rationale =
      `La descripción y las señales observadas apuntan a ${entry.path}: el código nuevo pertenece a un módulo ` +
  ...
  if (workspaceRoots.length > 0) {
    ...
    evidence.push(
      `el layout declara workspaces (${workspaceRoots.join(', ')}): el módulo es la unidad de cambio`,
    );
    ...
    if (reuseCandidates.length > 0) {
      evidence.push(
        `aun así hay ${reuseCandidates.length} símbolo(s) existente(s) parecido(s): revísalos antes de crear el módulo`,
      );
    }
    const rationale =
      `Nada observado coincide con la descripción, y el repositorio se organiza en workspaces, ` +
  ...
  evidence.push('ningún término de la descripción coincide con un módulo, responsabilidad o directorio observado');
  evidence.push(
    'el proyecto no declara workspaces ni un contenedor de módulos observable: no hay evidencia de que el módulo sea la unidad de cambio',
  );
  if (reuseCandidates.length === 0 && reuse.complete) {
    evidence.push('la búsqueda de reutilización se ejecutó y no encontró ningún símbolo parecido');
  }
  const rationale =
    'Sin coincidencia con lo observado y sin evidencia de un layout multi-módulo, no se puede responder dónde va ' +
...
export const writeCodeIntelligence = async (input: {
  ...
  modules?: ModuleMapEntry[];
  write?: boolean;
}): Promise<{ path: string; content: string; written: boolean; complete: boolean; detail: string }> => {
  ...
  const scripts: { scope: string; name: string; command: string }[] = [];
  const collectScripts = async (rel: string, scope: string): Promise<void> => {
    const { pkg } = await readManifest(cwd, rel);
    if (!pkg || !isRecord(pkg.scripts)) return;
    for (const [name, command] of Object.entries(pkg.scripts)) {
      if (typeof command === 'string') scripts.push({ scope, name, command });
    }
  };
  ...
  if (observedStack) stackLines.push(`- lenguaje: ${project.language}`);
  if (project.frameworks.length > 0) stackLines.push(`- frameworks: ${project.frameworks.join(', ')}`);
  if (project.packageManager) stackLines.push(`- gestor de paquetes: ${project.packageManager}`);
  if (project.buildTool) stackLines.push(`- build: ${project.buildTool}`);
  if (project.testFramework) stackLines.push(`- tests: ${project.testFramework}`);
  if (stackLines.length === 0) stackLines.push('- no se observó ningún indicador de stack (sin manifiestos ni config)');
  ...
  if (!observedStack) notInspected.push('El stack no se pudo determinar: no se observaron manifiestos ni configuración de lenguaje.');
  if (scripts.length === 0) notInspected.push('No se observó ningún script en los manifiestos: no hay comandos que reportar.');
  if (suppliedModules) notInspected.push('El mapa de módulos lo aportó quien llamó; no se volvió a verificar contra el código.');
  if (!built.complete) notInspected.push(`El mapa de módulos está incompleto: ${built.detail}`);
  if (!constitutionExists) {
    notInspected.push(
      `No existe ${constitutionRel}: todavía no hay autoridad constitucional que citar. Genera con \`open-sdd brownfield constitution <root> --write\`.`,
    );
  }
  ...
  lines.push('');
  lines.push(
    'Documento para agentes: resume lo que el reconocimiento observó en este repositorio para que el ' +
  ...
  for (const module of modules) {
    ...
    lines.push(
      `- responsabilidades: ${
        module.responsibilities.length > 0 ? module.responsibilities.join(' · ') : 'ninguna derivable de la evidencia'
      }`,
    );
  ...
  if (project.testDirs.length > 0) {
    for (const dir of project.testDirs) lines.push(`- \`${dir}\``);
  } else {
  ...
  lines.push('');
  if (scripts.length > 0) {
    for (const script of scripts) lines.push(`- **${script.scope}** \`${script.name}\`: \`${script.command}\``);
  } else {
  ...
  if (constitutionExists) {
    lines.push(
      `La constitución es la autoridad citable: \`${constitutionRel}\`. Este documento no la parafrasea; ` +
        'léela antes de proponer un cambio y cita sus principios (`C-…`) en los veredictos.',
    );
  } else {
    lines.push(
      `Todavía no existe \`${constitutionRel}\`. Genérala con ` +
  ...
  lines.push('');
  lines.push(
    'Antes de crear un símbolo nuevo, busca uno existente: `open-sdd brownfield reuse <feature> --symbols A,B`. ' +
  ...
  lines.push('');
  for (const item of notInspected) lines.push(`- ${item}`);
  lines.push('');
  ...
  if (write) {
    try {
      ...
      problems.push(`no se pudo escribir ${relPath}: ${(error as Error).message}`);
    }
  }
...
export const planBootstrap = async (input: { cwd: string; focus?: string }): Promise<BootstrapPlan> => {
  ...
  const artifacts: BootstrapPlan['artifacts'] = [];
  artifacts.push({
