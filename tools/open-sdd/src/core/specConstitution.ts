...
import type { Constitution, ConstitutionPrinciple } from './constitution.js';
import { principlesInForce } from './constitution.js';
import type { DeltaEntry, DeltaSpec } from './deltaSpec.js';
import { parseTasksMarkdown } from './specManager.js';

export interface ConstitutionalAlignmentFinding {
  severity: 'error' | 'warning' | 'info';
  /** Principle id when the finding cites one. */
  principleId?: string;
  /** Requirement id / task id / delta id when the finding is about one. */
  artifactId?: string;
  /** Stable machine code. */
  code: string;
  /** Spanish, actionable. */
...
export interface SpecAlignment {
  feature: string;
  /** Principles this spec declares as applicable. */
  declared: string[];
  /** Principles in force that the spec never mentions. */
  undeclared: string[];
  findings: ConstitutionalAlignmentFinding[];
  /** 0..1 — how much of the spec's declared authority resolves to a principle in force. */
  alignment: number;
  detail: string;
}
...
const isDeclarableId = (token: string): boolean =>
  ID_SHAPE.test(token) && !NON_PRINCIPLE_TOKENS.has(token) && !REFERENCE_PREFIX.test(token);

const escapeRegExp = (value: string): string => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

/** Identificadores en mayúsculas de un fragmento, ya filtrados los que no son citables. */
const declarationTokens = (text: string): string[] => {
  const out: string[] = [];
  for (const match of text.matchAll(ID_TOKEN_RE)) {
    if (isDeclarableId(match[0])) out.push(match[0]);
  }
  return out;
};

/** `_Constitution: A, B_` — declaración explícita, admite ids que no existen (para poder citarlos). */
const markerDeclarations = (text: string): string[] => {
  const out: string[] = [];
  const re = new RegExp(`${escapeRegExp(SPEC_PRINCIPLES_MARKER)}\\s*([^_\\n]+)`, 'gi');
  for (const match of text.matchAll(re)) {
    for (const token of declarationTokens(match[1])) out.push(token);
  }
  return out;
};
 ...
 */
const sectionDeclarations = (text: string): string[] => {
  const out: string[] = [];
  let inSection = false;
  for (const line of text.split('\n')) {
    ...
    for (const token of declarationTokens(line)) out.push(token);
  }
  return out;
};
 ...
 */
export const declaredPrinciples = (texts: string[], known: string[]): string[] => {
  ...
  for (const text of texts) {
    if (!text) continue;
    for (const id of markerDeclarations(text)) add(id);
    for (const id of sectionDeclarations(text)) add(id);
    if (inline) for (const match of text.matchAll(inline)) add(match[1]);
  }
  return found;
};

// ---------------------------------------------------------------------------------------------
// Vocabulario de tecnologías: solo nombres inequívocos
//
// Una lista corta y deliberadamente conservadora. Un nombre ambiguo («go», «fetch», «spring»,
// «request», «less») se deja FUERA: produciría acusaciones falsas de violación de stack, y una
// detección que no puede decidir debe callarse. Cada tecnología ocupa un HUECO (test, lenguaje,
// gestor de paquetes…): «una tecnología distinta en el mismo hueco» es lo que hace decidible una
// contradicción, porque «usa jest donde la constitución fija vitest» es un hecho, no una opinión.
// ---------------------------------------------------------------------------------------------
...
const TECH_SLOT_OF: Record<string, TechSlot> = {
  typescript: 'lenguaje',
  javascript: 'lenguaje',
  python: 'lenguaje',
  ruby: 'lenguaje',
...
const techsIn = (text: string): Set<string> => {
...
const slotsOf = (techs: Iterable<string>): Map<TechSlot, Set<string>> => {
  const out = new Map<TechSlot, Set<string>>();
  for (const tech of techs) {
    const slot = TECH_SLOT_OF[tech];
    if (!slot) continue;
    const bucket = out.get(slot) ?? new Set<string>();
    bucket.add(tech);
    out.set(slot, bucket);
  }
  return out;
...
const conflictingTech = (textTechs: Set<string>, locked: Set<string>): string | undefined => {
  ...
  for (const [slot, techs] of textSlots) {
    ...
    if (!lockedInSlot) continue;
    for (const tech of techs) {
      if (!lockedInSlot.has(tech)) return tech;
    }
  }
  return undefined;
};
...
const lockedTechInSlot = (locked: Set<string>, slot: TechSlot | undefined): string | undefined =>
  [...locked].find((tech) => TECH_SLOT_OF[tech] === slot);

/** Verbos que expresan intención de sustituir una tecnología fijada como hecho. */
const REPLACEMENT_INTENT_RE =
  /\b(migrar|migraci[óo]n|migra|migrate|migrating|reescribir|reescritura|reescribe|reemplazar|reemplazo|reemplaza|sustituir|sustituci[óo]n|sustituye|actualizar|actualizaci[óo]n|actualiza|rewrite|replace|upgrade)\b/i;

/** Un manifiesto de dependencias: tocar esto es declarar una dependencia nueva o cambiarla. */
const DEP_MANIFEST_RE =
  /(?:^|\/)(package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|requirements[.-]?[\w]*\.txt|pyproject\.toml|pipfile|poetry\.lock|go\.mod|go\.sum|cargo\.toml|cargo\.lock|pom\.xml|build\.gradle(?:\.kts)?|gemfile|gemfile\.lock|composer\.json|packages\.config\.json|paket\.dependencies)$/i;

const principleText = (principle: ConstitutionPrinciple): string =>
  [principle.id, principle.title, principle.restriction, principle.pattern, principle.justification]
    .filter(Boolean)
    .join(' ');

/** Lo que un principio FIJA: su restricción, su patrón de cumplimiento y la evidencia observada. */
const lockedText = (principle: ConstitutionPrinciple): string =>
  [principle.restriction, principle.pattern, ...(principle.evidence ?? [])].filter(Boolean).join(' ');
...
// ---------------------------------------------------------------------------------------------
// Enunciados de la spec: qué texto se juzga y a qué artefacto pertenece
// ---------------------------------------------------------------------------------------------
 ...
 */
const requirementStatements = (markdown: string): Statement[] => {
  const out: Statement[] = [];
  ...
  for (const raw of markdown.split('\n')) {
    ...
    if (heading) {
      ...
      if (heading[3]?.trim()) out.push({ artifactId: current, text: heading[3].trim() });
      continue;
    }
    if (/^#{1,6}\s/.test(raw)) {
      current = undefined;
...
const deltaText = (delta?: DeltaSpec): string | undefined => {
  ...
  for (const entry of delta.entries) {
    ...
    if (entry.previous) parts.push(entry.previous);
    if (entry.rationale) parts.push(entry.rationale);
  }
  return parts.join('\n');
};

const entryText = (entry: DeltaEntry): string =>
  [entry.id, entry.title, entry.statement, ...entry.targets].filter(Boolean).join(' ');

/** Fronteras declaradas por las tareas (`_Boundary:_`), con la semántica de `getSpecStatus`. */
const boundariesFromTasks = (tasks?: string): string[] => {
  if (!tasks) return [];
  const set = new Set<string>();
  for (const task of parseTasksMarkdown(tasks)) {
    for (const boundary of task.boundary ?? []) set.add(boundary);
  }
  return [...set];
};

const normalizePath = (value: string): string => value.trim().replace(/\\/g, '/').replace(/^\.\//, '').replace(/\/+$/, '');

/** Un `.` declarado cubre TODO (semántica de `getSpecStatus`); el resto es prefijo o ruta exacta. */
const coveredByBoundary = (file: string, boundaries: string[]): boolean => {
  const target = normalizePath(file);
  return boundaries.some((raw) => {
    const boundary = normalizePath(raw).replace(/\/\*\*$/, '');
    if (!boundary) return false;
    if (boundary === '.' || boundary === '*' || boundary === '**') return true;
    if (boundary.endsWith('*')) return target.startsWith(boundary.slice(0, -1));
...
const SEVERITY_RANK: Record<ConstitutionalAlignmentFinding['severity'], number> = {
  error: 0,
  warning: 1,
  info: 2,
};
...
export const alignSpecWithConstitution = (input: {
  ...
  tasks?: string;
  delta?: DeltaSpec;
  /** Source/boundary paths the change touches, when known, for the boundary principle. */
  changedFiles?: string[];
  ...
  const findings: ConstitutionalAlignmentFinding[] = [];
  ...
  const texts = [input.requirements, input.plan, input.tasks, deltaText(input.delta)].filter(
  ...
  const report = (finding: ConstitutionalAlignmentFinding): void => {
  ...
  if (delta) {
    for (const entry of delta.entries) {
      ...
      const introducesDependency =
        entry.kind === 'ADDED' || entry.targets.some((target) => DEP_MANIFEST_RE.test(normalizePath(target)));
      if (!introducesDependency) continue;
      for (const lock of locking) {
        const conflict = conflictingTech(textTechs, lock.techs);
        if (!conflict) continue;
        if ([...lock.techs].some((tech) => textTechs.has(tech))) continue;
  ...
  if (boundaryPrinciples.length === 0) {
    ...
    gaps.push(
      'fronteras no evaluadas: la spec no declara ninguna frontera (_Boundary:_), así que no hay contra qué contrastar los ficheros cambiados',
    );
  } else {
    for (const file of changedFiles) {
      if (coveredByBoundary(file, boundaries)) continue;
  ...
  if (!delta) {
    ...
    const structural = delta.entries.filter(
      (entry) => entry.kind === 'MODIFIED' || entry.kind === 'REMOVED' || entry.kind === 'RENAMED',
    );
    ...
    const removalsWithoutContracts = delta.entries.filter(
      (entry) => entry.kind === 'REMOVED' && (entry.contracts ?? []).length === 0,
    );
  ...
  if (locking.length === 0) {
    outOfScope.push(
      'contradicción de stack no evaluada: ningún principio en vigor nombra una tecnología reconocible, así que no hay nada que contrastar (no es un aprobado)',
    );
  }
  ...
  const undeclaredText =
    undeclared.length > 0 ? `; en vigor y sin mencionar: ${undeclared.join(', ')}` : '; ningún principio en vigor queda sin mencionar';
  const gapsText = gaps.length > 0 ? ` No evaluado: ${gaps.join('; ')}.` : '';
  const scopeText =
    outOfScope.length > 0 ? ` Fuera del alcance del pivote (no es un aprobado, es un no aplica): ${outOfScope.join('; ')}.` : '';
...
export const ADHESION_PENALTIES = { error: 8, warning: 3, info: 1 } as const;
...
export const adhesionHistoryPath = (sddDir: string = '.sdd'): string =>
  path.join(sddDir, 'state', ADHESION_HISTORY_FILE);

/** Las entradas que produjeron el número, para que un lector pueda recalcularlo. */
export interface AdhesionInputs {
  /** Principios que la spec declara. */
  declared: number;
  /** Declarados que resuelven a un principio EN VIGOR. */
  resolved: number;
  /** Principios en vigor que la spec no menciona. */
  undeclared: number;
  errors: number;
  warnings: number;
  infos: number;
  /** 0..1 — tasa de resolución de la autoridad declarada (el `alignment` del pivote). */
  alignment: number;
  /** Puntos restados por los hallazgos. */
  penalty: number;
  /** false cuando la spec no declara ningún principio: ahí 0 NO es un aprobado. */
  declaredAny: boolean;
}

export interface AdhesionScore {
  /** Entero 0..100. */
  score: number;
  inputs: AdhesionInputs;
  /** Una línea en español con el número y sus entradas. */
  detail: string;
}

/** Una medición guardada. `version` es la release/etiqueta opcional que el llamante conozca. */
export interface AdhesionHistoryEntry {
  /** ISO-8601. */
  date: string;
  /** 0..100. */
  score: number;
...
export interface AdhesionHistoryRead {
  /** Ruta del fichero consultado (relativa al repositorio). */
  file: string;
  entries: AdhesionHistoryEntry[];
  /** Presente cuando la historia no se pudo leer/parsear o tenía entradas inválidas. */
  warning?: string;
  /** true cuando la serie se considera nueva por culpa de una historia ilegible. */
  seriesRestarted: boolean;
}

export type AdhesionTrendStatus = 'first-run' | 'up' | 'down' | 'flat';

export interface AdhesionTrend {
  status: AdhesionTrendStatus;
  current: number;
  previous: number | null;
  /** current - previous; 0 en la primera medición. */
  delta: number;
  feature: string;
  /** Fecha de la medición contra la que se compara, si existe. */
  comparedTo: string | null;
  /** Cuántas mediciones previas de la MISMA feature hay. */
  entries: number;
  /** «subió 12 puntos…» / «bajó…» / «primera medición…». */
  detail: string;
}
 ...
 */
export const adhesionScore = (alignment: SpecAlignment): AdhesionScore => {
...
export const adhesionHistoryEntry = (
  score: AdhesionScore,
  options: { feature: string; date?: string; version?: string },
...
const isHistoryEntry = (value: unknown): value is AdhesionHistoryEntry => {
  const entry = value as Partial<AdhesionHistoryEntry> | null;
  return (
    typeof entry === 'object' &&
    entry !== null &&
 ...
 */
export const readAdhesionHistory = async (
  root: string,
  sddDir: string = '.sdd',
): Promise<AdhesionHistoryRead> => {
  ...
  try {
    ...
    if (code === 'ENOENT' || code === 'ENOTDIR') return { file: rel, entries: [], seriesRestarted: false };
    return {
      file: rel,
      entries: [],
      warning: `la historia de adhesión existe pero no se pudo leer (${(error as Error).message}): se empieza una serie nueva`,
  ...
  try {
    ...
    return {
      file: rel,
      entries: [],
      warning: `${rel} no es JSON válido (${(error as Error).message}): se empieza una serie nueva`,
      seriesRestarted: true,
    };
  }
...
const chronological = (entries: AdhesionHistoryEntry[]): AdhesionHistoryEntry[] =>
 ...
 */
export const adhesionTrend = (
  current: number,
  history: AdhesionHistoryEntry[],
  options: { feature?: string } = {},
): AdhesionTrend => {
  const feature = options.feature ?? '';
  const series = chronological(
    options.feature ? history.filter((entry) => entry.feature === options.feature) : history,
  );
  ...
  const release =
    previousEntry.version && previousEntry.version.length > 0 ? ` de la release "${previousEntry.version}"` : '';
  const status: AdhesionTrendStatus = delta > 0 ? 'up' : delta < 0 ? 'down' : 'flat';
  const movement =
    status === 'up'
      ? `subió ${delta} punto(s)`
      : status === 'down'
...
export interface AdhesionHistoryWrite {
  file: string;
  entries: number;
  warning?: string;
  seriesRestarted: boolean;
 ...
 */
export const recordAdhesionHistory = async (
  root: string,
  entry: AdhesionHistoryEntry,
  sddDir: string = '.sdd',
): Promise<AdhesionHistoryWrite> => {
  const rel = adhesionHistoryPath(sddDir);
  const abs = path.join(root, rel);
  ...
  try {
    ...
    const writeWarning = `no se pudo escribir la historia de adhesión (${rel}: ${(error as Error).message}): la puntuación es válida pero la tendencia no se guardó`;
