...
export const BACKUP_EXCLUDED_DIRS: readonly string[] = ['node_modules', 'dist', '.git'];

/** Nombres con forma de credencial: se omiten y se nombran, nunca se copian. */
export const BACKUP_CREDENTIAL_PATTERNS: readonly RegExp[] = [
  /^\.npmrc$/,
  /^\.env(\..+)?$/,
  /^\.netrc$/,
  /^\.git-credentials$/,
...
export interface BackupFileEntry {
  /** Ruta POSIX relativa a la raíz respaldada (p. ej. `state/progress.json`). */
  path: string;
  bytes: number;
  sha256: string;
}
...
export interface BackupManifest {
  ...
  tool: string;
  version: string;
  /** Raíz respaldada, relativa al proyecto (p. ej. `.sdd`). Es el destino de una restauración. */
  root: string;
  files: BackupFileEntry[];
  counts: { files: number; bytes: number };
  git: { branch: string | null; commit: string | null; dirty: boolean };
  /** Resumen humano; aquí se NOMBRAN las omisiones (credenciales, enlaces, directorios excluidos). */
  detail: string;
}
...
export interface VerifyBackupResult {
  ok: boolean;
  mismatches: { path: string; expected: string; actual: string | null }[];
  missing: string[];
...
const backupTimestamp = (date: Date): string => date.toISOString().replace(/[:.]/g, '-');
...
const isCredential = (name: string): boolean =>
  BACKUP_CREDENTIAL_PATTERNS.some((pattern) => pattern.test(name));

interface Walked {
  files: { path: string; abs: string; content: Buffer; bytes: number; sha256: string }[];
  credentials: string[];
  symlinks: string[];
  excludedDirs: string[];
  unreadable: string[];
...
const walkSource = async (
  baseAbs: string,
  opts: { outAbs: string; backupsAbs: string },
): Promise<Walked> => {
  const acc: Walked = { files: [], credentials: [], symlinks: [], excludedDirs: [], unreadable: [] };
  ...
  const visit = async (rel: string): Promise<void> => {
    ...
    try {
      ...
      acc.unreadable.push(`${rel || '.'} (${(err as Error).message})`);
      return;
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (isInside(childAbs, opts.backupsAbs) || isInside(childAbs, opts.outAbs)) continue;
        await visit(childRel);
        continue;
      }
      if (!entry.isFile()) continue;
      if (isInside(childAbs, opts.outAbs)) continue;
      ...
      try {
        ...
        acc.unreadable.push(`${childRel} (${(err as Error).message})`);
      }
    }
  };
...
const gitInfo = (cwd: string): BackupManifest['git'] => {
  if (!isGitRepo(cwd)) return { branch: null, commit: null, dirty: false };
...
const describeSkips = (walked: Walked): string => {
  const parts: string[] = [];
...
const emptyManifest = (root: string, createdAt: string, detail: string): BackupManifest => ({
...
export const createBackup = async (input: {
  ...
  const manifest: BackupManifest = {
    ...
    git: gitInfo(cwd),
    detail:
      `Respaldo de ${root}/ con ${walked.files.length} fichero(s) y ${totalBytes} byte(s).` +
  ...
  if (exists) await rm(outAbs, { recursive: true, force: true });
  ...
  try {
    ...
    return {
      ...
      manifest,
      written: false,
      detail: `El respaldo no se pudo escribir en ${outAbs} (${(err as Error).message}): no hay respaldo que verificar.`,
    };
  }
...
const readManifest = async (archiveAbs: string): Promise<ManifestRead> => {
  const manifestPath = path.join(archiveAbs, BACKUP_MANIFEST_FILE);
  let raw: string;
  try {
    raw = await readFile(manifestPath, 'utf8');
  } catch (err) {
    return { manifest: null, detail: `El respaldo ${archiveAbs} no tiene ${BACKUP_MANIFEST_FILE} legible (${(err as Error).message}): sin manifiesto no hay nada que comprobar.` };
  }
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (err) {
    return { manifest: null, detail: `El manifiesto de ${archiveAbs} no es JSON válido (${(err as Error).message}).` };
  }
  const candidate = parsed as Partial<BackupManifest> | null;
  if (
    !candidate ||
  ...
  for (const entry of candidate.files as unknown[]) {
    const file = entry as Partial<BackupFileEntry> | null;
...
const listRelativeFiles = async (baseAbs: string): Promise<string[]> => {
  const found: string[] = [];
  const visit = async (rel: string): Promise<void> => {
    const dirAbs = rel.length === 0 ? baseAbs : path.join(baseAbs, ...rel.split('/'));
    let entries;
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile()) found.push(childRel);
    }
  };
  await visit('');
  return found.sort();
...
export const verifyBackup = async (input: {
  ...
  const mismatches: VerifyBackupResult['mismatches'] = [];
  ...
  if (mismatches.length > 0) problems.push(`${mismatches.length} fichero(s) con hash que NO coincide con el manifiesto`);
  if (missing.length > 0) problems.push(`${missing.length} fichero(s) declarados y ausentes`);
  if (!countsMatch) problems.push('los recuentos del manifiesto no cuadran con su propia lista de ficheros');
  if (extras.length > 0) problems.push(`${extras.length} fichero(s) en la carga no declarados en el manifiesto: ${extras.slice(0, 5).join(', ')}${extras.length > 5 ? '…' : ''}`);
...
const matchesOnly = (rel: string, only: readonly string[] | undefined): boolean => {
...
export const restoreBackup = async (input: {
  ...
  const read = await readManifest(archiveAbs);
  if (!read.manifest) return { restored: [], skipped: [], detail: read.detail };
  ...
  for (const file of manifest.files) {
    if (!matchesOnly(file.path, input.only)) continue;
    ...
    try {
      ...
      skipped.push({ path: file.path, reason: `no se pudo escribir (${(err as Error).message})` });
    }
  }

  const verb = input.write ? 'Restaurados' : 'Se restaurarían';
  const detail =
    `${verb} ${restored.length} fichero(s) de ${manifest.counts.files} en ${destRoot}; ${skipped.length} omitido(s)` +
