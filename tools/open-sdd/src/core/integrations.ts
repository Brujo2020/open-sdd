...
const zedSnippet = (cliPath: string): string =>
  jsonDoc({ context_servers: { [MCP_SERVER_NAME]: { command: { path: 'node', args: [cliPath, 'mcp'] } } } });
...
const UNVERIFIED =
  'NO VERIFICADA en esta sesión: la forma y/o la ruta se recuerdan pero no se han confirmado contra la documentación del anfitrión. `open-sdd integrate --write` NO escribe configuraciones no verificadas: imprime el snippet para que lo pegues y lo compruebes.';
