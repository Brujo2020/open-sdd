...
export type ImpositionLevel = 'MUST' | 'SHOULD' | 'MAY';

/** Where a principle comes from: observed in the code, or authored as a requirement. */
export type ConstitutionProvenance = 'descriptive' | 'normative';
...
export interface ConstitutionPrinciple {
  /** Stable identifier cited by blocking verdicts (invariant I1). */
  id: string;
  title: string;
  /** Field 2 of the anatomy: the threat, ADR or control this principle answers to. */
  threatReference?: string;
  /**
   * Field 2 of the anatomy when the threat is a CWE: canonical `CWE-<n>` (e.g. `CWE-89`). Kept
   * separate from `threatReference`, which carries non-CWE references (OWASP/ATLAS, ADR ids).
   */
  cweReference?: string;
  level: ImpositionLevel;
  ...
  /**
   * Proof that the code already obeys a descriptive principle (path, symbol or command). Required
   * for `descriptive` principles: without it the claim is aspirational and belongs in an amendment.
   */
  evidence?: string[];
...
export interface Constitution {
  ...
  provenance: ConstitutionProvenance;
  generatedAt?: string;
  /** Facts the constitution declares immutable, e.g. the stack in force today. */
...
export interface ConstitutionIssue {
  ...
  message: string;
  /**
   * Machine-readable code (`MUST-THREAT`, `CWE-FORMAT`, `INJ-OVERRIDE`, …). Optional so existing
   * consumers that only read severity/id/message keep working unchanged.
   */
  code?: string;
}
 ...
 */
export const normalizeCweReference = (value: string): string | undefined => {
...
const LEVELS = ['MUST', 'SHOULD', 'MAY'] as const;
...
export const isPositionalIdentifier = (id: string): boolean => {
  const match = id.trim().toUpperCase().match(/^([A-Z]+)-?(\d+)$/);
  return match ? POSITIONAL_ID_WORDS.has(match[1]) : false;
};

/**
 * Indirect-prompt-injection patterns a constitution must not contain (CSDD §6.1, lesson 4:
 * "Constitutional Documents Must Resist Adversarial Manipulation").
 *
 * Because the document is read as instructions, a principle that can be re-interpreted through
 * injected context is a vulnerability rather than a rule. Two families are dangerous: an exception
 * clause a user or an input controls (`unless the user says…`, `if the user asks…`) and
 * instruction-override phrasing (`ignore previous…`, `override`, `bypass`, `disregard the above`).
 * One exported list keeps every pattern reviewable in a single place, each with its justification.
 */
export interface InjectionPattern {
  /** Stable code reported on the issue. */
  code: string;
  /** Matched case-insensitively against the principle's prose. */
  pattern: RegExp;
  /** Why this phrasing is dangerous in a document a model reads as policy. */
  justification: string;
}
...
export const INJECTION_PATTERNS: readonly InjectionPattern[] = [
  {
    ...
    justification:
      'Anular instrucciones anteriores es la forma canónica de inyección indirecta: convierte el documento en una instrucción sustituible.',
  },
  {
    code: 'INJ-DISREGARD',
    pattern: /\bdisregard\s+(?:all\s+)?(?:the\s+)?(?:above|previous|prior|preceding|foregoing|instructions?|rules?)\b/i,
    justification:
      'Descartar lo ya escrito permite que un contexto inyectado sustituya la política sin tocar el texto.',
  },
  {
    code: 'INJ-UNLESS-USER',
    pattern: /\bunless\s+(?:the\s+)?(?:user|users|caller|client|requester|input|request)s?\b/i,
    justification:
      'Una excepción condicionada al usuario o a la entrada delega la política en quien ataca: el control pasa a ser opcional.',
  },
  {
    code: 'INJ-EXCEPT-INPUT',
    pattern: /\bexcept\s+(?:when\s+)?(?:the\s+)?(?:user|users|caller|client|requester|input|request|payload)s?\b/i,
    justification:
      'Igual que "unless": una excepción controlable desde fuera anula la restricción en el caso que importa.',
  },
  {
    code: 'INJ-IF-USER-ASKS',
    pattern:
      /\bif\s+(?:the\s+)?(?:user|users|caller|client|requester)s?\s+(?:asks?|says?|requests?|demands?|wants?|specifies?|insists?)/i,
    justification:
      'Convertir la restricción en una condición sobre la petición del usuario hace que el cumplimiento dependa del atacante.',
  },
  {
    ...
    justification:
      'Justificar la desviación por lo que pida el usuario es una cláusula de escape legible por el modelo.',
  },
  {
    code: 'INJ-OVERRIDE',
    pattern: /\boverrid(?:e|es|ing|den)\b/i,
    justification:
      'El lenguaje de anulación ("override") autoriza a saltarse la restricción y es el objetivo declarado de la inyección de prompt.',
  },
  {
    code: 'INJ-BYPASS',
    pattern: /\bbypass(?:es|ed|ing)?\b/i,
    justification:
      'Nombrar una vía de "bypass" en un principio convierte la prohibición en un procedimiento documentado.',
  },
  {
    code: 'INJ-HIDDEN-COMMENT',
    pattern: /<!--/,
    justification:
      'Un comentario HTML no se renderiza pero sí se lee: es el canal oculto clásico para instrucciones escondidas en un documento de política.',
  },
];

export interface InjectionFinding {
  code: string;
  /** Matched text with a little surrounding context, so the issue is actionable. */
  excerpt: string;
  justification: string;
}

/** Scan prose for the exported injection patterns. Returns one finding per matching pattern. */
export const detectInjection = (text: string): InjectionFinding[] => {
...
const rationaleNamesThreat = (justification: string): boolean => {
  const text = justification.trim();
  if (text.length < MIN_RATIONALE_CHARS) return false;
 ...
 */
export const validateConstitution = (constitution: Constitution): ConstitutionIssue[] => {
  ...
  if (constitution.provenance === 'normative' && constitution.establishedFacts.length > 0) {
    issues.push({
      ...
      message:
        'Una constitución normativa no declara hechos del código: los hechos establecidos son propios de la constitución descriptiva (brownfield).',
    });
  }
  ...
  for (const principle of constitution.principles) {
    if (!ID_PATTERN.test(principle.id)) {
      ...
      issues.push({
        ...
        id: principle.id,
        message:
          'El identificador es posicional y no nombra la preocupación que gobierna: se cita en veredictos bloqueantes y en la matriz de trazabilidad (usa p. ej. SEC-002 o C-API-COMPAT).',
      });
    }
    ...
    if (!(LEVELS as readonly string[]).includes(principle.level)) {
      issues.push({
        severity: 'error',
        code: 'LEVEL-INVALID',
        id: principle.id,
    ...
    if (
      ...
      issues.push({
        ...
        id: principle.id,
        message:
          'Una referencia CWE pertenece al campo cweReference; threatReference queda para referencias no-CWE (OWASP/ATLAS, ADR).',
      });
    }
    ...
    if (
      (principle.level === 'MUST' || principle.level === 'SHOULD') &&
    ...
    if (principle.provenance === 'descriptive' && (principle.evidence ?? []).length === 0) {
      issues.push({
        ...
        id: principle.id,
        message:
          'Un principio descriptivo sin evidencia es un deseo registrado como hecho. Aporta la prueba de que el código ya lo cumple o trámitalo como enmienda.',
      });
    }
    ...
    if (principle.provenance === 'normative' && principle.level === 'MUST' && !principle.amendment) {
      issues.push({
        ...
        id: principle.id,
        message:
          'Un MUST normativo exige enmienda: la autoridad se introduce por un proceso gobernado, no por afirmación.',
      });
    }
  ...
  if (constitution.principles.length >= 4 && !levels.has('SHOULD') && !levels.has('MAY')) {
    issues.push({
      ...
      id: constitution.project,
      message:
        'Todos los principios son MUST. Una constitución donde todo es MUST no impone nada: usa SHOULD/MAY donde la desviación sea defendible.',
    });
  }
...
// ---------------------------------------------------------------------------------------------
// Compliance Traceability Matrix (CSDD §3.3 / §4.2)
//
// "The matrix maps each principle to specific implementation artifacts" with four named purposes:
// audit support (each principle has an implementation), change-impact analysis (which code affects
// which principle), gap detection (unmapped principles) and regression prevention (targeted testing
// when those artifacts change). The evidence a principle already carries is the mapping source.
// ---------------------------------------------------------------------------------------------

export interface ComplianceArtifact {
  /** The evidence entry exactly as written (path, `file::symbol`, command or fact). */
  reference: string;
  /** Path part, when the reference is path-shaped. */
  file?: string;
  /** First line, when the reference carries `file:line` or `file:14-24`. */
  line?: number;
  /** True when the reference names an artifact the matrix can point at. */
  resolvable: boolean;
}
...
export interface ComplianceMatrixEntry {
  principleId: string;
  level: ImpositionLevel;
  /** Artifacts the principle maps to, resolved from the principle's evidence. */
  artifacts: ComplianceArtifact[];
  /** True when the principle maps to at least one resolvable artifact. */
  covered: boolean;
}

export interface ComplianceMatrix {
  entries: ComplianceMatrixEntry[];
  /** Principles with no resolvable artifact — the gap list. */
  gaps: string[];
  /** covered / total. */
  coverage: number;
  detail: string;
...
const PATH_EXTENSIONS =
  /\.(?:[cm]?[jt]sx?|py|rb|go|rs|java|kt|kts|swift|php|cs|c|h|cpp|hpp|cc|sql|sh|bash|zsh|tf|tfvars|md|json|ya?ml|toml|ini|cfg|xml|html|css|scss|vue|svelte|dart|ex|exs|erl|hs|scala|clj|proto|graphql|gql)$/i;
...
export const parseEvidenceArtifact = (reference: string, options: { cwd?: string } = {}): ComplianceArtifact => {
...
export const buildComplianceMatrix = (
  ...
  const entries: ComplianceMatrixEntry[] = constitution.principles.map((principle) => {
  ...
  const detail =
    `Matriz de trazabilidad: ${covered}/${entries.length} principios con artefacto resoluble ` +
 ...
 */
export const impactedPrinciples = (matrix: ComplianceMatrix, changedFiles: string[]): string[] => {
  const normalize = (value: string): string => value.trim().replace(/^\.\//, '').replace(/\\/g, '/');
  ...
  for (const entry of matrix.entries) {
    ...
    if (hit && !impacted.includes(entry.principleId)) impacted.push(entry.principleId);
  }
...
export const promoteAmendment = (
  ...
  return {
    ...
    principles: constitution.principles.map((p) =>
      p.amendment?.id === amendmentId ? { ...p, amendment: updated } : p,
    ),
  };
};
...
export const renderConstitution = (constitution: Constitution): string => {
  const lines: string[] = [];
  lines.push(`# Constitution — ${constitution.project}`);
  lines.push('');
  lines.push(`Provenance: ${constitution.provenance}`);
  if (constitution.generatedAt) lines.push(`Generated: ${constitution.generatedAt}`);
  lines.push('');

  if (constitution.establishedFacts.length > 0) {
    lines.push('## Established facts');
    lines.push('');
    for (const fact of constitution.establishedFacts) lines.push(`- ${fact}`);
    lines.push('');
  }
  ...
  for (const p of constitution.principles) {
    ...
    if (p.cweReference) lines.push(`- CWE: ${normalizeCweReference(p.cweReference)}`);
    if (p.threatReference) lines.push(`- Threat: ${p.threatReference}`);
  ...
  if (constitution.amendments.length > 0) {
    ...
    for (const a of constitution.amendments) {
      ...
      lines.push(
        `- ${a.id} — ${a.title} [${a.status}]${attributes.length > 0 ? ` · ${attributes.join(' · ')}` : ''}`,
      );
    }
    lines.push('');
  }
...
export const parseConstitution = (markdown: string): Constitution => {
  const lines = markdown.split('\n');
  const generatedAt = markdown.match(/^Generated:\s*(.+)$/m)?.[1]?.trim();
  const constitution: Constitution = {
    project: (markdown.match(/^# Constitution — (.+)$/m)?.[1] ?? 'unknown').trim(),
    provenance: (markdown.match(/^Provenance:\s*(descriptive|normative)$/m)?.[1] as
  ...
  let section: 'facts' | 'principles' | 'amendments' | 'none' = 'none';
  let current: ConstitutionPrinciple | null = null;

  const push = (): void => {
    if (current) constitution.principles.push(current);
    current = null;
  };
  ...
  for (const raw of lines) {
    ...
    if (section === 'principles') {
      ...
      if (field && current) {
        ...
        if (key === 'Level') current.level = value.trim() as ImpositionLevel;
        else if (key === 'CWE') {
          const cwe = normalizeCweReference(value);
          if (cwe) current.cweReference = cwe;
        } else if (key === 'Threat') current.threatReference = value.trim();
        else if (key === 'Restriction') current.restriction = value.trim();
        ...
        // round-trip of an in-force document byte-identical to what it was before drafts existed.
          ...
          if (parsed) {
            current.amendment = {
              ...
              title: '',
              proposedBy: '',
              status: parsed[2] as AmendmentRecord['status'],
            };
          }
        }
      }
  ...
  for (const principle of constitution.principles) {
    if (!principle.amendment) continue;
    const declared = constitution.amendments.find((a) => a.id === principle.amendment?.id);
    if (declared) principle.amendment = { ...declared, status: principle.amendment.status };
  }
