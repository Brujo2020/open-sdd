...
import { RIGOR_SETTINGS_FILE, loadRigorSettings, type SddRigorLevel } from './rigor.js';
...
export interface RepairRecord {
  /** Artifact this repair regenerates, relative to the project root (posix). */
  target: string;
  /** The requirement the repair serves, e.g. REQ-MAT-015. */
  requirement?: string;
  /** The evidence that supports/falsifies the repair (a test, a check, a command). */
  evidence?: string;
  /** The declared generator command. */
  generator?: string;
  /** sha256:... the artifact must match, or null while no hash has been pinned yet. */
  hash: string | null;
  updatedAt?: string;
}

export const renderRepairRecord = (record: RepairRecord): string => {
  const lines: string[] = [`### ${record.target}`, ''];
  if (record.requirement) lines.push(`- Requirement: ${record.requirement}`);
  if (record.evidence) lines.push(`- Evidence: ${record.evidence}`);
  if (record.generator) lines.push(`- Generator: ${record.generator}`);
...
export const parseRepairRecords = (markdown: string): RepairRecord[] => {
  ...
  for (let i = sectionStart + 1; i < lines.length; i += 1) {
    ...
    if (key === 'Requirement') current.requirement = text;
    else if (key === 'Evidence') current.evidence = text;
    else if (key === 'Generator') current.generator = text;
    else if (key === 'Hash') current.hash = text && text !== 'pending' ? text : null;
    else if (key === 'Updated') current.updatedAt = text;
  }
  flush();
  return records;
};
 ...
 */
export const upsertRepairRecord = (markdown: string, record: RepairRecord): string => {
  ...
  for (let i = sectionStart + 1; i < sectionEnd; i += 1) {
    ...
    if (heading[1].trim() === record.target.trim()) recordStart = i;
  }
  if (recordStart >= 0 && recordEnd < 0) {
    recordEnd = sectionEnd;
    while (recordEnd > recordStart + 1 && lines[recordEnd - 1].trim() === '') recordEnd -= 1;
  }
  if (recordStart >= 0) {
...
export interface RepairFromSpecInput {
  cwd: string;
  feature: string;
  /** Artifact to regenerate, relative to `cwd`. */
  target: string;
  /** Requirement the repair serves (recorded in the spec). */
  requirement?: string;
  /** Evidence the repair rests on (recorded in the spec). */
  evidence?: string;
  /** The caller-declared generator command. Without it the loop refuses honestly. */
  command?: string;
  /** Default false: plan only, nothing is written. */
  write?: boolean;
  sddDir?: string;
  /** Injectable for tests: sha256 of a file, or null when it does not exist / cannot be read. */
  hashFile?: (absolutePath: string) => Promise<string | null>;
  /** Injectable for tests: run the generator command. */
  runGenerator?: (command: string, cwd: string) => Promise<GeneratorRun>;
}

export type RepairStatus = 'refused' | 'planned' | 'in-sync' | 'drift' | 'regenerated';

export interface RepairReport {
  feature: string;
  /** Artifact the repair is about, posix, relative to cwd. */
  target: string;
  level: SddRigorLevel;
  status: RepairStatus;
  /** False when the loop could not run to reconciliation (bad level, no authority, no generator…). */
  operable: boolean;
  /** The spec actually received a `## Repairs` record. */
  recorded: boolean;
  /** The generator changed the artifact. Never true when no file changed. */
  regenerated: boolean;
  /** The artifact did NOT match the hash the spec recorded BEFORE this run. */
  drift: boolean;
  /** After this run the artifact hash equals the hash the spec records. */
  matches: boolean;
  /** What the spec recorded before this run, if anything. */
  recordedHash: string | null;
  beforeHash: string | null;
  afterHash: string | null;
  /** Spec file that carries the repair log, posix relative to cwd. */
  specFile: string;
  refusals: string[];
  detail: string;
}

const posixRelative = (cwd: string, absolute: string): string =>
  path.relative(cwd, absolute).split(path.sep).join('/');

const sha256 = (content: Buffer): string => `sha256:${createHash('sha256').update(content).digest('hex')}`;

const defaultHashFile = async (absolutePath: string): Promise<string | null> => {
  try {
 ...
 */
const defaultRunGenerator = (command: string, cwd: string): Promise<GeneratorRun> =>
  new Promise((resolve) => {
    let stdout = '';
    let stderr = '';
    let settled = false;
...
const readIfExists = async (file: string): Promise<string | null> => readFile(file, 'utf8').catch(() => null);
 ...
 */
export const repairFromSpec = async (input: RepairFromSpecInput): Promise<RepairReport> => {
  ...
  try {
    ...
    refusals.push(
      `No se pudo leer el nivel de rigor declarado (${(error as Error).message}): no se repara a ciegas.`,
    );
  }
  if (levelKnown && level !== 'spec-as-source') {
    refusals.push(
      `El nivel declarado es ${level} y la reparación desde la spec exige spec-as-source, donde la spec ES la fuente y la regeneración es el mecanismo de reparación. Declara "level": "spec-as-source" en ${RIGOR_SETTINGS_FILE} (con su rationale) o repara el código a mano.`,
    );
  }
  ...
  if (constitutionRaw === null) {
    refusals.push(
      `La constitución no está en vigor (no existe ${posixRelative(cwd, constitutionPath)}): sin autoridad citable la reparación no tiene regla que la gobierne, así que se rechaza en lugar de ejecutarse.`,
    );
  } else if (principlesInForce(parseConstitution(constitutionRaw)).length === 0) {
    refusals.push(
      'La constitución existe pero no tiene principios en vigor: no hay autoridad que la reparación pueda citar.',
    );
  }
  ...
  if (!specDirExists) {
    refusals.push(
      `No existe la spec de "${feature}" (${posixRelative(cwd, specDir)}): la reparación se registra primero EN la spec, así que sin spec no hay dónde registrarla.`,
    );
  }
  ...
  if (!generator) {
    refusals.push(
      `No hay generador declarado para ${target}: en spec-as-source la regeneración es el mecanismo de reparación, así que sin comando declarado la reparación se rechaza en vez de afirmarse. Declara el comando del generador (input.command / --command "<comando>") para que sea auditable.`,
    );
  }
  ...
  try {
    currentSpec = upsertRepairRecord(
      currentSpec,
      {
        target,
    ...
    refusals.push(`No se pudo registrar la reparación en ${specFileRel} (${(error as Error).message}).`);
    return {
      feature,
      target,
      level,
  ...
  if (run.code !== 0) {
    refusals.push(
      `El generador "${generator}" terminó con código ${run.code}: la regeneración no se completó${run.stderr.trim() ? ` (${run.stderr.trim().split('\n')[0]})` : ''}.`,
    );
  }
  ...
  if (regenerated && afterHash !== null && run.code === 0) {
    ...
    try {
      ...
      refusals.push(
        `El artefacto se regeneró pero no se pudo fijar su hash en ${specFileRel} (${(error as Error).message}): la spec queda sin la verdad reconciliada.`,
      );
    }
