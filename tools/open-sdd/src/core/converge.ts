...
import { principlesInForce, type Constitution } from './constitution.js';
import { checkConsistency, type ConsistencyFinding } from './consistency.js';
import { deltaSpecFileName, parseDeltaSpec, traceDelta, type DeltaSpec, type DeltaTask } from './deltaSpec.js';
...
export type GapType = 'missing' | 'partial' | 'contradicts' | 'unrequested' | 'unprotected' | 'unbound';

export interface ConvergenceFinding {
  id: string; // F1, F2…
  gapType: GapType;
  severity: 'high' | 'medium' | 'low';
  /** The requirement id, task id or principle id this is about. */
  source: string;
  /** MACHINE-COLLECTED evidence: file:line, a command's output, a measured ratio. Never prose from a model. */
  evidence: string[];
  /** One line, Spanish, saying what is left to do. */
...
export interface ConvergenceReport {
  ...
  /** The text appended, or exactly the input when nothing was needed. */
  appended: string | null;
  tasksBefore: number;
  tasksAfter: number;
  converged: boolean; // true = nothing to do, tasks.md untouched
  checked: string[];
  notChecked: string[];
  complete: boolean;
  detail: string;
  /**
   * Componentes del SDD score (`sddScore.ts`) que este informe espera mover, con el motivo medido.
   * El número compuesto no se afirma aquí: se dice qué parte debería moverse y por qué hueco.
   */
  scoreImpact: { component: string; direction: 'up'; reason: string }[];
}
...
export const GAP_TYPES: GapType[] = ['missing', 'partial', 'contradicts', 'unrequested', 'unprotected', 'unbound'];

/** La severidad por defecto de cada tipo. `high` es lo único que hace salir con 1. */
export const GAP_SEVERITY: Record<GapType, ConvergenceFinding['severity']> = {
  missing: 'high',
  partial: 'medium',
  contradicts: 'high',
  unrequested: 'low',
...
export const GAP_DETECTION: Record<GapType, { detection: string; evidence: string }> = {
  missing: {
    detection:
      'traceDelta no mapea ningún requisito/entrada de la delta a una tarea, o la `_Boundary:_` declarada por una tarea no existe en disco (stat).',
    evidence: 'requirements.md:<línea> o tasks.md:<línea> + el resultado de `stat` sobre la ruta declarada.',
  },
  partial: {
    detection:
      'El requisito tiene tareas trazadas y no todas están `[x]`; se mide la ratio completadas/total (incluye 0/N).',
    evidence: 'la ratio medida + tasks.md:<línea> de cada tarea pendiente.',
  },
  contradicts: {
    detection:
      'traceDelta reporta una tarea que cita un id inexistente, o alignSpecWithConstitution emite un error de pivote.',
    evidence: 'tasks.md:<línea> + el id citado; o `alignSpecWithConstitution:<code>` + el principio y el artefacto.',
  },
  unrequested: {
    detection:
      'Un fichero cambiado dentro de una frontera/objetivo declarados exporta un símbolo que ningún token de la spec nombra (extractExportNames).',
    evidence: 'file:línea del export + el recuento de tokens de la spec que lo nombran (0).',
  },
  unprotected: {
    detection:
      'Un contrato declarado por la delta no existe ni en disco ni en el conjunto extraído por extractContracts.',
...
const SEVERITY_RANK: Record<ConvergenceFinding['severity'], number> = { high: 0, medium: 1, low: 2 };

const posix = (value: string): string => value.split(path.sep).join('/');
const norm = (value: string): string => posix(value.trim()).replace(/^\.\//, '').replace(/\/+$/, '');
...
const readText = async (abs: string): Promise<TextRead> => {
  try {
...
const lineOf = (content: string, needle: string | RegExp): number => {
  const lines = content.split('\n');
  for (let i = 0; i < lines.length; i += 1) {
    const line = lines[i];
    if (typeof needle === 'string' ? line.includes(needle) : needle.test(line)) return i + 1;
  }
  return 0;
};

/** Línea del encabezado de un requisito (`### REQ-X — …` o `## Requirement X`). */
const lineOfRequirement = (requirements: string, id: string): number =>
  lineOf(
    requirements,
    new RegExp(`^#{2,4}\\s*(?:${escapeRe(id)}|Requirement\\s+${escapeRe(id.replace(/^REQ-/, ''))})\\b`, 'i'),
  );

/** Línea de una tarea por su marcador de id (`- [x] 12.` o `- [ ] conv-1:`). */
const lineOfTask = (tasks: string, taskId: string): number =>
  lineOf(tasks, new RegExp(`^-\\s*\\[[ x-]\\]\\s*(\\*)?\\s*${escapeRe(taskId)}\\s*[:.]?(\\s|$)`));

const existsOnDisk = async (abs: string): Promise<{ exists: boolean; error?: string }> => {
  try {
    await stat(abs);
    return { exists: true };
  } catch (err) {
...
const stem = (value: string): string => norm(value).replace(/\.[A-Za-z0-9]+$/, '');
const canon = (value: string): string => stem(value).replace(/\/index$/, '');
 ...
 */
const isInside = (file: string, declared: string): boolean => {
  const d = norm(declared);
  const f = norm(file);
  if (!d || !f) return false;
  if (d === '.' || d === '*' || d === '**') return true;
  if (SOURCE_EXT.test(d)) return canon(f) === canon(d);
...
const isPathLike = (target: string): boolean => {
  ...
  if (/^(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\b/i.test(value)) return false;
 ...
 */
export const convergenceFingerprint = (gap: Pick<ConvergenceFinding, 'gapType' | 'source'>): string =>
  createHash('sha256').update(`${gap.gapType}|${gap.source}`, 'utf8').digest('hex').slice(0, 12);

/** Huellas ya anotadas en `tasks.md`. */
export const fingerprintsInTasks = (tasksMarkdown: string): Set<string> => {
  const found = new Set<string>();
  for (const match of tasksMarkdown.matchAll(/_Convergence:\s*F-([0-9a-f]{6,})_/g)) found.add(match[1]);
  return found;
};
...
export interface AnalyseConvergenceInput {
  cwd: string;
  feature: string;
  changedFiles?: string[];
  sddDir?: string;
  /**
   * Tope de hallazgos que se convierten en tareas anexadas (`--max N`). Los hallazgos y las métricas
   * siguen siendo los medidos; lo que se acota es cuánto se escribe en `tasks.md`. Sin tope se
   * anotan todos.
   */
  max?: number;
}
...
const deriveChangedFiles = (root: string, provided?: string[]): ChangedFiles => {
...
const specTokens = (texts: string[]): Set<string> => {
...
const isRequestedExport = (name: string, tokens: Set<string>): boolean => {
  if (tokens.has(name.toLowerCase())) return true;
  const parts = name
...
const isGenerated = (file: string): boolean => norm(file).split('/').some((segment) => GENERATED_DIRS.has(segment));
 ...
 */
const isTypeOnlyExport = (content: string, name: string): boolean => {
  const valueDecl = new RegExp(
    `\\bexport\\s+(?:default\\s+)?(?:async\\s+)?(?:function|class|const|let|var|enum)\\s+${escapeRe(name)}\\b`,
  );
  if (valueDecl.test(content)) return false;
  if (new RegExp(`\\bexport\\s+(?:default\\s+)?(?:type|interface)\\s+${escapeRe(name)}\\b`).test(content)) return true;
  if (new RegExp(`export\\s*\\{[^}]*\\btype\\s+${escapeRe(name)}\\b[^}]*\\}`).test(content)) return true;
  for (const block of content.matchAll(/export\s+type\s*\{([^}]*)\}/g)) {
    if (new RegExp(`\\b${escapeRe(name)}\\b`).test(block[1])) return true;
  }
...
const emptyGapCounts = (): Record<GapType, number> => ({
  missing: 0,
  partial: 0,
  contradicts: 0,
  unrequested: 0,
...
interface PushInput {
  gapType: GapType;
  severity?: ConvergenceFinding['severity'];
  source: string;
  evidence: string[];
  remainingWork: string;
  /** Requisitos que la tarea anexada debe declarar. */
  requirements?: string[];
  /** Principio constitucional, cuando el hallazgo es sobre uno y no sobre un requisito. */
  principle?: string;
  /** Frontera observada para la tarea anexada. */
  boundary?: string;
}
 ...
 */
class Collector {
  readonly findings: ConvergenceFinding[] = [];
  private readonly seen = new Set<string>();
  private readonly requirements = new Map<string, string[]>();
  private readonly principles = new Map<string, string>();
  ...
  push(input: PushInput): void {
    ...
    if (evidence.length === 0) return; // Sin evidencia de máquina el hallazgo no existe.
    ...
    if (input.requirements && input.requirements.length > 0) this.requirements.set(key, input.requirements);
    if (input.principle) this.principles.set(key, input.principle);
    if (input.boundary) this.boundaries.set(key, input.boundary);
  }

  ordered(): ConvergenceFinding[] {
  ...
  requirementsFor(finding: ConvergenceFinding): string[] {
    return this.requirements.get(`${finding.gapType}|${finding.source}`) ?? [];
  }

  principleFor(finding: ConvergenceFinding): string | undefined {
    return this.principles.get(`${finding.gapType}|${finding.source}`);
  }

  boundaryFor(finding: ConvergenceFinding): string | undefined {
    return this.boundaries.get(`${finding.gapType}|${finding.source}`);
  }
}

const SCORE_COMPONENT_REASON: Record<GapType, { component: string; reason: string }> = {
...
const scoreImpactFor = (findings: ConvergenceFinding[]): ConvergenceReport['scoreImpact'] => {
  const out: ConvergenceReport['scoreImpact'] = [];
...
const requirementsForTask = (tasks: ReturnType<typeof parseTasksMarkdown>, taskId: string): string[] => {
...
const boundaryForTask = (taskId: string, declared: Map<string, string[]>): string | undefined => {
  const boundaries = declared.get(taskId);
  return boundaries?.find((boundary) => isPathLike(boundary));
};

const boundaryForRequirement = (
  requirementId: string,
  tasks: ReturnType<typeof parseTasksMarkdown>,
  ...
  if (deltaTarget) return norm(deltaTarget);
  for (const task of tasks) {
    if (!requirementsForTask(tasks, task.id).includes(requirementId)) continue;
    const boundary = declared.get(task.id)?.find((candidate) => isPathLike(candidate));
    if (boundary) return norm(boundary);
  }
  return undefined;
};
...
const renderSection = (
  ...
  findings: ConvergenceFinding[],
  collector: Collector,
  tddCommand: string | null,
): string => {
  let next = 1;
  for (const match of tasksMarkdown.matchAll(/^##\s*Phase\s+(\d+)\b/gm)) next = Math.max(next, Number(match[1]) + 1);
  ...
  const lines = findings.map((finding, index) => {
    const parts: string[] = [`- [ ] conv-${base + index + 1}: ${finding.remainingWork}`];
    const requirements = collector.requirementsFor(finding);
    const principle = collector.principleFor(finding);
    if (requirements.length > 0) parts.push(`_Requirements: ${requirements.join(', ')}_`);
    else if (principle) parts.push(`_Constitution: ${principle}_`);
    parts.push(`_Boundary:_ \`${collector.boundaryFor(finding) ?? '.'}\`_`);
    if (tddCommand) parts.push(`_TDD:_ \`${tddCommand}\`_`);
 ...
 * inventa un comando.
 */
const realTestCommand = (
  testFramework: string | undefined,
  declaredScript: string | undefined,
): { command: string | null; declared: boolean } => {
  const derived = testCommandFor(testFramework, declaredScript);
  if (!derived.derived) return { command: derived.command, declared: true };
  if (derived.command !== 'npm test') return { command: derived.command, declared: false };
 ...
 */
export const analyseConvergence = async (input: AnalyseConvergenceInput): Promise<ConvergenceReport> => {
  ...
  if (!status.exists || !status.files.requirements) {
    return {
      ...
      detail:
        `Falta el prerrequisito: no existe ${requirementsRel}. ` +
  ...
  const delta: DeltaSpec | null = deltaRaw === null ? null : parseDeltaSpec(deltaRaw);
  ...
  checked.push(
    `requisitos: ${requirementItems.length} en ${requirementsRel}${delta ? `; ${delta.entries.length} entrada(s) en ${deltaRel}` : ''}`,
  );
  checked.push(
    `tareas: ${parsedTasks.length} en ${tasksRead.content === null ? `${tasksRel} (ILEGIBLE, no inspeccionado)` : tasksRel}`,
  );

  const requirementLine = (id: string): string => {
    const line = lineOfRequirement(requirements, id);
    return line > 0 ? `${requirementsRel}:${line}` : requirementsRel;
  };
  const taskLine = (taskId: string): string => {
    const line = lineOfTask(tasks, taskId);
    return line > 0 ? `${tasksRel}:${line}` : tasksRel;
  };

  // ── 1. Traza requisito↔tarea (traceDelta, reutilizado) ────────────────────────────────────────
  const traceSource: DeltaSpec = {
    feature,
    title: 'requirements.md + delta.md',
    status: 'approved',
    entries: traceIds.map((id) => ({ id, kind: 'ADDED', title: '', statement: '', targets: [] })),
  };
  const deltaTasks: DeltaTask[] = parsedTasks.map((task) => ({
    id: task.id,
    raw: task.raw,
  ...
  for (const entry of trace.mapped) ownersOf.set(entry.requirementId.toUpperCase(), entry.tasks);
  checked.push(
    `trazabilidad requisito↔tarea (traceDelta): ${trace.mapped.length}/${traceIds.length} requisito(s) con tarea, cobertura ${(trace.coverage * 100).toFixed(0)}%`,
  );

  // Fronteras declaradas por tarea y objetivos de ruta de la delta: el vocabulario de alcance.
  const declaredBoundaries = new Map<string, string[]>();
  for (const task of parsedTasks) {
    if (task.boundary && task.boundary.length > 0) declaredBoundaries.set(task.id, task.boundary);
  }
  const pathTargets = (delta?.entries ?? []).flatMap((entry) =>
    entry.targets.filter(isPathLike).map((target) => norm(target)),
  );
  ...
  for (const entry of trace.unmapped) {
    ...
    if (!traceIds.includes(id)) continue; // un id desconocido se reporta como `contradicts`
    const deltaTarget = delta?.entries
      .find((candidate) => candidate.id.toUpperCase() === id)
      ?.targets.find((target) => isPathLike(target));
    const boundary =
      deltaTarget ?? [...declaredBoundaries.values()].flat().find((candidate) => isPathLike(candidate)) ?? '.';
    collector.push({
      gapType: 'missing',
      source: id,
  ...
  for (const [taskId, boundaries] of declaredBoundaries) {
    ...
    for (const boundary of boundaries) {
      ...
      if (!probe.exists) missingPaths.push(candidate);
    }
  ...
  for (const [id, taskIds] of ownersOf) {
    if (!traceIds.includes(id)) continue;
    ...
    if (tasksOfRequirement.length === 0 || done.length === tasksOfRequirement.length) continue;
  ...
  checked.push(
    `evidencia (checkEvidenceLock): ${lock.checked} tarea(s) completada(s), ${lock.unprovenCompletions.length} sin _Evidence:_`,
  );
  ...
  const constitution: Constitution | null = constitutionRead.constitution;
  ...
  if (!constitutionRead.exists) {
    ...
    checked.push(
      `pivote constitucional (alignSpecWithConstitution): ${principlesChecked} principio(s) en vigor, ${alignment.declared.length} declarado(s), alineación ${(alignment.alignment * 100).toFixed(0)}%`,
    );
    for (const finding of alignment.findings) {
      if (finding.severity !== 'error') continue; // los avisos del pivote duplican a validateDeltaSpec
      const rawSource = finding.artifactId ?? finding.principleId ?? feature;
      const source = rawSource.toUpperCase();
      const gapType: GapType = finding.code === 'ORACLE_MISSING' ? 'unprotected' : 'contradicts';
      const evidence = [`alignSpecWithConstitution:${finding.code}`];
      if (finding.principleId) evidence.push(`principio ${finding.principleId}`);
      if (finding.artifactId) {
        evidence.push(
          /^REQ-/i.test(finding.artifactId)
  ...
  const declaredContracts = (delta?.entries ?? []).flatMap((entry) =>
    (entry.contracts ?? []).map((test) => ({ entry: entry.id.toUpperCase(), test })),
  );
  ...
  if (declaredContracts.length === 0 && changed.files.length === 0) {
    ...
    checked.push(
      `contratos (extractContracts): ${set.contracts.length} en el oráculo (${discovered.length} descubierto(s)), ${declaredContracts.length} declarado(s), comando propuesto \`${set.testCommand}\``,
    );
    if (!set.complete) {
  ...
  if (!changed.available) {
    notChecked.push(
      `código no pedido: ${changed.reason ?? 'no se pudieron obtener los ficheros cambiados'}, así que no se inspeccionó ningún diff`,
    );
    ...
    if (roots.length === 0) {
      notChecked.push(
        'código no pedido: la spec no declara ninguna frontera ni objetivo de ruta con el que contrastar el diff',
      );
    } else {
      const tokens = specTokens([requirements, plan ?? '', tasks, deltaRaw ?? '']);
      const sources = changed.files.filter(
        (file) =>
      ...
      for (const file of sources) {
        ...
        for (const name of extractExportNames(read.content)) {
          if (isRequestedExport(name, tokens)) continue;
          if (isTypeOnlyExport(read.content, name)) continue;
      ...
      checked.push(
        `código no pedido (extractExportNames): ${inspected}/${sources.length} fichero(s) fuente cambiado(s) dentro de las fronteras/objetivos declarados`,
      );
    }
  }
  ...
  checked.push(
    `consistencia cruzada artefacto↔artefacto (delegada a checkConsistency): ${consistency.checked.length} comprobación(es), ${consistency.findings.filter((finding) => finding.severity === 'error').length} error(es); converge no re-emite los códigos que ya posee`,
  );
  for (const reason of consistency.notChecked) notChecked.push(`consistencia cruzada — ${reason}`);
  const lifted = consistency.findings.filter(
    (finding: ConsistencyFinding) => finding.severity === 'error' && !ownedByConverge.has(finding.code),
  );
  ...
  if (skipped > 0) {
    checked.push(
      `idempotencia: ${skipped} hallazgo(s) ya constan como tarea en ${tasksRel} (\`${CONVERGENCE_MARKER} F-…_\`); no se vuelven a anotar`,
    );
  }

  // ── Comando de test REAL de las tareas anexadas ───────────────────────────────────────────────
  const rootManifest = await readText(path.join(root, 'package.json'));
  let declaredScript: string | undefined;
  if (rootManifest.content) {
    try {
      const parsed = JSON.parse(rootManifest.content) as { scripts?: Record<string, string> };
  ...
  if (!tdd.command) {
    notChecked.push(
      'runner de test: ningún manifiesto declaró `scripts.test` ni se detectó un framework con comando asociado, así que las tareas no llevan `_TDD:_` (no se inventa un comando)',
    );
  }
  ...
  let appended: string | null = tasksRead.content === null ? null : tasks;
  ...
  for (const finding of findings) byGapType[finding.gapType] += 1;
...
export interface AppendConvergenceInput {
  cwd: string;
  feature: string;
 ...
 */
export const appendConvergence = async (
  input: AppendConvergenceInput,
): Promise<{ written: boolean; path: string; tasksAfter: number; detail: string }> => {
  const root = await findRepoRoot(input.cwd);
  const sddDir = input.sddDir ?? (await resolveSddDir(root));
  ...
  if (input.write === true) await writeFile(tasksPath, after, 'utf8');
...
export const convergeExitCode = (report: ConvergenceReport): number =>
  report.findings.some((finding) => finding.severity === 'high') ? 1 : 0;
