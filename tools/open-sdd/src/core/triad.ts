...
export interface TriadFile {
  /** Canonical name in the paper. */
  canonical: string;
  /** Accepted aliases in this repository. */
  aliases: string[];
  role: string;
}

export const TRIAD: TriadFile[] = [
  {
...
export interface TriadPresence {
  canonical: string;
...
export interface TriadVerdict {
  ...
  files: TriadPresence[];
  missing: string[];
  /** True when the verdict says nothing about content quality — C1 checks presence, not sense. */
  approvalIsByProxy: boolean;
  detail: string;
}

/** Evaluate Triad presence from the file names actually found in a spec directory. */
export const evaluateTriad = (filesPresent: string[]): TriadVerdict => {
  const present = new Set(filesPresent.map((f) => f.toLowerCase()));
  const rows: TriadPresence[] = TRIAD.map((f) => {
...
export interface TaskEvidenceFinding {
  ...
  hasEvidence: boolean;
  /** The captured output, when present. */
...
export interface EvidenceLockResult {
  satisfied: boolean;
  checked: number;
  unprovenCompletions: TaskEvidenceFinding[];
  detail: string;
}
 ...
 */
export const checkEvidenceLock = (tasksMarkdown: string): EvidenceLockResult => {
  const lines = tasksMarkdown.split(/\r?\n/);
  const findings: TaskEvidenceFinding[] = [];
  const taskRe = /^\s*-\s*\[( |x|X)\]\s*(\S+)?\s*(.*)$/;

  let current: { id: string; status: 'completed' | 'pending'; line: string; evidence: string | null } | null =
    null;
...
// ---------------------------------------------------------------------------------------------
// Decidable properties of the implementer's discipline (§9.7)
// ---------------------------------------------------------------------------------------------
...
export interface DiffFacts {
  ...
  addedLines: number;
  removedLines: number;
  /** Paths the spec declares as the change's scope (the task's DAG node boundary). */
 ...
 */
export const checkDecidableDiscipline = (
  facts: DiffFacts,
  budget: DiffBudget,
): DecidableFinding[] => {
  ...
  return [
    ...
    },
    {
      ...
      violated: false,
      detail:
        'No medible desde el diff: exige el registro de supuestos para comprobar que los puntos de incertidumbre declarados aparecen como preguntas y no como supuestos silenciosos.',
    },
  ];
};

/** The non-decidable remainder, kept as a warning that stops nobody — by design. */
export const NON_DECIDABLE_DISCIPLINE_NOTE =
  'Elegancia, simplicidad y calidad del razonamiento no son decidibles: se emiten como advertencia con evidencia, alimentan la calibración del juez y no detienen a nadie. Una verificación débil es peor que una brecha reconocida.';
