...
export const RATIFY_INSTRUCTION =
  'Para ratificar: open-sdd govern constitution --ratify --by "<nombre>" --rationale "<texto>" --write';
...
export const constitutionArtifactPaths = (
  cwd: string,
  sddDir = '.sdd',
): { draft: string; inForce: string } => ({
...
export interface DraftProposal {
  ...
  /** Evidence observed in the code, each entry a path/symbol that exists. */
  evidence: string[];
  /** A first pass at the justification, derived from the evidence — the host model may rewrite it. */
  rationaleDraft: string;
  /** True when the proposal has no evidence and therefore must NOT become a principle. */
...
export interface ConstitutionDraft {
  ...
  /** Proposals that cannot be justified by evidence, phrased as questions for the human. */
  questions: string[];
  /** The document, marked as a draft and NOT in force. */
  text: string;
  detail: string;
  complete: boolean;
}
...
/**
 * Las reglas que la salida de un modelo anfitrión debe cumplir: los códigos que
 * `constitution.ts:validateConstitution` puede emitir, con su severidad real.
 *
 * La lista de inyección se DERIVA de `INJECTION_PATTERNS` para que no pueda quedarse atrás; el resto
 * se enumera aquí y por eso lleva el recordatorio de mantenerla sincronizada con `validateConstitution`.
 * Un paquete de evidencia que no nombre estas reglas deja al modelo anfitrión inventando el contrato.
 */
export interface DraftValidationRule {
  code: string;
  severity: 'error' | 'warning';
  /** Qué debe cumplir la prosa/estructura para no disparar el código. */
  rule: string;
}

export const DRAFT_VALIDATION_RULES: readonly DraftValidationRule[] = [
  {
    code: 'ID-FORMAT',
    severity: 'error',
 ...
 */
const verifiableEvidence = (entries: string[], root: string): { kept: string[]; dropped: string[] } => {
  const kept: string[] = [];
  ...
  for (const raw of entries) {
    ...
    if (artifact.file !== undefined && !artifact.resolvable) dropped.push(entry);
 ...
 */
export const buildConstitutionDraft = async (
  cwd: string,
  options: { sddDir?: string } = {},
): Promise<ConstitutionDraft> => {
  ...
  for (const principle of constitution.principles) {
    ...
    for (const entry of dropped) droppedEvidence.push(`${principle.id}: ${entry}`);
    if (kept.length === 0) {
  ...
  constitution.amendments.forEach((amendment, index) => {
    ...
    const question =
      `¿Se adopta "${amendment.title}"? El código NO muestra la práctica` +
  ...
  if (questions.length > 0) {
    ...
    for (const question of questions) header.push(`- ${question}`);
    header.push('');
  }
  ...
  const detail =
    `Borrador de constitución para "${constitution.project}" (${evidenced} propuesta(s) con evidencia, ` +
 ...
 */
export const evidencePackForHost = (
  draft: ConstitutionDraft,
  options: { maxEvidencePerItem?: number } = {},
  ...
  if (!draft.complete) {
    incompleteBecause.push(
      `El borrador está INCOMPLETO: ${draft.questions.length} pregunta(s) abierta(s) exigen una decisión humana. No las conviertas en principios: una práctica que el código no muestra no tiene evidencia.`,
    );
  }
  if (pending.length > 0) {
    incompleteBecause.push(
      `${pending.length} propuesta(s) sin evidencia (${pending.map((p) => p.principle.id).join(', ')}): el código no las demuestra, así que NO pueden ser principios.`,
    );
  }
  if (facts.length === 0) {
    incompleteBecause.push(
      'No se pudieron determinar hechos establecidos (proyecto sin manifiesto legible): no los inventes, declara lo que no se pudo determinar.',
    );
  }
  if (candidates.every((candidate) => !candidate.mayBecomePrinciple)) {
    incompleteBecause.push(
      'Ninguna propuesta tiene evidencia verificable: el borrador no puede producir ni un principio y debe declararse incompleto.',
    );
  }
  ...
  return {
    ...
    noModelShipped:
      'Esta herramienta no embarca ningún modelo: el texto final lo escribe tu modelo anfitrión a partir de este paquete, y lo ratifica una persona nombrada. Un borrador nunca es autoridad.',
  };
};

/** Principles still waiting for ratification. */
export const draftPrincipleIds = (constitution: Constitution): string[] =>
  constitution.principles.filter((principle) => principle.draft === true).map((principle) => principle.id);
 ...
 */
export const ratifyDraft = (
  constitution: Constitution,
  input: { by: string; rationale: string; ids?: string[] },
): { constitution: Constitution; ratified: string[]; refused: { id: string; why: string }[] } => {
  const by = input.by?.trim() ?? '';
  const rationale = input.rationale?.trim() ?? '';
  if (!by) {
    throw new Error(
      'La ratificación exige una persona nombrada: pasa --by "<nombre>". Sin nombre no hay puerta humana, y un borrador no se convierte en autoridad porque sí.',
    );
  }
  if (!rationale) {
    throw new Error(
      'La ratificación exige un motivo no vacío: pasa --rationale "<texto>". Sin motivo no queda rastro de por qué se aceptó el principio.',
    );
  }
  ...
  const promoted = new Map<string, ConstitutionPrinciple>();
  const newAmendments: AmendmentRecord[] = [];
  ...
  const keptAmendments = constitution.amendments.filter(
    (amendment) => !newAmendments.some((added) => added.id === amendment.id),
  );
