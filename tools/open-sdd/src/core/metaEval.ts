...
export interface KappaInput {
  /** Counts of judge-vs-gold outcomes. */
  truePositive: number;
  trueNegative: number;
...
export interface KappaResult {
  ...
  n: number;
  /**
   * Landis-Koch band. `undefined` is a distinct outcome, not the weakest one: when a rater has no
   * variance the coefficient is mathematically undefined, and labelling that as "almost perfect"
   * would report the strongest agreement exactly when there is no measurement at all.
   */
  band: 'undefined' | 'poor' | 'slight' | 'fair' | 'moderate' | 'substantial' | 'almost-perfect';
  /** True when the sample is too small for the point estimate to settle anything. */
  tooSmallToConclude: boolean;
  /** Pre-registered n that would make the estimate informative (kappa ~= 0.75, half-width 0.10). */
...
const landisKoch = (k: number): KappaResult['band'] => {
...
export const cohensKappa = (input: KappaInput): KappaResult => {
  ...
  return {
    ...
    detail: Number.isNaN(rounded)
      ...
      : `κ = ${rounded.toFixed(2)} (${landisKoch(rounded)}), n = ${n}${
          n < PREREGISTERED_N ? ` — por debajo del n preregistrado (${PREREGISTERED_N}): señal piloto, no validación` : ''
        }.`,
  };
};

/** The asymmetry direction matters: 0 FP / 1 FN means the judge errs STRICT (safe). */
export const judgeErrorDirection = (input: KappaInput): string => {
  if (input.falseNegative === 0 && input.falsePositive > 0) return 'lenient (dirección insegura)';
  if (input.falsePositive === 0 && input.falseNegative > 0) return 'strict (dirección segura)';
  if (input.falsePositive === 0 && input.falseNegative === 0) return 'sin errores observados';
  return 'mixta';
};
...
export interface ApprovalDriftResult {
...
export const checkApprovalDrift = (
  historical: number[],
  recent: number[],
  sigmaThreshold = 2,
): ApprovalDriftResult => {
  ...
  return {
    ...
    detail:
      sigma === 0
        ...
        : `Tasa móvil ${(movingAverage * 100).toFixed(1)}% vs línea base ${(baseline * 100).toFixed(1)}% (z = ${zScore.toFixed(2)}). ${
            zScore > sigmaThreshold
...
export interface SelfPreferenceTest {
  /** Sentinel artifacts of known quality injected blindly. */
  injected: number;
  /** How many were rated correctly. */
  correctlyRated: number;
  recalibrationForced: boolean;
  detail: string;
}

/** Monitor 3: blind sentinels of known quality; a bad rating forces recalibration. */
export const checkSelfPreference = (injected: number, correctlyRated: number): SelfPreferenceTest => {
...
export const checkJudgeIndependence = (input: {
  patchModelFamily: string;
  judgeModelFamily: string;
  temperature: number;
}): { independent: boolean; status: 'blocking' | 'advisory'; detail: string } => {
  const independent =
    input.judgeModelFamily.length > 0 &&
