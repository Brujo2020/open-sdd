...
import { traceDelta, type DeltaSpec } from './deltaSpec.js';
import { extractContracts } from './executionContract.js';
import { runChain, type GateRunReport } from './gateRunner.js';
...
export interface SddScoreReport {
  /** 0..100, media ponderada de los componentes MEDIDOS. */
  total: number;
  phase: SddPhase;
  phaseLabel: string;
  /** Los siete componentes, con sus pesos (suman 100). Los no medidos viajan con `score: 0` y su
   *  peso NO entra en el total: `notMeasured` y `detail` lo dicen explícitamente. */
  components: ScoreComponent[];
  /** UN comando, ejecutable. */
  nextAction: string;
  /** Lo que no se pudo inspeccionar, por nombre (componente o gate). */
  notMeasured: string[];
  detail: string;
}
...
const PHASE_SHORT: Record<SddPhase, string> = { 1: 'Especificar', 2: 'Implementar', 3: 'Verificar' };
...
const LABELS: Record<keyof typeof SDD_SCORE_WEIGHTS, string> = {
...
interface ComponentOutcome {
  id: keyof typeof SDD_SCORE_WEIGHTS;
  measured: boolean;
  score: number;
  evidence: string;
  /** Nombre con el que aparece en `notMeasured` cuando no es simplemente la etiqueta. */
  name?: string;
}

const measured = (id: keyof typeof SDD_SCORE_WEIGHTS, score: number, evidence: string): ComponentOutcome => ({
  id,
  measured: true,
...
const sameContractRef = (a: string, b: string): boolean => {
  const norm = (value: string): string => value.split('::')[0].trim().replace(/\\/g, '/').replace(/^\.\//, '');
  const left = norm(a);
  const right = norm(b);
...
const unreadable = (inspection: FeatureInspection | null, fileName: string): boolean =>
  Boolean(inspection?.unreadable.some((rel) => rel.endsWith(fileName)));
...
export const computeSddScore = async (
  ...
  const extraNotMeasured: string[] = [];

  // ── 1. Constitución: presente + válida + principios en vigor ──────────────────────────────────
  let constitutionValid = false;
  if (!constitutionRead.exists) {
    outcomes.push(
      measured('constitution', 0, `ausente (${constitutionCandidates(sddDir)[0]}): no hay autoridad que citar`),
    );
  } else if (!constitution) {
    outcomes.push(
      unmeasured('constitution', `existe pero no se pudo leer (${constitutionRead.error ?? 'error de lectura'})`),
    );
  } else {
    ...
    outcomes.push(
      measured(
        'constitution',
        constitutionValid ? 1 : 0,
        `presente y ${constitutionValid ? 'válida' : 'inválida'} · ${inForce.length} principio(s) en vigor` +
  ...
  if (!inspection || inspection.requirements === null) {
    ...
    if (report.total === 0) {
      ...
      outcomes.push(
        measured(
          'ears',
          report.conforming / report.total,
          `${report.conforming}/${report.total} requisito(s) conforme(s) · ${earsErrors} error(es) · ${warnings} aviso(s)`,
        ),
      );
    }
  }
  ...
  if (!inspection || inspection.delta === null) {
    outcomes.push(
      unreadable(inspection, 'delta.md')
    ...
    outcomes.push(
      unreadable(inspection, 'tasks.md')
    ...
    const delta: DeltaSpec = inspection.delta;
    ...
    if (tasksTotal === 0) {
      ...
      outcomes.push(
        measured(
          'evidence',
          evidenceScore,
          `${withEvidence}/${tasksTotal} tarea(s) con evidencia · ${lock.unprovenCompletions.length} completada(s) sin _Evidence:_ · ${tasksTotal - lock.checked} pendiente(s)`,
        ),
      );
    }
  }
  ...
  if (!inspection || inspection.delta === null) {
    ...
    try {
      ...
      if (declaredRefs.length === 0 && project.testDirs.length === 0) {
        outcomes.push(
          unmeasured('contracts', 'la delta no declara contratos y no se detectó ningún directorio de test'),
        );
      } else {
        ...
        if (declaredRefs.length > 0) {
          const fileExists = (ref: string): boolean => {
            const file = ref.split('::')[0].trim();
            return file.length > 0 && existsSync(path.join(root, file));
          };
          const backed = declaredRefs.filter(
            (ref) => fileExists(ref) || set.contracts.some((contract) => sameContractRef(contract.test, ref)),
          );
          const missing = declaredRefs.filter((ref) => !backed.includes(ref));
          outcomes.push(
            measured(
              'contracts',
              backed.length / declaredRefs.length,
              `${backed.length}/${declaredRefs.length} contrato(s) declarado(s) respaldado(s)${missing.length > 0 ? ` · faltan: ${missing.join(', ')}` : ''}`,
            ),
          );
        } else {
          ...
          outcomes.push(
            measured(
              'contracts',
              score,
              `${set.contracts.length} contrato(s) descubierto(s) · ${set.uncoveredChanges.length} cambio(s) sin cobertura`,
            ),
          );
        }
      }
    } catch (err) {
      outcomes.push(unmeasured('contracts', `no evaluables (${(err as Error).message})`));
    }
  }
  ...
  if (!constitution) {
    outcomes.push(
      unmeasured(
        'alignment',
        `constitución ${constitutionRead.exists ? 'ilegible' : 'ausente'}: sin autoridad no hay pivote que ejecutar`,
      ),
    );
    ...
    try {
      ...
      outcomes.push(
        measured(
          'alignment',
          errors.length > 0 ? 0 : alignment.alignment,
          `declarados: ${alignment.declared.join(', ') || 'ninguno'} · alineación ${(alignment.alignment * 100).toFixed(0)}% · ${errors.length} error(es)` +
            `${gitRepo ? '' : ' · origen del cambio sin determinar (no es un repositorio git)'}`,
        ),
      );
    } catch (err) {
      outcomes.push(unmeasured('alignment', `el pivote no se pudo ejecutar (${(err as Error).message})`));
    }
  }

  // ── El total: media ponderada SOLO sobre lo medido ───────────────────────────────────────────
  const components: ScoreComponent[] = outcomes.map((outcome) => ({
    id: outcome.id,
  ...
  if (!constitutionValid) incomplete.push('constitución ausente o no válida');
  if (!feature) incomplete.push('sin ninguna especificación');
  else if (!inspection || inspection.requirements === null) incomplete.push('requirements.md ausente');
  else if (earsTotal === 0) incomplete.push('EARS no encontró ningún requisito');
  else if (earsErrors > 0) incomplete.push(`${earsErrors} error(es) EARS`);
  const specSound = incomplete.length === 0;
  const phase: SddPhase = !specSound
    ? 1
  ...
  const detail =
    `Total ${total}/100 = Σ(peso·score de lo medido) / Σ(peso medido) = ${raw.toFixed(1)}/${measuredWeight} · ` +
...
const chooseNextAction = (input: {
  ...
  earsErrors: number;
  traceGap: number;
  traceabilityMeasured: boolean;
  evidenceMeasured: boolean;
  evidenceScore: number;
  contractsMeasured: boolean;
  contractsScore: number;
  gatesMeasured: boolean;
  gatesPassed: boolean;
  gates: string[];
}): string => {
  ...
  if (!input.feature) return 'open-sdd init <feature>';
  if (input.requirementsMissing) return `open-sdd init ${input.feature}`;
  if (input.earsErrors > 0) return `open-sdd govern --advise-ears ${input.feature}`;
  if (input.traceabilityMeasured && input.traceGap > 0) return `open-sdd delta validate ${input.feature}`;
  if (input.evidenceMeasured && input.evidenceScore < 1) return `open-sdd impl ${input.feature}`;
  if (input.contractsMeasured && input.contractsScore < 1) return `open-sdd brownfield contracts ${input.feature}`;
...
export const explainNextAction = (report: SddScoreReport): string => {
...
export const renderScoreFooter = (report: SddScoreReport): string => {
  ...
  if (report.notMeasured.length > 0) parts.push(`sin medir: ${report.notMeasured.join(', ')}`);
