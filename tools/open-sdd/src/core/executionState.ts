...
import type { ExecutionState } from '../cli/ui/progressBar.js';
...
export const getExecutionStateFile = (sddDir: string, feature: string, cwd: string = process.cwd()): string => {
  return join(cwd, sddDir, '.impl-state', `${feature}.json`);
};

export const saveExecutionState = async (
...
export const loadExecutionState = async (
  feature: string,
  sddDir: string,
  cwd: string = process.cwd(),
...
export const clearExecutionState = async (
  feature: string,
  sddDir: string,
  cwd: string = process.cwd(),
): Promise<void> => {
