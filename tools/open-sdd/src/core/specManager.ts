...
import type { RequirementItem, SpecMetadata, SpecPhase, SpecStatus, TaskItem } from './types.js';
import { createAndCheckoutBranch, loadGitSettings } from './git.js';

export const resolveSddDir = async (
  cwd: string = process.cwd(),
  preferredDir: string = '.sdd',
): Promise<string> => {
  try {
    const sddStat = await stat(path.join(cwd, preferredDir));
    if (sddStat.isDirectory()) return preferredDir;
  } catch {
    // fallback check for legacy .kiro
    try {
      const kiroStat = await stat(path.join(cwd, '.kiro'));
      if (kiroStat.isDirectory()) return '.kiro';
    } catch {
...
export const readSpecMetadata = async (
  cwd: string,
  feature: string,
  sddDir: string = '.sdd',
...
export const writeSpecMetadata = async (
  cwd: string,
  feature: string,
  meta: SpecMetadata,
  sddDir: string = '.sdd',
...
export const parseRequirementsMarkdown = (content: string): RequirementItem[] => {
  const lines = content.split('\n');
...
export const getSpecStatus = async (
  cwd: string,
  feature: string,
  sddDir: string = '.sdd',
): Promise<SpecStatus> => {
  ...
  const phase: SpecPhase = meta?.phase ?? 'initialized';
  ...
  if (hasTasks) {
    try {
      ...
      for (const t of parsed) {
        if (t.boundary) t.boundary.forEach((b) => boundSet.add(b));
      }
  ...
  const checkApproval = (val: any): boolean => {
    if (typeof val === 'boolean') return val;
    if (val && typeof val === 'object' && 'approved' in val) return Boolean(val.approved);
    return false;
  };

  const isApproved =
    checkApproval(approvals.requirements) &&
...
): Promise<{ specDir: string; branchCreated: boolean; branchName?: string }> => {
