...
const manifestPath = join(repoRoot, 'tools/open-sdd/templates/manifests/windsurf-skills.json');
...
describe('real windsurf-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecInit = join(cwd, '.windsurf/skills/sdd-spec-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.windsurf/skills/sdd-spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.windsurf/skills/sdd-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.windsurf/skills/sdd-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.windsurf/skills/sdd-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.windsurf/skills/sdd-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.windsurf/skills/sdd-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.windsurf/skills/sdd-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.windsurf/skills/sdd-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.windsurf/skills/sdd-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/sdd-spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.windsurf/skills/sdd-spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.windsurf/skills/sdd-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.windsurf/skills/sdd-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.windsurf/skills/sdd-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 20 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.windsurf/skills/sdd-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.windsurf/skills/sdd-impl/templates/reviewer-prompt.md');
