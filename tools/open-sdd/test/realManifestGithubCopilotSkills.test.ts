...
const manifestPath = join(repoRoot, 'tools/open-sdd/templates/manifests/github-copilot-skills.json');
...
describe('real github-copilot-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecInit = join(cwd, '.github/skills/sdd-spec-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.github/skills/sdd-spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.github/skills/sdd-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.github/skills/sdd-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.github/skills/sdd-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.github/skills/sdd-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.github/skills/sdd-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.github/skills/sdd-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.github/skills/sdd-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.github/skills/sdd-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-getspecs/rules/getspecs-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-spec-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.github/skills/sdd-spec-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.github/skills/sdd-spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.github/skills/sdd-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.github/skills/sdd-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.github/skills/sdd-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 20 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.github/skills/sdd-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.github/skills/sdd-impl/templates/reviewer-prompt.md');
