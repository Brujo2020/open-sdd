...
const mkTmp = async () => mkdtemp(join(tmpdir(), 'ccsdd-real-manifest-antigravity-skills-'));
...
const manifestPath = join(repoRoot, 'tools/open-sdd/templates/manifests/antigravity-skills.json');

describe('real antigravity-skills manifest', () => {
  it('dry-run prints plan for antigravity-skills.json with placeholders applied', async () => {
    const ctx = makeIO();
    const code = await runCli(
      ['--dry-run', '--lang', 'en', '--agent', 'antigravity-skills', '--manifest', manifestPath],
      runtime,
      ctx.io,
      {},
    );
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const ctx = makeIO();
    const code = await runCli(
      ['--lang', 'en', '--agent', 'antigravity-skills', '--manifest', manifestPath, '--overwrite=force'],
      runtime,
      ctx.io,
      {},
    ...
    expect(docText).toContain('/sdd-spec-status');
    expect(docText).not.toContain('$sdd-spec-status');
    ...
    const skillSpecInit = join(cwd, '.agent/skills/sdd-spec-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.agent/skills/sdd-spec-quick/SKILL.md');
    expect(await exists(skillSpecQuick)).toBe(true);
    const skillSpecQuickText = await readFile(skillSpecQuick, 'utf8');
    expect(skillSpecQuickText).toMatch(/name: sdd-spec-quick/);
    expect(skillSpecQuickText).toContain('/sdd-impl');

    const settingsTemplate = join(cwd, '.sdd/settings/templates/specs/init.json');
    expect(await exists(settingsTemplate)).toBe(true);

    const skillSpecDesign = join(cwd, '.agent/skills/sdd-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.agent/skills/sdd-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.agent/skills/sdd-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.agent/skills/sdd-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.agent/skills/sdd-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.agent/skills/sdd-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.agent/skills/sdd-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.agent/skills/sdd-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-getspecs/rules/getspecs-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-spec-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/sdd-spec-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.agent/skills/sdd-spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.agent/skills/sdd-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.agent/skills/sdd-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.agent/skills/sdd-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
    ...
    const noRulesSkills = ['sdd-spec-init', 'sdd-spec-status', 'sdd-spec-quick', 'sdd-spec-batch', 'sdd-impl', 'sdd-validate-impl', 'sdd-discovery'];
    for (const skill of noRulesSkills) {
      expect(await exists(join(cwd, `.agent/skills/${skill}/rules`))).toBe(false);
    }
  ...
  it('generates exactly 20 skill directories', async () => {
    ...
    for (const skill of expectedSkills) {
      const skillPath = join(cwd, `.agent/skills/${skill}/SKILL.md`);
      expect(await exists(skillPath)).toBe(true);
    }

    // sdd-impl has prompt templates
    const implPrompt = join(cwd, '.agent/skills/sdd-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.agent/skills/sdd-impl/templates/reviewer-prompt.md');
