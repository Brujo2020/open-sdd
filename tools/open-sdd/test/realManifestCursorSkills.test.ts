...
const mkTmp = async () => mkdtemp(join(tmpdir(), 'ccsdd-real-manifest-cursor-skills-'));
...
const manifestPath = join(repoRoot, 'tools/open-sdd/templates/manifests/cursor-skills.json');

describe('real cursor-skills manifest', () => {
  it('dry-run prints plan for cursor-skills.json with placeholders applied', async () => {
    const ctx = makeIO();
    const code = await runCli(
      ['--dry-run', '--lang', 'en', '--agent', 'cursor-skills', '--manifest', manifestPath],
      runtime,
      ctx.io,
      {},
    );
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const ctx = makeIO();
    const code = await runCli(
      ['--lang', 'en', '--agent', 'cursor-skills', '--manifest', manifestPath, '--overwrite=force'],
      runtime,
      ctx.io,
      {},
    ...
    const skillSpecInit = join(cwd, '.cursor/skills/sdd-spec-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.cursor/skills/sdd-spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.cursor/skills/sdd-spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.cursor/skills/sdd-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.cursor/skills/sdd-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.cursor/skills/sdd-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.cursor/skills/sdd-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.cursor/skills/sdd-spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.cursor/skills/sdd-spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.cursor/skills/sdd-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-getspecs/rules/getspecs-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-spec-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.cursor/skills/sdd-spec-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.cursor/skills/sdd-spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.cursor/skills/sdd-spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.cursor/skills/sdd-spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.cursor/skills/sdd-spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 20 skill directories', async () => {
    ...
    for (const skill of expectedSkills) {
      const skillPath = join(cwd, `.cursor/skills/${skill}/SKILL.md`);
      expect(await exists(skillPath)).toBe(true);
    }

    // sdd-impl has prompt templates
    const implPrompt = join(cwd, '.cursor/skills/sdd-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.cursor/skills/sdd-impl/templates/reviewer-prompt.md');
