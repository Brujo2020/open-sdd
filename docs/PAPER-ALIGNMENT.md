...
### G-36 — The commit hook resolves the CLI through PATH, so a stale global install makes it fail open

Paper: §6.3, Table 19 (levels A–D: a commit gate that silently degrades to "no checks" is worse than
none); §9.2 (`default-FAIL`). `templates/hooks/pre-commit.mjs` resolves the CLI in this order:
`$OPEN_SDD_CLI`, then eight project-relative candidates (`tools/open-sdd/dist/cli.js`,
`node_modules/…`, `node_modules/.bin/open-sdd`), and **last the first `open-sdd` on `PATH`**
(`resolveCli` → `findOnPath`). A project installed the documented way — `npx @brujo2020/open-sdd init
. --write` — has no `node_modules` of its own, so the hook falls through to whatever global `open-sdd`
the machine has.

Measured on this machine on 2026-09-21, in a throwaway repository installed from the published
`@brujo2020/open-sdd@3.4.0`:

| Probe | Result |
|---|---|
| `gates run C1 C2 C3 --staged --strict` with an AWS-shaped key staged | **exit 1**, C2 finds it |
| `govern rigor --no-drift --quiet` with the constitution missing | **exit 1**, blocking |
| the installed hook, same index, same moment | **exit 0** — it blocks nothing |
| `open-sdd --version` of `/opt/homebrew/bin/open-sdd` | `@brujo2020/open-sdd@2.0.0` (stale) |
| the hook with `OPEN_SDD_CLI` pointing at the 3.4.0 CLI | **exit 1**, `COMMIT BLOQUEADO` |

So the hook is not broken in itself: it is running an **old global CLI** that does not enforce, and the
commit gate therefore reports nothing while the configuration still reads "blocking". That is the
`hook-failed-open` case this repository's own `core/enforcement.ts` names, reached through the
resolution order rather than through an exit code. `doctor` does not catch it either: run through
`npx` it resolves the cached CLI and reports `CLI alcanzable … 3.4.0`, so the diagnostic and the hook
can disagree about which CLI is in play.

Workaround today, and the reason the level-B floor is only as good as the machine it is installed on:
upgrade the global (`npm install -g @brujo2020/open-sdd@latest`) or pin the interpreter explicitly
(`OPEN_SDD_CLI=<path to the project's CLI>`, or install the package as a project dependency so a
project-relative candidate resolves first). A fix would have `init` record the resolved CLI at install
time (`.sdd/settings/hook.json`) and make the hook prefer it, leaving the `PATH` shim as a last resort
**only after verifying its version** — an unpinned `PATH` lookup is not a resolution strategy for a
security gate.
...
11. Do **not** read "the commit hook is installed" as "the commit hook enforces". The hook resolves its
    CLI through `PATH` as a last resort, so a stale global `open-sdd` makes it exit 0 over a staged
    secret (G-36). State the CLI version the hook actually resolves, not the one `doctor` reports.
